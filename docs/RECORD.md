...
[2026-06-24] 首存-Phase2-Task4 报价侧合桶分发 precomputeQuoteDriverBuckets(P3-C1) | ConfigureSnapshotService.snapshotLines 内层逐(行×组件)expand → eligible 组件整单一次 expandMulti(全部不同料号,按 hf_part_no 回分)+不 eligible 逐行回落；新增 precomputeQuoteDriverBuckets(在 evictAll()之后保冷跑；LinkedHashSet 去重保序料号；eligibleForQuoteBucket 四闸门:①非EXCEL②$view路径③非composite④无lineItemId/quotation_line_item_id/spineKeys)；kill switch cpq.firstsave-quote-bucket 默认 false(灰度)；AP-37 可变共享面：每行写入前 MAPPER.writeValueAsString 深拷贝，materialize 重 parse 各行独立。等价测试 FirstSaveQuoteBucketEquivTest 6 TC(TC-1 A/B逐位:6行×8eligible组件全等价+nonEmpty=16 TC-2 连跑两次序列化一致无mutate TC-3 回落分支:全组件eligible无回落(符合预期) TC-4 无重复料号跳过 TC-5 往返度量 TC-6 空入参→空buckets) Tests run: 6, Failures: 0, Errors: 0, Skipped: 1；回归 Task1-3+P2C4+BatchExpand 21 tests 全绿 BUILD SUCCESS。 | cpq-backend/.../configure/service/ConfigureSnapshotService.java(新增 precomputeQuoteDriverBuckets + emptyExpandResponse + kill switch + snapshotLines 内层改桶分发 + LinkedHashSet import) + test/.../configure/service/FirstSaveQuoteBucketEquivTest.java(新,6 TC) | 关键决策:①报价视图($ys_view/$cp_view等)已确认不含 lineItemId 维度(全 eligible)，合桶纯等价；②不 eligible 逐行回落 8-arg(childLineItemIds=null)与现状 1:1；③默认关灰度，A/B 全绿+真实对拍后由 Task5 决定是否开启；④precompute 不进 expandCache(expandMulti 本身不缓存)→天然冷跑。
