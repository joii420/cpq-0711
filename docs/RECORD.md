...
[2026-07-16] task-0712 update-071501 后端 B1~B6 - 三模板(报价/核价/选配)统一到「客户产品分类」轴，换轴改造 | worktree `task-0712-category-axis`(分支 `feat/task-0712-category-axis`) | **依据**：`dev-docs/task-0712-选配模板和报价单选配功能/update-071501/{backtask.md,api.md,优化需求说明.md §9}`(D1~D13)。**核心**：给客户绑一个 `product_category_id`，报价/核价/选配三类模板此后全部以"客户所属产品分类"为唯一轴匹配，不再手选/按行业(`customer.industryCode` 保留不动，另有用途 D8)。 | **B1 迁移**：`V337__customer_add_product_category.sql`(幂等 seed `product_category.name='默认分类'` → `customer` 加 `product_category_id` 列(先可空)→ backfill 所有存量客户成默认分类 → `SET NOT NULL`；软引用不加物理 FK)+ `V338__sel_template_switch_to_product_category.sql`(先清空 `sel_template_item_value/_item/_template` 三表存量测试数据(D12，含前日 F6 QA 插入的 `__DEFAULT__` 种子模板，随此次换轴一并清空重配) → `DROP COLUMN industry_code`(内联 UNIQUE 随列一并删) → 加 `product_category_id UUID NOT NULL` + `UNIQUE` 约束)；两迁移均按 V336 先例做成幂等(`IF NOT EXISTS`/`WHERE NOT EXISTS`/`pg_constraint` 守卫)，应对共享 `cpq_db` 多会话并发重跑。 | **B2 客户**：`Customer`/`CustomerDTO`/`CreateCustomerRequest` 加 `productCategoryId`；`CustomerService.create` 为空兜底 `resolveDefaultCategoryId()`(查不到"默认分类"抛 500)，`update` 可改绑(不追溯已有报价单，D4)；`CustomerDTO` 不返 `productCategoryName`(D5，前端自映射)；list() 未加 join，无 N+1。 | **B3 选配模板换轴**：`SelTemplate.industryCode(String,UNIQUE)` → `productCategoryId(UUID,UNIQUE)`；`SelTemplateUpsertRequest.industryCode`→`productCategoryId`(`@NotNull`)；`SelTemplateDTO` 同步；`SelTemplateService.getByIndustry`→`getByCategory(UUID)`；`upsert` 按 `productCategoryId` upsert，且新增 api.md §1.1 要求的存在性校验(`ProductCategory.findById` 查不到报 400)。 | **B4 EffectiveTemplateService**：删 `DEFAULT_INDUSTRY="__DEFAULT__"` 哨兵常量，改真实查 `name='默认分类'` 的 `ProductCategory`(D10，兜底链结构不变：客户分类模板→默认分类模板→`hasTemplate=false`)；`EffectiveTemplateDTO.resolvedIndustryCode(String)`→`resolvedCategoryId(UUID)`；`ConfigureProductService.effectiveEnabledTypes` 只消费 `eff.params`，未引用改名字段，回归确认不受影响(0 处引用)。 | **B5 commit 一致性审计(关键决策，技术总监 2026-07-16 现场订正裁决)**：**实测发现**任务书原假设"commit 用 categoryId 匹配模板"不成立——`ImportSessionService.commit()`(对应前端真实调用的 `import-session/{id}/commit`，非未接线的平行路径 `basic-data-import/v6/quote`)里 `req.categoryId` 是**死字段**：`customerTemplateId`/`costingTemplateId` 由前端在调 commit **之前**用 `match-customer-quote`/核价 list 预匹配好塞进请求体，commit 本身不匹配模板、不持久化 categoryId。技术总监裁决：否决"commit 内重匹配并覆盖模板"的强版本(越界 D13 + 会破坏 MIXED 多模板下用户手选语义)，采纳弱版本——commit 内查一次 `Customer`(`session.customerId`，单条非循环无 N+1)，`req.categoryId != customer.productCategoryId` 时仅 `LOG.warn` 留痕，不改任何持久化/不重匹配；"以客户为准"的真实权威性由前端只读锁定保证(fronttask F3)。`api.md §5`/`backtask.md B5` 已同步订正为此口径。 | **B6 回归**：grep 全工程确认 `resolvedIndustryCode`/`getByIndustry`/`DEFAULT_INDUSTRY`/seltemplate 包内 `industryCode` 均清零(仅历史 test 文件 `EffectiveTemplateServiceTest.java` 因 JSON 断言字符串/原生 SQL 字符串未编译期报错但语义已过期，留给 cpq-tester 后续按新字段重写，未在本次范围内动)；`matchCustomerQuoteTemplate`/`customer.industryCode`/`snapshot_customer_industry` 零改动(D8)。 | 涉及文件：`cpq-backend/src/main/{resources/db/migration/{V337,V338}__*.sql, java/com/cpq/{customer/{entity/Customer.java,dto/{CustomerDTO,CreateCustomerRequest}.java,service/CustomerService.java}, seltemplate/{entity/SelTemplate.java,dto/{SelTemplateDTO,SelTemplateUpsertRequest,EffectiveTemplateDTO}.java,service/{SelTemplateService,EffectiveTemplateService}.java}, importsession/service/ImportSessionService.java}}` + `dev-docs/.../update-071501/{api.md,backtask.md}`(B5 订正段落)。 | **自检**：`./mvnw -q -DskipTests compile` BUILD SUCCESS 0 错误 ✅；grep `resolvedIndustryCode|getByIndustry|DEFAULT_INDUSTRY`(main) 空 ✅；grep `industryCode`(seltemplate 包) 空 ✅；未真跑 Flyway 连库(按约束，迁移真验证留给合并阶段技术总监做)。 | **已知限制/交接**：①`EffectiveTemplateServiceTest.java` 引用旧 `industry_code` 列的原生 SQL 会在真跑迁移后运行期报错(列已删)，需 cpq-tester 按新 `product_category_id` 语义重写或删除该测试；②B1 两迁移未在本会话真实应用到共享 `cpq_db`(仅编译期语法自检)，合并前需技术总监实跑验证 `flyway_schema_history` success + `customer.product_category_id` 0 空值 + `sel_template` 表结构。

---

[2026-07-16] task-0712 update-071501 后端 Tier1+Tier2 订正 - 客户产品分类 DB 可空(方案B) + 2 个 seltemplate 遗留测试换轴重写 | worktree `task-0712-category-axis`(分支 `feat/task-0712-category-axis`)，续上条 B1~B6 记录 | **Tier2(方案B，技术总监裁决)**：`V337__customer_add_product_category.sql` 删掉第 3 步 `SET NOT NULL`，`customer.product_category_id` 保持 **DB 可空**；`Customer.java` entity 同步去掉 `@Column(nullable=false)`(与实际 DB 物理约束对齐，避免文档/代码/schema 三处口径不一致)。非空改由**应用层三重保证**：①前端表单必填 ②`CustomerService.create` 兜底 `resolveDefaultCategoryId()`(保留不变) ③本迁移 backfill 存量客户。`EffectiveTemplateService` 对 `productCategoryId==null` 的"默认分类"兜底同样保留(客户找不到分类时仍走兜底链，不因 DB 可空而出现空指针/未处理分支)。**动机**：全后端约 31 个既有测试用原生 SQL 直插 `customer` 夹具、未带 `product_category_id` 列，若 DB 强制 `NOT NULL` 会让这些与本次改造无关的测试集体因约束违反报错；这 31 个测试**零改动**，未触碰。 | **Tier1**：`EffectiveTemplateServiceTest.java`(4 用例) + `SelTemplateResourceTest.java`(6 用例) 从行业维度全量重写到产品分类维度——native SQL `industry_code`→`product_category_id`；请求体 `industryCode`→`productCategoryId`；断言 `resolvedIndustryCode`→`resolvedCategoryId`；`__DEFAULT__` 哨兵→真实 `name='默认分类'` 产品分类(经 `SELECT id FROM product_category WHERE name='默认分类'` 取得，依赖 V337 幂等 seed)。两个测试类改用 `@TestInstance(PER_CLASS)` + `@BeforeAll`/`@AfterAll` 非静态生命周期(对齐 `ExistingProductResourceTest`/`ModelConfigResourceTest` 既有惯例)，各自**自建自清**一个测试产品分类(`TEST-EFF-CAT` / `TEST-SEL-CAT`，按 code 幂等 find-or-create，`@AfterAll` 精确按 code 删除)，customer 夹具 `product_category_id` 指向自建分类而非 NULL(确保测的是"客户分类命中/回退"这条真实业务路径，而非可空退化态)。`EffectiveTemplateServiceTest` 完整保留原有 4 场景(①客户分类命中②回退默认分类③customerNo 缺失 400④两者皆无→hasTemplate=false)及原有对"默认分类模板"的 `assumeTrue` 护栏(已有真实默认模板则跳过，避免误删共享库生产数据)。 | 涉及文件：`cpq-backend/src/main/{resources/db/migration/V337__customer_add_product_category.sql, java/com/cpq/customer/entity/Customer.java}` + `cpq-backend/src/test/java/com/cpq/seltemplate/{EffectiveTemplateServiceTest.java,SelTemplateResourceTest.java}`(全量重写) + `dev-docs/.../update-071501/{api.md,backtask.md,优化需求说明.md §9.7}`(方案B + B5 订正同步落文档)。 | **自检**：`./mvnw -q -DskipTests test-compile` BUILD SUCCESS 0 错误(含 main compile)✅；grep `industryCode|resolvedIndustryCode`(seltemplate 测试包)空 ✅；grep `sel_template.*industry_code`(全测试目录)空 ✅；**未真跑 mvnw test / 未真跑 Flyway 连库**(共享库尚无 V337/V338 新列，新测试此刻无法真跑，按约束只验证编译期正确性，真跑验证留给技术总监在主仓合并后做)。

---

[2026-07-16] task-0712 update-071501 后端代码质量评审 polish（Approved 后 4 项）| worktree `task-0712-category-axis`(分支 `feat/task-0712-category-axis`)，续上两条记录 | **Important #1 — 补 upsert 校验负测**：`SelTemplateResourceTest.java` 加 T7(`productCategoryId` 传随机不存在 UUID → 400，覆盖 `SelTemplateService.upsert` 新增的存在性校验分支) + T8(漏传 `productCategoryId` → 400，覆盖 `@NotNull`)。 | **Minor #2 DRY**：`ProductCategory` 实体加静态方法 `requireDefaultId()`（按 `name='默认分类'` 查，无则 `BusinessException(500,...)`，注释显式记录**不改按 code 查**的理由——seed/backfill/前端全链均按 name 判定业务身份，code 非约定值时按 code 查会误判查不到，对应评审 Minor #3 明确不采纳）；`CustomerService`/`EffectiveTemplateService` 各自的私有 `resolveDefaultCategoryId()` 删除，改调用 `ProductCategory.requireDefaultId()`。`TemplateService`/`BasicDataImportServiceV5` 的历史查找**未碰**(D13 范围外)。 | **Minor #4**：`ImportSessionService.commit` 审计逻辑把 `req.categoryId != null` 守卫提到 `Customer.findById` 之前，`categoryId` 为空时不再查库。 | **Minor #5**：`SelTemplateService.upsert` 内联全限定名 `com.cpq.basicdata.entity.ProductCategory.findById(...)` 改为顶部 import + 短名 `ProductCategory.findById(...)`。 | 涉及文件：`cpq-backend/src/main/java/com/cpq/{basicdata/entity/ProductCategory.java(新增requireDefaultId),customer/service/CustomerService.java(删私有方法+改调用),seltemplate/service/{EffectiveTemplateService.java(同上),SelTemplateService.java(import整理)},importsession/service/ImportSessionService.java(null守卫前置)}` + `cpq-backend/src/test/java/com/cpq/seltemplate/SelTemplateResourceTest.java`(+T7/T8)。 | **自检**：`./mvnw -q -DskipTests test-compile` BUILD SUCCESS 0 错误 ✅；未真跑 mvnw test / 未连库跑 Flyway(同上条记录约束，真跑验证留给合并阶段)。

---

[2026-07-16] task-0712 update-071501 前端 F1~F4 — 三模板统一到「客户产品分类」轴（前端交付）| worktree `task-0712-category-axis`(分支 `feat/task-0712-category-axis`)，承接后端 `a443153` | **F1** `CustomerManagement.tsx` 新建/编辑客户表单加「产品分类」Select（必填；新建默认预选 `name==='默认分类'`，处理 categories 异步加载竞态——两个 effect：drawer 打开时若已加载则预填，若未加载则等 categories 到位后补默认且不覆盖用户已选值；编辑回填 `editingCustomer.productCategoryId`）；列表/筛选不加（D5）；`industryService` 与「所属行业」字段原样保留（D8，不属本次轴）。`customerService.ts` 本身全用 `any` 无既有 `Customer`/`CreateCustomerRequest` 接口可加字段，故未新增类型（fronttask 假设的接口不存在，`any` 已透传 `productCategoryId`，功能不受影响）。 | **F2** `SelTemplateManagement.tsx` 换轴：`industryService`→`productCategoryService`，删 `RESERVED_INDUSTRIES`（`__DEFAULT__`/`__GLOBAL__` 哨兵，换轴后"默认分类"本身就是一条 `product_category` 记录承接兜底角色），`industryCode`→`productCategoryId` 贯穿下拉/列/upsert/回填/`categoryNameMap`/`usedCategoryIds`，文案「归属行业」→「产品分类」。`selTemplateService.ts` 新增 `SelTemplateUpsertRequest` 类型（原 `upsert(data:any)` 弱类型，顺手类型化）。 | **F3** 报价单创建改「客户带出 + 只读锁定」：`BasicDataImportV5ToQuotation.tsx`(customers map 带 `productCategoryId`) → `BasicDataImportV5Wizard.tsx`(customers prop 类型加字段，Step3 传 `lockedCategoryId`) → `QuotationCreateForm.tsx`(加 `lockedCategoryId` prop；删"自动预选默认分类"逻辑；分类 Select 锁定态 `disabled={readOnly||!!lockedCategoryId}` 只读展示、去 `onChange`/`allowClear`；tooltip 换文案）。**关键设计决策（超出 fronttask.md 字面 3 文件范围，判断后主动补齐）**：`grep -rn QuotationCreateForm src/` 发现实际有 **3 处**调用点而非 fronttask 假设的 1 处——另 2 处是 `QuoteBasicDataImportV6Drawer.tsx`（`QuotationList.tsx`"基础数据导入"入口）与 `QuotationWizard.tsx`（`/quotations/new` 手动新建入口，路由已验证）。若不处理，这两个真实存在的报价单创建入口会遗留"仍可手选产品分类"的缺口，直接违背本任务"统一到客户产品分类轴"的目标。**已同样贯穿 `lockedCategoryId`**：`QuoteBasicDataImportV6Drawer.tsx` 的 `customers` map 补 `productCategoryId` + 新增 `customerCategoryId` memo 传入；`QuotationWizard.tsx` 的 `selectedCustomer`（`customerService.getById` 返回，后端已含该字段）直接取值，但**做了 readOnly 互斥防护**——该组件调用点同时服务"新建"(`quotationId` 空)与"编辑已有单"(`readOnly=!!quotationId`)两态，若无脑透传会让"编辑已有单"场景的分类被客户*当前*绑定值覆盖，与既有"从已存模板反查回填"逻辑赛跑，且违反 D4（客户改绑分类不追溯已有报价单）；修法：调用侧 `lockedCategoryId={quotationId?undefined:selectedCustomer.productCategoryId}` + `QuotationCreateForm` 内部 sync effect 加 `!readOnly` 二次防护（双保险）。 | **F4** `types/configure.ts` 的 `EffectiveTemplateDTO.resolvedIndustryCode`→`resolvedCategoryId`；全仓 grep 确认无运行时读取该字段的逻辑（仅类型定义+透传），`ConfigureProductDrawer.tsx`/`AddPartSubDrawer.tsx` 未消费，未改动（回归对象）。 | 涉及文件：`cpq-frontend/src/pages/{customer/CustomerManagement.tsx, config/SelTemplateManagement.tsx, quotation/{QuotationCreateForm.tsx,BasicDataImportV5Wizard.tsx,BasicDataImportV5ToQuotation.tsx,QuoteBasicDataImportV6Drawer.tsx,QuotationWizard.tsx}}` + `cpq-frontend/src/{services/selTemplateService.ts,types/configure.ts}`。 | **自检**：`npx tsc --noEmit` 0 错误 ✅；`grep -a industryCode\|resolvedIndustryCode\|RESERVED_INDUSTRIES` 对 F2/F3 目标文件净（`CustomerManagement.tsx` 残留 3 处 `industryCode` 为 D8 保留的「所属行业」字段，非本次轴，非回归）✅；全仓 `resolvedIndustryCode` 0 命中 ✅；临时 vite(5199) 对全部 9 个改动文件 + 主入口 curl 200 ✅。**未做**：真实点击回归（客户表单/报价单创建/选配抽屉）——依赖后端迁移落库，留合并阶段由技术总监验证；`ConfigureProductDrawer.tsx` 内一处文案"该客户所属**行业**或默认模板"（L327）换轴后属术语过期但未改字（任务明确该文件"不改逻辑"边界内不含此文案调整，留 follow-up）。commit `8fd2a7d`。

---

[2026-07-16] task-0712 update-071501 前端评审必修 3 项（消除种子 effect 竞态）| worktree `task-0712-category-axis`(分支 `feat/task-0712-category-axis`)，承接前一条前端交付记录 | **必修1** `QuotationCreateForm.tsx`：name 种子 effect 与 `lockedCategoryId` 种子 effect 原本分开写，挂载同批次各基于 stale `value` 调一次整对象 `onChange`，后者用 stale value 展开覆盖前者的 patch —— 换轴后 `lockedCategoryId` 主流程恒存在，稳定复现"自动报价单名被清空"。合并为单一 effect 一次性 `patch` 对象、一次 `onChange`。 | **必修2** `CustomerManagement.tsx`：删除挂载 effect 里与补偿 effect 重复的默认分类种子逻辑。**复核环节发现机械删除会引入新的时序 bug**（协调者的原始修法未预见）：补偿 effect（第二个 `useEffect`）与挂载 effect（第一个）在 `drawerOpen` 同一次变更时于同一 effect-flush 批次内均触发，但挂载 effect 里的 `resetFields()` 走 `setTimeout(...,50)` 延后执行；若补偿 effect 同步读写（无自身延时），会在"categories 已预加载"这一最常见路径下**先于** `resetFields()` 写入默认分类值，随后被 50ms 后触发的 `resetFields()` 连同其它字段一并清空——默认分类种子 100% 被吞、从不生效。修法：给补偿 effect 包一层 `setTimeout(...,60)`（显式晚于 50ms），确保先看到 reset 后的干净表单状态再判断是否需要种默认值，`!current` 守卫覆盖"预加载"与"后到达"两种时序且不覆盖用户已选值。 | **必修3** `ConfigureProductDrawer.tsx:327` 空态文案"该客户所属**行业**或默认模板"→"该客户所属**产品分类**或**默认分类**"，仅文案不改逻辑。 | 涉及文件：`cpq-frontend/src/pages/{quotation/{QuotationCreateForm.tsx,ConfigureProductDrawer.tsx},customer/CustomerManagement.tsx}`。 | **自检**：`npx tsc --noEmit` 0 错误 ✅；临时 vite(5199) 对 3 个改动文件 + 主入口 curl 200 ✅；必修2 的竞态判断为代码走查推导（effect 执行顺序 + setTimeout 调度时序分析），**非运行时复现**，留合并阶段人工点击回归确认。commit `c39bb6f`。
