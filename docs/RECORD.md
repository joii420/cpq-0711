...
## [2026-08-07] task-0806 B18 —— FR-5「10 处」清单漏第 11 处：`BomTreeRenderService` 主树 driver 组件查询 | `BomTreeRenderService.java:14-16,79-80,215-247`

**背景**：FR-5 要求渲染期活表读全部收口到 `PublishedTemplateReader`，原清单 10 处（`CardSnapshotService` 5 /
`ConfigureSnapshotService` 1 / `ExcelViewService` 4）均已完成（B8）。B17 排查时发现清单漏了第 11 处：
`BomTreeRenderService.render()` 内部 `②-pre` 步骤查「模板 driver 组件清单」的 native SQL，与已收口的 5 处
同形（`SELECT DISTINCT c.id, c.bom_recursive_expand FROM template_component tc JOIN component c ...`），
注释自己写着「照抄 `CardSnapshotService#expandTemplateDriverBaseRows` 的既有查询」却没被列进清单。它在
核价树 + 报价树渲染主路径上，决定「模板有哪些 driver 组件 / 哪个是主树组件」。

**修法**（与 `ConfigureSnapshotService#loadDriverComponents`（B8）同款分支写法）：
- `Template.findById(templateId)` 取状态；`PUBLISHED`/`ARCHIVED` → 走
  `publishedTemplateReader.driverCompsOf(templateId)`，用 `Set<UUID> seen` 去重（同旧 SQL `DISTINCT c.id`
  语义），结果适配回 `Object[]{componentId, bomRecursiveExpand}` 形状，下方 `§④` 消费 `driverComps` 的
  既有循环体零改动；`DRAFT`（或模板查不到）→ 保留原 native SQL 活表查询（§5.1.2 冻结边界：DRAFT 不写
  快照，草稿期一律走活表）。
- 新增 `LOG.debugf("[task-0806 B18] driverComps via ...")` 标注实际走哪条分支，供端到端实证。

**端到端验证**（不是代码走读）：真实 PUBLISHED 核价模板 `bc99f083-...`（18 tab，17 个 driver 组件）+
真实 SUBMITTED 报价单 `ef1d779b-...`，走 `POST /api/cpq/admin/quotations/task0729-costing-whole-refresh-verify`
（内部经 `CardSnapshotService.refreshCostingCardValues` → `BomTreeRenderService.render`）：
- 命中证据：dev 日志 `[task-0806 B18] driverComps via PublishedTemplateReader templateId=bc99f083-...
  status=PUBLISHED count=17`；
- 值中性：改动前/改动后同一单同一行两次调用该端点，`quotation_line_item.costing_card_values` /
  `costing_excel_values` 的 md5 逐位相同（`4296ca1f763db51de65a05ce80850011` /
  `802be7411bd5b3c204e39b35e913908d`）；
- SQL 级交叉核对：`template_component ⋈ component`（活表）与 `template_component_snapshot`（快照）对
  该模板的 driver 组件结果集（id + bom_recursive_expand）逐行 diff 为空（17/17 一致，无漂移）。
- DRAFT 分支：库内当前无「DRAFT 模板 + BOM 树页签组件」的真实组合可端到端触发（且
  `CardSnapshotService.templateHasTreeTab` 本身基于 `hasRecursiveExpand`（B8 已收口 Reader），DRAFT 模板
  无快照行时天然短路返回 false，使得核价侧这条调用路径对 DRAFT 模板不会走到 `render()`）；改动后的
  DRAFT 分支 SQL 文本与改动前逐字节不变（`git diff` 确认），且与 B8 已验证过的同款分支模式结构一致，
  未做额外的活表侵入式实证（评估：分支代码零改动，实证边际收益低于在共享 dev 库搭建全套合成夹具的
  操作风险，故未做，如实记录该限制）。

**顺带发现（未处理，仅登记）**：`grep "JOIN component c"` 扫出另外 3 处疑似同类活表读，均**不在本次范围
内**，未改动：
- `CostingVersionService.java:266-274`（`driverComponentIdsOf`，供版本切换 `affectedTabs` 用）；
- `QuotationTreeService.java:72-94`（`loadTemplateComponents`，报价侧 BOM 树增删节点读组件元数据用）；
- `CostingBomTreeConfigService.java:172-183`（`invalidateTreeTabCostingCardValues`，递归 SQL 配置变更后
  失效存量核价卡片值缓存用，风险相对低——只影响缓存失效范围，不直接决定渲染输出）。
- `TemplateService.java:984` **不算** —— 那是 `publish()` 建快照本身的写路径，读活表天经地义。

**自检**：`./mvnw -o compile -DskipTests` 通过 ✅；`grep -an "JOIN component c" BomTreeRenderService.java`
只剩 DRAFT 分支那一处 ✅；N+1 自检——本次改动 0 处循环新增查库（`driverCompsOf` 委派 `allTabsOf` 单次
`IN`/等值查询，`seen` 去重是纯内存 `Set`操作）✅。
