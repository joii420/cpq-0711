...
[2026-06-17] 报价导入 - §5 元素回收折扣 投入料号空→按名匹配(只匹配不生成,更新型) | Q05ElementRecoveryHandler.java(+import MaterialNoResolver, +@Inject materialNoResolver, +BatchState batch, 替换料号解析块) / Q05ElementRecoveryResolveTest.java(新建,2测试) | 关键决策: §5 是唯一更新型 sheet,调 resolveMatchOnly 而非 resolve,绝不生成 9 字头料号(生成后 UPDATE 匹配 0 行毫无意义); 料号空+名称有值→按名查 material_master 命中返料号,未命中→recordError("投入料号为空，且按名称[xxx]在料号表查无对应料号")跳过,不注册; BatchState 贯穿整批降低重复查询; row.exact("投入料号名称") 避免 getStr contains 匹配误命中「投入料号」列; UPDATE 查询体(2键+is_current=TRUE)不变; Q05*Test 共 4 passed(HandlerTest 2 + ResolveTest 2); commit 036d1a0; spec review 独立通过(无生成无注册无 scope creep 确认)。
