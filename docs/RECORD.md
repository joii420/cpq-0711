...
[2026-08-03] task-0729 阻断修复：versions/generate supersede 400——HQL `now()` 在 Hibernate 6 类型推断失败 | 改动 `MaterialPriceUpdateJobItem.java`（`staleAllUnfinishedByJobIds` 的 `updatedAt = now()` 改绑定真实 `OffsetDateTime.now()` 参数）+ `QuotationAdminResource.java`（新增临时验证端点 `task0729-stale-mark-verify`）| **阻断现象**：`POST /price-adjust/versions/generate {confirmSupersede:true}` 稳定 400，`SemanticException: Cannot assign expression of type 'java.lang.Object' to target path 'alias_0.updatedAt'` | **根因**：HQL 函数 `now()` 在 Hibernate 6 返回类型推断为 `java.lang.Object`，赋值给 `OffsetDateTime` 字段语义校验直接拒绝 | **时间线核实**（回应"是否75文件同步引入"的怀疑）：直接查库确认当前 job 名下28个 job_item 全部已是终态，`staleAllUnfinishedByJobIds` 的 `if(jobIds.isEmpty())` 提前返回只看"该版本名下是否存在job"不看"job内是否有未完成项"——只要 pending 版本名下曾有过 job，这行 HQL 就会被编译执行。这是 B3 阶段既有代码（worktree/主仓75文件diff核对时逐字节一致，早于本次修复），不是同步引入的新内容；此前两次"成功" supersede 极可能发生在待 superseded 版本名下还没有任何 job 的窗口期，该 HQL 从未被真正编译执行过 | **意外抓到一次真实并发覆盖**：修复编译时报 `ComponentService.suggestElementBinding` 找不到，排查发现该文件在两次 diff 核对之间被某并发写入覆盖为旧版本（非我操作导致）——重新全量 diff 核对 75 文件发现 1 个 mismatch 并修复，证明主仓工作目录在调查期间存在其他会话并发写入 | **验证**：真实 `POST /price-adjust/versions/generate` supersede 成功（V26080302→V26080303），V26080302 正确转 SUPERSEDED；STALE 标记单独隔离验证（WAITING job_item → STALE + updated_at 正确写入真实时间），确认修复未影响 STALE 标记语义 | 涉及文件：`MaterialPriceUpdateJobItem.java`/`QuotationAdminResource.java`

---

[2026-08-03] task-0729 BomTreeRenderService 真根因修复落地——DEBUG日志锁死+全量验证 | 改动 `BomTreeRenderService.java`（`renderInternal` 内查 `Quotation.customerId` 传入 `expandUncached`，替换此前恒传的 null；保留 coordinator 已做的 `QuotationIdContext` 保存-恢复重入修复）+ `QuotationAdminResource.java`（2个新临时验证端点）| **三版假设演进**：①"BomTreeRenderService设计传null"→②"核价侧4条路径遗漏QuotationIdContext.set()"（coordinator用"有保护"路径复测证伪）→③**真根因**（临时DEBUG日志在`SqlViewExecutor.executeAllRows`渲染现场锁死，非推断）：`:customerCode`解析走`DataLoader.loadByPath`里`new RuntimeContext.QuotationContext(quotIdSv, customerId)`，`quotIdSv`确实来自`QuotationIdContext`（这是②看似合理的原因——priceBaseDate确实靠它算对了），但同一行的`customerId`是从`expand()/expandUncached()`透传下来的独立方法参数，**没有任何ThreadLocal兜底**，`BomTreeRenderService`§④调`expandUncached(compId, null)`时就是字面意义的null | **修法**：`renderInternal()`用`lineItems.get(0).quotationId`查一次`Quotation.customerId`，传给`expandUncached`替代null；`partNo/lineItemId/partVersion`继续传null不变；`expandUncached`本就绕开`expandCache`，不产生AP-37缓存串号风险 | **全量验证**（QT-20260726-0018/S-3120014539，探针清理完毕）：`refreshCostingCardValuesForLine`(B0 S5)✅Cu null→3650与报价侧逐位相同(验收#64核心断言满足)；`snapshotLineValuesWithUnion→snapshotCostingSideOnly`(加产品单行)✅同上；`refreshCostingCardValues`(整单批量刷新)✅同上；`CostingVersionService.switchVersion`(task-0713核价版本切换)⚠️**未做完整live-fire验证**（真实PENDING核价单有并发Playwright测试不便mutate+隔离环境需先materialize该行才有version-options候选，构造成本超预算，仅code-level确认调用同一个3参render()重载）；报价侧`ConfigureSnapshotService.snapshotLines`"有保护"路径回归确认✅隔离单外购件成本/加工费仍各1行未变，证明嵌套QuotationIdContext保存-恢复无副作用 | 隔离测试数据（TEST-0729-TREEVERIFY/TEST-0729-SWITCHVER 及其quotation/line item/costing_order）全部验证完已DELETE清理，CUST-0001真实数据核对未受影响 | 涉及文件：`BomTreeRenderService.java`/`QuotationAdminResource.java`/评估文档

---

[2026-08-03] task-0729 BomTreeRenderService §3.3矛盾验证解开——真根因是QuotationIdContext遗漏 | 更新`dev-docs/task-260729-客户价格调整策略和价格版本/BomTreeRenderService-customerId影响面评估.md`，`BomTreeRenderService.java`全程零改动 | **验证**：隔离测试customer(TEST-0729-TREEVERIFY)+独立material_bom_item(OUTSOURCED)/unit_price(PROCESS)+全新报价单/行项(quote_card_values插入时NULL确认首次创建态)+调用真实`ConfigureSnapshotService.snapshotQuotation`生产方法→外购件成本/加工费均1行非0行 | **真根因**（比原评估的"customerId设计传null"更精确）：`:customerCode`真正来源是`QuotationIdContext`(ThreadLocal)→DataLoader→`RuntimeContext.toNamedParams()`独立管线（`SqlViewExecutor.enrichPriceBaseDate`的javadoc早写明，第一版评估读代码时漏看），非`expandUncached()`那个被硬传null的customerId参数。逐个核实全部6个直接触发`render()`的方法：`ConfigureSnapshotService.snapshotLines`(saveDraft/选配/V6导入)+`CardSnapshotService.snapshotNewLinesCardValues`(ensureCardValues新行)两个"有保护"（都设了`QuotationIdContext.set`）；`snapshotCostingSideOnly`(加产品单行)+`refreshCostingCardValues`(整单刷新/复制单)+`refreshCostingCardValuesForLine`(task-0729 B0 S5，本次wl_ys_bom_view null就是从这条实测出来)+`CostingVersionService.switchVersion`(task-0713核价版本切换)四个"遗漏"，**全部是核价侧**——是核价侧4个方法遗漏了报价侧都有的`QuotationIdContext.set()`调用，是遗漏不是设计 | **推荐方案修正**：原"扩展`BomTreeVarsContext`携带customerId"地基不对（该context从不参与`:customerCode`解析）。新推荐=在`BomTreeRenderService.render()`方法体最前面统一补`QuotationIdContext.set(quotationId)`+`finally clear()`，一次改动覆盖全部6个调用方，只加两行代码；风险点=与调用方已有的set/clear可能有嵌套/重入语义问题，实施前需先读该类源码确认 | 测试数据6张表探针已全部DELETE清理，CUST-0001真实50张单计数核对未受影响 | 🔒 `BomTreeRenderService.java`等coordinator确认新方案后再改

---

[2026-08-03] task-0729 §2/§3 信封修正 + BomTreeRenderService customerId 影响面评估（不改代码）| **①信封**：`PriceAdjustReviewResource`(api.md §2，屏3/4/5)+`PriceAdjustJobResource`(api.md §3，屏6)同B3/B6问题一并改裸DTO，真实数据验证`GET /price-adjust/reviews`+`GET /price-adjust/jobs`裸体200 | **②影响面评估**：产出`dev-docs/task-260729-客户价格调整策略和价格版本/BomTreeRenderService-customerId影响面评估.md`，全程只读不改代码。核心发现：①7个直接调用方追到7条真实业务场景（saveDraft/详情页打开/选配加产品/V6导入/核价冻结提交/核价版本切换/报价单复制），且task-0729自己的B0升版流程也在用，是渲染主链路公共基础设施；②"customerId/partNo/lineItemId全传null"的注释理由只覆盖partNo/lineItemId维度（整单只跑一次+避免expandUncached缓存串号），没有视图依赖customerCode必须为null；③逐视图排查11个树页签模板下40个组件：核价侧只有COMP-0049（本次已修复）受影响，报价侧wg_view/jg_view(外购件成本/加工费)用customer_no做硬WHERE过滤理论上应结构性0行，但实测5张真实单普遍1行——**如实标注矛盾未100%闭环**，最支持假设是saveDraft"已有快照整行跳过expand"优化让存量行从未重新走过这条路径，给出不需要动代码的验证步骤；④推荐方案=扩展`BomTreeVarsContext`携带customerId/priceBaseDate（复用既有"树渲染下沉全局上下文"机制），不用expand换expandUncached（会重引入AP-37型缓存串号）| 🔒 **`BomTreeRenderService.java` 全程零改动，等coordinator确认后再动** | 涉及文件：`PriceAdjustReviewResource.java`/`PriceAdjustJobResource.java` + 新增评估文档

---

[2026-08-03] task-0729 核价 wl_ys_bom_view 元素单价 JOIN 键归一到销售料号（+ 发现独立更深层架构缺口）| 新增 `V373`+`V374`（V373 用 LATERAL 实现但通过应用真实链路验证元素单价仍为null，V374 跟进改用不含LATERAL的等价子查询写法，验证通过）| **背景**：coordinator 派 B2 时指令是"补 `AND cep.material_no=ebi.material_no`（平铺形态）"，coordinator 自述已确认这是指令判断失误——没意识到核价侧 `element_bom_item.material_no` 是「颗粒物料」（如 S-2120011658，被多个不同成品复用的共享子件配方标识，本库实测同时是 S-3120014539 与 S-3120018220 的子件），而 `material_price_version_ref` 指针只挂「销售/成品料号」，两个轴根本不是同一料号空间，JOIN 100%不命中（非偶发）| **修法**：仿报价侧 `mc_view` 的 `bom_closure` 手法加独立递归CTE（walk `material_bom_item` system_type='PRICING'+customer_no='_GLOBAL_'），只改价格JOIN键为归一后根料号，**hf_part_no输出保持不变**（用冻结数据实证运行时driver注入的过滤集合是颗粒物料集合非销售料号，若一起改会让驱动过滤条件对不上、行数归零，比现在更糟）| **多根共享场景验证**：真实数据里 S-2120011658/S-2120011659 同时被 S-3120014539 与 S-3120018220 引用，用 `ORDER BY (lvl=0?1:0), lvl, root_no LIMIT 1` 的确定性子查询取"层级最浅+字典序"单一候选根，避免LEFT JOIN笛卡尔重复行；已知限制：这只是确定性而非"当前渲染上下文精确对应"的根，真正精确解需运行时把"当前行属于哪个成品"当参数传入价格JOIN（类似`costing_bom_tree_config`的`:production_part_nos`机制），超出本次热修复范围，已如实标注 | **LATERAL踩坑**：V373直接psql验证正确（14行→14行，Cu=3650），但通过应用真实SQL编译层（`SqlViewExecutor`）跑`expand-driver`验证时仍null，怀疑LATERAL在自定义SQL改写层不受支持（未展开排查根因），V374改用相关子查询写在JOIN...ON条件里（不含LATERAL关键字）后验证通过 | ✅ **真实数据验证（POST /components/{id}/expand-driver，customerId=CUST-0001）**：Cu在S-2120011658/S-2120011659上正确归一到根S-3120014539解出3650.0（与报价侧升版后数值一致，验收#64核心断言满足）；S-3110520789归一到另一不同根S-3111320636解出Ag=217520.0/Cu=6290.0（多根场景验证正确、无串价）| 🔴 **发现独立更深层架构缺口（未修，已如实记账留给coordinator裁定）**：通过`CardSnapshotService#refreshCostingCardValuesForLine`刷新QT-20260726-0018真实`costing_card_values`后元素单价仍为null——定位到该核价模板含树页签组件（COMP-0048 `bom_recursive_expand=true`）→ 走`BomTreeRenderService.render`渲染路径→对同卡非树兄弟页签（如COMP-0049）调`componentDriverService.expandUncached(compId, null)`时`customerId`/`partVersion`/`lineItemId`/`partNo`**全部显式传null**（`BomTreeRenderService.java:210`附近注释：设计上让`$view`只靠`BomTreeVarsContext`的`:total_material_no`收窄，不走customerId维度）——任何需要`:customerCode`的`$view`挂在含树页签的核价模板下，价格JOIN从一开始（甚至早于V366）就永远拿不到customerCode，与本次JOIN键修复无关、也非task-0729引入。已用diff核实两次刷新调用对QT-20260726-0018的costing_card_values**零净改动**（完全幂等，未污染真实数据）| 涉及文件：`V373`/`V374` migration + `QuotationAdminResource.java`（新增临时验证端点 `task0729-costing-view-fix-preview`）

---

[2026-08-03] task-0729 策略CRUD补交（api.md §1.1-1.7）+ 响应信封全线修正 | 新增 `PriceAdjustStrategyResource.java`+`PriceAdjustStrategyService.java`（7端点：GET/PUT strategies/{customerNo}、GET/PUT .../materials、GET/PUT .../elements、GET .../logs）+ 10个DTO + 2个409确认异常（`MaterialRemovalNeedsConfirmException`/`ElementUnselectNeedsConfirmException`）| **背景**：coordinator backtask 划分 B1~B12 时漏了"策略CRUD"这块（B6只写了"比对列配置CRUD"），`com.cpq.priceadjust.resource` 从未有 Strategy 资源类，屏1入口对前端 404 | **意外发现并顺带修正（不修屏1仍断）**：curl 实测已上线的 B3 `PriceAdjustVersionResource`/B6 `PriceAdjustComparisonColumnResource`，发现均被 `ApiResponse.success()` 包了 `{code,message,data}` 信封，但 api.md §0.2 明确"沿用 `/api/cpq/element-price/*` 风格不另造包装层"（核实参照点 `StrategyResource.java` 确认裸DTO），前端 `priceAdjustService.ts`+`api.ts` 拦截器成功分支只做一层 `response.data` 解包——两个已上线端点组同样会让前端读到 `undefined`，与"策略CRUD缺失"同性质，一并改为裸DTO返回（错误响应 `ApiResponse.error` 保持不动，`buildApiError` 专读 `.data.data` 这条本来就对）| **§1.4/§1.6 二次确认设计**：移出料号/取消勾选元素未带 confirm 标记 → 409 + 影响面预览（`pendingReviewCount`/`unlockedQuotationCount`），沿用 `PendingVersionExistsException` 既有异常模式；确认后料号的 PENDING 审核记录置 VOIDED（APPROVED 不回滚），元素清单变化不联动审核记录（api.md 服务端行为条款没提，未臆造）| **§1.5 pivot 硬约束**：一次 IN 查询取回全部版本×元素（禁止逐元素查10次）；`NOT_IN_LIST`（该版未在清单，无 `element_price_version_item` 行）vs `NO_PRICE`（在清单但 `currentPrice` 为 null）两种空值语义靠行存在性+字段值双重判断分开，非猜测 | **重算范围差异化设计（如实标注推理依据）**：§1.2 策略主体变化 + §1.6 元素清单变化 → 该客户全部 PENDING 料号（元素变化影响面广，与策略同口径）；§1.4 料号矩阵变化 → 只对新增部分调 B4 `processMaterial`（复用其 pool-entry 判定，未变动料号不进池，同裁决39）| 验证（`cpq_db_0724` 真实数据，探针测完已删，测试工程师 CUST-0001 数据全程未碰）：GET elements pivot 对 CUST-0001 keyword=Cu 命中 unitPrice=3650.000000/1850.000000（与 coordinator 报告的"Cu 1850→3650"吻合）；隔离测试客户 TEST-0729-STRATEGY 全流程 PUT策略/PUT elements/PUT materials 的成功+409确认+确认后成功三态全验证，logs 5条审计记录 changeType/字段全核对；测后 DELETE 清理 + CUST-0001 逐字段核对未受影响 | 已知限制：`unlockedQuotationCount`（元素侧）用 `row_data::text ILIKE` 近似查询非精确定位（仅供确认弹窗预估，不影响写库判定）；§2/§3（Review/Job Resource，屏3-6）同款信封问题未修，记账留待 coordinator 决定 | 涉及文件：`priceadjust/{resource,service,dto,exception}/*` 新增18处 + `GlobalExceptionMapper.java`/`CustomerPriceAdjustStrategyLog.java`/`PriceAdjustVersionResource.java`/`PriceAdjustComparisonColumnResource.java` 修改

---

[2026-08-02] task-0729 B11 · 批量调价完成通知（复用 NotificationService）| 新增 `PriceAdjustNotificationService.java`（`notifyJobCompletion(jobId)`）+ 接入 `PriceAdjustJobExecutionService#finalizeJob` 末尾（非阻断 try/catch）+ 新增 `V371__task0729_notification_type_expand.sql` + `QuotationAdminResource` 加临时验证端点 `POST /admin/quotations/task0729-b11-notify-preview` | **两类收件人**：①触发批次财务人员（`job.triggeredBy`）——批次汇总，success/failed/conflict/stale 计数全量透出；②受影响报价单销售负责人（`quotation.salesRepId`）——按报价单聚合（同单多料号行只发一条），含"此前手工调整可能已被覆盖，请复核"，若该单同批次有失败/冲突项同一条里一并说明（"失败项两边都必须可见"）；销售只收自己名下报价单通知；STALE 项不计入销售侧聚合（等同未处理，不构成复核事件）| **幂等**：复用 `MaterialPriceUpdateJob.notified`（建表时预留但此前从未被写入的字段）——只在批次首次终态落库时通知一次，retry 触发的后续 `finalizeJob` 不重复通知（无更细粒度规格时的取舍，已在类注释写明，如需"重试后结果有变化应再通知"需另行调整）| **踩坑**：未提前查 `notification` 表既有 `chk_notification_type` CHECK 约束就写代码，首次真实调用直接被拒（约束原白名单仅 `APPROVAL_*`/`PASSWORD_RESET`/`ROLE_CHANGED`/`SYSTEM` 7 个值），补 V371 扩展白名单加 `PRICE_ADJUST_JOB_SUMMARY`/`PRICE_ADJUST_QUOTATION_REVIEW`；诊断手法：dev server 无独立日志文件，通过 `readlink /proc/<pid>/fd/1` 找到其 stdout 被重定向到的临时文件（另一并发会话启动时留下的路径）才拿到完整堆栈 | **不发明前端路由**：批次汇总通知 `link` 留空（"更新任务页"前端路由尚未接入 `router/index.tsx`，grep 确认过不存在），报价单通知 `link` 用已确认存在的 `/quotations/:id`（`router/index.tsx:111`）| **真实数据验证**（`cpq_db_0724`，探针数据验完已删除）：手工插入 1 个 job + 4 个 item（2 张真实报价单 QT-20260726-0001/QT-20260728-0022，覆盖 success-only / mixed success+failed / STALE 排除 三分支），调用 notify-preview 端点 → 3 条 notification 精确落库（财务1条含 stale 计数 + 2 张报价单各1条，QT-20260728-0022 的 STALE 项未计入"1个料号行"文案，证明排除逻辑生效）；二次调用验证幂等（notification 计数仍为3，未重复）；`job.notified` 正确翻 true；清理后 SQL 复核 `job_left=0/items_left=0/notif_left=0` | 验收：真实 HTTP 端到端（非仅单元测试）；`NotificationResourceTest` 跑测试连的是 `cpq_db`（非 `cpq_db_0724`），6 个用例里 5 个在 `login()` 步骤即 500——已核实 V371 在 `cpq_db` 上同样 `success=t`，login 500 与本次改动无关，是已知的 `cpq_db`/`cpq_db_0724` 环境分歧（历史已多次记录），非本次引入的回归 | 已知限制：`finalizeJob→notifyJobCompletion` 的自动接入点本身未跑全流程真实 `approve()` 触发（需真实 PENDING+BUDGET_READY review 前置数据，超出本轮验证成本），验证的是同一方法体在 admin 端点下的真实执行，接入代码仅3行且已人工复核

---

[2026-08-02] task-0729 B9 · 双端公式一致性黄金用例集 L1（后端侧交付）| 新增 `formula-golden/*.json`（仓库根，10 类目 33 用例）+ `formula-golden/README.md`（契约文档）+ `cpq-backend/src/test/java/com/cpq/quotation/service/FormulaGoldenTest.java`（`@TestFactory` 动态读 JSON） | **动机**：立项时实测前端 `formulaEngine.test.ts`（756行）与后端 `FormulaCalculatorTest.java`（465行）**零共享用例**——两端各自"绿"从未验证过同一输入同一输出。本轮建立仓库根共享 JSON 契约，双端读同一份，不再各自维护副本 | **反向验证（核心断言，非可选项，仿 B4/B6 手法）**：临时把 `FormulaCalculator.java` 的 `component_subtotal` 精确键匹配从 `compCode+"#"+colName` 颠倒为 `colName+"#"+compCode`，5 个用例（cs1-001/cs2-001/cs2-002/amt-001/amt-003）应声变红；`git diff --stat` 清零式还原后 33/33 复绿——证明这套用例真有拦截力，不是摆设 | **意外发现并记账（不在本任务范围内修正）**：编 `10-decimal-precision.json` 时按 `FormulaCalculator.java` 类注释（"4 位小数 HALF_UP"）手推 `expected`，3/4 用例实测失败（`dec-001` 10/3 期望 3.3333 实得 3.333333333333 等）。定位根因：**该类注释已过期**——并发的 task-0801 已把 `evaluateExpression` 改造为"计算精度与呈现精度分离"（`PrecisionPolicy.java` 类注释），内部不再 `setScale(4,HALF_UP)`，除法中间精度改用 `PrecisionPolicy.DIVISION_SCALE=12` 位 HALF_UP，其余运算不舍入，最终 6 位展示精度由调用方在落库/API/显示/导出边界统一规整。已按实测真实行为更正 4 条 `expected`+`notes`（不是掩盖旧文档，是让 `manual-computed` 名副其实），`FormulaCalculator.java` 第25行类注释本身未同步，跨任务边界不予修正，仅在 `formula-golden/README.md`"重要发现"章节 + 本条 RECORD 记账 | **对后续的信号**：若前端 `formulaEngine.ts` 除法仍按 4 位近似（历史文档一贯如此），补 `expectedSource:frontend-engine` 时 `dec-001/002/003/004` 可能暴露真实两端数值不一致——这正是 B9 要捕获的信号，不代表用例写错，需前端/协调方确认"最终一致性该在哪层达成" | **本轮交付边界（如实说明）**：仅后端侧（JSON骨架+JUnit读取器+反向验证），33 条全部 `expectedSource=manual-computed`（后端工程师按现有代码行为独立推导，未对照前端引擎实跑产出），不等价于两端一致性已验证；`frontend-engine` 权威值待前端后续补齐。`06-cross-tab-ref.json` 仅覆盖 KSUM 基础匹配+空集 I-1+COUNT，未覆盖 KAVG/KMAX/KMIN 空集分支（I-2）、多 source 广播、SUMIF predicate，留给前端补充 | 验收：`mvn -Dtest=FormulaGoldenTest test` 33/33 全绿；反向验证 5 红→还原→33 绿 | 注意事项：`mvn test` 连的是 `cpq_db`（非 dev 的 `cpq_db_0724`），本类零 DB 依赖（纯内存 `RowContext`），不受该分歧影响

---

[2026-08-01] 报价导入「物料BOM」两列表头别名兼容 —— 修「材料占比整列静默丢数」 | 改动文件：`basicdata/v6/quote/MaterialBomMergeHandler.java`（新增 `MATERIAL_RATIO_HEADERS={材质占比,材料占比}` / `USAGE_TYPE_HEADERS={产出料号类型,产出类型}` 两组别名常量，两处读列改走常量）/ 测试 `MaterialBomMergeHandlerTest`（+3 用例，`bomRowWithRatio` 抽成可指定表头的 `bomRowWithHeaders`）/ `docs/table/报价系统Excel导入落库方案.md` §物料BOM（两列标注别名 + 新增「表头别名」小节）/ `dev-docs/rule-260724-组件模板配置/报价侧.md` §7.2.1 §7.2.2 | **零 DDL、零 Flyway、无 field_type 改动（AP-44 不触发）** | **根因**：V365 落地时导入器只认「材质占比」，而同一概念在组件卡片列名、SQL 视图输出别名 `_材料占比`、客户 xlsx 里都叫「材料占比」。`SheetRow.getStr` 是 contains 匹配，「材料占比」不含「材质占比」⇒ **静默按"没填"处理**：不报错、不计失败行、`successRows` 照常全绿，整列丢数。属 AP-52「语义错配 + 契约不对齐」族 | **A/B 实证（真实导入端点 `POST /basic-data-import/v6/quote` + `cpq_db_0724`，探针数据验完已按行还原/删除）**：同一份 5 行数据只改表头——表头写「材料占比」→ `material_ratio` **全 NULL**、表头写「材质占比」→ 落 `0.3/0.7/0.35`（材质行）+ 零件/外购件行 NULL（设计如此）；两份的「产出料号类型」都正常落 `银点类/非银点类/组成件`（前导"N."被 `labelOnly()` 剥离），**故产出类型的写入链路本来就是好的**，加别名只是防客户按卡片列名写「产出类型」时重蹈覆辙 | **同期澄清的两个非代码问题**（用户报"产出类型没值"的真实成因，不在本次代码改动范围）：① 仓库测试件 `报价系统模板0723.xlsx` 的「产出料号类型」**整列空白**，故 `cpq_db_0724` 里 QUOTE 侧 `component_usage_type` 172 行 100% NULL——数据侧没值，不是 SQL 绑错（§7.2.1 早有预警）；② `客户组件模板/SQL视图-施耐德-物料-bom_view.sql` 是**可编辑副本不是运行时来源**，本地库 4 个「物料」组件的 `component_sql_view.sql_template` 仍是旧 8 列版（无 `component_usage_type`/`material_ratio`），且组件字段「产出类型」「材料占比」「回收价格」「回收比例」的 `default_source` 均为 null（未绑定）——要出数须先把 SQL 粘回 UI/API + 绑字段，或导入 `组件模板-施耐德-v3.json`（该 bundle 里四个字段都已绑好） | 验收：`MaterialBomMergeHandlerTest` 15→18 用例，RED 阶段精确只挂 2 个别名用例（`expected: not <null>` / `expected: <银点类> but was: <null>`），实现后 18/18 全绿；合并后主仓 8081 热重载再跑一次「材料占比」真实导入，`material_ratio` 落值 | 注意事项：匹配仍是 contains 且**按列序取首个命中**，故一份 sheet 同时出现两种写法时以左列为准；今后该 sheet 不得再引入含「占比」「产出类型」字样的旁列（会被截胡）；核价侧 `P06MaterialBomHandler` 未动（它读「使用特性」，语义不同）

---

[2026-07-30] task-0730 报价导入「来料回收折扣」Sheet 扩列 —— 项次/值/货币/计价单位 4 列落库 + 组内 upsert | 改动文件：`basicdata/v6/quote/Q09IncomingRecoveryHandler.java`（CONTENT 由 `[cost_ratio]` 扩为 `[seq_no, cost_ratio, pricing_price, currency, unit]` + 4 列读取 + 必填其一兜底 + 组内去重末值胜）/ `basicdata/v6/quote/QuoteImportValidator.java`（`validateIncoming` 加 `requireValueOrRatio` 形参，仅「来料回收折扣」传 true）/ 测试 `Q09IncomingRecoveryHandlerTest`（+7 用例）+ `QuoteImportValidatorTest`（+3 用例）/ `docs/table/报价系统Excel导入落库方案.md`（V3.5→V3.6）| **零 DDL、零 Flyway**：`unit_price` 的 `seq_no/pricing_price/currency/unit` 四列早已存在（V220 建表，V276 把 `pricing_price` DROP NOT NULL），此前 Q09 只是没写 | **需求方三次澄清的最终口径**：①`项次→seq_no` **不必填、不补号**（空即 NULL，明确否决了"必填校验"与"按序自动补号"两个方案）；②「值」与「回收折扣（%）」**并存**（可同时有值，不是二选一互斥）但**必填其一**；③**项次为 NULL 触发唯一键时走 upsert 而非报错** | **关键实现点（漏一个就静默出错）**：(a) **CONTENT 必须同步扩列** —— `VersionedV6Writer` 的 `loadCurrentGroup`/`multisetEqual` 只看 `contentColumns`，而 `assembleRow` 是 `all.putAll(row)` 照单全写；漏加会造成"值写得进去、但只改值不改折扣% 时 `triggerSame && contentSame` 成立 → 直接 return 不写库"的**静默吞改动**（已加 `changePricingPriceOnly_bumps` 用例钉死）；(b) **组内去重键必须精确镜像 `COALESCE(seq_no,0)`** —— NULL 与 0 视为同一键，否则"项次留空多行"仍撞 uq（`uq_unit_price` 13 维含 `COALESCE(seq_no,0)`，V315）；(c) `VersionedV6Writer.insertRowsBatched` 是**裸 INSERT 无 ON CONFLICT**，故 upsert 语义只能在 handler 侧做，手法照搬核价侧 `IncomingOtherMergeHandler`（其 javadoc 已写明"同键取最后一行，对齐 EXCLUDED 覆盖语义"）；(d) 跨次导入不需处理——版本化写入器本身即"整组比对→复用/原地更新/升版"，天然 upsert，需去重的只有**同批次组内** | **顺带修正文档两处历史漂移**：§9 主料号列名仍写「宏丰料号」（模板 0723 早已改「销售料号」，代码 `getStr("销售料号","宏丰料号","成品料号")` 有回退兼容）；§9 顶部注仍是 2026-06-17 老规则（已被 update-0723 U10 取代：材质走 `material_recipe` 查无即报错，只有零件/外购件才发 9 字头号）| 验收：`Q09IncomingRecoveryHandlerTest` 9/9 + `QuoteImportValidatorTest` 8/8 + 相邻回归 `Q06/Q07/Q08` 7/7 全绿；`com.cpq.basicdata.v6.quote.*` 包全量通过 | 注意事项：**无 `field_type` 改动，AP-44 不触发**；`getStr("计价单位","单位")` 是 contains 匹配，表头写「计价单位」或「单位」均可命中，但今后该 sheet 不得再引入含「值」「单位」的旁列（会按列序被首个命中截胡）；**这条数据链路目前无下游消费**——全库 `component_sql_view` 无一引用 `回收折扣`/`RECYCLE`，老视图 `v_q_incoming_merged` 已不存在（V128 时代基于已废弃的 `mat_fee`，AP-53），要在页签上显示需另配 SQL 视图按 `(customer_no, finished_material_no, code)` LEFT JOIN；Excel 模板加列由业务方自行在 xlsx 完成（代码对列顺序不敏感）

---

[2026-07-29] task-0729 报价单产品分类持久化 — 杜绝编辑页「请选择产品分类」丢失 | 改动文件：新增 `db/migration/V364__quotation_product_category_id.sql`（只加 `quotation.product_category_id` 列）/ `quotation/entity/Quotation.java`（加 `productCategoryId`）/ `quotation/dto/QuotationDTO.java`（加 **`categoryId`** + `from()` 映射）/ `quotation/dto/CreateQuotationRequest.java` + `SaveDraftRequest.java`（各加 `categoryId`）/ `quotation/service/QuotationService.java`（`create` 无条件落库、`saveDraft` **仅非 null 才覆盖**）/ `basicdata/v6/service/V6QuotationCommitService.java`（导入路径 `cq.categoryId = req.categoryId` 透传，复用 create 的赋值）/ 前端 `QuotationWizard.tsx`（建单 + `buildDraftPayload` 两个 payload 补发 `categoryId`）/ `QuotationCreateForm.tsx`（无模板时文案指向真实成因）| **根因（实证三段断链，不是竞态、不是"存丢了"）**：产品分类在报价单链路里**从来不是数据**，只是前端用来匹配模板的一次性参数——① 新建路径前端 `handleCreateQuotation` 的 payload **压根没发** `categoryId`；② 导入路径前端发了、`CreateQuotationFromImportRequest.categoryId` 也接住了，但 `V6QuotationCommitService` **全文零处** `category`（`/usr/bin/grep -ain category` 退出码 1）；③ `quotation` 表没有对应列，`SaveDraftRequest` 里 `categoryId` 出现 0 次。全后端读 `.categoryId` 的只有 product 模块。⇒ 编辑态（`lockedCategoryId` 被有意置 undefined 守 D4）唯一来源退化为「从 `customer_template_id` 反查 `template.category_id`」，该字段为空即永久空白 | **实证影响面**：`cpq_db_0724` 全库 29 单中 **8 张 DRAFT 必现**（`customer_template_id IS NULL`），且因 `QuotationCreateForm:227` 校验含 `customerTemplateId`，这些单「下一步」同时被禁用——这连带修正了一条既有归因：E2E `quotation-flow` 恒失败此前记为"夹具单缺产品分类"，按代码真正的阻断项是**缺 `customer_template_id`**，分类空白只是同成因的伴随现象 | **附带修正的语义偏差**：反查读的是**模板当前的** `category_id`，模板改分类会让所有已建单显示跟着变，**违反它自己想守的 D4**（客户/模板改绑不追溯已有单）；持久化后偏差消失 | 关键决策：①**存量不处理**（业务方裁定）——迁移只加列不回填、不加外键不加 NOT NULL，那 8 张无模板单不管；②**模板反查保留为兜底**不删（存量单唯一显示来源），DTO 带值后 `QuotationCreateForm:126` 的 `value.categoryId` 守卫命中自然短路；③**DTO 字段名必须叫 `categoryId`** 而非 `productCategoryId`——`QuotationWizard:353` 的 `q.categoryId \|\| prev.categoryId` 是 2026-07-16 修分类竞态时预留的口子（注释明写"DTO 将来若真带 categoryId 仍以 DTO 优先"），名字对上则**前端回填逻辑零改动**；④后端**不做二次推导**（categoryId 为空时不去反查客户绑定），否则违反 D4 | 验收（技术总监亲验）：`POST /quotations` 带 `categoryId` → 库内 `product_category_id` 落值 + DTO 回显；二次 GET（=重进编辑页）仍返回 `categoryId` 且该单 `customerTemplateId=None`（**原本必现丢失的无模板场景现在也正常**）；`saveDraft` 不带该字段 → 未被清空；V364 `success=t` 且 `product_category_id IS NOT NULL` 计数为 **0**（存量确未被动）；真实浏览器编辑页采样 `显示值="默认分类" 提示=""`（无红字）| 注意事项：无 `field_type` 改动（AP-44 不触发）；采样 antd Select 选中值要用 `.ant-select-content`（本仓版本不是 `.ant-select-selection-item`）；**worktree 起临时后端跑 Flyway 后，V364 先落到共享库而主仓无该文件，此窗口内 8081 完全重启会 validate 失败——须尽快合并**（本次合并用 `--ff-only` 绕过并发会话未提交文件导致的 ort 失败，且不触碰其工作区）

---

[2026-07-29] repair-0729 复制报价单数据丢失（P0，详情页/列表页「复制」入口） — 三症状同源修复 | 改动文件：后端 `quotation/service/QuotationService.java`（copy 同模板路径改值快照整份继承 + `migrateAndCreateComponentData` 分流 + AP-40/未匹配页签两条可观测告警）/ `quotation/resource/QuotationResource.java`（`ensureStructure` 移到 copy 事务提交后调用）/ `quotation/service/CardSnapshotService.java`（`refreshQuoteCardValues` 组件粒度空覆盖护栏 + `EMPTY_OVERWRITE_BLOCKED_COUNT`）/ `configure/service/ConfigureSnapshotService.java`（两处 `snapshot_rows` 写点同款护栏 + 整单预取复用 `byLine`）；新增测试 3 个类 9 用例；`docs/反模式.md` 立 **AP-62** | **三症状**：①新单 BOM 页签空 ②点「刷新基础数据」后更多页签空 ③走完向导后 `row_data` 被空值覆盖不可逆 | **根因五层（全部实证，非推测）**：A `pending` —— 基础数据大量是本单私有 pending（`pending_quotation_id=源单id`），实测该产品在 `material_bom_item` 的**全部结构边**都属源单、正式行只剩主件自身一行；新单 id 下 `QuotePendingScope` 看不见 ⇒ 重查即归零。B 树 —— 报价侧 `tab_type='BOM'` 不参与实时展开（`expandFlatDriverBaseRows` 显式 continue），只读 `snapshot_rows`，而 copy 置 null 且从不重物化 ⇒ BOM 恒空。C 空覆盖 —— `saveDraft` 用渲染态空值覆盖已持久化 `row_data`（**网络级拦截实证触发点是「下一步」而非 autosave**：打开编辑页 0 次写请求，autosave 拒绝门是生效的）。D 结构 —— copy 不建 `quotation_view_structure`（新单 0 份 vs 源单 4 份）。E 金额 —— 置 0 靠重算回填，数据丢了只是偏低、不报错，错价单可一路提交 | **方案裁决（否决"迁移 pending"）**：9 张带 `pending_quotation_id` 的表（capacity/element_bom/element_bom_item/material_bom/material_bom_item/material_customer_map/material_master/plating_scheme/unit_price）**唯一索引均不含该列** ⇒ 物理复制必撞 23505；可见性并集会让新单编辑时污染源单或撞键——pending 模型不支持"同一份基础数据两个并行草稿"，属架构约束不在本次范围。改走**同模板复制 = 值快照整份继承**（`snapshot_rows`/`row_data`/4 份值快照/金额/`annualVolume`/行级折扣 8 列/`deletedTreeNodes`+`deletedRowKeys` 两类墓碑），显式跳过 `refreshQuoteCardValues`/`refreshCostingCardValues`；换模板复制逐字节保持原重建逻辑 | **评审抓到的 Critical（计划本身的漏项）**：只继承 `costingCardValues`（单件成本）却漏 `annualVolume`（乘数）⇒ 核价总额与行折扣金额恒 0，是根因 E 的同族形态；连带补齐 Step3 行级折扣 8 列，否则 submit 权威重算会用缺位值覆盖已继承的头部金额 | **事务边界**：`ensureStructure` 必须在 copy 事务**提交后**调用——实现者验证 `REQUIRES_NEW` 同样不行（挂起事务里未提交的行对新事务不可见，会静默查空），最终按 `CreateQuotationMaterializer`/`saveDraft` 范例落在 Resource 层 | **亲验（技术总监，非采信自报）**：临时后端 8099 + `cpq_db_0724` 真实单 —— 复制后逐页签 `snapshot_rows`/`row_data`/`baseRows` 与源单**全等**（BOM 6 行，修复前 0）、结构 4 份齐全；调 `refresh-card-snapshot` 后行数不变且护栏日志实证拦下外购件组件的 0 行重查；合并后主仓 8081 + 真实浏览器走完向导「下一步」+ 点「刷新基础数据」，5 个页签行数逐一不变、`加载中`=0；主仓 `mvnw test` 9/9 BUILD SUCCESS | **踩坑**：Playwright 点 Ant Design 两字确认按钮时 `hasText:'刷新'` 匹配不到（渲染为「刷 新」带空格），确认框没关导致遮罩挡住页签点击，5 个页签全读到同一个当前页签的行数 —— 一度误判为"刷新仍会清空"，靠查库（数据完好）+ 看截图（对话框还开着）纠正；选择器应改 `.ant-modal-confirm-btns .ant-btn-primary` | 注意事项：无 Flyway 迁移、无 `field_type` 改动（AP-44 不触发）；核价侧已实证零影响（18 页签逐一行数与源单相同，核价链路不在报价 pending 域内）；护栏的代价 = 基础数据被真正清空时页签不会自动变空（保留旧值 + WARN），属有意取舍

---

[2026-07-26] repair-0727 报价侧 BOM 树页签删除行 BUG（task-0721 后续修复） — 三症状同源修复 | 改动文件：后端 `quotation/rowkey/DeletedRowKeys.java`（Tombstone 加可空 nodeId + 4 参 keepMask，旧 3 参 delegate 退化 fp 单键）/ 新增 `quotation/rowkey/PrunedTreeNodes.java`（剪枝前缀匹配抽取共享）/ `service/FormulaCalculator.java`（抽 `buildRawRowKeys` 单一口径）/ `service/CardSnapshotService.java`（`refreshQuoteProjection` 重构为共享 `materializeAndProject(id, rebuild)` + 新增树删除专用 `materializeRowDataAndProject`）/ `service/QuotationTreeService.java`（墓碑带 nodeId、`computeRowFpForNode` 同节点多行精确定位）/ `resource/QuotationTreeResource.java`（两段式事务）/ `service/rowkey/RowKeyUniquenessService.java`+`QuotationService.java`（B5 提交校验消费墓碑 + 树按 nodeId 判重）；前端 `deletedRows.ts`（keepRow 加 nodeId 维度，与后端逐字节镜像）/ `QuotationStep2.tsx`（buildSnapshotExpansions 传 __nodeId + applyTreeDeleteResult 走权威投影 + 树行加 data-node-id）/ `BomTreeDeleteConfirmDrawer.tsx`（onApplied 传整个响应体 + 在途窗口保护）/ `useCardSnapshots.ts`（F0 buildUniqueRowKeys nodeId 前缀 + getByKeyWithLegacyFallback 旧键回退）| **三症状根因（均实机复现确认，非推测）**：①树删除端点响应缺 `componentData` 权威投影 → 前端只回灌 quoteCardValues，而 baseRows 按设计不含行墓碑过滤 → 过滤所需的 deletedRowKeys 永远是旧值，须整页重载才生效；**①-b（勘察时新发现，比原判更严重）**：`executeDelete` 只调 `snapshotQuoteSideOnly` 不物化 row_data，且 `refresh-card-snapshot` 也不补 → 实测 `snapshot 6 / row_data 5 / 墓碑 2` 三存储错位（AP-54 族"删错行/值串行"的同一成因）；②墓碑按 fp 内容指纹单键匹配，而树的 `__nodeId` 在 row 顶层不在 driverRow 内 → 同料号多 occurrence 靠"driverRow 恰好含 parent_no"才偶然可分，视图不输出父件列即必然连删（BL-0055 残留被树场景激活）；③`RowKeyUniquenessService` 只读 snapshot_rows 全集、从不读 `deleted_row_keys`/`deleted_tree_nodes` → 软删的行永远参与判重，用户删光重复行仍 422 且 UI 无路可解（P0 阻断，实测原文「行键重复…在第 5,6 行重复」）| 关键决策（需求方逐项裁决）：Q1 树页签墓碑加 nodeId 维度（非树侧维持 fp 单键逐字节不变，旧墓碑无 nodeId 退化兼容）；Q2 树页签判重键 = `computeDedupKey + "@" + __nodeId`（同料号挂不同父=合法 DAG 结构放行，同节点内重复仍拦）——此裁决即 BL-0004 的产品结论；Q3 仅树页签（BL-0055 非树侧不结案）；Q4 不排查存量；Q5 一次交付 | **顺带修复 F-1（B0 前置）**：task-0721 B10 只给 `FormulaCalculator.computeRows` 的树行 effKey 加了 `__nodeId::` 前缀，`CardSnapshotService.buildResolvedRows`/`RowDataMaterializer`/前端 `buildUniqueRowKeys` 三处未同步 → 树页签 formulaResults 键带前缀而查表侧不带 → FORMULA 叶子值与 editRows 绑定静默 miss（实测 `formulaResults[0].rowKey="S-3120014539::主料1"`；当前全库 3 个树组件 FORMULA 字段数均为 0 故未显形）。已抽 `buildRawRowKeys` 四处同源 + 旧键回退兼容存量 editRows | **开发期发现并修复的 2 个真实 bug**：(a) 墓碑 effKey 双重前缀——B0 后前端 rowKey 本身已含 `nodeId::`，后端再拼一次会写出 `nodeId::nodeId::base`（技术总监裁决 10.6 拦下）；(b) JTA 60s 事务超时 `ARJUNA016102`——同事务内原生 UPDATE 持行锁与 `REQUIRES_NEW` 物化写同一行互等，仿 `delete-driver-row` 改两段式（tx1 写墓碑提交 → tx2 物化投影落 Resource 层），`QuoteBomTreeEndToEndTest` 70s 超时→10.4s 全绿 | 验收（技术总监亲验，非采信自报）：后端 7 类 47 用例 `mvnw test` EXIT=0；前端 3 文件 39 passed；真实单 QT-20260726-0006 实机——删除响应含 componentData、`row_data` 6→4 两存储对齐、旧格式墓碑仍精确命中（AC-9 存量兼容 live 证据）、两条重复行都删后 submit **422→200**（P0 闭环）、基线 B 下只删一条时另一条 AgNi 精确保留（症状②闭环）；E2E 专项 spec 三次连跑 2 passed（行数 6→5 当帧消失）| **测试环节两条 bug 的裁定**：Bug-1（"UI 行不消失"，测试判 P0 阻断）经技术总监三次连跑复验为**假阳性**——测试复用了 20:14 启动的 vite 实例，早于前端 20:24 / 后端 21:04 的提交，测的是旧代码；Bug-2（add-leaf 中部插入后删除该叶子致 row_data 错位/幽灵重复）**确认真实但归因更正为 pre-existing**——master 上做同样操作同样损坏且更严重（H65 整行丢失 + 4 条幽灵 AgNi vs worktree 的 3 条），是 task-0721 加叶子路径既有缺陷（手工叶子行不进 row_data 物化 → snapshot 7 行 vs row_data 6 行 → 按位置合并整体错位一格），本次改动只是让它更早暴露，已登记 BL-0077 | 注意事项：本次无 Flyway 迁移、无 field_type 改动（AP-44 不触发）；核价侧走 `deleted==null` 分支逐字节零回归；worktree 三代理共用 git index 期间发生过一次提交竞态（前端暂存文件被测试的 commit 带走，内容无损）——后续并发开发应 `git add` 后立即 commit 并用 `git show --stat` 自查
...
[2026-07-22] task-0722 罗克韦尔模板按最新规则改造 + 组件级 sort_field 行排序 | 罗克韦尔模板目录f4e2bca1(10组件全改) + V353迁移 + Component/ComponentDTO/CreateComponentRequest/ComponentService/ConfigureSnapshotService + ConfigureSnapshotServiceSortTest | **①罗克韦尔改造**:页签类型逐个问用户定(勿猜,用户纠正来料=BOM非零件):产品=主件/元素=材质元素/来料=BOM树/其他费用+外购件=空/电镀费用+自制加工费=零件/组装加工费=空;全`_`前缀别名+补料号字段+rowKeyFields;live验QT-20260717-2050(CUST-1269/S-3120014539)8页签全渲染,来料真树4节点带数据。**②QuotePendingRewriter框架bug**(已上升规则§4.6):DRAFT渲染时版本化表(unit_price等)整表换成带:pq子查询,写成`LEFT JOIN unit_price ON...AND is_current=true`(is_current在JOIN ON)→pending改写产参数错位畸形SQL→运行期`column index out of range`静默返0行(dry-run照过只真实渲染暴露);规避=改标量子查询(WHERE带is_current安全)。**③sort_field特性**(方案B,已合master merge eacdd235):视图ORDER BY对DRAFT报价单无效(pending改写复杂子查询+executor SELECT*外壳无外层排序,PG不保证透传;试UNION直挂/外层包裹/MATERIALIZED CTE三种全失效),故component加sort_field列(V353),ConfigureSnapshotService Pass1平铺页签按其数字感知升序(sortRowsBySortField返新副本不改缓存,树页签跳过),7单测通过;罗克韦尔其他费用/电镀/自制/组装=项次、元素=序号,live验其他费用1,2,3,4;不碰export/import(task-0721脚手架主仓未提交)+不碰核价CardSnapshotService(报价侧已覆盖)。**④V351迁移冲突**:并发会话task-0722-element-price也用V351先应用共享DB→我V351被同号跳过列没建8081挂500→改号V353自愈;教训=建迁移前find全worktree+查history max。
[2026-07-23] task-0721 报价侧BOM树状渲染+页签类型属性 - 全量交付结案(技术总监编排,子代理实现,全程亲验) | 核心:报价卡片复用核价spine树引擎(方案A物化侧接树,buildCardValues纯读snapshot_rows渲染层零改动/树随快照冻结);页签类型5类(BOM树状/材质元素/零件/外购件/主件,tab_type自动同步bom_recursive_expand,已被COSTING模板引用的组件改BOM拦400);类型判定不由用户声明由「料号出现在哪个类型页签」推导故不新增field_type/AP-44不触发;跨页签级联删除(DAG不误删,仅无剩余occurrence才删,走墓碑,弹窗确认三块含retainedParts);树上编辑(加叶子候选来自本单各页签本地采集/剪枝/行删)。 | 涉及:CardSnapshotService(overlayTreeTabsFromFrozenSnapshot)/BomTreeRenderService(核价CostingTreeRenderService改名)/BomNodeTypeResolver/BomTreeCascadeCalculator/QuotationTreeService/QuotationTreeResource/ConfigureSnapshotService/ComponentService/TemplateService(快照补tab_type/part_no_field/part_name_field/bom_recursive_expand四字段)/V346~V348 + 前端QuotationStep2(拆COSTING闸门数据驱动)/ReadonlyProductCard(AP-50同步)/bomTreeLeaf/BomTreeAddLeafDrawer/BomTreeDeleteConfirmDrawer/ComponentManagement/ComponentPalette(tabType Tag展示) | **真实数据/渲染验收抓到4个单测测不出的真bug**:①refresh对树组件live重算覆盖冻结spine致报价树UI空树→overlayTreeTabsFromFrozenSnapshot读冻结快照;②树组件坏$view(hf_part_no口径)abort刷新事务连累overlay→expandFlatDriverBaseRows跳过BOM树组件;③previewDelete缺@Transactional致连续剪枝读L1缓存旧墓碑误判;④V307遗留全局唯一索引致QUOTE配置激活500→V347下线。 | **裁决要点**:料号列自适应宽度(width:1+nowrap非贪婪);报价树不显版本列(cardSide==='COSTING'才出,核价保留);**匹配标识放宽为料号列或名称列至少一个**(2026-07-23,截图外购件页签用「料件名称」无料号列;单值字符串相等匹配,树节点__hfPartNo vs各页签自身标识列值,非同时比;spine节点料号身份只命中料号列页签,名称列页签只给名称身份的手工叶子定类型) | **并发教训**:master被报价升版/sort_field/element_price 3+会话密集推进,HEAD操作期间变多次+WIP实时churn;landing改用cherry-pick避三方合并撞WIP;ConfigureSnapshotService与task-0722 sort_field撞同一SQL列r[6](sort_field+part_name_field正交)手动合并两列都留(r[6]/r[7]);git reset清误暂存的他会话WIP用md5前后比对确认内容零改动 | **验证**:后端测试全绿(判定/级联/校验/名称列端到端/QuoteBomTreeEndToEnd/核价零回归);E2E quotation-bom-tree.spec 3 passed;真实浏览器亲验树渲染/DAG剪枝弹窗/版本列去除/料号列自适应/tabType Tag两处/QT-2085 99901渲染 | fixture:QT-20260721-2067(TASK0721-*组件/模板/QUOTE递归配置,持久化供E2E,勿删);B12存量清除因并发密集推迟(广泛删报价数据会毁他会话)


[2026-07-23] task-0722 元素价格策略 - 交付验收+合并结案(技术总监) | 合并 master(merge commit 见下);worktree feat/task-0722-element-price-strategy 待清理 | **交付**:后端B1-B11(策略表/历史表/f_customer_element_price表函数/:priceBaseDate注入/源CRUD/导入/价格表/策略CRUD+历史+试算/两标杆组件接通)+前端F1-F9(18文件)+测试168例(实跑137)+3轮返修。**技术总监亲验实测**:先乘后加107.00(非107.10)/窗口滚动边界(窗口外9999正确排除)/MAX117.5+MIN96.5+LATEST96.5/例外优先110而非107/无策略客户0行不兜底/_GLOBAL_独立(报价107 vs 核价100)/矩阵稠密dates长度5含3个null/CASE WHEN 6处0残留COALESCE/端点全活+矩阵缺参400+跨度91天400/试算44行。**发现并修复的阻断级缺陷(3方都漏、我的验证盲区)**:DataLoader.java:267多值合桶入口(loadByPath 4-arg,expandMulti专用)硬编码 new QuotationContext(null,customerId) 不读QuotationIdContext→**任何走合桶的批量expand priceBaseDate 退化成今天**,§11.2基准日=报价单创建日在生产基本失效(真实报价单多行料号即触发);单task走单值入口(198行)正确故三方(后端自测/测试168例/我的串价专项)全部漏过——**因为大家构造的都是单task场景**;实测复现 双task=999(今天价) vs 单task=100(创建日价),修复后双task=100(commit a3eca1c8)。**我造成的4处文档错误(均由下游发现)**:①backtask让读SqlViewRuntimeContext.quotationId(实测driver主链路恒null,照做基准日永远退化)②「计价单位」字段要求绑cep.price_unit(实测该列是BOM发料单位ebi.issue_unit,被毛用量/净用量以unit_source_field引用做真实g→KG换算,改了会读到非法token静默算错)③backtask B6 dates定义写成稀疏且自相矛盾(与api.md+原型定稿冲突,原型是07-16~07-22七连续日期含「—」)④初版误判核价视图缺客户过滤是漏洞(实为设计如此,PRICING数据customer_no恒_GLOBAL_)。**Flyway共享历史污染清理**:后端调试期留下 V351×2+V352×8 共10条孤儿记录(文件已改号V356/V357但记录残留)→备份后DELETE,否则下次启动validate挂;库内另有7组历史遗留重复版本号(V336×7等)非本次造成未动。**环境级已知问题(非本次引入,A/B验证)**:REST层@QuarkusTest测试全401(既有ComponentResourceTest同样4/4全红)致29个新增测试无法验证→改用真实HTTP+SQL直查验收。**遗留**:①合桶bucketKey虽已补quotationId但"同请求同单"仍是隐性假设(前端useDriverExpansions每hook实例绑死一个quotationId保证不可达)②未验证29条(纯UI视觉细节/5步向导walkthrough/部分边界状态码)③其余3个元素组件副本按§11.19业务自配。 | 文档 dev-docs/task-260722-元素价格策略/{需求说明(§11共24节+§11.24交付物对照矩阵+16验收要点),api,backtask,fronttask,单价字段配置规则,testcase,test-report}.md + 元素价格策略-原型图.html(v9定稿8屏);闭合 BL-0069#5(listAvailableElements 从废弃mat_bom改读element主表)

[2026-07-23] 定价策略页 - 折扣策略 Tab 下线(task-0722 follow-up) | cpq-frontend/src/pages/pricing/PricingStrategy.tsx | 用户发现「折扣策略」页签配了不生效,排查确认(codegraph+grep):该页签配的客户级折扣(pricing_strategy 表)在当前报价流程已悬空——①前端触发函数 handleCalculateDiscount(QuotationWizard.tsx:1213)只定义无调用方(按钮已摘)②后端 JavaDiscountCalculationService 唯一调用者是单测③现行报价折扣走 Step3 逐行 LineDiscountService(saveDraft 时 recompute)+手填 finalDiscountRate,完全不读 pricing_strategy。裁决(方向1·最省纯前端):去掉右侧内容区 Tabs 外壳,选中客户后直接渲染 ElementPriceStrategyTab,不再展示「折扣策略」Tab。删 activeMainTab state + 其 reset useEffect。**折扣策略前端逻辑(strategies/fetchStrategies/Create-Edit Modal/handlers)、后端 calculate-discount 端点、pricing_strategy 表数据均保留未删(死代码,可逆,noUnusedLocals=false 不报错)**,如需恢复见 git 历史/后端接回入口。客户等级筛选 Tabs(levelFilter)与元素价格策略无关,保留。自检:tsc 0 错误 ✅;PricingStrategy.tsx Vite 200 ✅;主入口 200 ✅。
[2026-07-23] task-0709-update0723 报价导入模板 0723 适配(B1~B10) - 后端实现，worktree feat/task-0709-update0723-quote-import，待技术总监验收合并 | 新增 PartTypeInferenceService(B2 料号类型推断+批量库内兜底+批量名称种子R2)/QuoteImportValidator(B7/B8 Phase1 全量校验器)/QuoteImportWriteFailedException | 重写 QuoteImportService(processImport 拆 Phase1 校验+Phase2 单事务写入 writeAll，移除组成件BOM 预扫，MaterialBomMergeHandler 并入 orderedHandlers 统一循环) + MaterialBomMergeHandler(B3 单sheet三态改造+B4 operation_no反填+B6 material_type) | 改动 Q06/Q07/Q09(B5 名称反查按类型分派材质/零件外购件) + Q13(item_seq getIntNth 3→2 必修bug + material_type 组成件→外购件 + 去掉repair-2 matNoSet特判) + Q02/Q04/Q05/Q08/Q10/Q11/Q14~Q19 全部 @Transactional REQUIRES_NEW→MANDATORY(join外层单事务) | 删除 Q01ElementPriceHandler(+测试)，B1元素价格改由独立元素价格策略项目(task-0722)维护，验证QUOTE侧unit_price(ELEMENT)无下游读取，安全下线 | **关键决策**：①Phase1校验器范围收敛为"新增的B2/B3/B4/B5必需检查"(类型冲突/材质缺库/CFG-/关键键必填)，其余sheet既有per-row校验留在Phase2、靠orchestrator"任一handler.failedRows>0即throw整体回滚"兜底，两阶段合力满足"零写库"净效果而非要求Phase1穷举复刻每个handler逻辑 ②R1(协调方补充)：pendingQuotationId恒非空导致material_master相关upsert实际写`pending_material_master_staging`非正表，验收/自测SQL全部改查staging表 ③R2(协调方补充)：MaterialNoResolver.resolve()按名查询命中material_master正表, 但本次导入新铸的料号只进staging对同批次其它handler不可见→加`MaterialNoResolver.batchStateFor(ctx)`+`ctx.sharedCache["materialNoBatchState"]`全handler共享一个BatchState，并用PartTypeInferenceService.TypeIndex.seedBatchState预灌"同行码+名"种子，解决跨sheet同码只给一次的重号风险 | **踩坑**：①13+个handler改MANDATORY后，~24个直接调用handler.handle()/merge()的既有单测在无外层事务的情况下抛TransactionRequiredException——按backtask预期(暴露误用)逐一在@Test方法补@Transactional(脚本化批量加，需处理@Test单行写法漏加的边界) ②Q04/Q05 两个既有测试文件在master上已经是坏的(fixture用"投入料号"而handler早改读"销售料号")，与本次改动无关，git diff master实证零业务代码改动，留作已知缺陷未修(超出本任务范围) ③Q13ComponentRecipeHandlerTest整个文件测的是被本任务删除的repair-2 matNoSet特判逻辑，已retire(其冲突检测语义由PartTypeInferenceServiceTest覆盖) ④Q13ComponentOtherFeeHandlerTest的旧fixture固定3个"项次"重复表头(对应旧模板)，item_seq修复后第2个项次被两行共用致唯一键撞车，改回2个项次匹配新模板 | **自测**：`./mvnw test -Dtest='*Quote*,*PartType*,*MaterialBomMerge*'` 全绿(exit=0，MaterialBomMergeHandlerTest 11/11、PartTypeInferenceServiceTest 10/10、QuoteMaterialNoIntegrationTest 9/9等)；用真实`报价系统模板0723.xlsx`(客户CUST-1269/罗克韦尔，该客户此前已合法导入过此数据)跑通QuoteImportService.processImport端到端：三态精确匹配验收(991/992=RECIPE、S-80011=ASSEMBLY含operation_no=Z380、W-1001=OUTSOURCED含composition_qty=2/issue_unit=PCS)、staging表material_type(S-80011→零件/W-1001→外购件)、主表bom_type=ASSEMBLY；热身后二次导入elapsedMs=1980ms(<2s但贴近阈值，判断主因是远程DB(10.177.152.12)网络往返而非算法N+1，非localhost场景下留意)；整单回滚验收：用POI改「组成件其他费用.组成件料号」为已存在材质码991构造真实类型冲突，Phase1精确报错"料号/名称「991」同时命中 材质(物料与元素BOM) 与 外购件(组成件其他费用)，类型冲突"，10张相关表count导入前后逐一相等(zero-write确认) | **已知限制/未完成**：Q04/Q05两个pre-existing坏测试未修(超范围)；性能仅验证了黄金样例(约25行)量级，未测千行级；Q10自制加工费投入料号维持原样(仅按backtask明确排除范围不改，登记material_master由MaterialBomMergeHandler路径覆盖)

**[同日 follow-up] U8性能返修**：测试工程师独立验收后回传——功能项全部PASS(含补测的Phase2中途失败整单回滚，MANDATORY传播经受住真实场景验证)，唯一不达标是U8性能：黄金样例3次采样2065/2256/2350ms均>2s，根因定位为`writeAll`每handler一次`updateProgress`独立REQUIRES_NEW远程往返(BEGIN+UPDATE+COMMIT=3次网络往返/次)，17次合计~800ms固定开销。**修法**（不改事务传播，只减写入次数，遵约束）：`QuoteImportService.writeAll`改"均匀分桶"确定性节流——整个handler序列按桶号`i*(N-1)/(handlerCount-1)`切成N个检查点(桶号变化才写，首尾天然落桶0/N-1天然满足"关键节点必写")+800ms静默超时兜底(防大文件单sheet耗时久时进度条长时间不动)；N从5→4→3→2逐档下调实测(黄金样例17 handler)，N=2(仅首尾)时5次采样1381/1746/1535/1838/1482ms全部<2s（较早前N≥3时仍偶发>2s，怀疑受当前共享DB环境噪声影响，下调后有更大安全边际）。**踩坑**：诊断过程中发现测试用`managedExecutor.runAsync(...).get()`包一层异步调度（与生产`BasicDataImportV6Resource`同款），internal `[v6import] QUOTE TOTAL`与外部实测elapsed之间有平均500-900ms、方差很大的gap（怀疑@ActivateRequestContext/executor调度/finalizeImportRecord等固定开销），此部分本次未展开处理(超出"只降低updateProgress"的授权范围)，若未来仍有性能余量诉求需另行评估。复测：`Update0723ImportAcceptanceTest` 2/2 PASS；`*Quote*,*PartType*,*MaterialBomMerge*` 回归全绿(exit=0)。

[2026-07-23] task-0722/update-0724 元素价格手工维护 - 前端 F1~F6 实现，worktree feat/task-0722-update0724-element-price-manual，待技术总监验收合并 | 新增 PriceEditDrawer.tsx(二级抽屉480) / PriceHistoryTab.tsx；改 types/element-price-strategy.ts(ElementPriceRowDTO 补 id/fetchStatus + 新增 CreatePriceRequest/UpdatePriceRequest/PriceChangeDTO/PriceHistoryDTO) / elementPriceStrategyService.ts(+createPrice/updatePrice/deletePrice/listPriceHistory) / PriceDetailTab.tsx(裸Table→SelectableTable+工具栏[新建/编辑/删除]，rowKey组合键→id，加「数据来源」列) / ElementPriceTableDrawer.tsx(加第3个「变更历史」Tab) / config/ElementEditDrawer.tsx(F6 空态文案) / QuotationStep2.tsx(摘除 ElementPriceHint 挂载点：import:19 + 判定2725-2731 + 渲染2777-2787，cellInner 直接取 else 分支) | 删除 ElementPriceCenterPage.tsx / ManualPriceEntryDrawer.tsx / quotation/components/ElementPriceHint.tsx / router/index.tsx 的 element-price-center 路由项+import / elementPriceService.ts 的 getReference/listHistory/upsertManual/listAvailableElements(只留活跃的 listLatestBySource) / **额外清理**孤儿类型文件 types/element-price.ts（4 消费方全删后零引用，`/usr/bin/grep` 确认，判断属于 U8"清到底"范畴主动删除，非 fronttask 删除清单明文列出，供技术总监复核） | **关键决策**：①PriceEditDrawer 元素下拉沿用 task-0722 StrategyExceptionEditDrawer 同款模式(打开时 elementService.list() 全量拉取 + 前端 optionFilterProp 本地过滤 ACTIVE)，未按 api.md 字面 `?keyword=` 做服务端搜索，理由=与同期姐妹抽屉行为一致、fronttask 明确要求"与之保持一致" ②价格编辑保存错误处理严格按 api.md：409/400 message.error 不关抽屉，404(仅编辑可能)提示"该价格已被删除"+关闭+刷新 ③货币/计价单位用自由文本 Input 非固定枚举 Select，因 api.md 未限定枚举值且价格导入侧本就是 Excel 自由列(非 v1 RMB/USD/EUR 写死枚举) | **自测**：`npx tsc --noEmit -p tsconfig.json` 0 错误(worktree 内软链 node_modules 后跑，非共享 5174)；改动的 10 个 .tsx/.ts 用 worktree 内临时端口 5199 Vite dev server 全部 200(非共享 5174，避免验的是主仓代码)；`/usr/bin/grep -a` 核查 ElementPriceCenterPage/ManualPriceEntryDrawer/ElementPriceHint/element-price-center 全工程 0 命中；deleted 4 文件磁盘确认不存在 | **未做**：E2E `quotation-flow.spec.ts`(动了协议级 QuotationStep2.tsx 按规范是硬门槛)——按任务委派说明，等技术总监确认后端 B3~B7 ready 后统一跑，本次未跑；手工验收 8 步清单(新建/409/编辑IMPORT行/多选删除/变更历史 Tab 显示)因后端本分支是否已合入 B4~B6 未知，未做端到端联调，标注「待技术总监 E2E+联调」；backend 侧 ElementPriceRowDTO.java 抽查已见 id+fetchStatus 字段(B3 疑似已并行落地，未做全面backend代码审查)

[2026-07-23] task-0709-update0723 报价导入模板 0723 适配 - 第二阶段测试工程师进场，补测 agent 自测盲区(Phase2真实中途失败回滚) | 新增 `cpq-backend/src/test/java/com/cpq/basicdata/v6/quote/Update0723ImportAcceptanceTest.java`(2个`@QuarkusTest`，直调`QuoteImportService.processImport`经`ManagedExecutor`+`V6QuotationCommitService.createQuotation`，连远程DB 10.177.152.12，不改主代码) + 测试报告 `dev-docs/task-260709-导入报价数据和导入核价数据的版本升级与版本维护/update-0723/test-report.md` | **核心发现**：agent 自测的"整单回滚"验收（RECORD 上一条记录）构造的是「组成件其他费用料号=991撞材质码」——该场景在 **Phase1** `QuoteImportValidator` 类型冲突检测阶段即被拦截，`writeAll`(Phase2 单事务写入体)从未被调用，因此**没有验证过"已发生的真实写入被回滚"这件事**，只验证了"Phase1 report + 零写库"（trivial，因为压根没写）。本次改为构造 **Phase1 全过、Phase2 才失败** 的真实场景：读码确认 `QuoteImportValidator.validateComponentOtherFee` 不做跨客户校验（只查必填），跨客户串号只在 Phase2 `MaterialNoResolver.resolve()→QuoteMaterialNoAllocator.ensureRegistered()`(真实DB owner查询)才会触发；构造一份全新客户B的4-sheet workbook：物料BOM(handler#1)+单重(#2)+客户料号关系(#3) 均为合法数据（真实写入 material_bom/material_bom_item/pending_material_master_staging/material_customer_map），第12个handler「组成件其他费用」引用一个已属客户A的料号触发 `CrossCustomerQuoteNoException`→per-row `recordError`(不抛异常)→`writeAll`检测`failedRows>0`→`throw QuoteImportWriteFailedException`→**前3个已完成真实写入的handler随之整体回滚**（日志证实 `writer{dbCalls=7...ins=2x/31ms}` 证明真实写入确实发生过）。断言：①`status=FAILED`非PARTIAL ②`material_bom/material_bom_item/unit_price/material_customer_map`(customer=B)计数前后一致(0→0) ③`pending_material_master_staging`(本次recordId)=0 ④具体验证bomMerge/Q02各自的写入确实不存在(非计数巧合) ⑤客户A既有归属不受影响。**结论：未发现"某表独立提交未回滚"问题**——与担心的相反，MANDATORY传播改法实测生效，13+ handler确实join了同一外层事务。 | **黄金样例全链路复测**（客户CUST-1269）：三态推断(991/992=RECIPE、S-80011=ASSEMBLY、W-1001=OUTSOURCED)、R1落staging非正表(S-80011→'零件'、W-1001→'外购件'，material_master正表count=0)、B4工序反填(operation_no='Z380')、B3 composition_qty(1/2)+issue_unit('PCS')、B5 Q13 item_seq修正(=1非null)、U13 pending语义(8表pending_quotation_id=recordId→create-quotation后过户为quotationId等量搬迁) 全部SQL实测PASS | **⚠️ 性能存疑(如实记录不淡化)**：3次连续采样 processImport内部elapsed=2009/1880/2147ms，测试端到端2256/2065/2350ms，**均超backtask U8的2s阈值**(略高于agent自测记录的1980ms)；handler级日志求和实际业务耗时仅约1.0~1.1s，差额主因判断为19个handler each一次`updateProgress`独立REQUIRES_NEW事务+Phase1 validate独立事务+远程DB(10.177.152.12)网络往返(每次事务begin/commit均计入RTT)，非算法N+1(dbCalls与分组数成正比非行数)；建议后续优化点=合并updateProgress为内存计数+批量回写，不阻断本轮功能验收但需PM/技术总监知悉 | **踩坑**：cleanup()方法首次实现FK删除顺序写反(先删quotation后删import_record，import_record.quotation_id是FK导致外键冲突)，致第一轮测试的新建import_record+quotation孤儿残留；已修正删除顺序(先删import_record断开FK引用)+手工核实清理孤儿行+重跑3次验证零残留 | **清理策略说明**：Test2(全新客户UPD0723-前缀)零写库+防御式清理=零残留已核实；Test1(复用CUST-1269)只精确清理本次新建的import_record+quotation及明细行，**不回滚版本化表(material_bom等)的is_current新版本指针**(是导入成功的正确副作用，强行回滚需精确重建改动前状态，误操作风险更高，且该客户已是既定自测客户有历史残留先例) | **回归**：`./mvnw -o test -Dtest='*Quote*,*PartType*,*MaterialBomMerge*'` 86/86 PASS(0F/0E, 9 Skipped为既有基础设施类跳过与本次无关)，新测试文件名不匹配该通配符是独立互补覆盖

[2026-07-23] task-0722/update-0724 元素价格手工维护 - 后端 B1~B8 实现，worktree worktree-task-0722-update0724-element-price-manual，待技术总监验收合并 | 新增 Flyway `V359__element_daily_price_log.sql`(变更历史表，price_id 不建 FK) / `ElementDailyPriceLog.java`(Panache 实体，逐字比照 ElementPriceStrategyLog) / `PriceMaintenanceService.java`(create/update/delete 三方法+私有 writeLog，同事务写价格与日志) / `CreatePriceRequest.java` / `UpdatePriceRequest.java`(刻意不声明键三字段) / `PriceHistoryDTO.java` / `PriceChangeDTO.java` | 改 `ElementPriceRowDTO.java`(末尾追加 id/fetchStatus) / `PriceTableService.java`(listDetail SELECT 末尾追加 edp.id,edp.fetch_status + mapDetailRow 末尾追加两下标；新增 findById 供写操作后取完整 DTO；新增 listHistory 两步 SQL：CTE 先收窄"价格身份"再取完整时间线，diff 算法逐字比照 StrategyService.listHistory) / `PriceTableResource.java`(新增 POST/PUT/DELETE /prices{,/{id}} + GET /prices/history 四端点，挂在类级 @RoleAllowed 下不单独加注解，响应不包 ApiResponse) | 删除 v1 元素价格中心：`ElementPriceResource.java`/`ElementPriceService.java`/`ElementReferenceDTO.java`/`UpsertManualPriceRequest.java` 四文件 + 依赖它们的两个测试 `ElementPriceResourceTest.java`/`ElementPriceQuotationFlowTest.java`（**backtask.md B7 未列出这两个测试文件，是我按 U8"清到底"精神主动一并删除**——留着会编译通过但运行时全部 404 失败，供技术总监复核是否认可） | 新增测试 `PriceMaintenanceResourceTest.java`(14 个 @QuarkusTest，覆盖撞键409原值不变/price≤0拒绝/元素或源非ACTIVE拒绝/键字段被Jackson丢弃/IMPORT→MANUAL翻转/三动作各一条日志/DELETE的snapshot为删除前值/事务原子性) | **关键决策**：①409 撞键用先查后插，INSERT 不用 ON CONFLICT（那是导入侧覆盖语义），并用 `org.hibernate.exception.ConstraintViolationException` 沿 cause 链查 SQLState 23505 兜底竞态（比照 CostingFreezeService 现成写法）②history 查询没有客户维度不能照抄策略侧"全表 load"，改两步 SQL：CTE 先按筛选条件收窄"价格身份"(element_name,source_id,price_date三元组)，再取这些身份的完整时间线做 diff，最后拍平按 changed_at 窗口过滤+分页 ③snapshot 内容含 price/currency/priceUnit/fetchStatus 四字段全部参与 diff（不仅 price），fetchStatus 变化会显示"数据来源: 导入→手工" | **踩坑（均自查自修，非遗留给验收）**：①`COALESCE(:s::text,'')` 在 Hibernate 原生查询里被误解析成参数名 `s::text`（"No parameter named ':s'"）——sourceId 在 create() 里已校验非空，直接改 `source_id = :s` 规避 ②新测试 T14(事务回滚用例)最初用固定 email `test-pm-longname@test.com` 在共享远程库上二次运行撞 `user_email_key`，且这段裸 `utx.begin()/commit()` 没包 try/catch，异常导致线程残留"已关联事务"，poison 了同一 Surefire fork 内后续所有测试类的 `@BeforeEach utx.begin()`(ARJUNA016051)——已改 email/username 按随机 UUID 唯一化 + 包 try/catch 保证失败必 rollback | **发现的预置环境问题（未修，超出本任务范围，已如实报告）**：①`mvn test` 不显式传 `-Dcpq.security.rbac.enabled=false` 时，`src/test/resources/application.properties` 里的 `false` 未生效、RBAC 仍按 true 生效导致全部端点 401——原因未查清（怀疑与本 worktree 的 target/ 缓存或多 worktree 共享 DB 并发有关），本次自测全程显式传该 `-D` 绕过；②`PriceTableResourceTest.matrixAlignsDatesWithNulls` 独立跑必失败（期望 dates.size()==2，实际==6）——`git diff` 确认我未改 matrix() 方法也未改该测试文件，是同日更早"稀疏→稠密返修"遗留的测试未同步更新，纯预先存在缺陷；③`StrategyResourceTest` 整个类独立跑必失败（ARJUNA016051 + FK violation on customer 表），`git diff` 确认零改动，判断是共享库多 worktree 并发写脏数据，非本次引入 | **自测**：`./mvnw -q compile`/`test-compile` 0 错误 ✅；worktree 内临时端口 8098 起 quarkus:dev（未碰主工作区 8081）→ `/api/cpq/element-price/prices` 401 ✅；`V359` success=t ✅；管理员登录后真实 HTTP 冒烟：新建 Ag/百川盈孚 201(fetchStatus=MANUAL)→撞键 409 且 SQL 直查原值 5860 未变→PUT 强传 elementCode=Cu/priceDate=2020-01-01 后 SQL 直查键三元组未变、fetch_status=MANUAL→手工插入 IMPORT 行改价后 SQL 直查 fetch_status 翻 MANUAL→DELETE 204+SQL 直查行已删+日志 snapshot 含删除前值 5750→`GET /prices/history` 返回 CREATE/UPDATE(含changes.price)/DELETE 完整时间线 ✅；**取价链路硬验证**：用真实策略客户 `_GLOBAL_`（配置源=长江有色网）手工新建 Ag 价 6234.5678 → `f_customer_element_price('_GLOBAL_', CURRENT_DATE)` 精确取到 6234.5678 → DELETE 后重新调用返回 0 行（按剩余数据正确重算）✅；`PriceMaintenanceResourceTest` 14/14 PASS（含事务回滚用例：changed_by_name 超长触发 SQLState 22001，SQL 直查价格行未落库）✅；`/usr/bin/grep -a -rn "element-prices" cpq-backend/src` 0 命中 ✅（前端仍有 1 处引用，是 fronttask F5 范畴不归我改）；`codegraph_impact` 复核 ElementPriceService 四方法均仅被同文件 ElementPriceResource 调用，无遗漏调用边 ✅ | **未做**：合并后主仓 `./mvnw clean test` 全量回归（按 B7 已知坑提醒，留给技术总监合并时执行）；前端 F5(下线 element-prices 消费方)不归我做

[2026-07-24] task-0722/update-0724 元素价格手工维护 - 测试工程师进场执行 107 条用例(testcase-update0724.md，106 原始+技术总监补的 TC-HIST-14) | 产出 `dev-docs/task-260722-元素价格策略/update-260724-元素价格手工维护/test-report-update0724.md` | **结论**：107 条全部执行完毕，0 个功能性 FAIL（89 条真实执行 curl/SQL/grep/screenshot + 17 条源码走查 deterministic 渲染逻辑 + 1 条 TC-REG-01 E2E 环境受限未测，如实标注未伪造）| **环境**：worktree 内临时后端 8098(`-Dcpq.security.rbac.enabled=false` 跑 API/SQL 组，另单独重启一次默认开权限跑 TC-PERM 真实登录 alice/bob/test_finance_fd726739/admin 四角色) + worktree 内临时前端 5199(软链主仓 node_modules, `VITE_API_TARGET=8098`) | **5 个 🔴 核心风险点全部真实证据确认**：①TC-FETCH-04/05 系数换算(factor=1.05/premium=2.00)，Ag AVG(50,55,60)×1.05+2=59.7500、删后 AVG(50,55)×1.05+2=57.1250，两次均 SQL 精确匹配到小数点后4位，MANUAL/IMPORT 混合计入无差异 ②TC-CRT-14 撞键 409 + SQL 直查确认原值 145.0000 未被 999 覆盖，另加测 TC-CRT-16 并发竞态(2 并发同键请求恰好 1×201+1×409 非 500) ③TC-UPD-08/09 键锁定：`UpdatePriceRequest.java` 源码确认不声明键三字段 + 强传键字段实测 200(非400)且 SQL 直查键三元组未变 ④TC-LOG-07/08 事务原子性：独立执行(非复用开发者报告)`mvn test -Dtest=PriceMaintenanceResourceTest -Dcpq.security.rbac.enabled=false` 14/14 PASS(含 T14 失败注入用例)，另补做黑盒 SQL 复核确认价格表与日志表均 0 行 ⑤TC-V1-08 `grep -a` 确认 `ElementPriceHint` 全工程 0 命中(非条件禁用是物理删除) | **技术总监 3 项裁决全部真实核对通过**：裁决1(TC-REG-07) `GET /prices` 默认窗口内确认 v1 脏行(`5c422615-...`,price_date 落窗口内)不出现，结构性过滤生效非巧合；裁决2(TC-HIST-13) `size=300` 实测 HTTP 200 + `size` 字段重置为 20 非 400；裁决3(TC-UPD-08/12) 确认 `application.properties` 无 `FAIL_ON_UNKNOWN_PROPERTIES` 配置 + 实测多传字段静默丢弃非 400 | **发现 1 个文档一致性缺陷**：需求文档 U12「须回写主文档」交付前置条件未落实——`git diff master...HEAD --stat` 确认 `dev-docs/task-260722-元素价格策略/需求说明.md`/`fronttask.md`/`api.md` 三份主文档零改动，§11.14A 仍写着已被推翻的"录价统一走价格导入"旧表述，一般严重度不阻断功能验收但建议合并前补齐(<15分钟工作量) | **3 个观察项(非缺陷)**：TC-V1-10 grep 命中 1 处 `elementPriceService.ts:9` 良性注释(非代码引用)；`f_customer_element_price` AVG 口径下 `price_unit` 取值不稳定(kg→元/kg，函数体本次未改动，task-0722 既有行为非本次回归)；TC-CRT-13 日期格式非法时错误响应信封是 Jackson 原生反序列化错误形态非统一 BusinessException 信封(HTTP码仍正确400) | **UI 验证方法论说明**：Playwright 浏览器自动化在本 worktree 隔离环境下多次于中段操作(`.ant-select` 下拉/跨 Drawer 定位)挂起超时(疑似 chrome 子进程生命周期不稳定，非功能问题)，成功获取 2 张真实截图证据(TC-V1-01 `/element-price-center` 路由 React Router 404 页面、TC-UI-02 禁用按钮 hover tooltip 精确显示"编辑一次只能选一行")，其余 UI-only 交互项(TC-UI-03~07/09、TC-CRT-15、TC-UPD-11、TC-DEL-05/06/09、TC-HIST-11/12、TC-REG-05/06)改用 `SelectableTable.tsx`/`PriceEditDrawer.tsx`/`PriceDetailTab.tsx`/`PriceHistoryTab.tsx` 源码走查替代(deterministic 渲染逻辑，非条件分支，已在报告逐条标注方法论供技术总监知悉) | **TC-REG-01 E2E 未测**：按任务委派说明"worktree 环境难搭建时优先保证 106 条 API/SQL/UI 真跑，E2E 标注环境受限留技术总监统一跑"处理，未伪造 E2E 结果；`git diff` 确认改动范围仅 `QuotationStep2.tsx` 单文件 22 行纯删除(无新增逻辑)，风险面小但仍建议技术总监在稳定环境补跑 A/B 对比作为最终合并前形式合规确认 | **测试数据清理**：§1.6 SQL 清理后逐条复核 0 残留，v1 脏数据行数改动前后一致(=1，未被误伤)；`test_finance_fd726739`(权限测试账号)密码哈希已重置为已知测试密码(纯测试账号非生产数据，如实披露)；临时 8098/5199 进程已停止未占共享 8081/5174

[2026-07-24] 组件模板配置(报价侧) - 罗克韦尔 4 组件导入 JSON 交付(按 rule-0724 AGENT 入口) | 产物 `客户组件模板/组件导入-罗克韦尔.json`(bundle 1.0, 4 组件, checksum sha256:8caed10d…) | 输入=用户填好的 `客户组件模板/罗克韦尔模板.xlsx`(4 sheet: 产品/材料成本/外购件成本/加工费)。**关键口径修订(用户当场纠偏，与 报价侧.md §7.2/§7.5「加材质料号/组成件料号字段」旧写法冲突)**：客户模板没有料号列是**真实业务场景**，不得为满足 partNoField 而凭空新增可见字段；**料号只在 SQL 里做 JOIN 键**，组件标识回落「名称列」partNameField —— 后端 2026-07-23 已放宽(`ComponentService.assertPartNoFieldRequirement`：4 类物料页签只需 料号列/名称列**至少一个**)，`QuotationTreeService` 类型判定亦是「优先 partNoField，为空回落 partNameField」，故此路径是一等公民。**4 组件最终配置**：①产品 主件 partNo=销售料号/partName=客户料号名称/rk=[销售料号]/$cp_view；②材料成本 材质元素 **partNo=空**/partName=材质/rk=[销售料号,材质,元素]/sort=项次/$mc_view —— material_part_no 仅在 SQL 内 JOIN material_recipe+material_master 逐行取材质名(一号多材质铁律)并进 ORDER BY，不出可见列；「元素单价」接价格策略 LEFT JOIN f_customer_element_price(:customerCode,:priceBaseDate) ON element_code=ebi.component_no，别名 `AS 元素单价`(逐字不加 `_`)，required=[customerCode,priceBaseDate]，禁 COALESCE(...,0)；③外购件成本 外购件 **partNo=空**/partName=组成件名称/rk=[销售料号,组成件名称]/$wg_view，component_no 仅作 JOIN 键取名；④加工费 零件 partNo=料号(=unit_price.code 非 finished_material_no)/partName=料件/rk=[项次,工序]/sort=项次/$jg_view，hf_part_no=finished_material_no，工序=COALESCE(pm.process_name,up.operation_no)，料件=COALESCE(mm.material_name,mr.name)(V341/V342 兜底口径) | **自检**：4 个 `$view` POST 200(dry-run 过)；psql 对 cpq_db_0724 实跑 4 条 SQL 零 ERROR、输出列与字段 1:1 无死列、别名 `_` 前缀合规(唯 hf_part_no/元素单价 例外)；cpq_db_0724 仅有 PRICING 数据故 mc/wg/jg 返 0 行 → 另在**旧库 cpq_db(只读, CUST-1269 真实报价数据)**背靠背实跑证明 JOIN 正确：mc_view 材质逐行出 AgC3/AgNi11#-Ⅰ/银镍合金、jg_view 工序出 酸洗/电镀(process_master JOIN 生效)、cp_view 出料号，wg_view 0 行=OUTSOURCED 全网无数据(已知)；bundle 回环导入临时目录 checksumValid=true/created=4/sqlViews=4，DB 复核 tabType/partNoField(空)/partNameField/sortField/rowKeyFields/required_variables 全还原且不触发 400；两个暂存目录(8 组件)删净，仅留用户自建空目录「罗克韦尔」，orphan sql view=0 | 用户自行 `POST /component-directories/{罗克韦尔目录id}/import/commit?conflictPolicy=RENAME` 导入。**规则文档已同步改写（同日完成）**：`dev-docs/rule-260724-组件模板配置/` 全套按新口径回写 —— `1-总则与工作流.md` 新增 §1.3.1「模板没有料号列 = 正常，禁凭空造字段」+ 原铁律降为 §1.3.2（仅模板确有料号列时适用）；`AGENT-配置入口.md` §3.1 partNoField/partNameField/rowKeyFields 三行改写 + §3.4 改为两步（先判有无料号列再谈绑哪列）+ §3.5 材质元素/外购件标注「模板有该列才出可见列」+ §6 自检新增第 0 条「字段与模板列 1:1、无凭空新增列、无没人绑的死列」；`4-页签属性与树.md` §4.1 partNoField 改「可空」+ 新增 🚫 段 + 组件映射表改写；`报价侧.md` §0/§5.2 表/§5.4 表/§5.5/§7 用法/§7.2/§7.5 全部改写（§7.5 顺带补 material_recipe 品名兜底 + 撞键提示）+ 已交付实例表新增本次罗克韦尔 JSON 行；`2-组件与字段.md` part_no_field 列说明 + 行键范例；`附录-速查.md` 常见坑加一行；`组件模板.xlsx`「0-填写说明」料号列/名称列/行键三条说明改写（openpyxl 改文本，下拉校验/合并单元格/配色实测保留）

[2026-07-24] 组件模板配置(报价侧) - 罗克韦尔 **5 组件**导入 JSON 重出(模板加 BOM sheet 后覆盖同日 4 组件版) | 产物 `客户组件模板/组件导入-罗克韦尔.json`(bundle 1.0, 5 组件, checksum sha256:974d1b0fd…) | 输入=用户填好的 `客户组件模板/罗克韦尔模板.xlsx`(5 sheet: 产品/BOM/材料成本/外购件成本/加工费)。**新增第 5 个组件 BOM(树页签)**：tabType=BOM → 走 Pass2 树契约(`:total_material_no` + 输出 `material_no`(子)/`parent_no`(父) 边键，**不用** hf_part_no/:customerCode，§5.6)；$bom_view = material_bom_item 边式全子件 + 根分支(NOT EXISTS 无父边的成品 UNION ALL)，字段绑 净重=`net_weight`/毛重=`rough_weight`/用量单位=`weight_unit`(重量口径，老库实测值 `g/kg`) 与 组成数量=`composition_qty`/组成单位=`issue_unit`(发料口径)分属两组单位，料件名称=COALESCE(mm.material_name, mr.name)；模板该 sheet 角色列全留空 → 按 `0-填写说明`「留空我按规则推荐」+ 老库权威母版 COMP-0028__imp1(来料页签, partNo 空/partName=料件/rk=[料件]) 推导为 partNoField 空 / partName=料件名称 / rk=[料件名称] / 无 sort(树序)。其余 4 组件配置同上一条(口径未变)。| **自检**：5 个 `$view` POST 200(dry-run 过，required_variables 自动推出 cp/wg/jg=[customerCode]、mc=[customerCode,priceBaseDate]、bom=[total_material_no])；bundle 字段与模板列**逐 sheet 1:1 全 ✅**(脚本对拍 xlsx)；`cpq_db_0724` 无任何 QUOTE 数据(全是 PRICING/_GLOBAL_) → 另在**旧库 cpq_db(只读, CUST-1269 真数据)**背靠背实跑 5 条 SQL：cp_view 出客户产品编号 PN0507945/汇率 6.9755、mc_view 材质逐行出 AgC3/AgNi11#-Ⅰ/银镍合金/银铜合金、jg_view 工序出 电镀/酸洗/正火退火/分条(process_master JOIN 生效)、bom_view 对 spine[0363-2607000009 + 2 子件] 出 3 节点(根 parent_no=NULL)、wg_view 0 行=OUTSOURCED 全网无数据(已知)；元素单价 LEFT JOIN 不掉行实证 21 行=21 行、无价返 NULL 非 0；一号多材质实证(S-3120014539 有 2 条材质链)→ 材质进 rk 必要性成立；bundle 回环 `POST /component-directories/{罗克韦尔}/import?conflictPolicy=RENAME` 预览 checksumValid=true / toCreate=5 / conflicts=0 / blockers=[] / canCommit=true，预览后该目录组件数仍为 0(证明只读未写库)；暂存目录 5 组件 + 目录本身删净，DB 只剩用户自建空目录「罗克韦尔」 | ⚠️ **遗留前置(bundle 带不走)**：BOM 树页签要出树还需一条 `costing_bom_tree_config`(usage=QUOTE, is_active, 递归 material_bom_item **全子件**不得限死 ASSEMBLY)，`cpq_db_0724` 当前 0 条(老库 cpq_db 有「报价BOM树-QUOTE口径v1」可抄)；导入后各页签渲染 0 行属正常(新库尚无 QUOTE 侧 V6 数据)。用户自行 `POST /component-directories/{罗克韦尔目录id}/import/commit?conflictPolicy=RENAME` 导入。

[2026-07-25] 树递归SQL配置 - 报价树+核价树递归 SQL 双侧配置落库并激活(jh 本地库) | `costing_bom_tree_config` 新增 2 条: 核价BOM树-PRICING口径v1(b349a275, usage=COSTING, active) + 报价BOM树-QUOTE口径v1(108b5840, usage=QUOTE, active); SQL 原文备份见会话 scratchpad `tree_costing.sql`/`tree_quote.sql` | 背景: jh 库 baseline 在 V361 的空 schema, 该表 0 行(全库无任何 INSERT 迁移种子), 导致报价单 QT-20260725-0001 的 BOM 页签落失败哨兵「未配置生效的报价树递归 SQL」。**核价侧**逐字沿用 `docs/archive/核价SQL配置手册.md` 参考实现(system_type='PRICING' + customer_no='_GLOBAL_' + is_current); **报价侧**按 2026-07-21 RECORD 已验证口径重建(system_type='QUOTE' + is_current + 客户口径自根传播 `_cust`, 因 TREE_PARAM 只注入 production_part_nos/__vfPart/__vfVer/pq, 无 :customerCode)。**关键约束(踩坑预防)**: ①~~树配置**不能用** `:versionFilter` 宏 —— `CostingTreeSqlValidator` dry-run 只做 `:production_part_nos`→`ARRAY[]::text[]` 字面替换, 不展开宏也不绑 :__vfPart, 带宏必然保存失败~~ **⚠️ 2026-07-26 已推翻并修复(772dd401)**: 这是校验器缺陷不是配置约束——核价树版本切换本就依赖该宏(运行期 `BomTreeRenderService:362` expandForExecution 正常), 是保存端漏调 `VersionFilterMacro.expandForValidation`(对照 `SqlViewValidator:151-152` 早已正确调用)。**照本条旧约束执行会让配置者主动放弃核价版本切换能力**; ②~~注释里**禁写** `:production_part_nos/:pq/:__vfPart/:__vfVer` —— `BomTreeRenderService.queryRecursive` 的 TREE_PARAM 正则不屏蔽注释~~ **⚠️ task-0725 已修复**: `BomTreeRenderService:400` 已用 `SqlTextMask.mask()` 屏蔽注释/字面量后再匹配 TREE_PARAM, 注释内写命名参数是正当文档习惯, 框架已兼容, 无须规避; ③端点是**单数** `/api/cpq/costing-bom-tree-config`(api.md 写复数是笔误) | 实测: 核价树 seed=S-3120014539 → 15 节点/4 层, 经 pending 改写后仍 15(零回归); 报价树 → 无 pending 改写时仅 1 行光根, 模拟 :pq 绑本单后 6 节点(根+00137+00006+S-80011(+00006)+W-1001), 与 Excel 物料BOM 5 行严格对应, 同料号 00006 双 occurrence node_path 不同未挂错枝 | 遗留: 报价树要真正出数仍依赖修复「quotationId 未传播到渲染链路」(ComponentDriverService:342 / ConfigureSnapshotService 均未 set quotationId → pending 感知改写门槛恒关), 否则报价树只渲光根; 跨客户同成品仍按 customer_no 字母序取根客户(需框架注入 :customerCode, task-0721 follow-up)
[2026-07-25] task-0725 T1 - SQL 文本屏蔽工具抽取 + 4 站点加固（根因2：命名参数替换未屏蔽 SQL 注释） | 新建 `SqlTextMask.java`（`com.cpq.datasource.sqlview`）/ 改 `QuotePendingRewriter.java:107-113`（mask 委派）/ `SqlViewExecutor.java:602-644`（rewriteNamedParams+extractNamedParams 先 mask 再定位，替换仍作用于原文）/ `SqlViewValidator.java:4,122-145,192-198,220-243`（4 处：NAMED_PARAM 提取 + hfPartNo/__sk/__vf 三个裸检查全部改用 masked 文本；`bindWithNullPlaceholders`/`stripCommentsAndWhitespace` 评估后保留不动，理由写进方法 Javadoc）/ `BomTreeRenderService.java:388-403`（TREE_PARAM 匹配改用 mask(withPending) 定位，替换仍作用于 withPending 原文——**保留"必须在 pending 改写之后匹配"的顺序不变量，为 T2 的 :pq 生成预留**）| 新增单测 `SqlTextMaskTest`(8)/`SqlViewExecutorNamedParamMaskingTest`(6，含 cp_view/bom_view 真实模板)/`SqlViewValidatorCommentMaskingTest`(7)/`BomTreeRenderServiceTreeParamMaskingTest`(2，反射调 private `queryRecursive`，用 `io.quarkus.arc.ClientProxy.unwrap()` 拿真实 CDI 实例避免 private 方法在代理上字段为 null)，`mvn test` 54 passed 0 failed 0 skipped | **真实性验证**：`cp_view`/`bom_view` 均从当前 `cpq_db` 库直接 `SELECT sql_template` 取出真实模板嵌入测试；`rewriteNamedParams` 改写后用真连接 `PreparedStatement.getParameterMetaData().getParameterCount()` 断言 pgjdbc 认到的占位符数与 Java 侧 `params.size()` 逐一相等（cp_view: 1==1；bom_view: 2==2）| **环境坑**：`mvn test` 默认走 `%test` profile → `application-test.properties` 硬编码 `DB_HOST=10.177.152.12`，但该 IP 在本 worktree 的 Bash 工具网络命名空间不可达（同机 `jh` profile 的 dev server 走的是 `localhost:5432` docker 端口映射）；跑测试须显式 `DB_HOST=localhost mvn test`，否则 Flyway 连接超时导致 Quarkus 启动失败（非代码问题） | **已确认的 2 组预置失败与本次改动无关**（用 `git stash` 切回改动前逐一复测同样失败）：`SqlViewExecutorPendingHookTest` 3 个 error（硬编码 componentId `4d8874c8-...` 对应的"组合工艺"组件在当前库不存在，纯 fixture 数据缺失）；`DataSourceResourceTest` 5 个 failure（期望 200 实际 401，认证 fixture 问题）| **未做/评估后放弃**：`SqlViewValidator.java:220` 附近 `stripCommentsAndWhitespace`（第三套注释处理实现，backtask 标注"评估是否收口，可选"）**未收口**——理由已写入该方法 Javadoc：其不屏蔽字符串字面量（预置潜在 bug，字面量里的 `--` 会被误判为注释起点），与 `SqlTextMask` 行为不完全等价，贸然合并无法满足"若收口须保证现有语义不变"的前置条件，建议登记 BACKLOG 独立处理

[2026-07-25] task-0725 T2 - `QuotePendingScope` pre-gated 窄作用域 + 传播接缝 + 两层缓存维度（根因1核心，方案D'） | 新建 `QuotePendingScope.java`（`com.cpq.datasource.sqlview`，ThreadLocal<UUID>，公开 API `open(quotationId,status)`/`restore(prev)`/`pendingOwner()`/`cacheTag()`，**故意不提供 public clear()**）| 改 `ComponentDriverService.java`：`:353-372`（私有 expand() 的 setNested 改传 `_pq=QuotePendingScope.pendingOwner()`，第4参 quotationStatus 恒传 null）+ `:390-392`（cacheKey 追加 `buildExtraCacheTags()` 新抽取的私有静态方法，含 QuotePendingScope.cacheTag()）+ `:134-147`（新增 `buildExtraCacheTags` 供反射单测）+ `:666-672`（expandMulti 同款 setNested 改动）| 改 `DataLoader.java`：新增 package-private 静态 `scopedCacheKey(String)` 统一入口（`:125-127`），3 个进缓存分支全部改用（`:95` 1-arg $view 分支 / `:114` 1-arg 非视图分支——**评估后仍加，理由：该分支走 CachedPathParser 直查物理表不经过 QuotePendingRewriter，本身结果不因 scope 开关而变，加 tag 纯防御性统一，关闭态逐字不变，不改变现有行为** / `:216` 5-arg $view 分支）| 改 `BomTreeRenderService.java` `resolvePendingOwner()`（原 :445-449）：改读 `QuotePendingScope.pendingOwner()`（3 行），修复报价树独立断点（`ConfigureSnapshotService:350` 调 render() 时从未 setNested 过，原实现恒返回 null）| **顺手修复 T1 遗留的测试退化风险**：`BomTreeRenderServiceTreeParamMaskingTest` 原用 `SqlViewRuntimeContext.set(...)` 驱动 resolvePendingOwner()（T1 编写时 T2 尚未接线），T2 落地后若不改会让该测试静默退化为空转（assertDoesNotThrow 仍通过但不再验证任何改写逻辑）——已改用 `QuotePendingScope.open()/restore()` 驱动，恢复真实覆盖面 | 新增单测 `QuotePendingScopeTest`(12：DRAFT开/SUBMITTED-APPROVED-PUBLISHED关/未开/嵌套/异常路径finally还原/cacheTag开关/SUBMITTED专项边界)、`DataLoaderScopedCacheKeyTest`(4，同包直调 package-private 静态方法，无需反射)；`ComponentDriverServiceCacheKeyTest` 新增3个反射测试（对 `buildExtraCacheTags` 私有静态方法：关闭态逐字不变/开关不同/qidTag与pending标签不可混淆） | **三个不变量落点**：①quotationStatus恒传null的注释——`ComponentDriverService.java` expand()内`:353-368`与expandMulti`:667-669`各一处，`QuotePendingScope.java`类级javadoc与`open()`方法javadoc各一处；②两层缓存补维度——第一层`ComponentDriverService.java:390-392`(buildExtraCacheTags)，第二层`DataLoader.java:95/114/216`(scopedCacheKey)；③无public clear()——`QuotePendingScope.java`全文只有 open/restore/pendingOwner/cacheTag 4个public方法，class javadoc显式写明理由 | `DB_HOST=localhost mvn -o test -Dtest='QuotePendingScopeTest,ComponentDriverServiceCacheKeyTest,SqlViewIsolationBoundaryTest,SqlTextMaskTest,SqlViewExecutorNamedParamMaskingTest,DataLoader*Test,SqlViewValidatorCommentMaskingTest,BomTreeRenderServiceTreeParamMaskingTest,SqlViewExecutorPendingHookTest,DataSourceResourceTest'`：Tests run 77 / Failures 5 / Errors 7 / Skipped 0，T1+T2 新旧共 64 个测试全绿，非零的 12 个全部逐一用 `git stash` 切回改动前复测确认为预置失败（签名逐字相同）：`DataSourceResourceTest` 5 failure（401 认证 fixture）+ `SqlViewExecutorPendingHookTest` 3 error（硬编码 componentId 不在库） | ⚠️ **新发现第3组预置失败**（backtask.md 原只登记前2组，本次补充确认）：`com.cpq.formula.DataLoaderTest`（注意与 `com.cpq.formula.dataloader.DataLoaderScopedCacheKeyTest` 不同包）4 个 error，`NullPointerException: this.sqlViewExecutor is null` @ `DataLoader.java:89`——该测试用 `new DataLoader()` + 反射注入 pathParser/sqlCompiler/dataSource 三个字段但**从未注入 sqlViewExecutor**，而 `loadByPath` 顶部无条件调 `sqlViewExecutor.isSqlViewPath(...)`（此行 T1/T2 均未改动，属纯前置代码），`git stash` 到完全干净 baseline 复测同样 4 error 逐字相同，与 task-0725 无关，系该测试自身与"阶段2 $ 前缀旁路"功能上线时不同步的历史遗留 bug，建议登记 BACKLOG | **AC-17 静态验证**：`grep -rn "QuotePendingScope.open(" src/main/java` 零命中（本 Task 只提供能力，不接线，接线是 T3 的工作）| **环境坑（并发纪律）**：本 worktree 与 F1/F2 前端 Task 并发共用，`git stash -u` 会连同 F1/F2 未提交的 `.tsx` 改动一并暂存/恢复——两次验证用后均用 `git stash pop` 完整还原且无冲突，但**后续应避免在共享 worktree 上做 stash 操作**，改用单独 clone/worktree 做 before/after 对比更安全 | **未做**：不接线任何 `open()` 调用点（T3 范围）、不动 `SqlViewExecutor:553/:567`、不动 `SqlViewRuntimeContext`、本期无 Flyway


[2026-07-25] 报价单页签数据修复(task-0725) F1/F2 前端 - 给实时 batch-expand 传业务侧别 usage | 涉及文件: `cpq-frontend/src/pages/quotation/useDriverExpansions.ts`(hook 新增第4参 `usage:'QUOTE'|'COSTING'`(必填), 写入每个 batchExpand task 的 `usage` 字段, 纳入 effect 依赖数组) / `cpq-frontend/src/services/componentService.ts`(`BatchExpandTask` 接口加可选 `usage` 字段) / `cpq-frontend/src/pages/quotation/QuotationStep2.tsx:3362,3364`(报价侧传 'QUOTE'、核价侧传 'COSTING') / `cpq-frontend/src/pages/quotation/QuotationWizard.tsx:135`(该实例只喂 `lineItems` 与下方 `buildSnapshotExpansions(lineItems,'QUOTE',...)` 同源，传 'QUOTE') / `cpq-frontend/src/pages/quotation/ReadonlyProductCard.tsx:284-286`(由已有 `isCosting`(`side==='COSTING'`) 派生，未新增 prop) | **关键决策**：①`usage` 只进请求体，不进 `driverExpansionKey`/fingerprint 缓存键——报价侧/核价侧本就是两个 hook 实例、两个独立 `useState` cache，结构上不存在跨侧命中，加维度反而有 AP-31 键错位风险(2026-07-25 评审已否)；②`usage` 纳入 useEffect 依赖数组，因 `ReadonlyProductCard` 的 `usage` 值会随用户切换报价/核价视图在**同一 hook 实例**上变化(非字面量常量)，QuotationStep2/QuotationWizard 三处仍是恒定字面量、加入 deps 对其无副作用 | **自测**：`tsc --noEmit` 0 错误；4 个改动文件 + 主入口 Vite 200；`git diff` 逐行核对 `driverExpansionKey`(:130-137)与 `fingerprint`/`tasks` useMemo 块未改动；真实浏览器(snap chromium, 因沙盒缺 google-chrome 改用 executablePath 绕过)登录真实后端(客户 32aea5b1 罗克韦尔、报价单 c670e9e7、lineItem 6ad49abc)、切到详情页「核价单」视图，实测捕获 `[Y1.5 expand-driver] batch 触发` + 临时 `console.debug` 打印的 batchTasks 载荷，逐字段确认 `"usage":"COSTING"` 真实出现在即将发往 `POST /components/batch-expand` 的请求体中(验证完删除该临时打点行) | **环境限制(与本次改动无关，如实记录)**：本 worktree 沙盒缺 `google-chrome`/`psql`/`redis-cli` 二进制，`npx playwright test`(`quotation-flow.spec.ts`/`composite-product-flow.spec.ts`)按仓库既定 `channel:'chrome'` 配置无法启动；且当前 `cpq_db_0724`(2026-07-24 起新默认库)缺该 spec 依赖的「苏州西门子」客户/「报价模板0608」等夹具数据，即便替换浏览器可执行文件也会在客户选择步骤失败——这两项均为环境/数据问题，与 F1/F2 代码改动无因果关系（同一现象在 `git stash` 回退到 HEAD 未改动代码时复现一致）；**遗留发现**：`ReadonlyProductCard` 切换核价视图时，该 line item 的 batch-expand 请求在真实浏览器中出现"发起但未达网络层"的静默中止(`page.route`/`performance` 双重确认零请求送达)，经 `git stash` 验证**在改动前的 HEAD 版本即可复现同一现象**，判定为与 task-0725 无关的既有行为，未纳入本次修复范围，供后续排查参考。

[2026-07-25] task-0725 T3 — 报价侧 4 个权威 set 点接线 + bucketKey 侧别维度（让 T1/T2 的能力真正生效） | `configure/service/ConfigureSnapshotService.java`(P1: `snapshotLines:273-283/542-546` 新增 `loadQuotationStatus` + `QuotePendingScope.open/restore` 包住 `:272 QuotationIdContext.set` 同一 try/finally，覆盖建单/加产品/saveDraft/从基础刷新/报价树) / `quotation/service/CardSnapshotService.java`(P2×2: `refreshQuoteCardValues(li,force):2124-2159` 包住 `expandFlatDriverBaseRows`；`dryRunTokenRows:2675-2706` 包住同款调用；P4×1: `ensureExcelValues:672-685` 只在报价分支 if 块内 open/restore，核价分支不动——两分支共用同一方法体+同一 for 循环，已按需求确认不能整方法包裹) / `component/resource/ComponentResource.java`(P3×3: phase1 循环 `:268-313`、bucket-merge `:386-403`、`runSingleTask:432-460` 均按 `task.usage` 条件 open；`usageTag`/`isQuoteUsage`/`prefetchQuoteStatuses` 新增私有方法；bucketKey `:343-345` 新增 `|u=` 维度) / `component/dto/BatchExpandDriverRequest.java`(Task 新增可选 `usage` 字段，无默认值字面量，由 `ComponentResource.isQuoteUsage` 统一按"非 QUOTE 一律 COSTING"兜底) / `datasource/sqlview/QuotePendingRewriter.java`(**T3 收尾发现的独立 bug，非 4 个 set 点范围**：新增 3-arg `rewrite(sql,conn,injectAnchor)`，`injectAnchor=false` 跳过主位表探测+`__v6_id`锚点插入整段；2-arg 签名保留委派 `true`，3 个既有调用方零改动) / `quotation/service/BomTreeRenderService.java`(`queryRecursive:380` 改调 3-arg `rewrite(...,false)`) | **背景**：T3 开工前 T1/T2 均已就绪但零调用点，本任务是"让修复真正生效"的一步。**关键决策**：①`quotationStatus` 恒从 quotation 表现查现取，不复用调用方已有变量，避免过期 status 误判冻结；②P4 `ensureExcelValues` 报价/核价分支共用同一方法体（同一 for 循环内相邻 if），open/restore 只包报价分支的 `buildExcelValues(...,q.customerTemplateId,...)` 调用，核价分支 `buildExcelValues(...,q.costingCardTemplateId,...,true)` 原样不动；③P3 三处均需 `Map<UUID,String> quoteStatusById`（批量预取，仅收集 usage=QUOTE 的 quotationId，避免 N+1 且 COSTING task 不产生多余 DB 往返），bucket-merge 处按 `pivot.usage` 整桶判定（桶内 usage 已因 bucketKey 新维度保证一致）；④bucketKey 新增 `|u=` + `usageTag()` 归一化（非 QUOTE 一律记为 "COSTING"，避免非法值裂桶）。**T3 实施期意外发现并修复的独立 bug（AC-17 安全，未扩大 open() 白名单）**：P1 生效后 `BomTreeRenderService.resolvePendingOwner()` 第一次在报价树递归 CTE 路径上真正返回非 null，激活了此前从未被真实执行过的 `QuotePendingRewriter.rewrite()` 调用——其"主位表探测"启发式（顶层无 FROM 则退化为"文本序第一个 FROM，任意深度"）对本单「报价BOM树-QUOTE口径v1」这种 `WITH RECURSIVE` + SELECT 列表内相关子查询（`(SELECT sd.bom_version FROM material_bom_item sd WHERE...) AS bom_version`）的 SQL 形态选错主位（选中了子查询内的 FROM，而非递归分支真正的行来源 FROM），把 `sd.id AS __v6_id,` 插进一个只能返回单列的标量子查询 SELECT 列表 → Postgres `subquery must return only one column`，BOM 页签整棵树渲染失败。修法：加 `injectAnchor` 参数让 `BomTreeRenderService` 跳过锚点注入（树 spine 本就不需要 `__v6_id`，其最终 wrapper 查询显式按列名只取 5 列，注入与否不影响既有输出），2-arg 签名（`SqlViewExecutor`/`QuoteBackfillColumnMapper`/`QuoteViewValidationService` 三个既有调用方）零改动零回归。 | **端到端实测**（样本 QT-20260725-0001 / c670e9e7）：`POST configure-product/quotations/{id}/refresh-snapshot`(P1) + `POST quotations/{id}/refresh-card-snapshot`(P2) 200/200；页签行数 产品=1 ✅ / BOM=6节点 ✅(node_path `__nodeId` 逐条不同，`00006` 根下与 S-80011 下各一次) / 材料成本=2 ❌(期望3，见下)/ 外购件成本=1 ✅ / 加工费=1 ✅(小计14) / SUBTOTAL=14非0 ✅；BOM `__renderError` 哨兵 0 条；连续3次刷新行数稳定不累加；他单 pending 行(978479fd)0条命中，隔离正确。**材料成本 2≠3 的诚实结论**：非本次 pending-visibility 根因 1 的回归，是独立、预先存在的设计缺口——`ConfigureSnapshotService.java:561` 注释明确写"报价侧无 BOM 闭包，分桶键就是产品料号本身（不做 BOM 递归展开）"，材料成本/外购件/加工费的 expand 种子 partNo 恒为 `li.productPartNo`（产品自身），从未纳入 BOM 树闭包发现的子件（如 S-80011），P1/P2 两条路径均如此、与 pending 作用域开关无关；需求方 §8 期望的"3 行含 S-80011"依赖一个当前代码库里尚不存在的"报价侧 BOM 闭包注入 hfPartNos"机制，不在 T3 的 4 个 set 点范围内，登记为独立 follow-up。P3 用真实 curl 验证：同一 task 仅改 `usage` 字段，QUOTE 侧 2 行+带 `__v6_id`，COSTING 侧 0 行+无 `__v6_id`（未开 pending 可见域时该数据本就不可见，AC-17 零回归的直接证据）；同批次混合 usage（api.md §2.3 场景）两个 task 各自独立求值，未被 bucketKey 合并污染。`DB_HOST=localhost mvn -o test` 全量因已知 ARJUNA016051 毒事务级联（task-0721 RECORD 已记录的环境级问题，与本次改动无关）不可用作信号；改按包 scoped 跑 973 测试/43 failures/33 errors/53 skipped，与 T3 直接相关的 9 个测试类（）全绿或按设计 skip；`RefreshCardSnapshotTest` 1 个失败经代码走查确认是测试自身对"新建行 `cardSnapshotAt` 初始为 null"的假设与"Excel 导入行 `cardSnapshotAt` 已在导入期设置"的现状不符（短路判断发生在我的 `QuotePendingScope.open()` 调用**之前**），与 T3 改动无因果；`ComponentResourceTest` 4 个 401 与 RECORD 既有记载的"REST 层测试类统一 401，`git stash` 验证 HEAD 同样 401"环境问题同类。 | **已自检**：编译 0 错误(`mvn -o compile` + `test-compile`)；后端热重载后业务端点 401 正常；本期无 Flyway 迁移；scoped 测试 973 跑/9 个直接相关测试类全绿。

[2026-07-25] task-0725 BUG-1 修复 — batch-expand 两处快照旁路无视 usage 门控（T3 遗留缺口，破 AC-17） | `component/resource/ComponentResource.java`(旁路点1 `doBatchExpandPhases` Phase1 `:306`：`if (hasContext)` 改 `if (hasContext && isQuoteUsage(t.usage))` 才调 `tryReadSnapshot`；旁路点2 `runSingleTask` `:446-461`：`hasContext` 分支按 `isQuoteUsage(t.usage)` 三元分派——QUOTE 走原 `expandWithSnapshot`，COSTING/非法/缺省改调不读快照的 9 参 `expand(...)` 重载) / 新建 `cpq-backend/src/test/java/com/cpq/component/resource/ComponentResourceSnapshotBypassUsageTest.java`(4 用例) | **根因**：`ComponentDriverService.tryReadSnapshot`/`expandWithSnapshot` 读的 `quotation_line_component_data.snapshot_rows` 是报价侧专属快照表（只由 `ConfigureSnapshotService`/`QuotationTreeService` 写），该方法本身完全不看 `usage`/`QuotePendingScope`——T3 只在"要不要 `QuotePendingScope.open()`"这一层做了 usage 门控，遗漏了"要不要读快照表"这一层，COSTING task 命中快照即会拿到报价侧 pending 数据 + `__v6_id` 锚点。修复原则：COSTING/非法/缺省一律不窥探快照、强制走真实展开（scope 天然关闭 → 无 pending 行/`__v6_id`），QUOTE 侧原有快照优化逐位不变。**未动** `!bucketEnabled` 遗留分支（`ComponentResource.java:286`，flag 关闭时的老路径，无条件调 `expandWithSnapshot`，同款隐患但默认 flag=true 不会命中，用户明确只要求两处，未扩大范围，如实登记供后续评估）。新增测试类：自建 component+`component_sql_view`(指向白名单表 `unit_price`，保证 COSTING 侧真实展开必然成功不报错)+自建 DRAFT quotation+line item+一条**预置**含 `__v6_id` 的 `quotation_line_component_data.snapshot_rows` 夹具；`@BeforeEach` 插入后立即原生 SQL 回查断言非空且含 `__v6_id`（前置非空断言，防重蹈"快照本来就 miss 所以测试空转通过"）；单 task 批量（`idxs.size()>=2` 才能合桶，1 个 task 必然走 `runSingleTask`，覆盖旁路点2）分别验证 usage=QUOTE 命中快照(driverPath="snapshot"+含 `__v6_id`)/COSTING/非法值"XXX"/缺省(null) 均不命中(driverPath≠"snapshot"+不含 `__v6_id`)；`@AfterEach` 清理 component/component_sql_view/quotation/line_item/component_data 五张表，跑完 DB 残留 0/0/0/0。 | **验证**：①真实 curl 复现用户原始 3 case（componentId=edfa54ff 材料成本/lineItemId=6ad49abc/quotationId=c670e9e7）：COSTING/XXX/缺省 三者均变为 `driverPath="$mc_view"`(真实视图) + `rows:[]`，不再含 `__v6_id`；QUOTE 侧仍 `driverPath="snapshot"` + 2 行各带 `__v6_id`，优化未受影响；②`refresh-snapshot` 重算后逐页签行数与 T3 记录完全一致：产品1/BOM6/材料成本2/外购件成本1/加工费1/`quotation_line_item.subtotal=14.0000`；③`QuotePendingScopeOpenWhitelistTest`(3)+`QuotePendingSqlTextAssertionTest`(3)+`ComponentDriverServiceCacheCrossScopeTest`(2)+`QuotePendingRewriterOfficialVisibilityAndSupersedesTest`(2)+新增`ComponentResourceSnapshotBypassUsageTest`(4) 合计 14 个测试全绿 Skipped=0，`open(` 调用点白名单总数仍为 7（本次修复未新增任何 `QuotePendingScope.open()` 调用点，只是给既有快照读路径加了 usage 门控）。**性能影响评估**：COSTING 侧失去快照快路径后必须走真实 SQL 展开——用户判断"核价侧自 2026-07-03 起就不走实时 batch-expand 了（`QuotationStep2.tsx:3353 useSnapCosting`），唯一还走的是存量单详情页"，经 grep `usage.*COSTING\|useSnapCosting` 及 F1/F2 RECORD 条目（前端固定核价侧传 `usage:'COSTING'`）交叉核实**成立**：核价侧主渲染路径已切到快照优先的 `useSnapCosting`，本次改动只影响"详情页等仍走 batch-expand 且传 COSTING/无 usage"的调用方，此前它们能命中快照本就是 T3 之前遗留的意外行为（且是错误行为——AC-17 要求核价侧本就不该有 pending 快照优化），代价可接受；未发现其它会因此显著变慢的核价侧调用方。**顺带发现但未修（越权/不在本次范围）**：`mvn test` 跑 `BatchExpandSnapshotPrefetchEquivTest`（2026-06-26 P0 批次的老测试，与 task-0725 无关）出现 1 个 pre-existing failure——`ExpandDriverResponse.Row` 严格反序列化 + `BomTreeRenderService`(task-0721 B3) 写入 BOM 树节点时在 row 顶层加 `__lvl`/`__nodeId` 等系统列，`SNAPSHOT_MAPPER`(`ComponentDriverService.java:74`，无 `FAIL_ON_UNKNOWN_PROPERTIES` 豁免)读回 BOM 页签快照时抛 `UnrecognizedPropertyException`（被 catch 吞掉降级为 WARN + 回落实时展开，不影响本次修复的正确性，仅让 BOM 页签快照优化静默失效）；该 bug 与本次 BUG-1 的两处改动均无代码交集（`tryReadSnapshot`/`SNAPSHOT_MAPPER`/`ExpandDriverResponse.Row` 本次一字未动），是被本次验证过程中调用 `refresh-snapshot` 材料化 BOM 树数据顺带暴露的独立缺口，建议登记 BACKLOG 由后续任务处理。

[2026-07-25] 报价单渲染 - task-0725 修复「报价单页签全空」三根因 + 交付验收 | 分支 worktree-task-0725-quote-pending-fix 5 提交(3f7b84e3 T1+T2 / 0fa27ebb 前端 / 84d14835 T3 / 62eaebc2 T4 / 89f0fcb7 BUG-1)；新增 SqlTextMask + QuotePendingScope；改 ComponentDriverService / DataLoader / SqlViewExecutor / SqlViewValidator / QuotePendingRewriter / BomTreeRenderService / ConfigureSnapshotService / CardSnapshotService / ComponentResource / BatchExpandDriverRequest + 前端 useDriverExpansions/QuotationStep2/QuotationWizard/ReadonlyProductCard/componentService；文档见 dev-docs/task-0725-*/（需求说明+backtask+fronttask+api+test+testcase+验收报告） | **三个独立根因**：①pending 感知改写在驱动展开链路失效——ComponentDriverService setNested(componentId,null,null,null) 把 quotationId 擦成 null(覆盖语义不继承)，门槛 SqlViewExecutor:555 恒关 → 4 页签返 0 行、报价树只渲光根；②rewriteNamedParams 不屏蔽 SQL 注释，注释里的 :customerCode 也被换成 ? 多绑一个参数而 pgjdbc 忽略注释内 ? → "column index is out of range: 3, number of columns: 2"(cp_view/bom_view 命中)；③costing_bom_tree_config 表空缺 usage=QUOTE 配置(全库无 INSERT 迁移，配置只活在 DB，环境重建即丢) | **架构方案 D'**(否决 A=no-op/B=20签名)：新建 QuotePendingScope pre-gated 窄作用域，AC-17 靠"核价侧不调用 open()"结构性保证 + AC-10 靠 open() 内建冻结判定；quotationStatus **恒传 null** 以同时满足 SqlViewExecutor:555 门槛与 ComponentSqlViewService:379 休眠分支两个消费者 | **两层缓存**都补 pending 维度(ComponentDriverService.cacheKey + DataLoader 三个 loadByPath 重载含 :114 仅 path 的粗粒度重载)，关闭态 cacheTag() 返空串保核价 key 逐字不变 | **顺带修复 2 个独立 bug**：(a)P1 生效后递归 CTE 首次进 QuotePendingRewriter，主位表探测 fallback 对 WITH RECURSIVE+标量子查询选错主位、把 __v6_id 插进单列子查询 → PG "subquery must return only one column" → BOM 树 100% 失败；修法加 3-arg rewrite(sql,conn,injectAnchor)。(b)BUG-1 快照旁路：ComponentResource:301/:446 无视 usage 直读 quotation_line_component_data.snapshot_rows(该表只由报价侧写)，修复前该表为空故无害、T3 让报价侧开域写入后就含 __v6_id → COSTING/非法/缺省 usage 全部拿到 pending 数据破 AC-17；用 isQuoteUsage 门控两处 | **实测**(技术总监独立复跑)：产品1/BOM6节点/材料成本2/外购件1/加工费1/subtotal=14；6 类错误标记 0 条(__renderError 哨兵消失)；00006 两次且 node_path 不同未挂错枝；他单 pending 泄漏 0；3 次 refresh 全 200 行数不变；14 测试类 86 tests 0F/0E/Skipped=0 | **AC-17 门禁改为环境无关 SQL 文本断言**(SqlDebugContext 负向+正向对照+前置非空断言+open() 白名单单测+skipped==0)——原 4 个等价性测试硬编码锚单不在库全部静默 skip；⚠️ **同一病症本轮暴露 6 例**(测试硬编码库内数据换库即失效)，已列 BACKLOG P1 全量审计 | **交付限制(不得表述为已端到端验证)**：AC-7 无集成级证据(样本单 costing_card_template_id 为 NULL 无核价卡可渲染、AP-37 交叉前提不成立、QUOTE 侧官方 current 行 0/0/0、pending_supersedes 非空 0/0/0)；AC-12 无法验证(模板 excel_view_config 为 NULL)；E2E 本环境非可靠门禁(缺 google-chrome+psql，默认 config 0 条执行) | **技术总监更正自己 4 处文档错误**：重算端点 configure→**configure-product**(错的返404)、材料成本 3→**2**(原 SQL 未按 hf_part_no 收窄，报价侧按设计无 BOM 闭包见 ConfigureSnapshotService javadoc+:371/:581)、外购件断言 W-1001/OUTSOURCED→二者都不出输出列、P-4 基线 10/6/20→**5/3/10**(原为两张 pending 单合计) | 并发纪律：多 agent 共用 worktree 时**禁止 git stash/checkout--/reset/clean**(本轮两次险情)

[2026-07-25] repair-0726 Task B1 - Flyway V362：material_master 加 `pending_quotation_id` UUID NULL 列(部分索引 `ix_mm_pending` WHERE NOT NULL) + 存量 `pending_material_master_staging`(90行/84 distinct material_no/4 distinct quotation_id) DISTINCT ON(material_no, ORDER BY created_at) 迁回正表 + DROP 暂存表退役 | `cpq-backend/src/main/resources/db/migration/V362__material_master_pending_flag.sql` | **关键决策**：ON CONFLICT(material_no) DO UPDATE 口径对齐 promoteStaging 的 upsertByMaterialNo(preserveDescriptive=true)——material_name/material_type 用 COALESCE(现值,新值)保留正表已有值优先，其余描述性列 COALESCE(新值,现值)以暂存表新值优先；**故意不在 UPDATE 分支写 pending_quotation_id**，防止正表已生效的老料号被暂存表同料号行降级为 pending。Docker 库(`cpq-jh-postgres`/cpq_db) BEGIN…ROLLBACK dry-run 验证：INSERT 0 84(83 新插入+1 冲突更新)，mm_pending_not_null=83(=84 distinct - 1 与正表重合的 S-3120014539)，mm_total=14+83=97，staging 表 to_regclass 为 NULL(已 DROP)；负向断言 S-3120014539(正表原有 14 行之一，与暂存表 material_no 唯一重合项)迁移后 pending_quotation_id 仍为 NULL，未被降级；ROLLBACK 后复核 staging 仍 90 行、mm 仍 14 行、列已消失，确认零残留。**未手工 psql -f 执行/未 COMMIT 任何 DDL**，迁移交由 Quarkus dev 启动时 Flyway migrate-at-start 自动跑(后续 Task 负责)；本任务只提交迁移脚本文件，不碰 Java 代码，未重启/新起 8081 dev server。

[2026-07-26] repair-0726 Task B1 follow-up - V362 代码质量评审修订(采纳 7 项/不采纳 4 项由需求方裁决) | 同上 + `backtask.md`/`需求说明.md` §12.1 代码块同步改名 | **采纳落地**：①补文件头版本自述+spec溯源指针(对齐 V349 风格)；②索引名 `ix_mm_pending`→`ix_material_master_pending`(对齐 V349 建的 `ix_*_pending` 家族 7/8 成员写全表名的惯例，`mm` 在同时有 material_master/material_bom/mat_* 的 schema 里歧义大)，两处文档同步改名防止文档与实现打架；③迁回行 `created_at` 由 `NOW()` 改存暂存表原值(暂存表 DROP 后这是仅存时间溯源)；④`ORDER BY material_no, created_at` 补 `quotation_id` tiebreak 防同时间戳归属不确定；⑤`DROP TABLE`→`DROP TABLE IF EXISTS`(对齐仓库 12/14 带 IF EXISTS 的惯例)；⑥"故意不写 pending_quotation_id"注释从分号后移到 DO UPDATE SET 块内紧邻最后一列赋值处；⑦纠正"逐列对齐"表述只对 ON CONFLICT 分支成立、对 DISTINCT ON 去重不成立(整行胜出 vs 逐行 upsert 归并语义不等价)，**实测复核当前数据集 3 组重复料号(S-3120014539/S-80011/W-1001 各 3 行)跨单描述性列值完全相同**(仅 quotation_id/created_at 不同)，故本次迁移去重无损，此为实测结论非假设。**不采纳项**(需求方裁决，非本任务推翻)：DROP 拆独立文件 V369(同 worktree 迁移目录必然同批应用，拆分不降低风险)；改 RENAME 式软删除(需求方 Q10 已裁决一次性 DROP)；改 array_agg 按列聚合去重(方案定稿指定 DISTINCT ON 且实测当前数据无损)；`ADD COLUMN IF NOT EXISTS`/uuid 小写(纯风格，最近邻 V349 未用)。**重新 dry-run 验证**：indexname 含 `ix_material_master_pending`(6 个索引名单列出无 `ix_mm_pending` 残留)；INSERT 0 84(83 新增+1 冲突)；mm_pending_not_null=83；mm_total=97；S-3120014539 pending 仍 NULL；用 TEMP TABLE 提前快照 staging 各料号最早 created_at(因迁移会 DROP 源表)，S-80011/W-1001 断言 `matches=t`(迁回行 created_at 精确等于暂存表最早行时间戳，非 NOW())；staging regclass NULL；ROLLBACK 后 staging 90 行/mm 14 行/无新列/无新索引，零残留。

[2026-07-26] repair-0726 Task B4+B5 - 料件主数据列表过滤 pending + Q02 销售料号补 material_type | `basicdata/v6/entity/MaterialMaster.java`(加 `pendingQuotationId` UUID 字段) / `basicdata/v6/service/MaterialMasterCrudService.java`(`list()` where 起手由 `1=1` 改 `pendingQuotationId is null`) / `basicdata/v6/quote/Q02CustomerMapHandler.java`(成品/销售料号同步由 `upsertBatchMaterialNoOnly` 改 `upsertBatchNameType(NameTypeRow(no,null,"零件"))`，preserveDescriptive=true) | **B4 关键决策**：count 与 find 共用同一个 `where.toString()` 变量，避免 total 与实际条数不符(仓库已知反模式)；`MaterialMasterDTO` 不加 `pendingQuotationId` 字段(接口零变化，按 api.md §2.1 口径)；`create`/`update`/`applyRequest` 不触碰该字段(新建恒 null=正式料号，update 走 findById 天然保留原值)；`GET /{id}` 详情走独立的 `MaterialMaster.findById`(非 list() 的 where)，未加谓词，未受影响、无需改动。**B5 关键决策**：口径依据 `PartTypeInferenceService.mapMasterType`——「客户料号与宏丰料号的关系」/「成品其他费用」/「组装加工费」销售料号(主件)权威口径=零件(ASSEMBLY)；名称仍传 null(该 sheet「客户料号名称」是客户维度语义，非料件品名，不得误写入 material_name)；preserveDescriptive=true 保护已有类型不被推断兜底覆盖；pendingQuotationId 原样透传不改语义。**明确未动的三处**(需求方已裁决)：报价渲染/SQL 视图 join material_master(`ExistingProductService`/`CustomerPartCandidateService`/`ConfigureProductService` 等)零谓词、`MaterialNoResolver` 发号查重零谓词、`ExistingProductService`+`CustomerPartCandidateService` 现有 pending 闸门本就挂在 `material_customer_map.pending_quotation_id`(别名 `m`/`mcm`)而非 `material_master`(别名 `p`)，未叠加——grep 逐一核对过。**验证**：`mvn test-compile` BUILD SUCCESS；本地 docker `cpq-jh-postgres`/cpq_db 内 BEGIN…ROLLBACK 手工加列验证 where 语法与过滤生效(14→13 行，标记 1 条 pending 后可见行减 1)，ROLLBACK 后列消失、零残留；V362 尚未 apply 到远程 dev DB，故未跑依赖新列的集成测试(预期内，交后续 Task 处理)。

[2026-07-26] repair-0726 Task B2+B3+B6+B7+B8 - 料号直落正表+行级 pending 标记：repo 写入语义/生命周期接线/回填口径/测试/全链路验收 | `basicdata/v6/repository/MaterialMasterRepository.java`(3 个批量 upsert + 单行 upsert 加 `pending_quotation_id` 列；新增 `flipPending`/`deletePendingWithGuard`/`listPending`；退役删除 `stageOne`/`listStaging`/`promoteStaging`/`clearStaging`) / `basicdata/v6/service/V6QuotationCommitService.java`(过户循环并入 material_master) / `basicdata/v6/quote/QuoteImportService.java`(重导清理追加带守卫回收) / `quotation/service/backfill/QuoteBackfillService.java`(promoteStaging→flipPending；`cleanupPending` 内 `clearStaging` 整行删除) / `quotation/service/QuotationService.java`(删单 8 表在前、料号回收在后) / `QuoteBackfillPlan+Collector+PreviewService`(`materialMasterStaging`→`materialMasterPending`，数据源换 `listPending`) / `test/.../MaterialMasterPendingTest.java`(新建 8 用例) / `Update0723ImportAcceptanceTest`+`MaterialBomMergeHandlerTest`(暂存表断言迁移) | **关键决策**：①写入语义靠 `INSERT ... pending_quotation_id=:pq` + `ON CONFLICT DO UPDATE` **不含该列** 一条 SQL 同时满足「新行打标记 / 正式行不降级 / 别单 pending 不被抢占」三条裁决，8 个 handler 的 call site 一行未改；②**顺序铁律证伪**——原方案称删单必须「先删 8 表后删料号」否则本单 bom_item 顶住守卫，实测不成立：守卫 `x.pending_quotation_id IS NULL OR x.pending_quotation_id <> :qid` 已排除本单自己的 pending 行，真正的承重墙是 `<> :qid` 子句本身（注释已按实情改写并警示不得移除该子句；代码顺序保持不动）；③守卫拦下时原本**完全静默**（返回值被丢弃）→ 两处 call site 加 WARN 日志（deleted/survivors 计数 + quotationId），把"pending 标记指向已删报价单的孤儿行"变成可观测；④`listPending` 固定 `ORDER BY material_no`（token 侧已 `Collections.sort`，此处只求结果集确定性）；预览 token 标签 `#staging=` **刻意不改名**，改了会让在途 preview token 重算→误报 409。**全链路验收（本地 docker cpq-jh-postgres/cpq_db，客户罗克韦尔 CUST-0001，`报价系统模板0723.xlsx`）**：AC-1 导入后 S-80011(零件,单重12.55)/W-1001(外购件) 即刻入正表且 pending=importRecordId、材质 991/992 不入表 ✅；AC-2 create-quotation 后两行过户为 quotationId 且与 8 表同步 ✅；AC-3 submit→preview→costing-approve 后 pending 清空转正、状态 APPROVED、**预览 token 跨 submit 仍有效未误报 409** ✅；AC-4 删单双向验证——被正式行/别单 pending 行引用时守卫拦下 2 条并打出 WARN（行保留），无外部引用时 2 行被正确回收、14 行正式料号不受影响 ✅；AC-5 渲染 `bom_view` 等 5 个 join material_master 的视图**零 pending 谓词**（全局可见），实测 pending 期间 S-80011 的品名「投入零件1」/单重 12.55 可正常 join 出（修复前该行根本不在正表→断链）✅；AC-6 主数据列表 pending 不可见（totalElements=14 vs 库内 16），核价通过后即出现 ✅；AC-7 同客户二次导入复用既有料号、无新铸号（total 16→16、9 字头 0）且**已转正的行未被降级** ✅；AC-8 V362 success=t、`to_regclass('pending_material_master_staging')`=NULL、存量 83 行带标记迁回 ✅。**环境注意**：①本地库缺材质 991/992（`material_recipe`），导入会以「未找到材质」失败 3 行 → 验收前已补种两条 ACTIVE recipe；②测试 profile 默认连不可达的 `10.177.152.12`，跑测试须 `DB_HOST=localhost`；③`./mvnw` 有 distributionSha256Sum 校验失败，用系统 `mvn -o -Dmaven.repo.local=/home/joii/project/repository`。**遗留技术债**（已登记 BACKLOG）：跨单复用 pending 料号无转正路径（BL-0074）；`MaterialMasterRepository` 位置性 NULL 填充与长参数列表（BL-0075）。

[2026-07-26] repair-0726 最终评审回写 - 两处方案层面的实质纠偏 + 部署前置检查 | `需求说明.md` §13(新增) / `backtask.md` §4 / `api.md` §3 / `fronttask.md` FR-4 / `dev-docs/task-0709-.../update-0723/test-plan.md`+`test-report.md`(失效横幅) / `BACKLOG.md` | **纠偏1「删单顺序铁律」被证伪**：守卫 `AND (x.pending_quotation_id IS NULL OR x.pending_quotation_id <> :qid)` 已排除本单自己的 pending 引用行，先删 8 表只是移除了本就对守卫不可见的行 → 删除顺序对正确性无影响；真正的承重墙是 `<> :qid` 子句本身（移除它顺序才成铁律）。实证 `MaterialMasterPendingTest#t4`（故意违反顺序、回收仍成功 deleted==1）+ 核实 material_master 无任何入向 FK。四处文档已加删除线纠偏，代码顺序保持不变（数据卫生）。**纠偏2「旧机制的第二个独立成因」**：暂存行键是 `quotation_id=importRecordId`，而 `repointPendingOwnership` 无法纳入暂存表（列名是 `quotation_id` 非 `pending_quotation_id`、也不在 PENDING_TABLES），可 `promoteStaging(quotationId)` 查的是 `WHERE quotation_id=:qid` → **任何真实核价通过都匹配 0 行，暂存料号从来没有 promote 成功过**。这是需求方「正表查不到」的第二个独立成因，本次改动顺带闭合；**勿误以为旧机制本来 work**。**部署前置检查（V362 在远程/团队库首跑前必做）**：`DISTINCT ON (material_no)` 是整行胜出、同料号跨单列值分歧会静默丢弃后行且同一迁移立刻 DROP 不可回溯；「无损」结论只对本地 90 行存量成立。目标库先跑 `SELECT material_no FROM pending_material_master_staging GROUP BY material_no HAVING count(DISTINCT (material_name,material_type,specification,dimension,old_material_no,usage_property,unit_weight,standard_unit,production_no))>1;` 期望 0 行，非空需先人工合并或改按列聚合写法。详见 `需求说明.md` §13.3。**其余修正**：需求说明端点名 `/api/cpq/basicdata/material-masters`→`/api/cpq/material-masters`；api.md+fronttask.md 把「预览后必现 409 = listPending 漏 ORDER BY」的错误因果更正（token 侧已 Collections.sort，与结果集顺序无关）；task-0709/update-0723 两份测试文档加「暂存表已 DROP、SQL 不可执行」失效横幅。**新登记** BL-0076（接线层回归测试缺口：B4 列表过滤/B5 Q02 类型/建单过户/flipPending 先于 cleanupPending 四项只有手工验收背书，其中第 4 项是 backtask 点名的最易错处却无测试锁死）。

[2026-07-26] 树配置校验 - CostingTreeSqlValidator 补 :versionFilter 宏展开(保存期与运行期对齐) | `component/service/CostingTreeSqlValidator.java` + `test/.../CostingTreeSqlValidatorTest.java`(3 新用例) | **缺陷**: 保存期 dry-run 把递归 SQL 原文直接发给 PG 做 probe, 未展开 `:versionFilter` 宏 → 含宏配置一律 `syntax error at or near ":"` 被拒。而**核价树版本切换(task-0713 B3)正依赖该宏**, 运行期 `BomTreeRenderService:362` 走 `expandForExecution` 一直正常 → 形成「运行期支持、保存期拒收」的不对称: 库里 active 的含宏配置**改一个字都存不回去**(只能直接写库, 这也解释了它们当初怎么进去的)。**归属**: pre-existing, task-0713 B3 加宏能力时漏配保存端, **非 task-0725 引入**; 对照组 `SqlViewValidator:151-152`(页签视图校验器)早已正确调用 `SpineKeysMacro` + `VersionFilterMacro` 两个 `expandForValidation`, 故属遗漏而非设计。**修法**: probe 前先 `expandForValidation`(展开为 `(is_current列)`, 不引入新占位符不污染 required_variables); 宏语法错误捕获 `IllegalArgumentException` 转校验失败 Result, 避免冒泡成 500 | **验证**: TDD 先 RED 后 GREEN——新用例修复前实测报 `syntax error at or near ":"`、畸形宏用例实测抛 IllegalArgumentException 冒泡; 修复后 `CostingTreeSqlValidatorTest` 6/0/0。**真实数据 A/B 对照**(cpq_db 全部 4 条真实配置, 用修复前 probe 逻辑在 psql 复跑): 含宏的 `BOMV2`(COSTING **active**) + `TASK0721-QUOTE-BOMV2克隆` 修复前 **REJECT** → 修复后 ok; 不含宏的 `BOM树1` + `报价BOM树-QUOTE口径v1`(QUOTE active) 前后均 ACCEPT(无回归)。回归 `CostingBomTreeConfigServiceTest` 5 + `SqlViewValidatorCommentMaskingTest` 7 + `SqlViewValidatorSpineKeysTest` 3 + `VersionFilterMacroTest` 8 全 0F/0E; codegraph 确认生产调用方仅 `CostingBomTreeConfigService.create:60`/`update:81` 两处, 均被上述测试覆盖 | **连带纠正**: 2026-07-25 树递归SQL配置条目的「关键约束①树配置不能用 :versionFilter 宏」**已作废**(是缺陷不是约束, 照此执行等于主动放弃核价版本切换能力), 「约束②注释禁写命名参数」亦已由 task-0725 的 `SqlTextMask` 修复 —— 两条均已就地划删除线标注。**worktree 环境坑**: 主仓未跟踪的 `V363` 已应用到共享库, 新建 worktree 缺该文件 → Flyway `Detected applied migration not resolved locally: 363` 启动失败; 需从主仓 cp 该文件进 worktree(不 git add)
[2026-07-26] 前端构建修复 - `npm run build` 两处 tsc 阻断错误 | `cpq-frontend/src/pages/config/ElementEditDrawer.tsx:148` / `cpq-frontend/src/pages/quotation/useDriverExpansions.ts:156-172` | **①antd v6 破坏性变更**：项目用 `antd@^6.3.5`，`Orientation` 已收窄为 `'horizontal'|'vertical'`（分割线自身方向），标题位置改由 `titlePlacement` 承担；该行仍是 v5 写法 `orientation="left"`，是全工程最后一处漏改（`V6BomItemDetailDrawer`/`CustomerManagement`/`OverridesDrawer` 等早已用 `titlePlacement`）。修法只改这一个 prop 名；`orientationMargin={0}` 虽被标 deprecated 但类型仍合法，**保留不动**——换成 `styles.content.margin` 会把左右两侧间距一起归零，标题会贴住右侧横线。**②TS1016 必选参数排在可选参数之后**：task-0725 提交 `0fa27ebb` 给 `useDriverExpansions` 末位加了必传的 `usage: 'QUOTE'|'COSTING'`，但它排在 `customerId?`/`quotationId?` 之后，语法不成立；Vite dev 不做类型检查故只在 `tsc -b` 暴露。**刻意不用 `usage = 'QUOTE'` 给默认值**（会让将来漏传的调用方静默拿到"打开 pending 可见域"的宽松档，违背 AC-17 的结构性保证），改为把前两参写成 `string | undefined`（位置必填、值仍可空）以保住 `usage` 的必传性；现有 4 个调用点（QuotationWizard/QuotationStep2×2/ReadonlyProductCard）本就传满 4 实参，**零调用方改动**。注意 `?:` 与 `| undefined` 在参数位**产出 JS 逐字节相同**（类型标注被擦除，不像默认值会 emit `if (x === void 0)` 赋值），故这个 AP-44/E2E 强制文件本次为纯类型改动、运行时行为零变化。**自检**：`tsc -b`/`--noEmit` 0 错误；`vite build` EXIT=0（dist 2.69MB/gzip 798kB）；两文件 Vite transform 200；主入口 200。

[2026-07-26] 部署建库脚本同步 - `deploy/cpq-init-empty-navicat.sql` 增量同步至 Flyway V362 | `deploy/cpq-init-empty-navicat.sql`（表结构 3 处 + Flyway 基线 1 处 + 文件头说明） | **增量确定方式**：该脚本 2026-07-24 从 `cpq_db` dump（129 表），而 `cpq_db_0724` 的 flyway 历史里 V361 记为 `<< Flyway Baseline >>`、V362 为唯一后续迁移 —— 说明现库正是由本脚本导入后 baseline 而来，故理论增量只有 V362。用脚本解析 + `information_schema` 做**列级全量比对**确证：差异恰为 `material_master` 缺 `pending_quotation_id`、多整张 `pending_material_master_staging`，索引差异恰为缺 `ix_material_master_pending`、多 `ix_pmm_staging_quotation`，与 V362 内容逐条吻合（约束比对的 100+ `chk_*` 差异是正则假阳性——pg_dump 把 CHECK 内联在 CREATE TABLE 里，已抽样核实）。**落地 4 处**：①`material_master` 末位加 `pending_quotation_id uuid`（对齐库内 ordinal 18）；②删除 staging 表定义 + `pending_material_master_staging_pkey` + `uq_pmm_staging_quotation_material` + `ix_pmm_staging_quotation` 共 4 个对象；③按 pg_dump 字母序在 `ix_material_bom_pending` 与 `ix_mcm_pending` 之间插入 `ix_material_master_pending`（语句与线上 `pg_indexes.indexdef` 逐字比对一致）；④**关键**：文件末尾手写的 Flyway 基线行 `VALUES (1,'361',...)` 上调为 `'362'` —— 脚本表结构既已含 V362，基线仍停在 361 会让新库连 Quarkus 时重放 V362 并因"列已存在"启动失败，同步改了该段注释并加警示。**未同步 V362 的 `COMMENT ON COLUMN`**：本脚本风格为全文零 COMMENT 语句（`grep -c "COMMENT ON"` = 0），保持一致。**验证**：改后脚本 129 表 ↔ `cpq_db_0724` 129 表，表集合差 0、共有表列差 0；staging 残留引用仅剩文件头我新写的说明文字 1 行（非 DDL）；`information_schema` 自检期望值 129/3/4 保持成立。**未做**：无本地 PG/docker，未真跑建库导入验证（可在共享库建临时库跑一遍再 DROP，需另行确认）；`deploy/cpq-init.sql` 与 `deploy/cpq-init-navicat.sql`（均 07-19 生成）属更早基线、本次未同步。

[2026-07-26] 树递归SQL配置 - 报价侧/核价侧 BOM 树递归 SQL 双侧配置纳入版本控制(V363 种子 + deploy 脚本同步) | 新建 `cpq-backend/src/main/resources/db/migration/V363__seed_bom_tree_config_both_usages.sql` / 改 `deploy/cpq-init-empty-navicat.sql`(文件头 + 种子块 + 自检 SQL 三处) | **背景**：`costing_bom_tree_config` 自建表起全库无任何 INSERT 迁移种子，配置只活在 DB —— 这正是 task-0725 根因③；`cpq_db_0724` 由空 schema 建库后该表 **0 行**，双侧皆缺。本次把配置本身纳入版本控制，根治"环境重建即丢"。**内容来源**：旧库 `cpq_db` 两条 active 行原文（COSTING=`BOMV2` md5 `784e51ed…` len 1586；QUOTE=`报价BOM树-QUOTE口径v1` md5 `0b8e458a…` len 1063），用 python 嵌入迁移以保证逐字节不走样。**关键取舍(用户裁决)**：核价侧**保留 `:versionFilter` 宏**——它是 task-0713 核价单版本切换作用到 BOM 主树的唯一通道(`BomTreeRenderService.queryRecursive:361` 对含宏模板调 `VersionFilterMacro.expandForExecution` 展开成 `:__vfPart/:__vfVer` 双数组谓词，空 override 时退化为 is_current 零回归)；代价是 `CostingTreeSqlValidator:48` 保存期 dry-run 只替换 `:production_part_nos`、不展开宏，故该 SQL **无法经 API/UI 保存**(必被判"递归 SQL 无法执行")，只能走迁移或直连改——这是显式取舍非缺陷，已写进迁移注释。报价侧不带宏(版本切换是核价侧专属，报价侧提交后 frozen)，因此仍可正常经 API 编辑。**⚠️ 2026-07-26 同日更新：该「代价」已被消除**——`CostingTreeSqlValidator` 补调 `VersionFilterMacro.expandForValidation`(提交 `772dd401`，对齐页签视图校验器 `SqlViewValidator:151-152` 的既有做法)，含宏递归 SQL **现已可正常经 API/UI 保存**，真实配置 A/B 实证 `BOMV2` 由 REJECT 转 ACCEPT。故本条「只能走迁移或直连改」的限制、以及 V363 迁移注释里的同款说明**均已过时**；保留宏的取舍本身依然成立且现在零代价。**幂等**：两条 INSERT 均带 `WHERE NOT EXISTS(该 usage 已有 active 行)` 守卫，与唯一索引 `uq_bom_tree_config_active_per_usage (usage) WHERE is_active` 不冲突；已配过的库(如旧 cpq_db)重复应用不插入不覆盖。**deploy 脚本同步**：种子块插在 Flyway 基线段之前，**基线仍停在 362 不上调**——V363 幂等，连 Quarkus 时会再跑一次并自愈(脚本漏配也能补回)，比 baseline=363 更稳；该块用**标准单引号转义而非美元引号**，规避文件头已注明的 Navicat `$$` 解析中断(两条 SQL 内无分号，`;` 切分器也不会截断)。**验证**：①迁移 BEGIN…ROLLBACK 干跑连应用两遍 → 第 1 遍 `INSERT 0 1` ×2、第 2 遍 `INSERT 0 0` ×2(幂等)、md5 逐字节一致、回滚零残留；②按 `VersionFilterMacro` 算法在库外复现宏展开(1586→2126 字符，`:__vfPart` 出现 6 次)后真实探针：契约 5 列(root_no/material_no/bom_version/parent_no/node_path)齐备，seed=`{S-3120014539,S-3111320636}` 展开 **20 节点/2 根/4 层**，同料号 `S-3111320636` 两处 occurrence 的 node_path 各自独立未挂错枝；报价侧 2 节点=仅光根(库内 `material_bom_item` 只有 PRICING 14 行、QUOTE 0 行，符合预期)；③Quarkus 重启实跑 Flyway **V363 success=t 106ms**，落库 md5 与源库一致；④API 层 `GET /api/cpq/costing-bom-tree-config?usage=COSTING|QUOTE` 各返 1 条 active、不传 usage 返 2 条；⑤deploy 种子块单独提取后在事务内清空重插 → md5 一致 + 二次应用 `INSERT 0 0` + 回滚恢复；转义字面量另交 PG 亲自解析回算 md5 双向确认；⑥回归 `QuoteBomTreeEndToEndTest` **4/4 passed**、`CostingBomTreeConfigServiceTest` **5/5 passed**、Skipped=0，跑完种子行完好未被测试残留污染。 | **顺带排掉一个环境坑(非本次引入)**：主仓 `target/test-classes` 残留 **16 个孤儿 .class**(源码已被 task-0723 退役等提交删除，如 `BasicDataImportV5*Test`/`CostingTemplateResourceTest`/`VersioningQueryResourceTest`)，Quarkus 测试引导扫编译产物 → `Unsatisfied dependency for BasicDataImportServiceV5` 等 6 个 CDI deployment problems → **所有 @QuarkusTest 在发现阶段全崩**(报 `Could not load class`，极易误判成自己的改动搞坏了测试)。已逐个用 `git log --diff-filter=D` 确认确有删除提交后定点删除这 16 个 class 文件；**`target/classes/com/cpq/datapath/grammar/CpqPath*.class` 共 17 个虽同样"无对应 src 源码"但是 ANTLR 生成产物(源在 target/generated-sources)，不可删**。根治办法是 `./mvnw clean`，但会丢已打好的 `target/quarkus-app`。

[2026-07-26] 组件模板配置(rule-0724) - 罗克韦尔报价组件 JSON 重出(模板补勾料号列) | 产物 `客户组件模板/组件导入-罗克韦尔.json`(覆盖 0724 版，bundle 5 组件 checksum `sha256:5386af25…`) + 文档 `dev-docs/rule-260724-组件模板配置/报价侧.md`「已交付实例」新增 0726 行、0724 行标注被覆盖 | **触发**：用户改了 `客户组件模板/罗克韦尔模板.xlsx` —— ①材料成本「销售料号」→「料号」并**勾了料号列** ②外购件成本「销售料号」→「料号」并勾料号列、「组成件名称」→「外购件名称」、**新增手填「单价」列** ③BOM「料件名称」→「料件」并勾名称列+行键。**关键判定**：模板既然确有料号列，按 `1-总则与工作流.md §1.3.2` 料号铁律绑**本页签语义层级**的料号 —— 材质元素 `partNoField=料号 → ebi.material_part_no`(材质料号)、外购件 `→ mbi.component_no`(组成件料号)、零件仍 `→ up.code`；**禁绑销售料号**(会让 `BomNodeTypeResolver` 把成品误判成材质/外购件)。代价：这两个页签不再出 `_销售料号` 可见列(字段与模板列严格 1:1)，`hf_part_no` 仍绑 `material_no` 当驱动键，故按行料号收窄不受影响。**交付路径**(AGENT-配置入口 §5 标准法)：暂存目录经 REST API 建 5 组件+5 视图+绑 driver → `GET /component-directories/{id}/export` 取 bundle → 删暂存(组件先删再删目录) → 落 JSON 文件；用户自建目录后 `POST /{新id}/import/commit?conflictPolicy=RENAME` 导入。**自检**：①字段与模板列 1:1 + 视图输出列无死列/无缺列(脚本比对 PASS) ②5 视图保存期 dry-run 全过(无 400) ③`cpq_db_0724` 报价侧 V6 表当前 0 行 → 改在旧库 `cpq_db`(CUST-1269 真数据)背靠背实跑 5 段 SQL：材质料号出 `00002/992/AgNi90/991`、材质名逐行经 `material_part_no` 出 AgC3/AgNi11#-Ⅰ/银镍合金、工序出 电镀/酸洗/分条、BOM 树 12 条边(含无父根分支)、`f_customer_element_price` 无策略返 NULL 且**元素行 12 行不掉行**(LEFT JOIN 硬约束成立) ④`characteristic='OUTSOURCED'` 全库 0 行 → 外购件页签暂 0 行是**数据未导入**非配置错(去掉该过滤同 SQL 出数，已对照) ⑤bundle 导入预演 `checksumValid=true / canCommit=true / 5×CREATE / 0 blocker`(预演目录已删)。 | **顺带纠正一条已过时的文档前置**：0724 交付时记的「`cpq_db_0724` 一条 `costing_bom_tree_config` 都没有、BOM 页签导入后仍不出树」**已消除** —— V363 迁移把 QUOTE/COSTING 两条递归 SQL 纳入版本控制，实测新库各 1 条 active。BOM 页签导入后即可出树，无需人工补录。 | ⚠️ 注意：本次只出配置产物，未落用户目录、未绑模板、未跑 refresh-snapshot（新库无报价侧 V6 数据，渲染级自检须待用户导入数据后在真实报价单上做）。

[2026-07-26] 核价基础数据组件模板 — 按 `核价系统功能基础数据功能结构所需字段新版.xlsx` 产出 17 个核价组件的导入 JSON | 新增 `客户组件模板/组件导入-核价基础数据.json`（bundle v1.0，17 组件 + 17 个 `$view` + 依赖清单 + checksum） | **范围**（用户拍板）：物料BOM → 其他外加工成本共 18 个 sheet，「单重」合并进物料BOM（取**组成料号/子件**的 `material_master.unit_weight`），「电镀方案」**不建组件**（无料号维度的全局方案库，同元素价格表/汇率表口径）→ 实建 17 个。**字段口径**（用户拍板「只建有数据源的列」）：品名/规格/尺寸全部保留并 JOIN 主档取真值；删 6 处「是否有效」（`labor_rate`/`production_energy`/`auxiliary_energy`/`unit_price` 只有系统位 `is_current`，仅 `tooling_cost.is_effective` 有真列故模具页签保留）、删「要素名称」@来料其他费用（P16 落 `cost_type`=**要素编号**）、删「要素编号」@成品其他比例费用（P19 落 `cost_type`=**要素名称**）、删 2 处「一级项次」（`item_seq` 恒 NULL，P16/P17 的 `seq_no`=**二级项次**）、删「项次」@来料加工费（P15 CONTENT 无 `seq_no`）。**对 2026-07-16 旧版核价通用模板 v1（旧库 dir d35342a9）纠正 3 处绑定错**：①来料 3 张原走 `JOIN material_master ON production_no` 桥接 → 改直取 `up.finished_material_no`（P15/P16/P17 groupKey 本就是销售料号）；②其他外加工成本原 `JOIN ON mm.production_no = up.code` → P23 的 `code` 是**销售料号**不是生产料号，改直取；③来料其他费用/固定费用的一级/二级项次原绑反（`seq_no`↔`item_seq`）。另：`rowKeyFields` 按规范改用**字段名**（旧版误用视图列名）；物料BOM 设 `tabType=BOM`（bundle 不承载 `bomRecursiveExpand`，靠 `ComponentImportService:247` 由 tabType 自动同步树标记）。**自检**：17/17 组件 + 视图 dry-run 200；psql 直跑 17 视图 0 语法错、14 张出数（物料BOM 14 行父子边 + 单重带出、元素BOM 7 行、产能/折旧/生产能耗/辅助能耗各 4、模具 2、耗材 3、包装 1、来料加工 2、来料固定 1、加工费 4、成品比例 2、成品固定 2）；expand-driver 13/17 出真实行（物料BOM 树视图 standalone 0 行=无 BOM 闭包上下文的通性，与线上 pj_view 同理，已用 psql 直跑证明取数正确）；**交付 JSON 导入全新目录端到端验证 `checksumValid=true / blockers=[] / commit 200`，导入后 code 按 sheet 顺序 COMP-0065~0081、driver/tabType/sortField/requiredVariables 全还原、取数与暂存目录逐行一致**；暂存目录与验证目录均已删除，库恢复干净。 | ⚠️ **3 张页签当前 0 行 = 数据缺口非配置错**：来料其他费用(比例)（库中 `INCOMING_OTHER` 唯一行 `cost_ratio` 为空 → 归固定费用）、电镀成本（**P22 对「电镀方案编号」非空的行直接 `continue` 跳过不落库**，而 xlsx 该 sheet 唯一行方案编号=A0001 → `unit_price` PLATING 0 行）、其他外加工成本（`OUTSOURCE_PROCESS` 0 行，xlsx 该 sheet「销售料号」列为空、值填在生产料号位）。⚠️ **工序名称列当前全空**：`process_master` 新库 0 行，且 `labor_rate`/`production_energy`/`auxiliary_energy`/`tooling_cost` 自带 `process_name` 也全空；视图已按 `coalesce(pm.process_name, 表自带)` 双路兜底，待主档补 Z 码工序即自动出数。⚠️ 本次只出配置产物，未落用户目录、未绑核价模板、未跑 refresh-snapshot（渲染级自检须待用户建目录导入 + 挂模板后在真实核价单上做）。

[2026-07-27] repair-0727 回填语义与预览重做 F1~F3 前端 - 核价通过预览抽屉从 V6 物理字段串改为产品级中文变更说明 | 涉及文件：`cpq-frontend/src/services/costingOrderService.ts`（F1，按 `dev-docs/repair-260727-回填语义与预览重做/api.md` §1.1/§1.2 扩展类型：`CostingApprovePreviewSummary` 加 `affectedProducts`；`CostingApprovePreviewRow.changes`/`values` 由 `Record<col,[old,new]>`/`Record<col,val>` 改为数组 `CostingApprovePreviewChange[]`/`CostingApprovePreviewValue[]`（带中文 `label`），行加 `rowLabel`/`conflict`；`CostingApprovePreviewGroup` 加 `productNo`/`productName`/`categoryLabel`/`route`/`baseSource`/`baseRowCount`/`resultRowCount`/`axisLabels`；`CostingApprovePreviewResult` 加 `products: CostingApprovePreviewProduct[]` 与 `globalShared: CostingApprovePreviewGlobalShared`）/ `cpq-frontend/src/pages/quotation/CostingApprovePreviewDrawer.tsx`（F2，渲染层重写：摘要条一行文案取代 4 格 Statistic；按 `products` 渲染产品 `Card`，卡内按 group（即 `categoryLabel`）用 `Collapse` 分节，`route=FLIP` 且 0 行变更的分节默认折叠+标灰「仅版本转正，内容无变化」，其余默认展开；行级用 `rowLabel` + 中文 `changes`/`values` 展示，`conflict=true` 加橙色 Tag；`globalShared` 单独进红色边框 Card「全局共享变更（影响所有客户）」；新增防御性「未归类变更」兜底区——某 group 若既不在任何 `product.groupIndexes` 也不在 `globalShared.groupIndexes` 里则单列展示，不静默丢弃）| **关键决策**：①未在 `quotationService.ts` 发现 `CostingApprovePreview*` 重复定义（grep 全工程确认），故无需类型收敛，`costingOrderService.ts` 是唯一权威定义源；②`buildSectionItem` 按「1 group = 1 分节」直接映射，未按 `categoryLabel` 二次分桶合并同类 group——因 api.md 契约里 `categoryLabel`/`axisLabels` 已是 group 级字段（1 group 通常即对应 1 类别 + 1 轴组合），二次分桶属未经契约证实的臆测结构，遵守「不擅自加功能」；③摘要区由原 4 格 `Statistic` 卡片改为单行 `Space` 文案（`影响 N 个产品 · 新增 X 行 · 删除 Y 行 · 改值 Z 行`），严格贴合 fronttask.md ASCII 结构图原文措辞；④交互契约（loading Spin / 加载失败 Alert+重试 / 409 自动重拉不关抽屉 / 500 关抽屉 / 提交带 `previewToken`）逐行核对未改动，只重做了 `preview.groups` 之后的展示层 | **自测**：`npx tsc --noEmit -p tsconfig.json` 0 错误（worktree 无 `node_modules`，按已知坑软链主仓 `node_modules` 后跑，tsc 查的是本 worktree 真实文件）；worktree 内另起临时 `vite --port 5199 --strictPort` 验证（非主工作区 5174），`curl 127.0.0.1:5199/src/pages/quotation/CostingApprovePreviewDrawer.tsx` 与 `.../costingOrderService.ts` 均 200 且返回体是真实转译后的 JS（非 error overlay），验证完已 kill 临时进程；未跑真实报价单核价通过全链路截图（后端契约按需求文档所述"此刻可能还没落地"，本次未联调，属已知未覆盖点）| **已知限制/未覆盖**：①fronttask.md F3 自检清单里「真实报价单走一遍出截图」「空变更场景截图」未执行——后端 D1~D5/patch 语义改动为并行 agent 同批次开发，本次未等后端联调；②`noImpact` 判定仍沿用原 4 字段（`versionedGroups`/`addedRows`/`deletedRows`/`changedRows`）零值判断，未额外校验 `affectedProducts===0`（按 api.md 语义两者应恒一致，`groups` 为空则 `products` 必为空）；③未改动 `useDriverExpansions.ts`/`QuotationStep2.tsx`/`QuotationWizard.tsx`/`ReadonlyProductCard.tsx`，按 fronttask.md 明示 E2E 非强制，未跑 Playwright。

---

[2026-07-27] 报价升版（repair-0727） - **核价通过回填改 patch 语义 + 顶层 UNION 锚点 + 预览重做**（task-0721 返修）

**起因**：用户实机反馈两条 —— ①核价通过预览"财务看不懂"②HJ-20260726-0007 通过后新建报价单 BOM 页签空白。排查发现②不是渲染故障，是**核价通过那一刻把基础数据写坏了**：`material_bom_item` 组 (QUOTE,CUST-0001,S-3120014539) 4 行→1 行空壳（`component_no`/`characteristic` 全 NULL），`element_bom_item.base_qty` 0.624610→NULL，而预览显示"12 组 0 增 0 删 0 改"。

**四联根因（详见 `docs/反模式.md` AP-60）**：
- D1 页签投影被当整组权威行集 → REBUILD 抹掉未表征行（`$wg_view` 只取 4 行里的 OUTSOURCED 1 行）
- D2 REBUILD 只写页签暴露列 → 未暴露列写 NULL（`mc_view` 不暴露 `base_qty`）
- D3 `$bom_view` 顶层 `UNION ALL` → `QuotePendingRewriter` 安全降级不注锚点 → 真实全量 BOM 对回填不可见，发言权让给了投影页签
- D4 预览按逐列 diff 判断有无变更，"未表征行将被删除"不在模型里 → 预览撒谎

**涉及文件**：后端 `QuotePendingRewriter`（逐分支注入 `__v6_id` + `Result.primaryBranchSql`）、`QuoteViewValidationService`、`QuoteBackfillColumnMapper`（set-op 取主位分支探测）、`QuoteBackfillCollector`（全面重写：基底行集装载/列级 patch 合并/四路径/Profile 计数）、`QuoteBackfillPlan`、`QuoteBackfillService`、`QuoteTableAxis`、`QuoteBackfillPreviewService`、新增 `BackfillLabelResolver`、`dto/backfill/*`；前端 `CostingApprovePreviewDrawer.tsx`（重做为产品卡片 + 中文行差异）、`costingOrderService.ts`。**无 Flyway 迁移**（零 schema 变更），**不触发 AP-44**（不新增 field_type / 不改组件配置 / 不改 snapshot_rows 结构）。

**关键决策（技术总监裁决）**：
1. **patch 语义取代原"丙 全量对齐"**：以 DB 真实行集为基底，页签只给列值，删除**必须有显式墓碑**。"页签里没出现"不再等于"用户删了它"。
2. **新增第四路径 NOOP**（基底=current 且零变更 → 整组跳过），由此得不变式 `route==REBUILD ⇒ rowChanges 非空`，用 `IllegalStateException` 兜底。原三路径设计会让"引用已有产品未编辑"这种常见场景走 REBUILD 并触发告警误报。
3. **effectiveNewRows 以基底行为遍历主轴、按 id 一一对应**，禁止"遍历 candidate 生成行"（那正是 D1 的形状）；多页签表征同一行合并成一份 patch，冲突按 sortOrder 先到先得 + `conflict` 标注。
4. **纯 pending 行被墓碑删除 = 物理删除**（`cleanupPending`），"老版本降 `is_current=false` 留存"只适用于曾经官方生效过的行——两套机制，已在 javadoc 写清。

**验证**：后端 46/46 + 电镀全局升版回归绿；**master 基线反向对照**（tester 从 master detach 临时 worktree 实跑）单个用例同时复现 D1+D2+D4：`is_current 行数=1（应为 4）`、幸存行 `characteristic/base_qty=null`、`groupShownInPreview=false`；**同库 A/B 亲验**（8081 跑 master、8099 跑修复分支，同一张 QT-20260726-0015 调只读 preview）：master 12 组且该 BOM 组整组不出现，修复后 17 组且该组 `route=FLIP / 4 行→4 行`；预览文案实机截图核对（Playwright 真实抽屉）。

**⚠️ 遗留**：S-3120014539 那个**已被写坏的组不会自愈**（现库里就是 1 行空壳），需重导或修数据，见 BACKLOG BL-0080。
[2026-07-26 晚] 组件模板配置(rule-0724) - 罗克韦尔报价组件 JSON 再出（「料号列」改语义为**所属销售料号** + 闭包去重修复） | 产物 `客户组件模板/组件导入-罗克韦尔.json`（覆盖当日早版，bundle 5 组件 checksum `sha256:1a78965e…`）+ 文档 `dev-docs/rule-260724-组件模板配置/3-SQL视图.md §3.8.2/§3.8.3(新增铁律3)/§3.8.6` + `报价侧.md`「已交付实例」新增本版行 | **触发**：用户改 `罗克韦尔模板.xlsx` —— 材料成本/外购件成本/加工费三个数据页签的「料号」列**取消勾选「料号列」、备注改为「所属销售料号」并勾「行键」**，外购件「外购件名称」→「料件」。这是 task-0726 §10.4-2 遗留项（`_归属料号` 是否做成可见字段）的用户裁决：**做成可见字段**。**关键判定**：①三视图 `_料号` 改绑 `ebi.material_no` / `mbi.material_no` / `up.finished_material_no`（本行归属的销售料号），原绑的 `material_part_no`/`component_no`/`up.code` 退回只作 JOIN + ORDER BY 键；②三页签 `partNoField` **一律留空**（把成品级料号绑给材质/零件/外购件页签＝§5.5 料号铁律硬错，会让树类型判定把成品误判成子件），标识由名称列（材质/料件）承担，后端 2026-07-23 已放宽为「料号列或名称列至少一个」；③「料号」已按 §3.8.4 进 `rowKeyFields` 防撞键；④加工费 rk 在模板勾选之外**补了「项次」**（§5.3 硬规则：同料号同工序可重复，`seq_no` 才是料号内唯一保证）——唯一一处偏离模板勾选处，已向用户明示。 | 🔧 **顺带修掉 §3.8.2 闭包标准写法的行翻倍缺陷（新增铁律 3）**：`material_bom_item` 同一对父子常并存 `RECIPE`+`ASSEMBLY` 两条边 → 递归 `UNION ALL` 产生重复路径 → `LEFT JOIN bom_closure` 把数据行成倍复制，**页签小计直接翻倍且静默无错**。实测旧库 CUST-1269 `0363-2607000009` 材料成本 **8 行（每行×2）→ 去重后 4 行**。修法=递归段改 `UNION` + 增 `bom_closure_d AS (SELECT root_no,node_no,MIN(lvl) … GROUP BY root_no,node_no)`，数据页签 JOIN 去重后的 CTE；加第二个 CTE 不破铁律 1（顶层 FROM 仍是白名单表，实测框架照常包成 `) inner_q WHERE inner_q.hf_part_no = ANY(?)`）。**⚠️ 已落库的 COMP-0027/0028/0029 三个 live 视图仍是未去重版本**（当时新库数据每对父子只有单边故未暴露），需重导 JSON 或补丁；`costing_bom_tree_config` 树递归 SQL 有同源风险未核。 | **自检**：①字段与模板列严格 1:1、视图输出列无死列无缺列（脚本比对 5/5 PASS）②5 个 `$view` 保存期 dry-run 全过 ③`expand-driver` 真管道实跑 4 组件无报错、谓词注入正常 ④旧库 `cpq_db`(CUST-1269 真数据)背靠背实跑最终导出版 SQL：材料成本闭包出**子件** `…07/…08` 的 Ag/Cu/Ni 且 `_料号` 正确显归属（4 行不翻倍）、加工费出「分条/长管质量处理」（工序经 `process_master`）、铁律 2 兜底探针（闭包为空时单料产品仍出自己的行）通过 ⑤导入预演 `checksumValid=true / canCommit=true / 5×CREATE / 0 blocker`；暂存/探针目录均已删除，库恢复干净。 | ✅ **pending 开域实测（补做，纠正一处误判）**：`cpq_db_0724` 报价侧 V6 数据 `is_current` 全 false，我起初误判为"导入版本化链路坏了"——**实为正常态**：每行都带 `pending_quotation_id`，是"导入后待审核、未转正"的 pending 数据，task-0725 已把 `QuotePendingScope` 接进报价驱动链路，DRAFT 单渲染时 `QuotePendingRewriter` 把白名单表整表替换成 `(t.is_current OR t.pending_quotation_id = :pq)` 开域可见（规则见 `3-SQL视图.md §3.6` 生效时间线）。按改写器算法原样模拟后实跑 DRAFT 单 `2168c574`/料号 `S-3120014539`：**材料成本 4 行**（主件 H65/Cu + AgNi11#-Ⅰ/Ag，子件 `S-80011` 的 AgC3 的 Ag/C；元素单价 1850/118478 出数、无价元素 C 留 NULL 不掉行）、**加工费 2 行**（主件 Z380 + 子件 Z381）、**外购件成本 2 行**（同名「组成件1」数量 2/3，靠新的「料号」归属列区分——正是 §3.8.4 的案例），与 §3.8.6 落地清单逐条吻合。**关键机制确认**：`QuotePendingRewriter.rewrite` 对**任意深度**的白名单表 token 都做替换（`for (TableMatch mt : matches)` 无深度过滤），故闭包 CTE 里的 `material_bom_item` 同样开域，子件闭包在 pending 数据下成立；而主位表探测/锚点注入只认 **depth==0** 的 FROM，本次新增的 `UNION`(去重) 与 `GROUP BY`(bom_closure_d) 都在 CTE 内(depth 1)，不触发 `hasTopLevelSetOp`/`hasGroupByAtDepth` 的安全降级，`__v6_id` 锚点照常注入。⚠️ 另：`characteristic='OUTSOURCED'` 全库 0 行（外购件页签暂无数据）；`up.code` 对应料号在 `material_master` 无名 → 加工费「料件」当前为空，而 `partNoField` 已留空，故零件页签对树类型判定暂无有效标识值。


[2026-07-27] 组件模板配置(rule-0724) - **施耐德-1 报价组件套装 JSON（8 组件，新客户首出）** | 产物 `客户组件模板/组件导入-施耐德-1.json`（bundle v1.0，8 组件 + 8 个 `$view` + 依赖清单，checksum `sha256:5d49d3b5…`）；输入 `客户组件模板/组件模板-施耐德-1.xlsx`（用户填好 8 个 sheet 的字段名 + tabType + 角色勾选） | **组件构成**：产品(主件,$cp_view,partNo=销售料号,partName=客户料号名称,rk=[销售料号],**不做闭包**) / 物料(**BOM 树**,$bom_view,partNo 空,partName=料件,rk=[料件],树契约 `:total_material_no`) / 材料成本(材质元素,$mc_view,**partNo=料号→`ebi.material_part_no`(材质料号)**,partName=料件,rk=[销售料号,料号,元素],sort=项次,元素单价+货币接价格策略) / 外购件成本(外购件,$wg_view,partNo 空,partName=料件,rk=[销售料号,料件]) / 加工费(零件,$jg_view,**partNo=料号→`up.code`(零件料号)**,partName=料件,rk=[销售料号,料号,项次,工序],sort=项次) / 组装加工费(主件,$zz_view,**源 `capacity`**,partNo=销售料号,partName=料件,rk=[项次,工序],sort=项次) / 来料固定加工费(tabType 空,$ll_view,`price_type='INCOMING_MATERIAL_PROCESS'`,rk=[销售料号,料件,项次,工序],sort=项次) / 其他费用(主件,$qt_view,`price_type='FINISHED_MATERIAL_OTHER'`,partNo=销售料号,rk=[项次,类别],sort=项次) | **相对罗克韦尔模板的新增页签与判定**：①**组装加工费**在 `unit_price` 无对应 price_type，源是 **`capacity`(QUOTE)**（沿用旧库罗克韦尔 `zz_view` 口径）——`capacity` 无 `customer_no` 列故不加 `:customerCode`，靠 `hf_part_no=material_no` 按本行成品收窄；现网工序名写在 `process_no`（`process_name` 空、`process_master` 新库 0 行）→ 工序绑 `COALESCE(NULLIF(c.process_name,''), c.process_no)`，实测出「焊接/铆接」。②**来料固定加工费**取 `INCOMING_MATERIAL_PROCESS`（cost_type=来料加工费），该类型**金额存在 `base_value`、`pricing_price` 恒空** → 绑 `COALESCE(pricing_price, base_value)`，实测出 0.043260/0.034140；`operation_no` 全空 → 工序列留空手填（数据缺口非配置错）。③**其他费用**料号在 `code` 列（`finished_material_no` 空），按 §7.3 绑 `up.code AS hf_part_no`。④材料成本「产出类型」= `ebi.component_usage_type`（§7.2 标准映射）。 | **用户拍板 4 项（模板推不出的才问）**：报价侧 / 材料成本+材料损耗成本**两列都手填**（不配公式，`INPUT_NUMBER` is_amount content="0" 无 `default_source`）/ 其他费用「费用」列**只取金额**（现库该客户 4 项只登记了比例 4.5/4/13 → 该列当前为空，是数据口径不是配置错）/ 物料「单位」= **重量单位 `weight_unit`**（非发料 `issue_unit`）。 | **闭包口径（task-0726 §3.8.1 判据）**：材料成本/外购件成本/加工费/来料固定加工费 **做 BOM 闭包**（子件数据进平铺页签，含铁律 1 顶层白名单表 + 铁律 2 `LEFT JOIN`+`COALESCE` 兜底 + 铁律 3 递归 `UNION`+`bom_closure_d` 去重）；产品/组装加工费/其他费用 **不做闭包**（本产品自身属性/费用）；物料是树页签走 Pass2。 | **自检**：①字段↔模板列严格 1:1（脚本比对 8/8 PASS，无凭空新增列）②8 个 `$view` 保存期 dry-run 全过（POST 200）③`expand-driver` 真管道实跑 7 个平铺组件全部无报错、谓词正常包成 `inner_q WHERE hf_part_no = ANY(?)`（产品 1 行 / 材料成本 2 行 / 加工费 1 行 / 组装加工费 2 行 / 来料固定 2 行 / 其他费用 4 行 / 外购件 0 行）；物料是 BOM 树页签走 Pass2 `:total_material_no`，平铺 `expand-driver` 报 `column inner_q.hf_part_no does not exist` 属**契约不适用**非缺陷 ④`cpq_db_0724` 真数据背靠背 psql 直跑 8 段 SQL：材料成本出 H65/AgNi11#-Ⅰ 且元素单价 1850/118478（无价元素留 NULL 不掉行）、BOM 树出 5 条边+根分支、闭包按 pending 开域模拟后子件 `S-80011` 的 AgC3(Ag/C) 与 `W-1001`(数量 3) 正确进页签 ⑤导入预演 `checksumValid=true / canCommit=true / 0 blocker / 8 组件`；暂存目录与预演目录均已删除，库恢复干净。 | ⚠️ **已知数据缺口（非配置错）**：外购件成本严格态 0 行（`characteristic='OUTSOURCED'` 仅 pending 有）、其他费用「费用」列空（只登记比例）、来料固定加工费「工序」空（`operation_no` 无值）、产品「客户料号名称/单位」空（主档未填）。⚠️ pending 态实测发现同一逻辑行在库里**current 行与 pending 行并存且 `pending_supersedes` 为空** → 开域后行数×2，这是**数据/框架既有现象**（与本配置无关，罗克韦尔同库同样命中），转正后即恢复单行。⚠️ 本次只出配置产物，未落用户目录、未绑模板、未跑 refresh-snapshot（渲染级自检须待用户建目录导入 + 挂模板后在真实报价单上做）。

[2026-07-27] 组件模板配置(rule-0724) - **施耐德-1 报价组件套装 JSON 重出（模板改版 v2：删 3 列 + 外购件接组成件其他费用）** | 产物 `客户组件模板/组件导入-施耐德-1.json`（覆盖同日 04:36 版，bundle v1.0，8 组件 + 8 `$view`，checksum `sha256:cf3a9f42…`；旧版备份在本次会话 scratchpad）；输入 `客户组件模板/组件模板-施耐德-1.xlsx`（18:43 用户改版） | **相对 04:36 版的 3 处实质改动**（均经用户拍板，非我方推断）：①**产品删「单位」列**（`mm.standard_unit AS _单位` 一并从 `cp_view` 移除，5 字段）②**材料成本删「材料成本」「材料损耗成本」两列**（原为 07-27 上午用户指定的手填列，本版模板已撤；两列无 `default_source`，故 `mc_view` 逐字未动，12 字段）③**外购件成本新增「要素/费用/单位/货币」4 列**（8 字段），源 = `unit_price` `price_type='COMPONENT_OTHER'`（组成件其他费用）：要素=`cost_type` / 费用=`pricing_price`(is_amount) / 单位=`up.unit`(**计价单位，≠已有的「组成单位」=BOM 发料单位 `issue_unit`**) / 货币=`currency`；`rowKeyFields` 由 `[销售料号,料件]` 扩为 **`[销售料号,料件,要素]`**（用户采纳防御性扩展：`cost_type` 是 Q13 groupKey 一维，同件多要素会撞键）。 | **🔑 取价维度定错会漏价（本次关键判定）**：`Q13ComponentOtherFeeHandler` 的 `finishedMaterialNo = row.getStr("销售料号","宏丰料号","成品料号")` ⇒ 费用登记在【**成品** × 组成件】维度，**不是【直接父件 × 组成件】**。实测该库 `W-1001` 同时直挂成品 `S-3120014539`(数量2) 与挂在子装配 `S-80011` 下(数量3)，而费用只登记了一条 `(code=W-1001, finished_material_no=S-3120014539)` → 若 ON 写 `mbi.material_no`(直接父件)，S-80011 那条取不到价。**正解：`ON co.code=mbi.component_no AND co.finished_material_no = COALESCE(cl.root_no, mbi.material_no)`（闭包根，即 hf_part_no）**；实测两条 W-1001 边在成品根下均出 材料费/55/RMB/PCS，而 S-80011 自当根时留 NULL 且不掉行。 | **⚠️ 偏离文档推荐写法的地方（已实证安全，勿照搬结论前先读这段）**：`3-SQL视图.md §3.6` 要求版本化表改**标量子查询**，但 `cost_type` 需产生**多行**，标量子查询不适用 → 改用**派生表** `LEFT JOIN (SELECT … FROM unit_price up WHERE … AND up.is_current AND customer_no=:customerCode) co ON …`，把 `is_current` 落在**子查询 WHERE**（§3.6 明列的安全位置），**JOIN ON 内不含 `is_current`**，规避的是同一个参数错位坑。 | **自检（全部实跑，非推演）**：①字段↔模板列严格 1:1 —— openpyxl 读模板 vs API 读组件逐列比对 **8/8 PASS**（含顺序），无凭空新增列 ②3 处 PUT 全 200（视图保存期 dry-run 过）③`expand-driver` 真管道 7 个平铺组件全绿（产品1/材料成本2/加工费1/组装2/来料固定2/其他费用4/外购件0）；物料是 BOM 树页签走 Pass2，平铺 expand 报 `column inner_q.hf_part_no does not exist` 属**契约不适用**非缺陷（同上版）④**用真实编译的 `QuotePendingRewriter` 离线跑 8 个视图**（`target/classes` + pg jdbc，不重启共享 dev server —— 启动期 `QuoteViewValidationService` 是 fail-fast，跑挂会让 8081 起不来）：8/8 `LIMIT 0` 探测 OK；`wg_view` **主位表仍 = `material_bom_item`(mbi)**（派生表里的 `unit_price` 在 depth>0 未夺主位，铁律1 未触发）、`__v6_id` 锚点基表 `material_bom_item.id`（页签保持可回填）、输出 10 列含新增 4 列 ⑤`cpq_db_0724` 真数据 psql 背靠背：严格态 0 行（外购件数据缺口）、忠实单张 `:pq` pending 开域后取价正确 ⑥拿**交付文件本体**做导入预演 `checksumValid=true / canCommit=true / 0 blocker / 8 组件`；暂存目录 + 2 个预演目录均已删除，库恢复干净。 | ⚠️ **踩坑记录（供后人复用验证手法）**：自建的"模拟 pending 改写"脚手架若用朴素正则数 `:命名参数`，会把**注释里**的占位符也算成绑定 → 报 `The column index is out of range: N, number of columns: M`，看着像视图坏了其实是**脚手架坏了**（正是 §3.7 记的 task-0725 根因）；正解是复用真实的 `SqlTextMask.mask()` 先屏蔽注释/字面量。首轮 4 个视图"失败"全是此假阳性，屏蔽后 8/8 通过。 | ⚠️ **已知数据缺口（非配置错，与上版一致）**：外购件成本严格态 0 行（`characteristic='OUTSOURCED'` 仅 pending 有）、其他费用「费用」列空（只登记比例）、来料固定加工费「工序」空（`operation_no` 无值）、产品「客户料号名称」空（主档未填）。⚠️ COMPONENT_OTHER 实测 10 个版本行中 v2009 已转正(`is_current=t`)、v2007 是某单 pending，二者 `pending_supersedes` 均为空 → DRAFT 开域后该费用行 ×2（**既有数据/框架现象，与本配置无关**，冻结态单行）。⚠️ 本次只出配置产物，未落用户目录、未绑模板、未跑 refresh-snapshot（渲染级自检须待用户建目录导入 + 挂模板后在真实报价单上做）。

[2026-07-27] 组件模板配置(rule-0724) - **新增 R7 金额→小计联动规则（配置期单向联动，非渲染期派生）** | `dev-docs/rule-260724-组件模板配置/2-组件与字段.md`（新增 R7 正文块 + 字段配置表「小计」行 + fields[] JSON 注释 + R6 段落加指向）+ `AGENT-配置入口.md`（§3.2 字段级属性表新增 `is_subtotal` 行 + §3.3 推导表 + §6 自检新增第 1 项「金额/小计成对自检」，原 1~4 顺延为 2~5） | **规则**：`is_amount=true` 且**语义可沿行累加**的字段（费用/金额/成本/加工费/金额类小计）→ **必须一并 `is_subtotal=true`**；**例外**：单价/汇率/费率等**单位量纲**的金额虽同为 `is_amount=true`（要 ¥ 前缀），但 `is_subtotal` **保持 false**。判据一句话：*该列各行相加是不是一个有业务含义的钱数？* | **为什么要设例外（用户拍板前我方提出的语义冲突）**：`is_amount` 是**显示属性**（只管 ¥ 前缀/货币格式化），`is_subtotal` 是**聚合属性**，喂三处消费口 —— ① 列 footer 列总计 `FormulaCalculator.computeTabSubtotalsByColumn` ② **页签合计** `computeTabSubtotal`（= 把所有 `is_subtotal` 列的列总计**再相加**，见 `FormulaCalculator.java:564-573`）③ 跨组件 `component_subtotal` 公式 token。字面执行"所有金额都勾"会把单价累加进②③ —— 用施耐德材料成本实例算过：2 行元素单价 1850 + 118478 → 页签合计变 120328，那不是成本。**两类错都不报错，只体现为金额不对**，故 §6 自检加成对核对项。 | **口径边界（勿混淆）**：R6 段落原有的"footer 加 ¥ 严格按 `is_amount`，与 `is_subtotal` 无关"**依然成立且未改** —— 那说的是**渲染期**两个标记互不派生；R7 是**配置期**的单向联动（金额且可累加 → 一并勾小计），两句不冲突，已在 R6 段落就地加括注防误读。 | **用户决策 2 项**：①范围 = 可累加的金额才勾，单价/汇率除外（另两个候选：字面全勾 / 反过来收窄 is_amount 定义，均未采纳）②**已交付的施耐德-1 JSON 不动**（新规则只对今后新配客户生效）——故该套 6 个 `is_amount` 字段仍全部 `is_subtotal=false`，属**有意保留的历史口径**，日后若重出需按 R7 重判（可累加的 5 个：其他费用/加工费/外购件成本 的「费用·加工费」；不勾的 1 个：材料成本「元素单价」）。 | 表达方式遵循记忆 `docs-express-rules-not-hardcode-fields`：写成按语义判断的类型级通则 + 举例，不逐字段钉死取值。**本次纯文档，未改任何代码/数据，无需自检编译。**


---

[2026-07-27] repair-0727 工序编号与工序名称落库优化 - 组装工序 Phase1 解析(编号/名称两段匹配)取代原样写名称的错误落库 | 涉及文件：新增 `cpq-backend/.../basicdata/v6/service/ProcessNoResolver.java`（process_master 全表索引一次性载入 + 两段匹配：先按 process_no 精确匹配，再按 process_name 精确匹配，同名多条取 process_no 升序第一条并 Log.warn 留痕全部候选；纯函数式不依赖 CDI/DB，`Index.of(List)` 可脱容器单测）；改 `QuoteImportValidator.java`（Outcome 新增 `assemblyProcessNo: Map<List<String>,Resolved>`，key=[sheetName,料号,原始值] 防 Q14/Q15 撞键；新增 `validateAssemblyProcess`/`validateAssemblyAnnualDiscount`/`finalizeAssemblyGroups` 三方法，按销售料号做**组级判定**——组内任一工序解析失败则整个料号作废：errors 只加 1 条聚合消息，`failedRows` 补齐为该料号全部行数（含本来能解析成功的行），`successRows` 不含该料号任何行，保证 `totalRows==successRows+failedRows` 恒成立；必填校验（销售料号/组装工序为空）文案独立不并入聚合，沿用 `Q14:55` 原文案）；改 `QuoteImportService.java`（Phase1 结果 `assemblyProcessNo` 放入 `ctx.sharedCache`，位置在 `vo.hasErrors()` 判空之后）；改 `Q14AssemblyProcessFeeHandler.java`（`CONTENT` 加 `process_name`，`VERSION_TRIGGER` 不变仍为 `process_no,seq_no`；取值改从 sharedCache 读已解析结果，不再重复解析/查库）；改 `Q15AssemblyAnnualDiscountHandler.java`（`operation_no` 落解析出的真编号，不写名称；工序列允许为空则 operationNo=null）；DB 配置 `component_sql_view.zz_view`（cpq_db_0724，id=8ca940dd-…，1 条）加 `LEFT JOIN process_master pm ON pm.process_no=c.process_no` + `_工序` 列 COALESCE 口径改为 `pm.process_name → c.process_name → c.process_no`，与 `jg_view` 统一；文档 `docs/table/报价系统Excel导入落库方案.md` §14/§15 补充解析规则说明（V3.5）。**关键决策（技术总监二次裁决，纠正了我最初的实现）**：①"组级拒绝"精确语义是**该料号在本 sheet 的全部行数计入 failedRows**（不是"失败料号数=1"），否则报告行数对不上；②必填项为空的错误文案必须与"未登记"聚合文案**保持独立可区分**（前者业务改 Excel、后者补主数据）。**Q15 双 current 时序约束**：`operation_no` 在 `unit_price` 的 groupKey 里（不像 Q14 只在 CONTENT/VERSION_TRIGGER），值从中文名换成真编号后若 `COMPONENT_REDUCTION` 已有老数据，重导会建全新组、老组 `is_current` 不会自动切走——本次改动上线时该 price_type 实测 0 行，无迁移成本，但**必须在其产生数据前上线**，晚了需手工下线老组。 | **自测**：`mvnw -q compile` ✅；`ProcessNoResolverTest`(6) + `QuoteImportValidatorTest`(5) + `Q14AssemblyProcessFeeHandlerTest`(6) + `Q15AssemblyAnnualDiscountHandlerTest`(3) 共 20/20 ✅（含同名多条 Log.warn 实际触发验证）；`*Quote*,*Capacity*,*Process*` 回归 147 个测试仅 3 个失败，均为 `ProcessResourceTest`（`/api/cpq/processes` 401，与本次改动完全无关的 auth 基础设施问题）——**已在干净 master 同步复现同样 3 处失败**，确认是预置回归非本次引入；`zz_view` 改动前后条数均为 1（psql 直查 `component_sql_view`）；改后用 `BEGIN;INSERT INTO process_master...;SELECT...;ROLLBACK;` 验证 COALESCE 优先级生效（主数据命中后 `_工序` 立即切到主数据名，回滚后库内 0 残留）；后端 `/api/cpq/components` → 401 ✅。**已知限制**：本次开发在隔离 worktree，主工作区共享的 8081 dev server 跑的是 master 代码树，Java 层改动无法通过该 8081 做运行时联调，只能靠单测/集成测试 + 直接 psql 验证覆盖；`application-test.properties`（`@QuarkusTest` 走的 profile）仍连旧库 `cpq_db`（非 `cpq_db_0724`），是仓库既有配置非本次引入，测试数据与 zz_view 改动分处两个物理库，互不影响但需知悉。

[2026-07-27] 主数据维护(task-0728) - **主数据维护版式优化：4 页签统一 + 列排序 + 分页 + 导入抽屉统一** | 后端 `PricingMaintenanceService`/`ProcessMasterRepository`/`ProcessMasterReadService` + 2 个 Resource + `SheetHandler`(加 `templateHeaders()` default) + P01~P24 共 24 handler + 新增 `PricingHandlerCatalog`/`PricingTemplateService`；前端 `MasterDataHubPage`/`PartCostingTab`/`V6ProcessCrudTab`/`MaterialRecipeManagement`/`ElementManagement` + 4 个导入抽屉 + `router/index.tsx` + 新增 `listConventions.ts`/`clientSorters.ts`/`CompactUploadDragger.tsx(.css)`；文档 `dev-docs/task-260728-主数据维护版式优化/{需求说明,api,backtask,fronttask}.md` + 原型 HTML | **需求 7 条**：移除 BOM/数据模板页签、页签重排(料号核价→材质→元素→工序)、每页签分页、搜索/过滤/分页/按钮统一、导入抽屉缩小文件框+统一模板按钮、导入核价数据移入料号核价页签、列标题点击三态排序。**拍板 6 项**：核价数据导入补后端模板端点 / 列排序后端加参数(四页签一致) / 元素 3 个价格入口收下拉 / 数据模板连路由下线(组件文件保留) / 过滤维度按页签定 / 抽屉宽度统一 840。 | 🔑 **三条硬约束与其代价**：①**`ORDER BY` 白名单 Map 查表，零拼接** —— 实测 `sortBy='a.mno;DROP TABLE unit_price'` 安全回退默认序不抛异常；②**修 `kwClause` 裸 OR 括号隐患** —— 原 `" WHERE a ILIKE :kw OR b ILIKE :kw"` 无括号，叠加 `configured` 会被解析成 `A OR (B AND C)` 致过滤静默失效(结果变多)，改为逐 predicate 加括号，**单 predicate 时不加**以保证关键字-only 路径与改造前逐字节相同(有机械化断言兜底)；③**过滤 predicate 必须同时进 count 与 page 两条 SQL**，否则「共 N 条」与实际行数不符。**`isOutsource` 用 `= false` 而非 `IS NOT TRUE`**：`is_outsource IS NULL` 的行既不算外协也不算自制(实测 3 条数据 true→1/false→1/NULL 行两侧都不出现)。**B4 模板 sheet 名遍历 handler 注册表取**(禁手写常量数组)，与导入侧 `wb.getSheet(h.sheetName())` 同源。 | ⚠️ **发现：项目 tsc 自检命令一直空转** —— CLAUDE.md「修改后强制自检」第 1 条的 `npx tsc --noEmit -p tsconfig.json` 因 `tsconfig.json` 是 `{"files":[],"references":[...]}` 的 solution 文件、不带 `-b` **不下钻子项目，实测检查 0 个文件恒返 0**。历史上所有「TS 0 错误 ✅」声明均未真正检查过代码。**正确命令 = `-p tsconfig.app.json`(2444 文件)**，跑出 master 既有 2 错：`ElementEditDrawer.tsx:148`(antd v6 `Divider orientation` 类型收窄) + `useDriverExpansions.ts:172`(TS1016 必选参数跟在可选参数后)。本任务验收改用**净增量 0** 口径。 | ⚠️ **发现：全站分页页大小显示英文 `20 / page`** —— antd 6.3.5 `Pagination` 走 `useLocale('Pagination', en_US)` 未拿到 ConfigProvider 的 zhCN(App.tsx 配置正确、无重复 antd、`optimizeDeps` 强制同构预构建无效，根因未定位)。**A/B 实证共享 5174(master) 与本分支同样英文** → 全站既有、非本任务引入；本任务把材质/元素的页大小切换打开后它变得更可见。 | ⚠️ **发现：P16 sheet 名与真实客户文件不一致** —— `P16IncomingOtherRatioFeeHandler.sheetName()`=「来料其他费用（比例）」，客户导入文件里叫「来料其他费用」，`PricingImportService` 精确匹配 → **现网该 sheet 数据从未被导入过**(既有缺陷，本任务「导入逻辑一字不动」故未改)。 | **自检**：后端 33 个新测试全绿(技术总监亲跑)；**A/B 对照干净 master worktree 同为 21F/6E**(master 282 tests vs 本分支 315 tests，多出的 33 即新测试) → 零回归，全量套件的红是既有环境问题(Redis CONNECTION_CLOSED→登录 500→RBAC 401)；实现方还做了**变异测试**(故意去掉括号 → 5 测试红 → 还原)。前端 `tsc -p tsconfig.app.json` 净增量 0。**端到端亲验**(worktree 内起临时 8099 后端 + 5250 前端，验完全部清理、共享 8081/5174 未受影响)：A1 升降序互逆 + 注入回退 + `configured` 二分完备(7=0+7) + **括号坑区分力测试**(`keyword=S-31 & configured=true` → 0 条，坏了会漏 4 条)；A2 三态 `isOutsource` + `processCategory` 精确匹配(「制造」不误命中「制造二部」)；A3 去重升序 + 空表返 `[]`；**A4 闭环**(下载的 24 sheet 模板原样导回 → `SUCCESS`/0 失败行/0 个「缺少 Sheet」)；浏览器实测 4 页签结构/顺序/默认页签、四页签均可排序且 `size=small`、材质元素已拆 Card、元素价格下拉 3 项、3 个导入抽屉 840 宽 + 68px 上传框(原 895px) + footer 统一、**核价模板前端真实下载成功**、`/config/config-templates` 返 404、报价单列表 19 行无回归。临时造的 3 条工序数据已删除(库恢复 total=0)。

[2026-07-27] repair-0727-process-no 测试执行 - **testcase.md 25 条用例全量执行，24 PASS + 1 CONDITIONAL PASS + 2 PARTIAL** | 产物 `dev-docs/task-260708-导入报价单和导入核价单的数据落库规则澄清/repair-260727-工序编号与工序名称落库优化/testreport.md`（走 8097 专用实例，未碰共享 8081；FX-1 `Z100=焊接`/`Z101=铆接` 已保留） | **本次改动本身（Q14/Q15 组装工序两段匹配解析 + Phase1 拦截 + zz_view 三级 COALESCE）验证充分且正确**：写入层（`pending_quotation_id` 过滤）+ 日志（WARN 三要素）+ 单测（20/20）+ 性能（-17.4%，无回归）+ 文档（6/6 子项）全部 PASS。**关键附带发现（非本次改动引入，`git diff --stat` 已核实 `VersionedV6Writer.java`/`QuotePendingRewriter.java` 均不在本次改动清单）**：task-0721 B2 起报价 V6 导入**所有写入无条件带 `pending_quotation_id`**（`QuoteImportService.java:97`），`capacity`/`unit_price` 等 7 张版本化表因此**永远 `is_current=false`**，除非完整走完报价单生命周期到某个未定位到的 promote 环节；真正渲染靠 `QuotePendingRewriter` 运行时按 `:pq` 动态改写 `WHERE`（表替换 + UNION pending/official）。这导致：①`writeVersionedGroup` 的"内容相同→复用旧版本号不写"短路判断依赖 `existing`(`WHERE is_current=TRUE`)，pending 材料永远查不到 existing → **任何 V6 导入重复导入同一份未变内容的文件，`calc_version` 都会持续增长，不会稳定复用旧版本号**（不限于本次改动，AC-8"再导一次不升版"实测不成立）；②`AC-6b`"双 current"检查在纯导入场景下**天然真空通过**（因为压根不会有任何行到达 `is_current=true`，风险未被真正排除）；③纯 SQL `WHERE is_current` 断言测不出新导入数据，须改用 `WHERE pending_quotation_id=<recordId>` 或模拟 `QuotePendingRewriter` 改写才能验证。**另发现**"组装加工费"组件(`f170b0a8`,`$zz_view`)当前未被任何已发布模板引用（`template_component`/`quotation_view_structure` 全表零命中），AC-7 无法通过真实 UI 打开报价单验证，改用 SQL 层模拟 `:pq` 改写替代（证实经 `process_master` JOIN 正确解析，非巧合命中旧脏数据 fallback）。**建议**：PM/架构澄清"验收断言应查 pending 态还是 official 态"+ 排查 official 态促成机制；不阻塞本次改动合并。

[2026-07-27] 组件模板配置(rule-0724) - **施耐德-1 补配第 9 个组件「来料其他费用」（单组件增量 JSON）** | 产物 `客户组件模板/组件导入-施耐德-来料其他费用.json`（bundle v1.0，**1 组件 + 1 `$view`**，checksum `sha256:d389d866…`，`dependencies` 全空）；输入 `客户组件模板/组件模板-施耐德-1.xlsx` 新增的 `来料其他费用` sheet（8 字段） | **组件配置**：`tabType` 空（B1=(空)，费用类不参与树）/ `partNoField`+`partNameField` **双留空** —— 模板既没勾料号列也没勾名称列，且 tabType 空**不受**「材质元素/零件/外购件/主件 4 类料号或名称至少一个」约束（兄弟组件「来料固定加工费」同样双空且已落库成功，是先例不是猜测）/ `rowKeyFields=[销售料号,料件,要素]`（模板勾选，照配）/ `sortField=项次`（模板「排序列」留空，按 §3.1「多行页签建议配项次」推导，与两个兄弟费用页签一致）/ `dataDriverPath=$lqt_view`（避开已用的 `$ll_view`/`$qt_view`/`$bom_view`/`$jg_view`/`$mc_view`/`$wg_view`/`$zz_view`/`$cp_view`）。8 字段：销售料号/料号/料件/项次/要素（`INPUT_TEXT`×4 + `INPUT_NUMBER`×1）+ 费用（`INPUT_NUMBER` is_amount）+ 比例（`INPUT_NUMBER` 非金额）+ 计价单位（`INPUT_TEXT`）。 | **🔑 数据源由后端导入 handler 反查确定（非猜测）**：`Q07IncomingOtherFeeHandler.java:45,100` 明确 sheet「来料其他费用」→ `unit_price` `price_type='INCOMING_MATERIAL_OTHER'`，`groupKey=(cost_type=要素名称, code=投入料号, finished_material_no=销售料号)`、`content=[seq_no=项次, pricing_price=值, cost_ratio=比例, currency=货币, unit=计价单位]`。据此绑定：销售料号=`up.finished_material_no`(**成品**) / 料号=`up.code`(**投入料件**) / 料件=`COALESCE(mm.material_name, mr.name)` via `up.code` / 项次=`seq_no` / 要素=`cost_type` / 费用=`up.pricing_price` / 比例=`up.cost_ratio` / 计价单位=`up.unit`。**「货币」handler 有但模板没这列 → 不出**（§3.4「禁为凑字段凭空造列」）。**费用列不加 `base_value` 兜底**：那是「来料固定加工费」(`INCOMING_MATERIAL_PROCESS`)专属现象，Q07 写的是 `pricing_price`，与「其他费用」07-27 用户拍板的「只取金额」口径一致。 | **闭包判定（task-0726 §3.8.1）**：**做 BOM 闭包**，与同族兄弟「来料固定加工费」一致（费用挂在【成品 × 投入料件】维度，页签展示整个产品结构的成本构成 → 子件的来料其他费用须进本页签）；三条铁律齐备：顶层 `FROM unit_price`（铁律1）/ `LEFT JOIN bom_closure_d` + `COALESCE(cl.root_no, up.finished_material_no)` 兜底（铁律2）/ 递归 `UNION` + `bom_closure_d` 按 `(root,node)` 去重（铁律3）。「销售料号」已在 `rowKeyFields` 里，天然承担 §3.8.4 的 `_归属料号` 角色。 | **R7 联动（用户本次二次确认）**：「费用」列 `is_amount=true` 且各行相加是有业务含义的钱数 → `is_subtotal=true`；「比例」是比率非金额 → `is_amount=false`。⚠️ 与 RECORD:4185「施耐德-1 不动，R7 只对今后新配客户生效」存在读法冲突（本组件属同一套施耐德-1），**已提请用户裁决 → 用户选择「按 R7 保持 true」**。故施耐德-1 目录现状为：本组件 `is_subtotal=true`，另 7 个组件的 6 个 `is_amount` 字段仍全为 `false`（有意保留的历史口径）；**页签合计口径因此在本客户内不一致，是已知且经确认的选择，非缺陷**。 | **自检（全部实跑）**：①字段↔模板列 **8/8 严格 1:1**（含顺序），无凭空新增列 ②`$lqt_view` POST 200（保存期 dry-run 过）+ `PUT driver-view` 200 ③`cpq_db_0724` psql 真跑（`ON_ERROR_STOP=1`）：`INCOMING_MATERIAL_OTHER` 严格态 **0 行但可执行**、9 列列名正确 ④**同结构烟测**（因真类型 0 行）：把 `price_type` 换成有数据的 `INCOMING_MATERIAL_PROCESS` 实跑出 2 行 —— `hf_part_no` 正确收敛到根 `S-3120014539`、料号 `00137`/`992` 经 `material_recipe` 取到名 `H65`/`AgNi11#-Ⅰ`、**无重复行**（证闭包去重 + 两个取名 LEFT JOIN 不倍增；已先验 `material_recipe.code` 254/254、`material_master.material_no` 27/27 唯一）⑤走 §5 标准交付路径：暂存目录 `__暂存-施耐德-来料其他费用-0727` 建组件(COMP-0116) → export → **删组件 + 删目录**，目录列表复核已恢复 `[罗克韦尔, 核价模板, 施耐德-1]` 干净。 | ⚠️ **已知数据缺口（非配置错）**：`unit_price` 里 `price_type='INCOMING_MATERIAL_OTHER'` **全库 0 行**（含历史版本），故 §6.3 `refresh-snapshot` 出数自检**无法证明**，只验到"视图可执行 + 返 0 行"；真验须先导一份带该 sheet 的施耐德数据。⚠️ **行键撞键风险（照配模板，供后续观察）**：Q07 的 `groupKey` 不含 `seq_no`（`seq_no` 在 content 里）→ 同一 `(销售料号,要素,投入料件)` 理论上可有多个项次行，而 `rowKeyFields` 未含「项次」→ 会触发行键消歧(`#序号`)；兄弟「来料固定加工费」的行键含项次可作对比。模板已明确勾选故照配，若日后出现编辑串行/末值塌缩，补勾「项次」进行键即可。⚠️ 本次只出配置产物，未落用户目录、未绑模板。 | **踩坑**：用户首次给的 `组件模板-施耐德-1.xlsx` 是损坏文件（无 `PK` zip 签名、`file` 报 `data`、24173B 高熵乱码），同目录 `罗克韦尔模板.xlsx` 同法可读 → 判定为文件本身损坏而非环境问题，请用户重存后正常。

[2026-07-28] 组件模板配置(rule-0724) - **修正「产出类型」绑错表（施耐德-1 材料成本 `$mc_view`）+ 沉淀"同名列陷阱"通则** | 改 `dev-docs/rule-260724-组件模板配置/报价侧.md`（§7.2 字段表 + §7.2 配方 SQL + 新增 §7.2.1 踩坑实录）、`AGENT-配置入口.md`（§3.5 材质元素映射补「产出类型」+ 新增「同名列陷阱」提示块 + §6 自检新增第 2 项「每个绑定列必须有写入路径」，原 2~5 顺延为 3~6）；**未改代码、未改交付 JSON**（用户只要 SQL） | **根因**：`_产出类型` 绑成 `ebi.component_usage_type`。`element_bom_item` 上**确实有**这个列（`ElementBomItem.java:64`）且列名与业务叫法完全对得上，但它**没有任何写入路径** —— 报价侧「物料与元素BOM」sheet 表头压根没有这一列；全工程写 `component_usage_type` 的 3 个点**全部写 `material_bom_item`**（`MaterialBomMergeHandler:148` 报价侧「产出料号类型」经 `labelOnly()` 剥前导编号 / `P06MaterialBomHandler:95` 核价侧「使用特性」/ `ConfigureProductService:724` 选配）；库里 `element_bom_item.component_usage_type` QUOTE 83 + PRICING 14 行 **100% NULL**。**误导源是我自己**：施耐德-1 交付时把该绑定写进了 `报价侧.md §7.2` 配方（line 306/324），本次一并改掉。 | **正解**：`material_bom_item.component_usage_type`，按 `(material_no, component_no=ebi.material_part_no)` **两维**取（实证是逐行的：罗克韦尔文件同一 `S-3120014539` 下 `991/H65带→铜件`、`992/AgNi11#-Ⅰ→触点`；且那份文件物料BOM 的料号列就叫「材质料号」，正好对上材料成本的「料号」列）。**必须用相关标量子查询而非 LEFT JOIN**：① `material_bom_item` 是版本化表（§3.6 纪律）② 同一对父子常并存 RECIPE+ASSEMBLY 多条边（§3.8.3 铁律3）。`ORDER BY CASE WHEN characteristic='RECIPE' THEN 0 ELSE 1 END` 保证取材质边而非组装边。 | **A/B 实证（`cpq_db_0724`，单事务造数 + `ROLLBACK`，库零留痕）**：①旧绑定**给 `material_bom_item` 造数后仍全 NULL** ← 铁证绑错表（不是数据问题）②新绑定 `(S-80011,00002)` 出「触点」，无对应 BOM 边的两行留 NULL **且不掉行**（4 行不变）③反例改 `LEFT JOIN material_bom_item` + 造第二条 ASSEMBLY 边 → **4 行涨到 6 行**且值被「组装件」污染 ④采用的标量子查询在同样两条边下**仍 4 行**、取到「触点」⑤`ROLLBACK` 后复核 QUOTE 侧非空 0 行、测试行 0 行。最终 SQL 独立（非事务）执行 OK。 | **⚠️ 修好后当前数据下该列仍显示空**：施耐德那份 `报价系统模板0723.xlsx` 物料BOM「产出料号类型」整列空白，本库 QUOTE 侧 `material_bom_item` 全部 146 行该列亦 100% NULL（只有 PRICING 侧 ASSEMBLY 有 16 行 `0:必要`，走 P06「使用特性」是**另一个语义**）。要出数须业务先在导入文件填该列。**别把"改完还是空"误判成没修好**（已写进 §7.2.1）。 | **沉淀的通则（本次真正的产物）**：`AGENT-配置入口.md §6` 自检新增第 2 项 —— **「列名对得上 ≠ 该列有数据」**：绑任何 V6 列前必须两步查证 ①`grep -rn "<列名>" cpq-backend/src/main/java` 找到写它的 handler 并**确认目标表就是你要绑的那张**（同名列常跨表并存，只有一张有写点）②查库非空率；**两者结论不同处理** —— 无写点=配置错须改绑，有写点但全空=数据缺口照配 + 交付说明写明。这类错**全程静默**（不报编译错、`$view` dry-run 照过、`expand-driver` 照常返回），只体现为该列永远空白，故必须进自检清单而非靠记忆。 | 取值样例（真实客户文件「产出料号类型」= 产出物品名/类型，非枚举）：罗克韦尔 `铜件`/`触点`；伊顿 `AgNi10触点`/`AgSnO2(12)-H65接触板`；森萨塔·西安中熔 `Cu/Ag/Cu带`；老版 V2 `1.银点类`→`labelOnly()`→`银点类`。

[2026-07-28] 组件管理 / 公式引擎 - **条件公式多分支共享依赖被误报「循环引用」** 修复 + 环报错改为可定位文案 | **涉及文件**：`FormulaCalculator.java`（① `buildFormulaDeps` 去重 = 根因修复；② 新增 `DepEdge(to, via)` 依赖边 + `buildFormulaDepEdges`（刻意保留重复边供定位）/ `dedupeEdges`（供算法）；③ 新增 `describeFormulaCycles` + Tarjan `stronglyConnected` / `cyclePathIn` / `renderCycle`；④ `FormulaField`/`CondRule` 带公式名与**原始规则序号**；⑤ `resolveFormulaExpression` → `resolveFormula` 返回 `ResolvedFormula(name, expression)`）、`ComponentService.validateFormulas`（环报错改「环路径 + 每条边的引用位置」，并保留 `cyclicFormulaNodes` 作兜底）、前端 `ComponentManagement.tsx`（新增 `showSaveError`：多行结构化错误走常驻 `notification`，单行仍 `message`）、新增测试 `FormulaCycleDetectionTest`(13) + `ComponentServiceConditionalValidationTest` 补 2 例；文档 `docs/反模式.md` 新增 **AP-61**。**无 Flyway 迁移**（零 schema 变更），**不触发 AP-44**（未改 `field_type` 枚举 / 白名单 / 任何字段类型协议）。

**根因**：`buildFormulaDeps` 对条件公式做「并集依赖」时用 `List` 累加**不去重**，而 `cyclicFormulaNodes`/`topoOrder` 的 Kahn 消解是 `indeg = deps.size()`（含重复）对 `if (deps.contains(cur)) indeg--`（每前驱只减 1）——**建图与消解计数口径不一致**，重复边让入度永远归不了零。COMP-0112「物料成本」是条件公式（非银点分支 / 银点默认分支），两分支都引用 `[来料包装费]` → `deps=[来料管理费, 来料包装费, 来料包装费]`(size 3)、只有 2 个不同前驱各减 1 → indeg 停在 1 → 一张干净 DAG 被判成环。**用户视角的困惑点**：报的是「入度没归零的节点」而非「环上的节点」，所以改的是 `非银点类材料成本公式`、被点名的却是 `物料成本`。

**⚠️ 潜伏危害（比报错更糟的那一半）**：同一个 `buildFormulaDeps` 还喂给 `topoOrder`。命中时该字段掉进 `// 环 → 尾部追加` 兜底路径——本例中「物料成本」本就该排最后而**侥幸算对**；换成中间层公式字段就是依赖未算先算的**静默错值**（不报错、不告警）。故本次修的不只是「存不进去」。

**关键决策**：①**去重表与溯源表分离**——`buildFormulaDepEdges` 保留全部重复边（定位提示要列出每一处引用位置），`dedupeEdges` 产出算法用邻接表，两者不混用；②**只报真正落在环上的节点**（Tarjan SCC），不再像 `cyclicFormulaNodes` 那样把「仅依赖了环」的无辜下游节点一起点名；③**规则序号取 `conditional_formula.rules` 原始下标**——解析不到公式的规则会被跳过，用 list 位置当序号会让提示指向错误的规则行（已有专项测试 `cycleDescription_ruleIndexIsOriginalPosition`）；④**保留 `cyclicFormulaNodes` 兜底**：描述器万一提取不出路径也绝不放过环（放过 → topoOrder 静默错值，比报错难查得多）；⑤前端多行错误改常驻 `notification`（`duration:0`）——`message.error` 会把换行折叠成一行且 3s 消失，配置员来不及照着定位；`notification` 属 UI 规范「轻量即时反馈」例外，不违反「抽屉替代弹窗」。

**自检**：后端全量 `mvnw -o test` **1806 tests / 155F+364E**，与**干净 master 同环境基线 1791 tests / 155F+364E** 做失败用例集比对——**双向差集均为 0（519 = 519），零回归**；测试数差 15 = 本次新增用例数。那 519 个红是既有环境问题（Redis `CONNECTION_CLOSED` → 登录 500 → RBAC 401），非本次引入。新测试 `FormulaCycleDetectionTest` 13/13 + `ComponentServiceConditionalValidationTest` 7/7 全绿。前端 `tsc --noEmit -p tsconfig.app.json` **2 错、净增量 0**（`ElementEditDrawer.tsx:148` + `useDriverExpansions.ts:172` 均为 master 既有，已在 task-0728 记录）；worktree 临时 vite(5251) 下 `/` 与 `ComponentManagement.tsx` 均 200。**同库 A/B 亲验**（8081 跑 master、8099 跑修复分支、同一份 PUT payload、真实 COMP-0112）：master 返 `400 公式存在循环引用: 物料成本`（**与用户报错逐字一致**），修复分支返 `200` 且 `[材料毛重]/[来料管理费]/[来料包装费]` + `外购件成本.小计` 正确落库；再造真环（来料管理费取值公式 反引 `[物料成本]`）两侧均 `400`——master 旧文案 `公式存在循环引用: 来料管理费, 物料成本`，修复后为「`「物料成本」→「来料管理费」→「物料成本」` + 逐边 `·「物料成本」的 条件规则1命中的公式「非银点类材料成本公式」 中引用了 [来料管理费]`」。**验完已还原**：COMP-0112 用备份表 `UPDATE ... FROM` 还原并校验 `formulas/fields` 逐字节一致后 DROP 备份表（该组件 `template_component` 引用数 = 0，故 `refreshSnapshotsByComponent` 无 snapshot 受影响）；临时 8099/5251 已停，共享 5174→200、8081→401 未受影响。**未跑 E2E**：本次不触及 driver expansion / 渲染协议，且 `quotation-flow` 夹具在 `cpq_db_0724` 已知失效（BL-0078）。

[2026-07-28] 公式引擎 / 单位换算 - **KPCS 系数 ×1000 → ÷1000 + 新增 kg/KPCS**（业务规则修订） | **涉及文件**：`cpq-backend/.../engine/unit/UnitConversion.java`（FACTORS：`KPCS`/`千片` 1000 → **0.001**；新增 `KG/KPCS` = **0.001**）、前端镜像 `cpq-frontend/src/utils/unitConversion.ts` 同步、两端测试各补「中英别名同系数」断言 + `KG/KPCS` 大小写/空格变体 + `KG/KPCS === G/PCS` 恒等断言、设计文档 `docs/superpowers/specs/2026-06-15-unit-conversion-design.md` §3 表格与修订说明。**无 Flyway 迁移**、**不触发 AP-44**。

**语义变更**：`KPCS / 千片` 从「千片这个**数量单位**换算成片」(×1000) 改为「**每千片**的量归一到**每片**」(÷1000)。

**关键决策**：①**中英别名必须同值** —— 「千片」是 `KPCS` 的中文写法，用户只提了 KPCS，但若不同步改，同一行写 `KPCS` 与写 `千片` 结果相差 **100 万倍**且配置员无从察觉；已在前后端各加 `aliasPairs` 断言把这条约束固化（覆盖 KPCS/千片、KG/千克、G/克、T/吨、PCS/片 五对）。②**`KG/KPCS` 取 0.001 而非最初提的 1** —— 用户复核后修正：`1 kg/千片 = 1000g/1000片 = 1 g/片`，与 `G/PCS` **数学等价**，故两者系数必须恒等（已加恒等断言）。这也使表内 canonical 目标继续统一为 `kg/PCS`，无需改动其他档。③**`G/KPCS` 保持 0.000001 不动** —— 在 kg/PCS 口径下 `1 g/千片 = 0.001kg/1000片 = 1e-6 kg/片` 本来就正确，一度担心的「canonical 冲突」在 KG/KPCS 定为 0.001 后自动消解。

**⚠️ 存量数据影响（改前实测，零影响）**：`cpq_db_0724` 全表扫 `quotation_line_component_data` 的 `snapshot_rows` + `row_data`：裸 `"KPCS"` **0 处**、`"千片"` **0 处**；含 KPCS 的 **28 处全部是 `g/KPCS`**（字段 `_毛用量单位`，见 COMP-0021/0027/0090 材料成本页签），而 `g/KPCS` 系数不变 → **既有报价结果逐值不变**。若日后有数据真用裸 KPCS，需注意本次是语义反转（1e6 倍差），非等价重构。

**自检**：后端 `UnitConversionTest`(7，含新增别名一致性用例) + `FormulaCalculatorUnitConversionTest`(4) + `SubtotalUnitConversionTest`(1) + `CardEffectiveRowsUnitConversionTest`(2) + `FormulaCalculatorUnitConvDataSourceTest`(2) = **16/16 全绿**；前端 `npx vitest run src/` = **61 files / 810 tests 全绿**（裸跑 `npx vitest run` 会误抓 `e2e/*.spec.ts` 报 Playwright `test.beforeAll` 错，属既有配置问题，package.json 无 `test` 脚本只有 `test:e2e`，须显式限定 `src/`）；`tsc --noEmit -p tsconfig.app.json` **2 错、净增量 0**（`ElementEditDrawer.tsx:148` + `useDriverExpansions.ts:172` 均为 master 既有）；worktree 临时 vite(5252) 下 `/` 与 `/src/utils/unitConversion.ts` 均 **200**。
...
**修复**：新增私有方法 `QuotationService#validateTemplateBinding(UUID templateId, String expectedKind, UUID currentValue)`（`QuotationService.java:221`），5 步语义：① `templateId==null` 直接放行（非绑定动作）；② 模板不存在 → 400；③ `templateKind != expectedKind` → 400（`templateKind` 为空按 `QUOTATION` 兜底，对齐 `TemplateService:125-127` 默认值口径）；④ `templateId == currentValue` → 放行，**豁免状态校验**（维持原绑定不是新绑定，防模板归档后历史草稿单被锁死 saveDraft）；⑤ `status != PUBLISHED` → 400。5 个调用点接入：`create` customerTemplateId(`:274`, currentValue=null) / costingTemplateId(`:301`, currentValue=null，**行为变更**：原「查不到→warn+静默忽略」改为 400) / `saveDraft` customerTemplateId(`:378`)+costingCardTemplateId(`:382`)（currentValue=库内现值）/ `copy` newTemplateId(`:1445`)（currentValue=`source.customerTemplateId`，同模板复制含 `templateId==null` 天然相等→豁免；`copy.costingCardTemplateId = source.costingCardTemplateId` 一行**不加校验**，因为是延续既有绑定非新绑定）。

**关键决策**：①**豁免"值未变化"是本次唯一容易做反的地方** —— 若无条件要求 PUBLISHED，模板一归档，所有引用它的历史草稿单就永久保存不了（前端每次 saveDraft 都回传同一个 templateId）；`templateId==currentValue` 的豁免必须先于状态检查判定。②**类型校验是本次新增的第二个不变量，此前完全没有** —— 把核价模板绑到报价字段会让下游整套渲染错配，比状态错更严重。③**fixture 修复只改 status 不改断言**：全仓 13 个测试文件含 `status = "DRAFT"` fixture，逐个核查后只有 `QuotationCopyValueSnapshotInheritanceTest` 的 T4（`quotationService.copy(source, t4TemplateBId)` 换模板到显式 DRAFT 目标）真正命中新校验，改 `t4TemplateBId` 的 `status` 为 `PUBLISHED` 即修复；其余 12 个文件要么不调用 `create/saveDraft/copy`，要么调用时未传对应模板字段（`request.customerTemplateId==null`），或走 sameTemplate 豁免路径，均无需改动。

**涉及文件**：`cpq-backend/src/main/java/com/cpq/quotation/service/QuotationService.java`（新增校验方法 + 5 处接入，唯一改动的主文件）；`cpq-backend/src/test/java/com/cpq/quotation/service/TemplateBindingInvariantTest.java`（新增，T1~T12 共 12 条用例，`@TestTransaction` 自动回滚不留脏数据）；`cpq-backend/src/test/java/com/cpq/quotation/service/QuotationCopyValueSnapshotInheritanceTest.java`（仅改 `t4TemplateBId` fixture 的 `status: DRAFT → PUBLISHED`，断言零改动）；`dev-docs/task-260729-模板绑定状态校验/需求与实现计划.md`（已裁定语义，逐字实现）。

**自检**：`./mvnw -q compile` 通过；`TemplateBindingInvariantTest` **12/12 全绿**；`QuotationCopyValueSnapshotInheritanceTest` **4/4 全绿**（修复前 T4 因 fixture DRAFT 报 400，符合预期，改 fixture 后绿）；直接调用 `create/saveDraft/copy` 的既有测试全集（`BatchStage1PersistEquivTest` / `SaveDraftCardValuesInvalidationTest` / `SaveDraftExcelSnapshotTest` / `SaveDraftRestrictedTabValidationTest` / `SaveDraftSerializeLockTest` / `Task0721PersistentFixtureBuilder`）**零新增失败**；全量 `com.cpq.quotation.**` 包（550 tests）26 failures + 4 errors，**逐一 A/B 比对 stash 到 base commit `266c3108` 复测，全部同样失败**——均为该环境预存在问题（`POST /api/cpq/customers` 等 REST 层稳定 401，与本次改动的 `QuotationService` 三方法无关的鉴权/会话环境问题；`GetByIdBatchEquivTest`/`CostingVersionServiceTest`/`QuotationDetailColumnsTest` 三例是硬编码 golden-anchor id 随共享库迁移失效的 BL-0078 同族问题；`RefreshCardSnapshotTest` 一例是查询共享 DB 里"任意一条 DRAFT quotation"的数据依赖型 flaky test），无一例可归因于本次改动。

[2026-07-29] task-0729 客户价格调整策略与价格版本 - **独立架构评审 → 技术方案定稿(§11.15) + 裁决 3/4/21 作废 + 全量裁决冲突扫描** | **涉及文件**：`dev-docs/task-260729-客户价格调整策略和价格版本/需求说明.md`(+497，新增 §11.4.2/§11.5.3/§11.15)、`AGENT-交接.md`(+105)、`价格调整策略-原型图.html`(屏2废除+屏3关键样例行+裁决速查7处修正)、**新建 `同事交接.md`**(209 行，面向人非 Agent)、`CLAUDE.md`(profile 对照表)。**纯文档，零代码零迁移**。 | 🔴 **本次最重要产出 = 独立架构评审**：派**无澄清会上下文**的架构师逐条读代码核实，结论**「需求不能直接进入出任务书阶段」** —— 业务口径（44 裁决+16 不做清单）很扎实，但**技术地基有 7 条阻断缺口，核心裁决 12/32/37/41 在现有架构上落不了地**。**A1**（最致命）：元素单价冻结在 `snapshot_rows`，而 `CardSnapshotService` **全类只读不写**，全后端无"按新价刷新"入口 → 只重算 `quote_card_values` 的话，**销售存一次草稿就被 `ensureCardValues` 从旧 `snapshot_rows` 重建、新价静默回滚**；**A2**：裁决 37 称"系统本就不记录手改"是**错的** —— `CardEffectiveRows:13-15` editRows 优先级最高，`CardSnapshotService:2168-2176` 还主动把 `row_data` 手改单价合并回 editRows，**系统不但记录还让它赢**，不主动清则裁决 37 完全落空；**A3**：裁决 32 的乐观时间戳**不成立** —— `quotation.updated_at` 只由 `@PreUpdate` 维护无 DB trigger，而销售最常见编辑路径（`quote-card-edit`/删恢复 driver 行/BOM 树增删叶/选配加产品）**全都不 bump 它**，验收 #16 必然失败且是**静默覆盖**；**A5**：裁决 31 的 `cep.material_no = ebi.material_no` 在 **BOM 闭包形态**下错 —— `hf_part_no=COALESCE(cl.root_no, ebi.material_no)`，`ebi.material_no` 在子件行上是**子件料号**，指针只挂父件 → 子件 JOIN 不中、**不吃版本**，静默破坏裁决 29；**A7**：`LineDiscountService.recompute` 的 S0 优先采信 `li.subtotal`，而后端 3 处写入**全是搬前端 DTO**，无重算源 → 升版后**行金额与单据总额都不会变**。 | **技术方案：双通道分离**（§11.15.1，**不采纳**评审建议的"给取价函数加 `override_version_id` + `SqlViewExecutor` 注入 `:priceVersionId`"，因其会把版本上下文以 ThreadLocal 渗进整条渲染链路，是新漂移源，参照 AP-37）：**通道 A**（新建单初次取价）走 `f_material_element_price(客户,基准日,侧别)` + 视图 JOIN，**必须有 fallback 分支**否则视图切换那刻未升版料号价格全空；**通道 B**（升版执行+审核预算，**同一段代码只差 `dryRun` 布尔**）**直读 `element_price_version_item`，不走视图不走函数** —— 使裁决 41「预算与实际逐位一致」**从靠纪律变成结构上不可能违反**，并直接消除评审缺口 A4、半个 A6。**重算通道 8 步**（§11.15.2）：S1 读版本价 / S2 定位价格字段 / **S3 字段级写回 `snapshot_rows`（保留行集合与 driverRow，不增删换行）** / S4 按列清手工值 / S5 重算双侧卡片 / S6 提取 SUBTOTAL 写回 `li.subtotal`+`recompute` / S7 `quoteExcelValues=null`（**已核实 `ExcelViewService:782-786` 有服务端 fallback 重算，故置 NULL 即够，无需新代码**）/ S8 聚合单据。 | **价格字段推导（零新增配置，业务方质疑后重验推翻我先前"元数据不存在"的判断）**：链路 = 组件字段 `default_source.path="$mc_view.元素单价"` ↔ 视图 `cep.unit_price AS 元素单价`。算法：①正则捕获取价函数后的**别名**（**禁硬编码 `cep`** —— 它只是配方习惯写法未被强制）②用别名找 `<别名>.unit_price AS <列名>`（**禁猜列名** —— 罗克韦尔是「元素单价」、规则文档举例是「单价」，**逐客户不同**）③找 `default_source.path` 匹配的字段。地基是 `rule-0724/核价侧.md:33` 的明文规则「**别名=字段名逐字**」。**推导失败即漏配信号，B11 的检测器免费获得**。建议迁移时推导结果落库 `component_sql_view.price_column/currency_column`，运行期不解析 SQL。 | **六项框架决策（业务方逐条拍板，改动须回问）**：**F1** 并发控制 → **行级 JPA `@Version`**（`QuotationLineItem`+`QuotationLineComponentData`）；**F2** 价格写回 → **字段级只改价格/货币两键**；**F3** `R` 快照 → **整单**（业务方明确「以切换版本的稳定性为优先」，裁决 38 不动 —— 逐行稀疏快照虽省空间但预览会退化为"升版行取历史值+其余行取当前值"的近似语义）；**F4** `R` 快照 = **升版后**状态（业务方"切历史版本看到**当时**的报价"直接验证此选择：选"升版前"会导致**版本号与内容错位一期**且当前生效值永远无版本号）；**F5** `submission_snapshot` **不刷新**+追溯页标注（它是审批凭证，事后改就不是凭证；`R` 轨迹已能答"之前什么价"，两者各管一段）；**F6** 撤回（原是工程问题误列为决策项）。 | **修正 5 条既有裁决**（§11.15.7：12 快照时点/31 签名+JOIN 键铁律+**只迁报价侧**/32 → `@Version`/37 理由证伪但结论保留/41·42 不改判仅换实现路径）+ **更正 §11.0 中 4 条"已实证事实"**（§11.15.8）：`@Scheduled` **实为 7 个非 3 个且全未配 `concurrentExecution`**；`ebi.material_no` **不是销售料号**；`quotation.updated_at` **不是可靠水位线**；比对视图"直接复用"**仅对取值成立** —— 差异/阈值/着色 **100% 前端算**（`comparisonMapping.ts:84-115`），后端**连 `ColumnDef` 类型都没有**（`ComparisonConfigDTO:20` "只存不解释"；`basicdata.v6.maintenance.dto.ColumnDef` **同名不同物**），故 §11.5.3(5) 预算逐列落库 = 后端新写一份 diff/着色算法，是**新的前后端漂移源**。 | **裁决 3/4/21 作废 —— 「优先使用勾选框」整体废除**（§11.4.2）：与裁决 35「料号版本指针权威」直接对立（升版后新建单该用新价还是实时算，两裁决给出**相反**答案，后者正是前者明令避免的"存量单已新价、新建单反而旧价"分裂）；职责已被**策略启用状态**覆盖（停用→不生成版本→指针永不推进→天然不参与）；与裁决 31 实现路径不咬合（`mc_view` **模板级共享**）；业务上无存在空间（升版时存量活单**被强制更新**，存量新价而新单旧价说不通）。屏 2 整屏废除（**保留锚点 `#s2`、屏号不重编**以免屏 3~7 交叉引用错乱），**元素价格策略 Tab 本期零改动**。 | **全量裁决交叉冲突扫描（44+4 条）**：因连查两起「改判后旧裁决未清理」而做。需求说明侧全对，**原型侧 7 处陈旧**已修：裁决 9/19/26 速查行仍讲被**裁决 32 推翻**的"跳过"模型（**屏 5 设计说明第 967/968 两条相邻列表项自相矛盾** —— 一条说"有没有会被跳过的单"、下一条说"没有编辑锁故改用乐观时间戳"）；不做清单 #2 依据仍引用**已被推翻**的裁决 16 原文（"核价侧完全不动"）；总览取价表残留"该**客户**当前生效版本"（第二次结构性修正后应为**料号**级）；不做清单**缺 #11~#15**（这张表正是"防止顺手加回"的看板，缺 1/3 等于失效）。另新增 #16 核价单切版本调价（`BL-0006`）不在本期。**裁决 41/42 在 §11.6 与 §11.8 的重复是有意跨章交叉列出，非冲突。** | **验收 24 → 31 条**：新增 #25 行标红判定(含反向用例)/#26 配置变更重算/#27 列表抽屉一致/**#28 升版不被 saveDraft 回滚**/**#29 手工值按列清除**(单价被覆盖、毛重不误伤)/**#30 不打开报价单直接导出即新价**/**#31 BOM 闭包子件同样吃版本**；**#14 改双向断言** —— 原文"料号 B 的 `snapshot_rows` 逐字节不变"是**永远不会失败的无效测试**（`CardSnapshotService` 从不写它，即使 A 的价格根本没更新也会绿），须补正向对照。 | **`BL-0006` 核价单切版本调价：业务方裁定后续单独立项、不并入本期** —— 它切的是**料号 BOM 版本**，与本期**元素价格版本**是两条不同的轴；故裁决 42 与不做清单 #3 保持不变，且不得改动 `:versionFilter` 宏与 `VersionFilterMacro`（task-0713 通道）。**顺带查证：`BL-0005`/`BL-0016` 引用的 `BomClosureService`/`updateLineItemPartVersion`/`regenerateAllSnapshots` 均已被 task-0723 B3「料号版本族整族下线」删除**，两条目实质失效；`BL-0005` 想要的"版本感知 BOM 闭包"看起来已被 `VersionFilterMacro` 覆盖（**未复测**）。 | **环境更正**：本环境用 **`jh` profile**（`./mvnw quarkus:dev -Dquarkus.profile=jh` → `localhost:5432/cpq_db` 本机 docker PG16）；**`cpq_db_0724` 是另一套开发测试库**，不带 profile 启动会连错它（症状"改动看不到效果"）。`CLAUDE.md` 与 `AGENT-交接.md §7` 原说法均已更正（交接文档原写"2026-07-24 起用 cpq_db_0724 不要连旧 cpq_db"是错的）；**BL-0078「E2E 夹具失效」是对 `cpq_db_0724` 实测的结论，对本环境未验证**，两处已改为"跑 E2E 前先确认夹具单号在本库存在"。 | ⚠️ **6 项开工前置验证仅完成 1 项**（§11.15.9，**未跑完不得出任务书**）：P2 ✅ 已核实（`:customerCode` 在核价侧可用 —— `RuntimeContext:153-157` 无条件放 `customerId`/`customerCode`，`SqlViewExecutor:285/:360` 无条件调 `enrichCustomerCode`；**但该需求已因双通道方案消失**）；**P1**（全库 `mc_view` 实际数量+逐个 `sqlTemplate`，**直接决定迁移脚本怎么写**，因两形态并存**禁止统一字符串替换**）/ **P5**（整单快照字节数基线）**卡在本机 docker PG 未启动**（`localhost:5432` 不可达，本 shell 无 `psql` 无 `docker`）；P3（后端提取 SUBTOTAL vs 前端 `li.subtotal` 对拍，**已知两处口径差：`product_attribute` token 恒取 0、`__amount_total__` 哨兵键不登记；对拍不通过则不做 S6**）/ P4（`getExcelView` fallback 取数源）/ P6（`saveDraft` 置 NULL 触发条件）待做。 | **自检**：§11.15 子章节 1~9 无缺号；F1~F6 全部落文；5 条被修正裁决与 §11.0 中 4 条证伪**均已就地标注**（防止只读原章节就照旧实现）；验收 31 条编号连续且 §11.11/§8/交接**三处计数一致**；不做清单 16 条**三处一致**；原型 DOM 13 类标签全配平、屏 3 表格 8 行×13 列配平、10 个导航锚点**零悬空**（屏 2 废除后保留 `#s2`）。**纯文档改动，不走 tsc/Vite/Flyway 自检链路。**

[2026-07-31] 报价导入 / 物料BOM - **「材质占比」列落库：新增 `material_bom_item.material_ratio`（V365）** | **涉及文件**：`cpq-backend/src/main/resources/db/migration/V365__material_bom_item_add_material_ratio.sql`（新增）、`entity/MaterialBomItem.java`、`basicdata/v6/quote/MaterialBomMergeHandler.java`（`CHILD_CONTENT` + `c.put`）、`quotation/service/backfill/QuoteTableAxis.java`（`MATERIAL_BOM_ITEM.contentColumns`）、`quotation/service/backfill/BackfillLabelResolver.java`（列标签）、`docs/table/报价系统Excel导入落库方案.md §3`、测试 `MaterialBomMergeHandlerTest`（+4 用例）。 | **为什么新增而不复用**：`material_bom_item` 50+ 列逐个排查——`composition_qty`（组成数量）/`base_qty`（底数）/`scrap_rate`/`defect_rate`/`fixed_scrap` 均已被两侧 handler 占用；`upper_limit_pct`/`lower_limit_pct` 虽全表 0 行有值、无 handler 写，但语义是 ERP 标准「用量上下限 %」，复用即语义错配（AP-52）；`content`（组成含量）只在 `element_bom_item` 有，本表没有。故新增独立列，口径对齐 `element_bom_item.content`：**小数（0.3 = 30%）+ `numeric(18,6)` + nullable**（非必填）。 | **关键决策**：①**仅材质行写值** —— 报价侧物料BOM 是三态单表，handler 按 `RECIPE.equals(characteristic) ? row.getDecimal("材质占比") : null` 显式置 NULL，防零件/外购件行误填污染；②**不加 CHECK 约束** —— 「同一销售料号下多材质占比合计 = 1」在非必填前提下必然误伤部分为空的行，该校验属业务/公式层；③**不动 `uq_material_bom_item`** —— 占比是内容列非键列；④**核价侧 P06 不写该列**（PRICING 行恒 NULL），不进核价 `CHILD_CONTENT` 故不参与其版本比对，无副作用；⑤`ConfigureProductService.insertMaterialBomItemV6`（选配写材质行）**刻意不加** —— 选配请求无该数据来源，沿用该方法「未采集的列留 NULL、不臆造数值」既有原则。 | 🔑 **改这列必须同步 3 处，漏一处即静默失效（只 grep handler 找不全）**：`CHILD_CONTENT`（漏 → 「只改占比」被 `multisetEqual` 判无变化 → 整组不写库不升版，静默丢数据）／`QuoteTableAxis.MATERIAL_BOM_ITEM.contentColumns`（漏 → repair-0727 回填 patch 与 pending→正式投影把该列抹成 NULL）／`BackfillLabelResolver.COLUMN_LABELS`（漏 → 回填预览显示英文列名）。 | **自检**：`./mvnw -o compile` 0 错误 ✅；V365 在 `cpq_db_0724`（dev）与 `cpq_db`（test）**双库 `success=t`**、列 `numeric(18,6) nullable` 已建 ✅；`MaterialBomMergeHandlerTest` **15/15 全绿**（11 原有 + 4 新增：材质行落值 / 留空落 NULL 不兜底成 0 / 零件行强制 NULL / **仅改占比必须升版 v2000→v2001**）✅；`QuoteBackfill*` 4 个验收测试 + `MaterialBomMaterialNoResolveTest` / `AssemblyBomMaterialSyncTest` 全绿 ✅。⚠️ **`Update0723ImportAcceptanceTest.goldenTemplate_fullImport_endToEndAcceptance` 失败已 A/B 归因为 pre-existing**：stash 掉本次 java 改动（保留 V365 文件以免 Flyway validate 因孤儿 history 记录挂）在 master 代码上复跑，**同样失败、同样错误** `sheet=[组成件其他费用] 报价料号跨客户串号`——共享 test 库 `cpq_db` 占号夹具漂移，与本次改动无关。

[2026-07-31] 组件管理 / 公式序列化 - **SUM 内跨组件小计列免行键：修「顶层放行 / SUM 内拒绝」的口径分裂** | **涉及文件**：`cpq-frontend/src/pages/component/formulaSerialize.ts`（FN body 的 `bracket_expr` 分支 + `renderTargetExprParts` 回显分支）、`formulaSerialize.test.ts`（新增 E2 describe 5 用例）、`cpq-backend/.../FormulaCalculatorCrossTabTest.java`（新增 1 用例）。**后端生产代码零改动**。 | **现象**：物料（COMP-0032，行键 `[料件]`）公式里用 `[产品.税率]`（COMP-0135，行键 `[销售料号]`）保存报「页签「宿主」与「产品」行键不可比（互不包含），无法同进一个 SUM；请改用 KSUM…」。但「税率」`is_subtotal=true`，是**整列总计标量**，与行无关，本不该参与行键对齐。 | **根因**：`formulaSerialize.ts` 的 FN body 解析把 `[页签.列]` **无条件** `srcTabsSeen.push(td)`，缺顶层早有的 `subtotalCols` 判断（顶层「跨组件小计列引用(无总计) → component_subtotal(整列总计标量)」分支），于是标量被当明细列送进 N≥2 两两可比校验（`comparable()` 互不包含即抛）。**同一个 `[产品.税率]` 写在 SUM 外放行、写进 SUM 就拒**。 | 🔑 **回显侧是第二个坑，比序列化更危险**：`renderTargetExprParts` 的 switch 无 `component_subtotal` case → 落 `default: return ''` **吞 token**，回显出 `SUM([材料成本.元素单价] *)` 残缺串，用户再保存即**静默丢乘数**。改 targetExpr 内 token 形状时**必须同步改回显**，否则序列化对了也会在 round-trip 丢数据。 | **关键决策**：①**小计列不进 `srcTabsSeen`** —— 标量不参与行键校验是修复的实质，仅换 token 类型不够；②**求值两端零改动，因本就支持**：前端 `formulaEngine.evalRowExpr` 透传 `componentSubtotals` 给 `evaluateExpression`，后端 `FormulaCalculator` C1 `sub.componentSubtotals = ctx.componentSubtotals`——补后端用例**锁死该透传**（移除即 6.78 → 0 失败）；③**后端校验零改动实测确认**：`TokenMappabilityValidator` 对 targetExpr 内非 `cross_tab_ref` 子 token 直接 `continue`，`ComponentService` 只校验顶层 token；④**KSUM 内层不动** —— 后端 `KSUM_INNER_ALLOWED_TYPES` 明确拒 `component_subtotal`，维持现状；⑤**行为边界**：仅把「原本抛错、根本存不下来」的路径改为产出正确 token，**任何已存量公式的序列化结果逐字不变**，故未跑 E2E（不触渲染链路，`formulaSerialize.ts` 不在 CLAUDE.md 强制 E2E 清单内）。 | **给配置者**：跨页签取小计**不需要行键可比**——`[页签(总计)]`（整页签合计）与 `[页签.小计列]` 都走 `component_subtotal`；只有引**明细列**才要求公共行键。前提：被引页签须有列标 `is_subtotal`（否则 `backfillSubtotalsFromResolved` 的 `subtotalFields.isEmpty() → return` 使取值**静默为 0**）、且必须是 NORMAL 页签（SUBTOTAL/EXCEL 被跳过）、不能成环。 | **自检**：前端 `vitest run src/` **61 files / 815 tests 全绿**（新增 5）；`tsc -p tsconfig.app.json` **0 错误**；后端公式与校验相关 **189 tests 全绿**（新增 1，实证 targetExpr 内嵌 `component_subtotal` 求值 = **6.7800** 精确匹配）；合并回 master 后**主仓复跑**前端 815/815 + 后端 172/172 全绿；`formulaSerialize.ts` → Vite **200**，主入口 **200**，后端 `/api/cpq/components` → **401**（鉴权正常）。

[2026-07-31] 报价单渲染 / 卡片值 - **「产品卡片小计 214 / 报价总额 14」同卡双值：报价侧算值配置源归一到冻结结构** | **涉及文件**：`cpq-backend/.../CardSnapshotService.java`（新增 `loadFrozenQuoteTabs` + `loadQuoteTabsForValues`；`buildCardStructure` 补搬 tab 级 `formula_assignments`；4 处 `loadComponentsSnapshot(q.customerTemplateId)` 改经统一入口）、新增测试 `CardSnapshotFrozenShapeTest`。**零 DDL、零 Flyway、前端零改动**。 | **现象**：详情页产品卡片小计 ¥214.00、同页报价总金额 ¥14.00，用户判断为"保存草稿后数据丢失"。**实际一行数据没丢**（BOM 6 行 / 材料成本 5 行 / 加工费 1 行俱在），是两条计算路径各用各的配置源。 | 🔑 **根因（双源分叉，非某处漏写）**：前端渲染 + 前端算 `li.subtotal` 读 `quotation_view_structure`（**建单时冻结、永不更新**，68fed021「草稿默认冻结」移除了 `rebuildStructureForDraft` 调用并有 `CardSnapshotFreezeTest` T3 锁死）；后端算 `quote_card_values` 读 `template.components_snapshot`（**随组件保存自动刷新**，`ComponentService` → `refreshSnapshotsByComponent`）。给 `COMP-0090` 材料成本新加 `conditional_formula`（元素=="Ag"→公式2）后两源分叉 → 后端按新配置算材料成本 200 → 卡片小计 214；前端拿不到条件公式"退回单一解析" → 0 → 提交 `li.subtotal=14`。而后端 3 处 `li.subtotal` 写入**全是搬前端 DTO、无重算源**（task-0729 架构评审缺口 A7），故总额恒采信前端错值。 | **裁决（用户拍板"严格冻结"）**：配置源归一到**冻结结构**——组件配置变更不再回溯影响已建单，新配的条件公式要新建单才生效，这正是 68fed021 的原意，本次把**算值侧**也纳入该语义（另一候选"草稿跟随当前配置"被否）。 | ⚠️ **修复分两次，第一次不完整（教训）**：首版只改 `buildCardValues`，live 验证卡片小计仍 214 —— 用户点「刷新基础数据」走的是 `refreshQuoteCardValues`，它经 `loadComponentsSnapshot` 直读模板快照，**根本不过 `buildCardValues`**。补全后共 5 条路径归一（`buildCardValues` 首次物化 / `refreshQuoteCardValues` 草稿刷新 / `editCardValue` 单元格编辑 / `materializeAndProject` 存草稿投影 / `dryRunTokenRows` 公式试算）。**改「配置源」这类横切点必须先枚举全部读取点（grep 读取函数名 `loadComponentsSnapshot`），只 grep 一个方法名就会漏。** | 🔑 **为什么换源安全（两条实证）**：① 算值链路对 tab 只消费 `componentId/componentCode/componentType/tabName/fields/formulas/formula_assignments` 七键，冻结结构除 `formula_assignments` 外均已搬运（本次补上——`FormulaCalculator.calculate` 第 3 参直接读它且**不做双读**，缺了该 tab 所有指派公式失效）；② field 侧 `FormulaCalculator` 访问器**本就 camelCase + snake_case 双读**（`fieldType`/`basicDataPath`/`datasourceBinding`/`defaultSource`/`content`/`isSubtotal`/`conditional_formula` 全覆盖），其注释「同时兼容 structure camelCase + snapshot snake_case」即为此设计。 | **自检**：`./mvnw -o compile` 0 错误；新增 `CardSnapshotFrozenShapeTest` **3/3**（camelCase `conditionalFormula` 生效 / `isSubtotal` 被识别 / **两形状逐值等价**——换源不得静默改数）；`CardSnapshot*+CardStructure*+CardValues*` 16/16 全绿；**A/B 零回归实证**：`com.cpq.quotation.**` 全量 base(stash) 551 tests 26F+4E vs 改动后 554 tests 26F+4E，**失败类清单 diff 为空**；唯一可疑的 `RefreshCardSnapshotTest.refresh_preservesEdits_andLeavesCostingUntouched`（正测本次改动的方法）单独 A/B 复跑，base 同一断言、**同一行号 206、同一错误消息逐字一致** → pre-existing。 | ✅ **live 验证（dev 库真单，双向）**：`QT-20260727-0019`（冻结结构**无**条件公式）重算后 小计 214→**14**、材料成本 200→**0**，与 `li.subtotal=14` **一致**；反向对照 `QT-20260728-0023`（冻结结构**有** `conditionalFormula`）重算后仍 材料成本 200 / 小计 **214**，与其 `li.subtotal=214` **一致** —— 证明是"各按各自冻结配置算"而非一律归零。

---

[2026-08-01] task-0801 页签连表公式配置优化 - **立项：澄清会 + 前后端任务书下发 + worktree 就绪（技术总监角色，未写业务代码）** | 产出 `dev-docs/task-260801-页签连表公式配置优化/{需求说明.md(§11 新增),fronttask.md,backtask.md,api.md}`（commit `b323b626`）；worktree `.claude/worktrees/task-0801-tabjoin-formula-layout` 分支 `feat/task-0801-tabjoin-formula-layout`（基于 HEAD b323b626） | **需求**：公式抽屉（`TabJoinFormulaDrawer`）四点改版 —— 左右分栏 / 删试算 / 公式框 52→170~340 / 括号配对着色。**技术总监独立核实推翻原稿 2 处事实**：①F1 抽屉唯一入口是**组件管理页**（`ComponentManagement.tsx:1789`）而非模板管理，且服务 NORMAL/EXCEL/SUBTOTAL **三类组件**（验收须三类各走）；②F2 `TabJoinPlanEvaluator` 是 **Excel 视图正式渲染链路**组件（`ExcelViewService.java:50`，codegraph callers 实证），非试算配套、本就动不得，真正无调用方的只有 2 个 dry-run 端点。 | **6 项决策**：C1 试算彻底移除（含删 `SampleCardPicker.tsx` + service 3 方法）/ C2 左栏加搜索框（**实测 `核价模板1` 达 35 页签**、字段最多 19，仅分栏解决不了左栏 5 屏高）/ C3 宽度 1520→`min(1520px,92vw)`（原型只在 <1100px 降级，1100~1520 是盲区）/ C4 **后端零改动**、2 个 dry-run 端点原样保留不标 Deprecated（其背后 `CardSnapshotService.dryRunTokenRows` 挂 `CardSnapshotDryRunParityTest`，实际保护**渲染**路径正确性 + 在 pending 域开放白名单内）/ C5 验收 = `scanParens` 单测 + 1 个 Playwright spec（AC-3/8/12 是 contentEditable+Selection 行为，jsdom 测不出；**本 spec 挂组件管理页、不依赖报价单夹具故不受 BL-0078 影响**）/ C6 抽屉 925 行适度拆左右栏子组件、SUMIF 构造器不搬 | ⚠️ **实现红线（原型 3 处演示级简化，照抄必出 bug）**：R1 `restoreCaret:84` 用 `data-raw ?? ''` 计长且**不递归**、而 `readBack:41` 递归 → 新括号 span 长度按 0 计致光标错位，**必须改递归**（禁用「给括号加 data-raw」捷径，会与退格删块判定冲突）；R2 着色扫描必须与 `checkParenBalance` **同源**（抽 `scanParens` 一处维护），原型漏跳 `{...}`；R3 括号 span 禁设 `contenteditable=false`（否则删不掉）。**括号可视化必须两通道分离**：内容驱动才重建 DOM，光标驱动只改 style —— 否则中文输入法 composition 被打断 + 光标丢。 | 基线：worktree tsc 0 错误、`src/pages/template/` vitest **59/59 全绿**（工程师返修对照基准）。**不触发 AP-44**（无 field_type 变动）。

---

[2026-08-01] task-0729 客户价格调整策略与价格版本 - **出任务书：E14 澄清批次 + 前后端任务书 + 接口文档 + worktree 就绪（技术总监角色，未写业务代码）** | 产出 `dev-docs/task-260729-客户价格调整策略和价格版本/{需求说明.md(§11.17 新增),backtask.md,fronttask.md,api.md}`（commit `ff194af9`）；worktree `.claude/worktrees/task-0729-price-adjust` 分支 `feat/task-0729-price-adjust`（基于 HEAD ff194af9，⚠️ 本地 master 领先 origin 19 提交，必须从 HEAD 建不能 fresh） | **E14 十一条裁定**：E14-1 一次性全量交付/单分支 · **E14-2 消解「活单」二义**（裁决 33「最近一张活单」与 D5「不在任何活单」与 §11.6.0 三处同一口径 = 5 个可更新状态，`ACTIVE_STATUSES` 单常量贯穿；料号只存在于 SENT/ACCEPTED 单 ⟹ 视同无活单走 D5）· **E14-3 版本生成与预算异步分离**（生成毫秒返回、预算逐料号投队列、待办池新增「预算计算中」不可审中间态、改配置重算复用同一管道）· **E14-4 D3 细化为「全局补齐 + 强制对拍清单 + 确认后合并」** · **E14-5 双端公式一致性三层机制** · E14-6 核价组件配置归技术总监且产物必须 Flyway · **E14-7 归位性能红线 saveDraft 增量 ≤300ms + 整单一次预取** · E14-8 通知=受影响单创建人+触发财务 · E14-9 手动生成弹二次确认但服务端单一代码路径 · E14-10 空元素清单不生成版本 · E14-11 升版口径运行时守卫（阈值 0.01 可配） | 🔑 **实测更正 2 条**：①`mc_view` **实为 7 个**（文档写"6 个未核实"，P1 关闭），全属「材料成本」组件，且**两种 `hf_part_no` 形态确实并存**（`ebi.material_no` 平铺 / `COALESCE(...)` 闭包）→ §11.15.4.2 JOIN 键铁律成立、迁移禁止统一字符串替换；②`PRICING_MANAGER` 角色**已存在**无需新建。开发库规模仅 1 客户/27 料号/35 单 → **性能问题在此库测不出来**，只能靠设计纪律。 | 🔴 **查明 E14-4 那处口径差的来历 = BL-0017 的遗漏路径**：`__amount_total__` 哨兵键机制非本期发明，是 BL-0017（2026-06-30 合并 `e6c53db`）的产物，当时登记了 `ComponentDataEffectiveRows:198-199`（LineDiscountService 用）与 `ConfigureSnapshotService:1332`（配置快照用）**两条**路径，**漏了 `CardSnapshotService`（算 `quote_card_values`、正是本期 S6 要用的那条）** —— 该类全文件**只有 `:1806` 的"防泄漏排除"、无任何 put**。故本期是"补 BL-0017 漏掉的第三条路径"而非从零加能力，实现须照抄 `ComponentDataEffectiveRows:180-199` 口径。实测影响面：`product_attribute` token **当前库 0 使用**（纯防御）；`__amount_total__` **5 报价模板 + 核价模板1 全在用**涉 26 单（真实影响）。小样本对拍 11 行中 9 行一致、**2 行对不上**（26 vs 14、755.93 vs 214）**但未定性**（可能是历史时点漂移），故立《SUBTOTAL 双端对拍清单》为合并前置。 | 🔴 **双端公式一致性升为重点检查项**（业务方特别强调）：实测前端 `formulaEngine.test.ts`(756 行) 与后端 `FormulaCalculatorTest.java`(465 行) **零共享用例**（前端测 `cross_tab_ref`×9 后端 0 条；后端测 `global_variable`×3 前端 0 条）—— 两边都绿但**从未验证过"同一输入两端同一结果"**。定三层机制：**L1** 仓库根 `formula-golden/*.json` 前端 vitest 与后端 JUnit **读同一份文件**（`expected` 由前端引擎产出、后端必须命中，覆盖 10 类含反向验证）/ **L2** 全库真实数据对拍 / **L3** 升版写回前用旧价自对拍、差异 >0.01 该单标「失败·口径不一致」不写回。 | **验收 64 → 70 条**（新增 #65 预算中间态 / #66 归位性能 / #67 手动生成确认 / #68 空清单不生成 / **#69 双端黄金用例同源** / **#70 升版口径守卫**），就地修正 #9 #36 #52④。**验收三分制**：后端自测 51 条 / 前端自测 14 条 / 跨端 14 条（后端主导前端配合，附复现步骤）/ 技术总监亲验 #14 #21 #28 #40 #41 #64。 | ⚠️ **技术总监待办（工程师开工前置）**：按 rule-0724 配核价 `物料与元素BOM` 组件的 `元素单价` 字段 + 视图 JOIN `f_material_element_price` + 三个角色字段，**产物为 Flyway 迁移脚本**（E14-6）—— 这是验收 #21/#64 的硬前置，组件空壳时"核价侧联动"无落点可测。

[2026-08-01] task-0729 核价元素组件配置（E14-6 前置） - **核价「物料与元素BOM」补元素单价字段 + 视图接客户取价函数，产物为 Flyway V366** | worktree commit `681dfa18`（`feat/task-0729-price-adjust`），文件 `V366__task0729_costing_element_price_field.sql`；DB 已 `success=t` | **背景**：核价 `COMP-0049` 是空壳（10 字段无任何价格列），而核价小计 `COMP-0082` 的公式恰恰是 `component_subtotal of COMP-0049` + `value/tab_name=__amount_total__`（Σ金额列）→ **小计恒 0**。没有价格字段，验收 #21（核价侧联动）/#64（两侧同一套价）根本没有落点可测。 | 🔑 **勘察查明报价侧真实口径**（文档 §11.8.3 没说透）：报价侧「材料成本」`COMP-0021` **9 个字段无任何成本计算列、组件公式为空**，是**「元素单价」自己 `is_amount=true`** —— 即「页签合计 = Σ元素单价」而非 Σ(单价×含量×损耗)。故"给核价加元素单价"若不同时明确 `is_amount`，小计仍是 0。 | **业务方 2026-08-01 拍板两条**：① **严格对称报价侧**（单价 `is_amount=true`，不加成本列）—— 两侧同口径，E13 的成本差额才是真实差异而非口径差；②形态 **`INPUT_NUMBER` + `default_source.path`**（非 BASIC_DATA）—— 本期单价列只读机制 + 版本徽标走 `ComponentCell` 的 INPUT_* 分支，S3 写回也按 INPUT_NUMBER+basicDataValues 通道设计，用 BASIC_DATA 两个机制都不适用。 | **有意留给后端的两件事**：①三个角色字段（列由 B1 新增）→ B7 统一预填，**COMP-0049 期望值 `元素代码`/`元素单价`/空**（字段名与报价侧的 `元素` 不同，是推导算法的反例样本）；②切 `f_material_element_price` → B2 统一迁移**两侧 8 个视图**（报价侧 7 + 核价侧 1），届时核价视图须补 `AND cep.material_no = ebi.material_no`（平铺形态，`hf_part_no` 逐字一致铁律）——**现阶段只按 element_code JOIN，因为现有 `f_customer_element_price` 不返回 material_no**。 | ✅ **四层验证全通**：Flyway `success=t`；取价函数返 Ag=216770/Cu=1850；核价视图端到端取到价**且与报价侧逐位相同**（E12「两侧同一套客户价」在数据层成立）；`expand-driver` 运行时 `driverRow` 与 `basicDataValues` **双通道均命中**、**同一元素多行全部取到值**（预演验收 #33），证明核价渲染链路确实注入了 `:customerCode`/`:priceBaseDate`；Ni/301 返 NULL 属正常（301 是材质编码不在元素主表，正是 §11.15.3.3 一致性体检要提示的场景）。 | ⚠️ **主仓有一份 untracked 的 V366 副本，合并前不要删** —— 为在共享 dev server(8081) 验证而 copy；V366 已进共享 DB `flyway_schema_history`，删文件会让下次 Quarkus 重启 **validate 失败**（教训 V311/V322 孤儿迁移）。正确处置：留着直到本分支合并 master 那一刻，先删副本立即 `git merge` 带回正式版本，中间不重启 Quarkus。

[2026-08-01] task-0729 B1+B2 表结构 + 取价函数 + 8 视图迁移 - **12 张新表 + 3 处加列 + `f_material_element_price` + 报价侧 7 个 mc_view + 核价侧 wl_ys_bom_view 全部切新函数** | `cpq-backend/src/main/resources/db/migration/V368__task0729_price_adjust_schema.sql`（B1）+ `V369__task0729_material_element_price_and_views.sql`（B2），worktree `feat/task-0729-price-adjust`；DB 均 `success=t`（`cpq_db_0724`） | **B1**：12 表按 backtask §1 逐条约束落地——`element_price_version` 3 个唯一约束（含 `UNIQUE(customer_no) WHERE status='PENDING'` 部分索引）、`quotation_price_revision` 用 `UNIQUE(quotation_id) WHERE based_version_id IS NULL` 锁定唯一初版（D6，PG NULL 不进普通唯一约束）、`material_price_review` 含 `budget_status`(QUEUED/COMPUTING/READY/FAILED, E14-3)、`comparison_column_config` 维度 `UNIQUE(customer_no, template_series_id)`（未叫 `customer_comparison_config`）、`material_price_version_ref` 用 `UNIQUE INDEX ... INCLUDE(version_id)` 覆盖索引兼当唯一约束（取价函数热路径）；`quotation_line_item`/`quotation_line_component_data` 各加 `row_version BIGINT DEFAULT 0`（原生 SQL 自带乐观锁，非 JPA `@Version`，§11.15.5.2）；`component` 加 3 个组件级角色字段（不进 fields config JSON，绕开 AP-44）。customer_no 统一 `VARCHAR(64)`（对齐 `element_price_strategy` 精度）、material_no `VARCHAR(50)`、element_code `VARCHAR(32)`（对齐 `element.element_code`）；金额列统一 `NUMERIC(20,6)`（对齐并发的 V367 精度放宽）。额外补了 backtask 未逐字给出但 api.md 需要的 `material_price_review.previous_version_id`（记录升版前指针，供「当前版本→目标版本」列）与 `material_price_review_column.missing_side`。 | **B2 函数设计**（比字面"按料号二选一"更细，按 (料号,元素) 粒度合并版本价与实时价）：`versioned` CTE 读指针 + 版本明细（`current_price IS NOT NULL` 才出，天然覆盖"无价"两类语义）；`candidate_materials` CTE 从 `material_bom_item ∪ element_bom_item` 取 `customer_no IN (p_customer_no, '_GLOBAL_')` 的候选料号——**开发期查证**核价侧 BOM/元素主档挂在字面量 `'_GLOBAL_'` 下（客户维度只体现在元素价格策略，不体现在 BOM 结构），若只用真实 customer_no 会让核价侧候选集为空、fallback 失效；`realtime` CTE 对候选料号 `CROSS JOIN f_customer_element_price` 做兜底，`LEFT JOIN versioned ... WHERE v2.material_no IS NULL` 去重，未被版本覆盖的 (料号,元素) 组合才出现——这使得"指针存在但该元素不在策略参与清单里"也能正确兜底到实时价（比 backtask 字面描述的"整料号二选一"更细粒度，仍满足三条硬要求）。 | **8 个视图迁移**：由脚本从 DB 现网 `sql_template` 原文生成（非手写），diff 验证每个视图仅改了「函数名」+「补一行 AND material_no 匹配」，其余字符逐字未动，规避人工转录 JOIN 键铁律时的抄写错误。报价侧 7 个 mc_view 中 COMP-0021 平铺（`hf_part_no=ebi.material_no`）、其余 6 个（COMP-0027/0090/0102/0122/0130/0133）闭包（`hf_part_no=COALESCE(cl.root_no, ebi.material_no)`）；核价侧 COMP-0049（`wl_ys_bom_view`）平铺，在 V366 已接 `f_customer_element_price` 的基础上按嘱咐切新函数并补 `AND cep.material_no=ebi.material_no`。每个 UPDATE 用 dollar-quoting（`$tpl$...$tpl$`）避免单引号转义出错，且带 `sql_template NOT LIKE '%f_material_element_price%'` 幂等守卫，文末保留逐字回滚参考块。 | ✅ **6 项完成判据全部带证据**：①Flyway 368/369 均 `success=t`；②7 个唯一约束逐条故意插重复触发报错验证（customer_no / scheduled_slot / version_no / PENDING 部分索引 / 初版部分索引 / based_version_id 组合 / material_price_version_ref / comparison_column_config / element_price_version_item，另验证 MANUAL 生成 `scheduled_slot=NULL` 两次不冲突）；③函数三情形直接 SQL 验证（指针有值→版本价 999999/USD；Cu 不在版本里→实时价 1850/CNY 兜底；currency 两种情形均非空），另做了一次**通过 HTTP `expand-driver` 端到端**的版本价命中测试（COMP-0027 闭包视图，Ag 从 216770 切到 777777/EUR，Cu 仍 1850 未受影响）；④8 个视图逐个贴出改写后 JOIN 条件 + hf_part_no 形态说明；⑤重启后 `/api/cpq/components` 返 401；⑥`expand-driver` 验证了 1 个平铺（COMP-0049，Ag=216770 与任务给定预期值逐位相同）+ 1 个闭包（COMP-0027，Ag=216770/Cu=1850，无版本指针时的 fallback 值），并额外做了闭包视图的版本价命中回归。测试数据（TEST-CUST-01 / CUST-0001 的临时版本+指针+对比列配置）已全部清理，复查 5 张相关表 count=0。 | **文档同步**：`dev-docs/rule-260724-组件模板配置/3-SQL视图.md §3.8.4b`（技术总监已预写 JOIN 键铁律，本次补注"V369 已落地生效"）+ `报价侧.md` 新增 §5.2.2（`f_material_element_price` 配方：函数签名、JOIN 键铁律速查、三角色字段配置表、自检步骤）。 | **与 backtask 不符之处**：backtask B2 原文措辞是"按 (customer_no, material_no) 查指针 → 有则取版本明细 → 无则整体回落"（材料级二选一），实现改为按 (料号,元素) 粒度合并（材料有指针但某元素不在版本明细里时，该元素单独回落实时价而非让整料号回落）——更细但仍满足"三条硬要求"字面要求，且更贴合 §11.4.1 取价优先级表"元素∈清单看指针、元素∉清单不受影响"的意图；此差异已在函数注释与迁移脚本头部注明。 | **已知限制（未验证，如实说明）**：BOM 闭包"子件继承父件版本价"（验收 #31）只用现有数据的根级材料验证了 JOIN 语法与 `COALESCE` 正确性，未构造真实多层闭包（父件+≥2 子件同为 Ag）做端到端验证——`cpq_db_0724` 当前 CUST-0001 的 QUOTE 侧 BOM 数据没有深度 ≥2 的闭包结构；该场景更完整的验证属于 B0（通道 B 升版重算，验收 #31/#33 专测）范畴，非本次 B1/B2 交付边界。B3~B12（定时扫描/预算计算/审核 API/比对算法/归位机制/通知等）均未开始，按 backtask 依赖顺序留给后续工作块。

[2026-08-01] task-0729 B8 SUBTOTAL 口径补齐 + 双端对拍清单 - **CardSnapshotService 补齐 productAttributes 上下文 + `__amount_total__` 哨兵登记，且额外发现并修复 camelCase/snake_case 字段形态不匹配的第三处口径差；产出《SUBTOTAL 双端对拍清单》正式报告，S6 写回逻辑未动** | `cpq-backend/src/main/java/com/cpq/quotation/service/CardSnapshotService.java`（新增 7 参 `assembleTabsWithFormulaResults` 重载 + `buildProductAttributesContext` + `backfillSubtotalsFromResolved` 补 `__amount_total__` 登记 + 新增只读诊断方法 `reconcileQuoteSubtotalsForTask0729B8`）、`cpq-backend/src/main/java/com/cpq/quotation/service/card/ComponentDataEffectiveRows.java`（`amountColsFromFields` 改双形态兼容）、`cpq-backend/src/main/java/com/cpq/quotation/resource/QuotationAdminResource.java`（新增只读端点 `GET /api/cpq/admin/quotations/task0729-subtotal-reconcile`）、`cpq-backend/src/test/java/com/cpq/quotation/service/CardSnapshotAmountTotalTest.java`（新增，2 个用例）、`dev-docs/task-260729-客户价格调整策略和价格版本/subtotal-reconcile-report.md` | **B8.1 两处口径**：①`productAttributes` 恒传空 map（4 处 calculate 调用点 + SUBTOTAL 公式求值 subCtx）→ 新增只读辅助 `buildProductAttributesContext`（短路：`productAttributeValues` 为空/`{}` 时零查询，当前库 0 实际引用，纯防御）；②`${key}#__amount_total__` 哨兵键未登记（BL-0017 遗漏的第三条路径，只有 buildTabNode:~1806 的排除逻辑无任何 put）→ 照抄 `ComponentDataEffectiveRows` 口径补登记，`buildTabNode` 排除逻辑保留不动。 | 🔴 **深挖过程中发现的第三处口径差（backtask 原文未列，纯靠真实数据验证挖出）**：`ComponentDataEffectiveRows.amountColsFromFields` 判断金额列只查 snake_case(`is_amount`/`is_subtotal`)，但 `CardSnapshotService` 走的**冻结结构**（`quotation_view_structure.structure`）字段是 camelCase(`isAmount`/`isSubtotal`，由 `buildCardStructure:292` 冻结时显式转写)——只补①②登记，对生产路径（100% 走冻结结构）完全不生效，只有自造的平铺测试 JSON（恰好没模拟这层大小写）会通过，是"测试绿但生产假"的典型案例。改为两键名都查，对齐 `FormulaCalculator.findSubtotalFieldNames` 已有的双形态兼容写法，纯增量修复（对既有 snake_case 调用方零影响）。技术总监独立复核（查 QT-20260726-0016 冻结结构确认 3 个金额列均为 camelCase）与本结论一致。 | ✅ **自测**：`CardSnapshotAmountTotalTest`(2/2，新写，验证 Σ 只算 is_amount 列 + buildTabNode 排除逻辑仍生效) + `ComponentDataEffectiveRowsTest`(6/6) + `ComponentDataEffectiveRowsDiscountTest`(2/2) 全绿；A/B stash 验证过 `RefreshCardSnapshotTest` 一处失败是共享 test DB 环境噪声（与本次改动无关，stash 掉改动后同样失败）；`mvnw test` 全量跑出 1849 跑/158 失败/380 错误，但失败集中在 Department/Region/User/Notification 等与本次改动完全无关的模块且呈"transaction aborted"级联特征（典型共享测试库并发污染），未逐一排查全部归因，如实说明未做全量 clean-baseline 对比。 | **B8.2 对拍**：新增只读诊断端点 `GET /api/cpq/admin/quotations/task0729-subtotal-reconcile`（`buildCardValues` 零持久化 + 复用既有 `CostingSubtotalUtil.extractUnitSubtotal`），对 `cpq_db_0724` 全量 25 个 line item（料号 S-3120014539）跑出：21 条一致(diff=0)、1 条跳过(无 customerTemplateId)、3 条差异全部逐条手算 SUBTOTAL 公式定性为「历史时点漂移」（QT-20260726-0008 diff 0.37、QT-20260726-0016 diff 12.0——与技术总监此前手工抽查"26 vs 14"吻合、QT-20260731-0037 diff 541.93——与"755.93 vs 214"吻合），报告含 §3 "S6 影响面预告"表（若这 3 条未来触发升版，行金额分别下降 -0.37/-12.0(-46%)/-541.93(-72%)，需业务方知情）。 | ⚠️ **进程受多次 API 连接中断打断**（技术总监代为完成部分验证并接续指导），协作方式：技术总监独立复核关键结论（camelCase 根因、QT-20260726-0016 定性）与本报告结论完全一致，互为印证。 | **未完成/如实说明**：E2E `quotation-flow.spec.ts` 因时间预算限制未跑出结果（超时终止，无回归证据）；QT-20260726-0008 差异来源只做到"数值方向吻合"级别的推断（外购件成本"成本"列现值 0 与缺口方向一致），未追溯到该列历史真实值；B3~B12 均未开始。

## [2026-08-02] 客户价格调整策略与价格版本(task-0729) B0 - MaterialVersionUpgradeService 升版重算通道（S0~S8 全链路）

**背景**：`worktree feat/task-0729-price-adjust`。B0 是「按价格版本一键升版」的后端核心通道——给定 `lineItemId` + `targetVersionId`，把该行卡片里所有接了价格策略的组件从版本表取新价，写回 `snapshot_rows`/`row_data`，重算报价侧+核价侧卡片、行小计、折扣、整单合计，同时保证只动被升版的这一行（硬约束1）、核价侧 `costing_order` 冻结快照绝不移动（硬约束2）。

**实现**：新增 `com.cpq.priceadjust.service.MaterialVersionUpgradeService`（`cpq-backend/src/main/java/com/cpq/priceadjust/service/MaterialVersionUpgradeService.java`），单一入口 `upgrade(UUID lineItemId, UUID targetVersionId, boolean dryRun)`，`@Transactional`，S0~S8 顺序执行：
- **S0（口径守卫，包在 S6 之前但代码上先跑）**：用旧版本价重算 `quote_card_values`（`CardSnapshotService.buildCardValues`），`CostingSubtotalUtil.extractUnitSubtotal` 取值与 `li.subtotal` 比对，diff>0.01 直接 `FAILED`、整单零写入（防止在已知漂移的脏单上继续叠加错误）。
- **S1**：读目标版本 `element_price_version_item`（`element_code→ElementPrice{price,currency}`，跳过 `current_price IS NULL`）。
- **S2**：从冻结结构 `quotation_view_structure.structure`（`view_kind='QUOTE_CARD'`）里按 `component.element_code_field/element_price_field/element_currency_field`（V368 新增三列，V370 临时手填 8 个组件的值，未做完整 B7 谓词算法，coordinator 已确认本轮范围）定位「价格承载组件」清单。
- **S3a**：`snapshot_rows` 的 `driverRow` 用 `FormulaCalculator.resolveRowByFieldName` 按字段名解析出元素码，命中版本价则写回 price(+currency)；**同一元素在多行必须全部命中**（验收#33，已用真实多行数据验证）。
- **S3b+S4b**：`row_data`（已扁平化，直接按字段名取值）按 `_origin==='manual'` 分流——手工行覆盖新价，非手工行**删除**价格键（不是改成新值，逼下次 saveDraft/expand 重新走正常取价路径，避免升版逻辑和 driver 展开逻辑打架）。S3a/S3b/S4b 共用一次 native SQL UPDATE，`WHERE row_version = :seen`，0 行受影响判定为并发冲突，直接整单回滚（`TransactionSynchronizationRegistry.setRollbackOnly()`），返回 `CONFLICT`。
- **S4a**：清理 `quote_card_values.tabs[].editRows[].values` 里手工改过的价格键（只删价格/币种键，元素键和其它手改字段如毛重/损耗率原样保留——验收#29/#34）。
- **S5**：报价侧 `CardSnapshotService.buildCardValues` 重算；核价侧新增 `refreshCostingCardValuesForLine(UUID lineItemId)`（**只算这一行**，区别于已有的会扫全单所有行的 `refreshCostingCardValues(UUID quotationId)`，复用后者会违反硬约束1）。
- **S6**：复用 `CostingSubtotalUtil.extractUnitSubtotal`（可见性从包私有提升为 `public`，未新写第二份提取逻辑）写回 `li.subtotal`，再走 `LineDiscountService.recompute(li)`（未新写折扣计算）。
- **S7**：`li.quoteExcelValues = null`，触发 `ExcelViewService` 既有兜底重算。**已实地追踪确认**兜底实际读 `quotation_line_component_data.row_data`（`getExcelView→buildRowData→buildTabJoinEffectiveRows→ComponentDataEffectiveRows.compute`），**不是** backtask P4 假设的 `quote_card_values`——如实记录在类 Javadoc：S4b 对非手工 `row_data` 是删键不是改键，若升版后、下次 saveDraft 前触发 Excel 导出，导出结果可能短暂看不到新价（可自愈，本轮未展开修 `ConfigureSnapshotService.materializeLineRowData`）。
- **S8**：`quotation.totalAmount = Σ` 其它行现有 `lineTotalAmount`（只读不重算其它行，硬约束1）；`taxAmount` 全工程无任何计算逻辑引用过，本轮未臆造税额公式，原样保留。
- `dryRun=true`：S1~S8 全部真实执行（同事务内可见，供 B4 预算预览复用同一条真实计算路径），方法返回前 `setRollbackOnly()`，真实验证 DB 零残留；`dryRun=false` 同数据同版本正常落库。

**关键决策**：①`row_version` 乐观锁走**原生 SQL**（`WHERE row_version = :seen` + 0 行=冲突），明确不用 JPA `@Version`，因为 `snapshot_rows`/`row_data` 现有写路径已 100% 是 native SQL。②CDI 代理陷阱：`upgradeComponentRows` 等需要单测直调的方法必须是包私有（非 `private`），否则通过反射调用会绕开 ARC 客户端代理直接打到未初始化实例上，`em` 等注入字段为 null（沿用 `loadVersionPrices`/`locatePriceBearingComponents` 已有模式）。③`ACTIVE_STATUSES` 硬编码为 `{DRAFT,SUBMITTED,APPROVED,REJECTED,COSTING_REJECTED}` 白名单常量，明确不用"排除 EXPIRED/CANCELLED 其余全更"的反向写法（会误伤 SENT/ACCEPTED）。

**真实单验证**（QT-20260726-0003，测试后已用逐列/逐表备份 + `diff` 字节级校验完整恢复，含 6 个 `quotation_line_component_data.row_version` 归零、`quote_card_values` diff 确认字节级一致、测试用 `element_price_version`/`element_price_version_item` 已删除）：`dryRun=true/false` 两遍均验证通过；S0 用 3 个已知真实漂移单（QT-20260726-0008 diff≈0.37、QT-20260726-0016 diff≈12.0、QT-20260731-0037 diff≈541.93）全部正确拦截、零写入；测试单 COMP-0027「元素单价」`isAmount=true` 但 `isSubtotal=false`，价格改动未移动该 Tab 小计——核实为该单冻结结构的模板配置属性，非本次代码引入的 bug。单测 `MaterialVersionUpgradeServiceS1S2Test`(2) + `MaterialVersionUpgradeServiceS3Test`(2) 共 4 个全绿。

**已知限制**：①未跑 `quotation-flow.spec.ts` E2E——协调者已在本环境亲测 400s 超时、前端工程师独立复现 180s 超时，环境已知限制，改用真实单+SQL 断言验证。②S2 的价格承载组件定位用 V370 临时手填的 8 个组件角色字段，未实现完整 B7 谓词算法（coordinator 已确认本轮范围，后续若加新组件需要接价格策略必须手工补 `element_code_field`/`element_price_field`/`element_currency_field` 三列，或后续做 B7）。③S7 发现的 P4 差异点（Excel 导出兜底读 `row_data` 非 `quote_card_values`）未展开修复，留作后续可能的 follow-up。

**涉及文件**：`cpq-backend/src/main/java/com/cpq/priceadjust/service/MaterialVersionUpgradeService.java`（新）、`cpq-backend/src/main/java/com/cpq/priceadjust/dto/UpgradeResult.java`（新）、`cpq-backend/src/main/java/com/cpq/priceadjust/dto/ElementPrice.java`（新）、`cpq-backend/src/main/java/com/cpq/quotation/service/CardSnapshotService.java`（`buildCardValues` 转 public + 新增 `refreshCostingCardValuesForLine`）、`cpq-backend/src/main/java/com/cpq/quotation/service/CostingSubtotalUtil.java`（可见性 public）、`cpq-backend/src/main/java/com/cpq/quotation/resource/QuotationAdminResource.java`（临时验证端点 `task0729-b0-upgrade-preview`）、`cpq-backend/src/test/java/com/cpq/priceadjust/service/MaterialVersionUpgradeServiceS1S2Test.java`（新）、`cpq-backend/src/test/java/com/cpq/priceadjust/service/MaterialVersionUpgradeServiceS3Test.java`（新）。commits: `4c6ee1e1`/`12f1354a`/`631aa2e0`/`e530991a`/`2fe94d9b`（分步提交，每步真实验证后立即 commit）。

## [2026-08-03] 客户价格调整策略与价格版本(task-0729) B3+B4+B5 - 定时扫描/待办池预算/审核通道全部落地，通道 B 全流程闭合

**背景**：`worktree feat/task-0729-price-adjust`。B0（升版重算通道）之后的三个后端工作块，每块独立验证后立即提交（`c94e10d4`/`0a4a207e`/`08be0e1f`）。至此「版本生成→待办池预算→审核通过→异步真实升版」全链路打通。

**B3 · 定时扫描 + 版本生成**（`c94e10d4`）：新增 `com.cpq.priceadjust` 包 11 张表的 Panache 实体（B3/B4/B5 共用基座）。`PriceAdjustVersionGenerationService#generateVersion` 单一入口，定时/手动共用（验收 #5/#67③）：幂等键 `UNIQUE(customer_no, scheduled_slot)` 命中直接返回既有版本；版本号 `V+YYMMDD+两位当日流水`（硬约束8）；E2/§11.3.2.1 无价处置（沿用上一版/彻底无价）；已有 PENDING 版本+`!confirmSupersede`→409 `PendingVersionExistsException`。`PriceAdjustScheduledScanService`：`@Scheduled(every="1m", concurrentExecution=SKIP)`（现有 7 个 `@Scheduled` 全未配此项，本期新增必须配）；周期边界（`MONTHLY_DAY` 小月顺延月末 / `MONTHLY_NTH_WEEK` 第N周不足按当月最后一个该星期几）10 个纯逻辑单测覆盖闰年2月/31号月末/5周对4周。**关键架构决策**：async 预算入队派发必须放在 `@Transactional` 方法之外（`generateVersionAndEnqueueBudget` 包一层，事务提交后才 `managedExecutor.runAsync`）——沿用 `QuoteImportService` 既定模式，防止异步线程抢在外层事务提交前跑起来读不到刚插入的行。真实数据验证：CUST-0001 首次生成 Cu=1850 实时价 + Ag=216770（命中 AVG 1周×1.5+20 策略）；confirmSupersede 409→201 流程正确；STRATEGY_NO_ELEMENTS(400)/策略不存在(404) 验证。单测 13/13（`PriceAdjustScheduledScanServiceTest`10 + `PriceAdjustVersionGenerationServiceTest`3）。

**B4 · 待办池成员判定 + dryRun 预算 + 比对算法移植**（`0a4a207e`）：`PriceAdjustBudgetService#onVersionGenerated` 补全 B4.1+B4.2+B4.3。**B4.1**：`resolveScopeMaterials`（ALL 模式=该客户历史报价单出现过的销售料号去重集合，backtask 未给出具体 SQL，已如实标注为需确认假设）；`findBasisLine` 按 `ACTIVE_STATUSES`（复用 `MaterialVersionUpgradeService.ACTIVE_STATUSES` 同一常量）+建单日期倒序取首张活单；**D5**：无活单料号不进池、指针直接推进；**D5 反例外**：存在过 REJECTED 记录→仍建 review 行交人工过一遍；逐料号 `@Transactional(REQUIRES_NEW)` 独立提交（单料号失败不连累同版其它料号）。**B4.2**：`runDryRunSnapshot` 隔离子事务调 `MaterialVersionUpgradeService.upgrade(dryRun=true)`，同一子事务内立即把调整后卡片值解析成分离态 `SideValues` POJO 带出（子事务因 `upgrade()` 内部 `setRollbackOnly()` 自动回滚、零残留）；未触发整单 `ensureCardValues`（硬约束4）。**B4.3**：新增 `ComparisonColumnDef`（复用 task-0717 前端 `ColumnDef` schema）+ `ComparisonColumnEvaluator`（`getColumnValue`/`computeDiff`/`classifyDiffThreeState` 逐行移植 `comparisonMapping.ts:84-115`，20 单测对齐 `comparisonMapping.test.ts` 黄金用例；`evaluate()` 在此之上扩出后端专属 MISSING/STALE 五态：STALE=componentId 在两侧卡片值 tabs 里都找不到、MISSING=componentId 命中但字段值缺失、E4 MISSING 计入 breached_count）；反向验证：故意把 AMBER 判定 `<` 改 `<=`，1 用例正确变红，revert 后与 worktree 逐字节一致再提交（B9 L1 精神）。`ComparisonViewService.extractSide`+`SideValues`/`TabVal` 三处 private→public（只提可见性，不新写一份）。真实数据验证：CUST-0001/S-3120014539/基准单=QT-20260731-0037（已知 L3 会拦截的真实脏单）→ review 落库 `budgetStatus=FAILED`，`budget_error` 精确复现 541.925200（与此前 B0 报告 diff≈541.93 完全吻合）；合成客户 TEST-D5-0729 验证 D5 两分支行为均吻合设计。单测 20/20（`ComparisonColumnEvaluatorTest`）。

**B5 · 审核 API + 异步更新任务**（`08be0e1f`）：api.md §2/§3 六端点全部实现。`approve`=同步推进指针+异步建 job（202），批量前置校验（`budgetStatus=READY ∧ reviewStatus=PENDING ∧ 所属版本status=PENDING`）不满足整批拒绝 409；按（客户×料号）名下**全部活单**（非仅判断依据单）建 `job_item`。`PriceAdjustJobExecutionService` 参照 `QuoteImportService`：`ManagedExecutor`+逐条 `@Transactional(REQUIRES_NEW)`，调用 `upgrade(dryRun=false)` 做真实升版；三种非成功态（FAILED含SUBTOTAL_MISMATCH/CONFLICT/STALE）精确映射；`finalizeJob` 汇总 job 状态（SUCCESS/PARTIAL/FAILED/STALE）；指针推进纪律——个别 job_item 失败不回退料号指针。**真实联调抓到 2 个 bug 并修复**：①【关键】`approve()` 最初把 `managedExecutor.runAsync` 直接写在 `@Transactional` 方法内部，真实调用复现 HTTP 500（review 已提交 APPROVED 但响应异常、job_item 全卡 WAITING）——与 B3 同一类"异步抢跑"问题，修法同样拆成非事务对外入口+`@Transactional` 内部方法；②`GET /reviews`/`GET /jobs` 用 `"1=1"+Parameters.with("dummy",1)` 占位起手在真实联调中触发 400（Panache 校验参数必须在查询串出现），改用恒有效字段起手/按有无过滤条件分叉查询方法。真实数据验证：临时改 `QT-20260726-0003` 的 `product_part_no_snapshot` 为隔离测试料号（避免影响 CUST-0001 名下其余 24 张共享同料号的活单）→ approve 202 → 3 秒内 job 收敛 SUCCESS，`quotation_line_component_data` 的 COMP-0027 行 `row_version` 0→1、`quote_card_values` 长度 9014→7719 字节证实真实升版执行；reject 无 reason 400、有 reason 200 且指针不动/不产生 job；测试数据测后逐字节恢复（diff 确认 IDENTICAL），12 张 task-0729 相关表 count 归零核实。单测 7/7（`PriceAdjustReviewServiceTest`）。

**已知限制（如实说明，均在各自 commit message 中已注明）**：①`resolveScopeMaterials` ALL 模式的数据源、屏4「为什么变」`usageQty`/`unitPriceImpact` 精确分解未做；②`material_price_review_column` 的着色口径统一取"调整后"评估；③B0 阶段发现的 R 版本（`quotation_price_revision` 表）写入路径至今仍无任何代码路径触达——backtask 把相关验收项（#63/#64 等）记在 B0 名下，但 S0-S8 实现从未写这张表，本轮 B3/B4/B5 也未涉及，如实标注为待确认的功能缺口，非本轮遗漏。④未跑 `quotation-flow.spec.ts` E2E（环境已知限制）。

**涉及文件**：`cpq-backend/src/main/java/com/cpq/priceadjust/` 下新增 entity 11 个 + service（`PriceAdjustVersionGenerationService`/`PriceAdjustScheduledScanService`/`PriceAdjustBudgetService`/`PriceAdjustReviewService`/`PriceAdjustJobExecutionService`/`ComparisonColumnEvaluator`）+ resource（`PriceAdjustVersionResource`/`PriceAdjustReviewResource`/`PriceAdjustJobResource`）+ dto 若干 + exception 3 个；`com.cpq.costing.service.ComparisonViewService`（`extractSide`/`SideValues`/`TabVal` 可见性提升）；`com.cpq.common.exception.GlobalExceptionMapper`（3 个新异常 handler）。单测共 40/40（`ComparisonColumnEvaluatorTest`20 + `PriceAdjustScheduledScanServiceTest`10 + `PriceAdjustVersionGenerationServiceTest`3 + `PriceAdjustReviewServiceTest`7）。commits: `c94e10d4`/`0a4a207e`/`08be0e1f`。

## [2026-08-03] 客户价格调整策略与价格版本(task-0729) B0-R+B6+B7 - R版本快照补派 + 比对列CRUD + 组件绑定完整版

**背景**：`worktree feat/task-0729-price-adjust`。coordinator 核对 backtask 后确认 R 版本快照（验收 #17/#53/#55/#56/#63 的载体）是任务书本身的遗漏（B0 章节写全了要求，但 S0~S8 八步表没列这步），补派为本轮首个模块；同轮补 B6（比对列 CRUD）+ B7（组件绑定完整版）。三块独立提交：`27afafcb`/`b1bba062`/`bb47b479`。

**B0-R · R 版本快照（补进通道 B 作为 S9）**：新增 `QuotationPriceRevision` 实体。`MaterialVersionUpgradeService` 两处插入点：① S3 之前（数据未改动）懒建初版（`based_version_id` 留空、`sealed=true`，物化升版前整单原貌，`first_effective_at` 取该单真实 `createdAt` 非 "now"）；② S8 之后写本期 R（F4，升版后状态），`UNIQUE(quotation_id, based_version_id)` 天然去重键，find-or-create 命中既有行时**整单双侧快照覆写**而非合并追加（E11-5，防"先通过新价后通过旧价"）。快照内容=整单双侧（§11.7.0）：`buildWholeQuotationSnapshot` 遍历该单**全部** line item，`quote_card_values`/`costing_card_values` 按 lineItemId 建索引 JSON 对象，`snapshot_rows` 再按 componentDataId 展开一层。版本号 `R+YYMMDD+两位当日流水`。真实数据验证（QT-20260726-0003，隔离测试料号法+测后逐字节恢复）：首次升版正确懒建初版+本期R（6 个 componentDataId 全齐，非稀疏存储）；同版本二次升级 R 行数仍为2（未新增）、`upgraded_material_nos` 去重正确、`last_updated_at` 正确刷新。**已知限制**：初版懒建跳过 backtask 描述的"建单时创建 unsealed 占位行"这个中间态，所有初版都是首次升版时直接 sealed=true 创建，功能等价但非字面实现，已在 commit 如实标注。

**B6 · 比对列配置 CRUD**：api.md §1.8/§1.9/§1.10 三端点（含 §1.8 模板系列列表，api.md 本身未给对应 backtask 小节但前端标注过缺口，一并补上）。`PUT` 为唯一写入口（屏4只读），强制保留 PRODUCT_TOTAL 列（删除400），写审计日志（`customer_price_adjust_strategy_log.strategy_id` 是 NOT NULL FK，客户无策略记录时 best-effort 跳过不阻断），保存后触发该「客户×模板系列」下 PENDING 料号重算（非该客户全部）复用 B4 的 `processMaterial`。**真实联调抓到并修复1个隐蔽bug**：`ComparisonColumnDef.isProductTotal()` 是 boolean getter，Jackson 按 bean 约定序列化成 JSON 属性 `"productTotal"`，PUT 落库时多余字段写入 JSONB，GET 读回反序列化因该属性无可写字段触发 `UnrecognizedPropertyException`，被 `parseColumns` 兜底 catch 悄悄吞掉、静默降级为默认列——现象是"保存200成功但立刻读回来还是老数据"，不是幂等而是异常被吞。修法 `@JsonIgnore`；反向验证过（加注解前后对比行为差异）。真实数据验证：CUST-0001 罗克韦尔模板1/2/3 三系列正确列出；PUT 2列配置完整往返；建真实 PENDING review 验证保存后异步重算正确流转到 FAILED（复现 541.925200，与 B4 一致）。**已知限制**：B6 复用 B4 的 `processMaterial` 做"重算既有 PENDING 料号"时，若该料号当下没有任何活单会误触发 D5 分支直接 return，导致该行卡在 QUEUED——真实场景下 D5 保证入池时必有 basis，仅当入池后基准单状态变化才会复现，窄但存在，本轮未修。

**B7 · 组件元素列绑定完整版**：`Component` 实体补齐 `element_code_field`/`element_price_field`/`element_currency_field` 三列映射（V368 已建列，Java 实体此前从未映射，属遗漏补齐）。保存期校验 `assertElementBindingRequirement`（接入 create/update）：组件任一 `component_sql_view.sql_template` 检测到取价函数 → 两个角色字段必填，否则 400 `COMPONENT_ELEMENT_BINDING_REQUIRED`（验收#32②）；未接取价函数留空可正常保存（验收#32③）。新增 `ElementBindingDerivation`（§11.15.3.4 五步推导算法纯函数）+ `GET /components/{id}/element-binding-suggest` 端点。**单测踩坑记录**：最初用 `@QuarkusTest`+`EntityManager` 直查现网 COMP-0027/COMP-0049，断言失败排查发现 `mvn test` 走 `test` profile 连的是 `cpq_db`（非手工 psql 核对用的 `cpq_db_0724`），两库同 code 组件的 `component_sql_view` 内容早已分叉（`sql_view_name` 一个 `mc_view` 一个 `cp_view`）——改为纯函数+内嵌真实文本（Java text block，逐字节取自 `cpq_db_0724`）写法，从根上避免跨库不一致，测试也快 60 倍（5s→0.09s）。**这是本轮一个值得记录的环境教训：`@QuarkusTest` 依赖开发库现网真实数据的测试在本仓库不可靠，纯函数应保持纯函数测试、不路由 DB**。真实数据验证（HTTP 直连）：`GET element-binding-suggest` 对 COMP-0027 返回 `元素/元素单价/null HIGH`，对反例样本 COMP-0049 返回 `元素代码/元素单价/null HIGH`，与 V370 人工回填值逐位一致（验收#32④）；PUT 清空 elementCodeField（组件仍接价格函数）→ 400 + DB 复核零写入（事务正确回滚）；PUT 恢复正确值 200；未接价格函数组件空 body 200（无误伤）。

**涉及文件**：`cpq-backend/src/main/java/com/cpq/priceadjust/entity/QuotationPriceRevision.java`（新）、`.../service/MaterialVersionUpgradeService.java`（S9 两处插入点）、`.../dto/{ComparisonColumnsDTO,PutComparisonColumnsRequest,TemplateSeriesDTO}.java`（新）、`.../entity/CustomerPriceAdjustStrategyLog.java`（新）、`.../resource/PriceAdjustComparisonColumnResource.java`（新）、`.../service/PriceAdjustComparisonColumnService.java`（新）、`com/cpq/component/entity/Component.java`（补三列映射）、`.../dto/{ComponentDTO,CreateComponentRequest,ElementBindingSuggestDTO}.java`、`.../exception/ComponentElementBindingRequiredException.java`（新）、`.../resource/ComponentResource.java`、`.../service/{ComponentService,ElementBindingDerivation}.java`（新）、`cpq-backend/src/test/java/com/cpq/component/service/ElementBindingDerivationTest.java`（新）、`com/cpq/common/exception/GlobalExceptionMapper.java`。单测共 54/54（新增 `ElementBindingDerivationTest`5 + 回归 49）。commits: `27afafcb`/`b1bba062`/`bb47b479`。

## [2026-08-03] 客户价格调整策略与价格版本(task-0729) ①R版本占位行时序修正 + B10价格列归位机制

**背景**：`worktree feat/task-0729-price-adjust`。coordinator 核对验收 #53① 后指出 B0-R 的 `createInitialRevision` 在"首次升版时才懒建"与 §11.10.6 要求的"建单时(首次保存且已有产品行)就创建 sealed=false 占位行"在**可见性**上不等价（内容等价但升版前屏7看不到未定型初版），要求与 B10（本期风险最高）一起做（同一条 saveDraft 链路）。commit：`abd48221`。

**① 修正**：拆成两段。`PriceReconciler#ensureInitialRevisionPlaceholder`（新，saveDraft 钩子）在建单时机（首次保存且已有产品行）创建 `sealed=false` 占位行，snapshot 三列留 NULL（零额外开销）。`MaterialVersionUpgradeService#createInitialRevision` 改名+改写为 `materializeAndSealInitialRevision`：首次升版时若占位行已存在则原地物化+`sealed=true`；若占位行不存在（存量单，早于本功能上线）则直接创建并当场定型（懒建兜底）；已定型则 no-op。真实数据验证：POST 归位后 `quotation_price_revision` 正确出现 1 条 `sealed=false/based_version_id=NULL` 占位行。

**B10 · 价格列归位机制**：新增 `PriceReconciler`，把 §11.4.1 取价优先级表幂等应用到每行价格列：元素∉清单→不动；元素∈清单+指针有值→写版本价+清手工值；元素∈清单+指针为空→task-0722 实时算（基准日=报价单 `created_at` 日期，非执行当天）+清手工值。🔑 "写"（driverRow 价格键+`__priceLocked`标记，要求元素∈清单且解出了价）与"清"（row_data 陈旧手工键，只要求元素∈清单，与是否解出价无关）是两个独立判定，`reconcileRows()` 故意分两段不合并。作用域三条件（元素∈清单 ∧ 料号∈范围 ∧ 策略启用）全部成立才归位。性能纪律（E14-7）：`prefetch()` 整单一次批量预取策略/元素清单/料号范围/指针/版本明细/实时价/冻结结构/组件fields/component_data，逐行处理全在内存，只有真正命中变更的行才发 UPDATE（原生 SQL + row_version 乐观锁，同 S3/S4 既定模式）。插入位置（`QuotationResource#saveDraft`，🔒不可变通）：`snapshotService.snapshotQuotation`（row_data落库/snapshot_rows重建）之后、`quoteCardValues=null` 之后（已在 `quotationService.saveDraft` 内完成）→ 后续独立请求触发的 `ensureCardValues` 懒重算之前。归位失败降级 warn 不阻断保存，幂等可重试。**与 S3/S4 的关系**：判定条件逐条对齐但本轮未把 S3/S4 字面改成调用 `PriceReconciler`（避免对已验证的升版核心路径引入重构风险），已在类注释如实标注为刻意范围裁剪。

真实数据验证（QT-20260726-0003 + CUST-0001 真实调价策略，测后逐字节恢复）：**指针为空分支**——策略含元素[Cu]无指针，Cu 行正确写入实时价 1850.0（`f_customer_element_price` 真实结果）+`__priceLocked=true`+`__priceVersion="实时"`；同组件 Ag 行（不在清单内）完全未被触碰——"元素∉清单→不动"精确验证。**指针有值分支**——为 S-3120014539 建版本 V99991001(Cu=2200)+指针后重新归位，Cu 行正确切换为 2200.0+`__priceVersion="V99991001"`；Ag 行仍完全未变；row_data 侧同步验证一致。**性能自检**（E14-7 要求的关归位 vs 开归位对比，各3次）：关归位（客户无策略）61-141ms；开归位（1料号1价格承载组件2元素行）220-295ms，增量约150-220ms，在300ms阈值内（单料号场景，已如实标注更大规模单需要的话后续可再测）。

**已知限制（如实说明，4 项未做，均已写入 commit message）**：①`__priceLocked`/`__priceVersion` 只写入 `snapshot_rows`，未打通 `CardSnapshotService` 卡片渲染管线透传进最终 `quoteCardValues` JSON——前端读不到这两个标记，是本轮最大缺口，需要后续单独立项（协议级、AP-44 同量级多点传播）；②`elementCodeField`/`elementPriceField` 透传到结构快照未做；③单元格失焦编辑（`quote-card-edit`/`editCardValue`）未接入归位——coordinator 已预判是"唯一需要小心的地方"，本轮未做；④用等价的临时管理端点 `task0729-b10-reconcile-preview` 直接验证 `PriceReconciler`（更聚焦风险更低），未跑 `saveDraft` 端到端的开/关归位耗时对比。
...
[2026-08-02] task-0729 B9 后续三件套（coordinator 裁定后执行）—— ②除零归零已修 / ③tab_name 复合键生产审计=0影响 / ①精度对齐影响面清单=当前0金额变化+发现关键分支时序问题 | `cpq-frontend/src/utils/formulaEngine.ts`（②，已合并至上一条目 commit `22d08f49`） / `dev-docs/task-260729-客户价格调整策略和价格版本/前端精度对齐影响面清单.md`（新增） | coordinator 呈报业务方后对 7 条黄金用例分歧给出三项裁定，按"② 最快先做 → ③ 只读审计 → ① 出清单"顺序执行，三件事都做完一起汇报：
...
[2026-08-03] task-0729 B0 阻断回归排查 —— 批量升版后核价卡片值清零假报 SUCCESS，真根因 = `@ActivateRequestContext` 未随 `REQUIRES_NEW` 嵌套事务正确激活 request scope，被静默吞异常掩盖 8 轮排查 | `cpq-backend/src/main/java/com/cpq/priceadjust/service/PriceAdjustJobExecutionService.java` / `cpq-backend/src/main/java/com/cpq/quotation/service/BomTreeRenderService.java` / `cpq-backend/src/main/java/com/cpq/quotation/service/CardSnapshotService.java` | 现象：批量升版（`approve`→异步 job）后部分报价单核价侧「物料与元素BOM」等 tab 从 14 行变 0 行，`job_item.status` 却仍是 `SUCCESS`；单条 `dryRun`/直调 `upgrade()` 均无法复现，只有走真实 `approve`→`executeJob`→`executeItem` 路径才 100% 复现。

**排查过程（8 轮假设，7 轮被实测证伪，如实记录不隐藏走过的弯路）**：
1. `lineItems.get(0)` 跨 item/跨 quotation 错位污染 `QuotationIdContext`/customerId 查找（coordinator 提出）——**证伪**：逐个查完 `render()` 的全部 5 个真实调用点，涉及本次回归的路径（`refreshCostingCardValuesForLine`）从头到尾都是单条 `List.of(li)` 调用，代码层面不存在跨条错位的条件；直接在真实犯罪现场 line item 上加 S5~S9 五个检查点的 DEBUG 日志跑 `dryRun=true`（全程真实执行、最后回滚不落库）+ `dryRun=false`（真实提交），**两次 18 个 tab 全部逐步正确、真实提交把 0 行的单修回 14/17 行**——证明 S5~S9 的确定性代码逻辑本身是干净的。
2. 外层事务持有 `QuotationLineItem` 托管实体，`upgrade()` 以为是 `REQUIRES_NEW` 独立提交，外层提交时整行覆写回旧值（coordinator 基于"`QuotationLineItem` 无 `@DynamicUpdate` + `costing_card_values` 无乐观锁"这一真实事实提出的第二个假设）——**证伪**：注册 `TransactionSynchronizationRegistry.registerInterposedSynchronization` 打印 `beforeCompletion`/`afterCompletion`，实测走真实 `approve`→job 路径复现时，`afterS5` 阶段就**已经是 0/0**（在任何 commit/completion 逻辑运行之前）——数据从渲染那一刻起就是错的，不存在"先对、后被覆写"的时序；且 `upgrade()` 本身是裸 `@Transactional`（REQUIRED），会加入调用方 `executeItem` 的 REQUIRES_NEW 事务而非独立开启，"外层持有另一份托管实体"的前提本身不成立。
3. **真根因**（服务器日志直接吐出异常原文，无需再猜）：`ComponentDriverService.expand`→`DataLoader`（`@RequestScoped`）在 `executeItem`（`@Transactional(REQUIRES_NEW)`）内解析时，**100% 抛 `ContextNotActiveException`**（"RequestScoped context was not active..."）——34 项批次里出现 272 次，从第一项开始每次必现。旧代码 `@ActivateRequestContext` 挂在外层 `executeJob` 上，未能让内层 `REQUIRES_NEW` 事务边界内的 request-scoped bean 保持可解析。

**为什么躲过前 7 轮排查（本次最有价值的发现）**：`BomTreeRenderService` §④ 每个 driver 组件的 expand 调用外面裹了 `catch(Exception e) { LOG.warnf(...) }`——异常被打成一条 WARN 日志就吞掉，`byKey` 留空，该组件（连带整卡除结构 spine 外全部）业务行清零，`render()` 照常返回、`upgrade()` 照常报 `SUCCESS`。**这个吞异常的 catch 是全部现象的共同放大器**：272 次真实异常，一次都没有以可见形式出现过，直到把 catch 块本身改成 `LOG.errorf` 才在日志里第一次看到原文。

**修法（两处，缺一不可，coordinator 明确要求）**：
1. **`@ActivateRequestContext` 从 `executeJob` 下移到 `executeItem`**（即 `REQUIRES_NEW` 事务边界方法本身）——一行改动，实测后 272 次异常归零。
2. **`BomTreeRenderService` §④ 不再静默吞异常**：逐组件捕获仍然保留（不因一个组件失败打断其余组件展开），但循环结束后若有任何组件真正抛过异常，聚合成 `BusinessException` 整体抛出——不能让"组件 expand 异常"降级成"该组件 0 行 + 整体报 SUCCESS"。**连带调整**：`render()` 从"总是安全返回"变成"会抛异常"后，逐个核对全部 5 个调用点的异常处理：`CostingVersionService.switchVersion`（无保护，任其向上抛给 REST 层，属既有默认容错路径，可接受）；`snapshotNewLinesCardValues`/`ConfigureSnapshotService.snapshotLines` 已有 BL-0030 同款失败哨兵模式（`costingRenderError`→`failedSentinelWithError`），天然兼容；`refreshCostingCardValues`（整单批量刷新）**原先对 `render()` 零保护**，补齐同款哨兵模式避免一个组件异常拖垮整单其余行；`refreshCostingCardValuesForLine`（本次 S5 唯一生产调用方）**刻意保持不接**、让异常原样传到 `executeItem` 既有 `catch`→`markItemFailed`（`FAILED`+错误原文，可重试），并在 javadoc 里明确写死这条"升版失败必须可见"的硬约束，防止未来有人顺手加哨兵把它悄悄焐平。

**验证（严格按 coordinator 要求走完整 `approve`→异步 job 路径，不走 `task0729-b0-upgrade-preview` 这种绕开问题本身的直调）**：① 真实 34 item 批次里 `ContextNotActiveException` 从 272→0；② 目标 line item「物料与元素BOM」及其余 16 个 tab 全部对齐修复前基线（17/14/4/4/4/4/2/3/1/2/0/1/4/2/2/0/0/0）；③ `snapshot_rows` 里元素单价（Cu=6290.000000/Ag=217520.000000）与目标版本 `element_price_version_item` 逐位相等，不只是"有值"；④ 临时在 §④ 注入一次针对性 fault（对准真实 costing 驱动组件，而非 `locatePriceBearingComponents` 返回的报价侧组件——首次注入选错组件导致假阴性，已订正）验证 item 正确落 `FAILED`/`UNEXPECTED_ERROR` 且事务整体回滚（数据未被污染），随后原样移除注入代码。批次里另有 10/34 项 `FAILED(SUBTOTAL_MISMATCH)`——如实说明这是本 session 反复测试积累的存量脏数据（历史损坏行的 `li.subtotal` 与干净重算不符），S0 口径守卫按设计正确拦截，与本次修复无关、非本次引入。

**顺带修复（同一次排查中发现并处理的独立阻断，不属于本任务范围但阻塞了共享 dev server）**：`task-0803` 并发合并 `master`（`merge(task-0803)` commit）覆盖了本 session 早前未提交同步进主仓工作区的 `ComponentService.suggestElementBinding`（B7 交付物），导致全项目编译失败、8081 对所有并发会话返 500。定位后最小面补回该方法（未触碰 task-0803 的任何新增内容），`./mvnw -q -o compile` 验证通过。本 session 第 3 次遇到同类并发覆盖。

**自检**：`./mvnw -q -o compile` 主仓 + worktree 双跑 0 错误；4 个文件（`MaterialVersionUpgradeService`/`PriceAdjustJobExecutionService`/`BomTreeRenderService`/`DataLoader`）临时 DEBUG 日志已在验证通过后逐条移除，`grep -c TASK0729-DEBUG` 全部归零；`MaterialVersionUpgradeService.java`/`DataLoader.java` 净改动为 0（只做了"加日志→删日志"，无其它逻辑变更，`git diff --stat HEAD` 确认）；真实改动落在 `PriceAdjustJobExecutionService`（+14/-2）、`BomTreeRenderService`（+21/-4）、`CardSnapshotService`（+28/-1）三个文件。

---
（以下为 merge master 带入的 task-0801/task-0803 系列开发记录）

[2026-08-01] task-0801 页签连表公式配置优化 - **交付并合并 master（merge 7954db51）** | 改动全在前端：`TabJoinFormulaDrawer.tsx`(925→约400行) + 新建 `tabjoin/TabFieldPanel.tsx`(左栏:搜索+卡片) / `tabjoin/FormulaEditorPanel.tsx`(右栏) + `tabjoin/FormulaRichInput.tsx`(高度/括号/光标) + `tabjoin/formulaBracketCheck.ts`(抽 scanParens) + **删** `tabjoin/SampleCardPicker.tsx` + `services/tabJoinFormulaService.ts`(删3方法) + 新增 `e2e/tabjoin-formula-drawer.spec.ts`；文档 `dev-docs/task-260801-页签连表公式配置优化/{需求说明,fronttask,backtask,api,testcase}.md` | **后端零代码改动**（2 个 dry-run 端点 + `/sample-cards` 原样保留，仅前端停调；`TabJoinPlanEvaluator` 经 codegraph 实证是 **Excel 视图正式渲染链路**组件而非试算配套，本就动不得） | **交付内容**：左右分栏 42:58 各自滚动 / 抽屉宽 `min(1520px,92vw)` / 移除试算 / 公式框 52→170~340 自动增高 / 括号深度 4 色 + 光标处配对高亮 + 未闭合红波浪线 / 左栏搜索（匹配页签名+字段名，总计 chip 恒保留、置灰语义不受影响） | 🚨 **三条实现红线（原型的演示级简化，照抄必出 bug，改本区代码必读）**：①`restoreCaret` **必须递归**且与 `readBack` 同口径（有 `data-raw` 的块整体跳过不下降、无 `data-raw` 的括号 span 递归下降按文本长度计）；**禁用「给括号 span 加 data-raw」的捷径** —— 会让 `handleKeyDown` 退格删块判定把括号当整块吞掉。②**括号可视化必须双通道分离**：内容驱动才重建 DOM，光标驱动只 `classList.add/remove`、`composingRef` 为真时直接 return —— 一旦在光标移动时碰 `innerHTML`，中文输入法 composition 被打断且光标丢失。③括号扫描抽 `scanParens` **单一口径**，与 `checkParenBalance` 同源消费（跳过 `[]`/`{}`），杜绝两套规则漂移。 | **验收 AC 22/22**（AC-13 真实 IME 候选框无头环境无法自动化，标部分 PASS）：vitest 6 files/248 tests；`scanParens` 新增 7 用例（原 13 条原样过）；E2E 2 passed；**存量兼容硬判定** = 含 SUMIF 公式保存后 `updated_at` 01:29:08→11:13:48（证明真落库）且 `formulas` 列 md5 与逐字节均不变 —— ⚠️**只看 md5 会被骗**：表达式为空会被 `save()` 拦下、点错按钮会点到「保存全部草稿」，两种情况 md5 同样不变，必须用时间戳证明保存真的执行。 | ⚠️ **执行期发现 4 处「用例自身缺陷」（详见 testcase.md §9.2）**：antd v5 走 CSS-in-JS 致 `.ant-tooltip-inner` 恒空（A/B 复核 master 同样为空）/ TC-26 与 TC-27 自相矛盾（总计 chip 本就该保留）/ TC-09 用 5 行撑不破 `minHeight:170` / `.ant-select-item-option` 全局匹配抓到隐藏残留项。**若直接采信这些 FAIL 会误报 4 个不存在的产品缺陷 —— 任何 FAIL 都必须先区分「产品坏了」与「用例写错了」**。 | ⚠️ **antd 双字按钮渲染为「保 存」「取 消」（中间带空格）**，`getByRole('button',{name:'保存'})` 永远匹配不到且表现为 **15s 超时而非报错**，极易误判成功能 bug；用 `button.ant-btn-primary`。其余实测 selector 已沉淀在 `fronttask.md` Task F6。 | 🚨 **并发撞号事件**：合并前发现本仓同时并行着**两个不同的 task-0801**（本任务 = 页签连表公式配置优化；另一个 = `dev-docs/task-260801-公式计算精度优化/`），BL 编号三重撞车，本分支 0091/0092/0093 全部避让为 **0093/0094/0095**；合并时主仓 `BACKLOG.md` 有他人未提交的 BL-0092，用 `git stash push BACKLOG.md` 单文件保护后合并再 pop，他人改动零丢失。 | 新登记 [[BL-0093]] dry-run 端点待清理(P2) / [[BL-0094]] `QuotePendingScopeOpenWhitelistTest` 恒红致安全护栏报警能力失效(P1) / [[BL-0095]] 测试库 V366 撞号致**所有 @QuarkusTest 起不来**(P1，master 上同样失败，与本任务无关)。**不触发 AP-44**（无 field_type 变动）。
---

[2026-08-01] task-0801 公式计算精度优化 — 后端交付（B1~B9） - 2026-06-21 拍板的旧口径（计算列/列小计/页签合计=4 位、产品小计+对外导出总额固定 2 位）正式作废，统一改为「内部全程 BigDecimal 不截断、只在落库/API/显示/导出 4 个边界规整到 6 位 HALF_UP 去尾零」。7 个后端求值点（6 JEXL + 1 自写 ArithParser）逐点改造 + 12 列 Flyway 迁移 + 全套单测。 | **新增文件**：`common/PrecisionPolicy.java`（DISPLAY_SCALE=6/DIVISION_SCALE=12/MC=DECIMAL128/HALF_UP；`round`/`divide`（除0→ZERO 不抛异常）/`of(double)`（走 valueOf 禁 new BigDecimal(double)）/`of(Object)`/`sum`）+ `common/DecimalJexl.java`（JEXL 引擎工厂，`JexlArithmetic(false,MC,DIVISION_SCALE)`）+ Flyway `V366__widen_amount_columns_to_scale6.sql`（12 列 numeric(18,4)→(20,6)）+ 6 个新测试类（`PrecisionPolicyTest`/`DecimalJexlTest`/`FormulaCalculatorGoldenCasesTest`(G-1~G-14+T3/T4 链路基线)/`TemplateFormulaServicePrecisionTest`/`ExcelViewServicePrecisionTest`/`CostingSheetServicePrecisionTest`，后 3 个用反射调私有方法验证、无需 CDI/DB）。 | **7 个求值点改造**：①`FormulaCalculator.ArithParser`（B2，递归下降解析器 double→BigDecimal，`*`用`multiply(MC)`、`/`走`PrecisionPolicy.divide`、`+/-`保持精确不设 MC；`ZERO4`→`ZERO`去 4 位常量）②`FormulaCalculationService`（JEXL#1，engine 换 DecimalJexl.newEngine + `toNumericString`/`component_subtotal`/`global_variable`/`number` 四处字面量追加"B"后缀）③`FormulaEngine`（JEXL#2，仅加 `.arithmetic(new JexlArithmetic(false,MC,DIVISION_SCALE))`——该引擎 `{path}`/binding 取值已是 BigDecimal 对象直接绑定 JexlContext 变量而非文本拼接，天然精确，无需字面量后缀改造）④`TemplateFormulaService.rowJexl`（JEXL#3，同③加 arithmetic；`toNumericLiteral`/`@变量`替换/`resolveAggregates`(SUM_OVER 结果) 三处追加"B"）⑤`ExcelViewService.evaluateFormulaColumn`（JEXL#4，内联引擎换 DecimalJexl.newEngine；`toNumericStr` 追加"B"）⑥`TabJoinPlanEvaluator`+`SafeArithmetic`（JEXL#5，`SafeArithmetic` 3 参构造器传 MC/DIVISION_SCALE 保留其除零兜底特有逻辑不用 DecimalJexl 整体替换；`numLit`/总计令牌/`reduceAggS` 结果追加"B"；AVG 除法 10→12 位）⑦`CostingSheetService.evaluateInlineFormula`（JEXL#6，engine 换 DecimalJexl.newEngine；`[col]`替换追加"B"；**安全校验正则 `[\d+\-*/().\s]*` 必须同步加 `Bb` 否则每条字面量替换都会被正则挡回 null**——这是容易漏改的隐藏耦合点）。 | **R-3 验证方式**：`DecimalJexlTest` 直接断言 `evaluate("0.1B+0.2B")==BigDecimal("0.3")` + 反例断言不加后缀仍解析成 Double（对照组）；`javap` 反编译 commons-jexl3 3.3 字节码证实 JexlArithmetic 构造器第 3 参 `bigdScale` 只在"非 BigDecimal 操作数强转 BigDecimal"的 `toBigDecimal()`/`roundBigDecimal()` 兜底路径生效，纯 BigDecimal 间除法走 `BigDecimal.divide(BigDecimal,MathContext)` 走 34 位有效数字（比 DIVISION_SCALE=12 更宽松非退化）——纠正了任务书原文"3 参构造器直接约束除法 scale=12"的表述误差，已在 `DecimalJexlTest` 注释存档避免后人误解。 | **B4-2 审计追加发现 2 个未列名漏点（均已修复，非仅列表里的 2 个已知点）**：①`ComponentDataEffectiveRows.computeScaled` 的折扣缩放 `v=v*discountScale` 是裸 double 乘法（3*0.8 类值二进制不精确≈2.4000000000000004），旧代码靠 `evaluateExpression` 顶层 `setScale(4)` 掩盖，B2 去掉截断后原样冒出，实测 `ComponentDataEffectiveRowsDiscountTest` 真的红了（17.4000000000000004）——改 `e.getValue().multiply(BigDecimal.valueOf(discountScale)).doubleValue()` 单次 BigDecimal 乘法算完再转 double；②`FormulaCalculator.evalCrossTab` 的 SUM/AVG/MAX/MIN 聚合原走 `toNumber()`→`Double`→`nums.stream().mapToDouble().sum()`→`BigDecimal.valueOf(r)` 双重转换+double 累加，`FormulaCalculatorCrossTabTest` 实测 2*1.13+4*1.13 不精确等于 6.78——改新增 `toBigNumber()` 全程 BigDecimal 收集 + `PrecisionPolicy.sum`/除法/`BigDecimal::compareTo` 选值；③`ConfigureSnapshotService.accumulateColumnSubtotals`（`CardSnapshotService.backfillSubtotalsFromResolved` 的孪生方法，同一 double 几十行累加根因）同步改 BigDecimal 收集。**教训**：去中间截断后必须真的跑一遍受影响的既有测试，光看"改动点列表"会漏掉被截断掩盖多年的裸 double 运算。 | **ArithParser 语义保真**：`cross_tab_ref` 的 I-2（KAVG/KMAX/KMIN 空集）分支原注入字面量 `"(0/0)"` 依赖 double 除零产生 NaN 触发外层 `Double.isNaN` 降级；BigDecimal 化后 `PrecisionPolicy.divide` 除零优雅返 ZERO 不抛异常，"(0/0)"这条路径失效——改为直接 `throw new IllegalStateException(...)`（与旁边 `FormulaErrorMarker` 分支手法一致），整表达式塌 0 的最终语义不变，但改造方式不同，逐行 diff 才能发现这类"外表無改动，内在依赖已断"的坑。 | **B5 落库边界**：`LineDiscountService.recompute` 5 个金额字段赋值前统一 `PrecisionPolicy.round()`（原 5 处 `setScale(4)`）；折扣系数由 `1.0-rate.doubleValue()/100.0`（double 中转）改 `BigDecimal.ONE.subtract(rate.divide(100,12,HALF_UP))`；`QuotationService` 5 处（664/706/850/1943/2385 附近，行号随并发改动漂移）`originalAmount`/`totalAmount` 赋值改 `PrecisionPolicy.round`，除法 scale 4→12；`CostingFreezeService`/`CostingVersionService` 的 `computeCostingTotalAmount`/`recomputeTotal` 同改。 | **B6 导出**：`QuotationExportService.formatAmount` 从固定 `setScale(2)` 改委托 `NumberFormatUtil.format(v,null,true)`（6 位去尾零），默认值 "0.00"→"0"；`createAmountStyle` POI DataFormat `"#,##0.00"`→`"#,##0.######"`（**若漏改这处，写入的高精度 double 值会被 Excel 单元格样式强制显示成 2 位，看起来像没生效**）；新增 `writeAmountCell`/`writeAmountCellValue` helper（`ExcelViewService`+`QuotationExportService` 各一份）：`stripTrailingZeros().precision()>15` 则 `setCellValue(String)` 保精度，否则 `setCellValue(double)`（POI NUMERIC 单元格 IEEE754 只保证 15 位有效数字，亿级金额 9 位整数+6 位小数=15 位正好触顶）。 | **B7 迁移**：`V366__widen_amount_columns_to_scale6.sql`（12 列）+ 4 个实体（`Quotation`/`QuotationLineItem`/`QuotationLineComponentData`/`CostingOrder`）`@Column(precision=20,scale=6)` 同步；test 库(`cpq_db`)验证 success=t + 12 列 information_schema 核对全部 20/6。**⚠️开发中撞见 flyway 版本号并发冲突**：主工作区已有另一并发会话未提交的 `V366__task0729_costing_element_price_field.sql`（不同内容，同版本号），临时把本迁移 copy 进主工作区验证时两份 V366 文件类路径冲突导致 Quarkus dev（8081）瞬间 500，**已立即删除主工作区副本恢复 401**；dev 库 `cpq_db_0724` 的 366 版本槽位已被那个并发会话的 task0729 迁移占用（`SELECT version,description FROM flyway_schema_history` 证实），本迁移**只在 test 库(`cpq_db`)成功落地，dev 库未应用**——按"迁移一旦应用禁止改名改号"规则不能擅自把本迁移改成 V367（会打乱 test 库已记录的历史），此坑留给合并时处理（建议合并前由集成者对其中一支重新编号）。 | ✅ **自检**：`./mvnw test`（test 库）**1886 用例，158 failures + 380 errors**——逐条排查证实**全部与本次改动无关**：380 errors 中 271+241+25≈537 条根因唯一，是 `customer` 表一条 2026-07-23（早于本任务立项）就存在的孤儿 `quotation` 外键引用导致 `UserResourceTest` 等清理例程连锁 `current transaction is aborted`（SQL 查证时间戳实锤，非本次引入）；94 条 `ARJUNA016051` JTA 线程复用、16 条 `mat_part` 已废弃表（AP-53）、其余若干 Excel 测试夹具文件缺失（"Cannot find zip signature"）——均为共享测试库既存环境问题；158 failures 同源级联或 `CostingVersionServiceTest`/`GoldenCardValuesEquivTest` 类的**已有自保护机制**（`assumeFixtureIntact`/`assumeTrue`）文档化的硬编码夹具 UUID 漂移（历史 `cpq-golden-cardvalues-preexisting-drift` 已知问题类）。**真正因本次改动出现过的 6 处失败已全部修复**：`FormulaCalculatorTest`(2，1/3、2/3 内部精度断言从 4 位改 12 位)、`ComponentDataEffectiveRowsTest`(1，产品小计断言从压缩后 0.2245 改全精度 0.224474)、`FormulaCalculatorCrossTabTest`(1，即上述 evalCrossTab 双转换 bug)、`ComponentDataEffectiveRowsDiscountTest`(1，即上述 computeScaled 裸乘法 bug)、`DecimalJexlTest` 自建用例(1，误判 JEXL 3 参构造器行为已订正)；`cpq-frontend/src/utils/__fixtures__/formula-reconcile-cases.json`（前后端共享黄金样本）同步把 `round_third`/`round_twothird` 两条 `expected` 从 4 位改 12 位除法精度值。**只涉及我改动文件的定向复测（245+54 用例）全绿**。V366 success=t（test 库）+ 12 列 information_schema 核对全 20/6；`/api/cpq/components`→401（鉴权正常，dev server 未受影响）。 | 遗留：dev 库(`cpq_db_0724`)迁移待合并时重新编号解决；`elementprice/**`（`PriceImportRowWriter`/`StrategyService`/`PriceTableService`）3 处 `setScale(4` 按 backtask 明确"不改"清单保留（取数值类别 B）。

---

[2026-08-01] task-0801 公式计算精度优化 — 前端交付（F1~F8） - **2026-06-21 拍板的旧口径正式作废**：计算列/列小计/页签合计=4位、产品小计+对外导出总额固定 2 位 → 统一改为「至多 6 位、HALF_UP、去尾零」；除法内部精度 12 位；公式引擎去 `new Function`/`eval` 改十进制精确递归下降解析器 | **涉及文件**：新增 `cpq-frontend/src/utils/precision.ts`（`DISPLAY_SCALE=6` / `DIVISION_SCALE=12` / `PAYLOAD_SIGNIFICANT_DIGITS=15` / `toDecimal` / `sumDecimal` / `roundToDisplay` / `normalizeNumber` / `evaluateArithmetic`）+ `precision.test.ts`（G-1~G-14 黄金用例 + T1~T8）；改 `formatNumber.ts`（`COMPUTED_FALLBACK` 引用 `DISPLAY_SCALE`，旧 4 位/2 位口径注释标废）、`formulaEngine.ts`（2 处 `new Function`→`evaluateArithmetic`，去 `toDecimalPlaces(4)` 中途截断）、`ExcelView.tsx`（第 3 处求值点）、`QuotationStep2.tsx`（`sumColumnsCanonical`/`subtotalsFromResolvedRows`/`computeTabSubtotal`/`getComponentSubtotals` 累加改 `sumDecimal`，去 `round4`；`formatCurrency` 去 `decimals:2`）、`tabTotalLines.ts`（两处 `+=` 改 `sumDecimal`）、`QuotationStep3.tsx`（`grandOriginal/grandDiscount/grandTotal` 全程 Decimal，链路二最高优先级）、`QuotationWizard.tsx`（`normalizeDraftPayloadNumbers` 改 `normalizeNumber`；两处 `originalAmount` reduce 全程 Decimal；4 处展示 `toLocaleString`→`formatNumber`）、`ReadonlyProductCard.tsx`/`QuotationList.tsx`/`ProductDetailViews.tsx`/`SnapshotTab.tsx`/`CostingSummaryDetailPage.tsx`（显示口径统一）；测试更新 `formatNumber.test.ts`/`formulaEngine.test.ts`/`draftPayloadNormalize.test.ts`/`formulaReconcile.test.ts`（详见下）；`package.json` 加 `"test": "vitest run src"`（原缺 test 脚本 + 缺此 scope 会把 `e2e/*.spec.ts` 误当 vitest 用例导致 61 个假失败）。 | 🔑 **两条链路边界（核心设计）**：链路一（单元格求值→列小计→页签合计→产品小计，量级 ≤10⁶）内部精确但跨层仍以 number 承载；链路二（产品小计→×年用量(可达几十万)→行合计→Σ整单合计，量级冲到亿级 = 15 位有效数字已达 double 极限）**全程 Decimal，禁止中途 `.toNumber()`**，只在最终 `roundToDisplay`。 | ⚠️ **额外发现并修复（不在 fronttask.md 原始 F3 清单内，超出文档字面范围但在"链路二"定义内，已在交付说明中向技术总监报备）**：`lineDiscount.ts#computeLineDiscount` 的 `s1*qty`/`(s0-s1)*qty`（折后单价×年用量，产出 `lineTotalAmount`/`lineDiscountAmount`，正是 Step3 两个 `grandXxx reduce` 直接求和的源头）原用 `Math.round(n*10000)/10000` 四位数字 number 乘法，未被文档点名但符合"链路二危险操作"定义，已同步改 Decimal + `roundToDisplay`。 | 🔧 **Task F5 两次修订（务必看清最终版，勿改回中间态）**：v1（fronttask.md 原稿）`PAYLOAD_NORMALIZE_SCALE=10` 按小数位数 `toFixed`；v2（技术总监实测 `production_energy.unit_price` 现网列声明精度达 12 位后修正）改按**有效数字** `toPrecision(15)`——按小数位数一刀切在「小值高精度取数列(12位)+大值金额(亿级)」并存场景下无解（位数低压坏取数列 / 位数高在大数值暴露 double 表示噪声，如 `(98765431.2).toFixed(14)` → `98765431.19999999552965`）；15 位是"已知最深声明精度(12位小数)+安全边际"，非拍脑袋。**若下一个人想把 `normalizeNumber` 改回 `toFixed(N)`，先看 `precision.ts` PAYLOAD_SIGNIFICANT_DIGITS 注释里的反例。** | 🐛 **实现期抓到的真实回归（并已修复，非虚报）**：`formulaReconcile.test.ts`（前后端共享 `formula-reconcile-cases.json` 防漂移红线）在后端工程师**同一 worktree 并发**把 fixture 的 `round_third`/`round_twothird` 两条 `expected` 从 `0.3333`/`0.6667` 改为 12 位除法精度值 `0.333333333333`/`0.666666666667`（与本 PrecisionPolicy/`DIVISION_SCALE=12` 独立对齐一致，验证前后端黄金用例真对上了）后，本文件内部残留的 `round4()` 包装器（把前端结果强砍回 4 位再比对）变成错误比较基准，2 条测试假红；改为直接比较原始值（`toBeCloseTo(expected, 9)`，容许 decimal.js↔number 往返噪声）后 12/12 绿——**这是并发协作暴露的真实测试基础设施 bug，不是我引入的语义回归**，已修复并在交付说明逐条说明修改依据。 | ✅ **自检**：`tsc --noEmit -p tsconfig.app.json`（注意不是空转的 `tsconfig.json` 根，BL-0085）2446 文件、0 错误（含 sanity-check 故意注入错误验证 tsc 真检查）；`npm test`（新增 `vitest run src`，避开 e2e glob 碰撞）63 文件 879 用例全绿；worktree 本地临时 Vite（端口 5201，**不是共享 5174**——5174 服务主工作区旧代码，其 SPA fallback 对不存在文件也返 200，直接用会得假阳性）对 14 个改动文件转译 200 + 非 SPA-fallback 内容校验；手工 Playwright（真实罗克韦尔客户+模板1/2/3+核价单数据，非固定 e2e spec）验证编辑页 5 Tab / Step3 / 详情页(报价+核价两视图) / 核价工作台 / 列表 全部 0 处"加载中"残留；**实测精度证据**：编辑页外购件成本 Tab 手填单价 `0.123456789`×组成数量`3`＝`0.370370367`，页面渲染 `0.37037`（6 位 HALF_UP 去尾零，旧 4 位口径会显示 `0.3704`）；列表页真实数据 `¥755.9252`（旧 `toLocaleString()` 默认 3 位会显示 `¥755.925`，AC-5 直接实证）。 | ⚠️ **E2E 双 spec BLOCKED（非本次回归，已做 A/B 对照）**：`quotation-flow.spec.ts` 硬编码客户"西门子"（库中 0 命中）+ 模板"报价模板0608"（库中 0 命中）、`composite-product-flow.spec.ts` 硬编码模板"组合产品 v1.16"（0 命中）——`cpq_db_0724` 现库只有"罗克韦尔"+"罗克韦尔模板1/2/3"+"核价模板1"共 7 个模板，与 **`BL-0078`（E2E 夹具随迁库集体失效）同一根因族**，BL-0078 列出的清单未含这两个 spec（本次是新发现的同类实例，建议后续把这两个 spec 补进 BL-0078 范围一并处理）；master 基线同环境同 spec 复跑同样卡死在选客户步骤（A/B 对照实证，日志见交付说明）。 | 已知限制：核价侧 `CostingSummaryDetailPage.tsx` 只手工截图未跑该模块专属 E2E（不存在）；后端 `PrecisionPolicy`/`DecimalJexl`/V366 迁移由后端工程师同 worktree 并发交付，未包含在本条目改动文件清单内（各自记录各自 RECORD）。

---

[2026-08-02] BL-0096 亿级金额端到端验证（task-0801 精度优化的验收补做） - **技术总监造数实测，四处逐位一致 + EDGE-13 兜底实证，BL-0096 关闭** | 无代码改动，纯验证 + 文档（`BACKLOG.md` BL-0096 就地标记完成并附完整实测数据） | **背景**：task-0801 全部设计依据是「年用量几十万 → 整单冲到亿级 → 15 位有效数字达 double 极限 → 小数第 5/6 位不可信」，但现网 `max(annual_volume)=1`、最大行金额 14 元，该场景**根本不存在**，合并时只能靠算法级单测覆盖，端到端留作 BL-0096。 | **数据设计要点**：20 行、单价 `123.456789+(i-1)*0.000007`（每行互异，避免"20 个相同数相加"被优化掉）、年用量 `799999`（**非 10 的幂次**，确保乘完仍带满 6 位小数而不是 `.2` 这种一位小数）；金标准用 psql `numeric` 精确计算 `SUM(subtotal*annual_volume)`。**绕开 BL-0078 的 UI 加产品阻断**：直接造 `quotation_line_item` 行 + 调 `POST /{id}/submit` 触发后端权威重算（`LineDiscountService.recompute` → `Σ lineTotalAmount`），验的仍是真实链路二计算路径，只是不走 UI 加产品那一段（那段属 BL-0078、与精度无关）。 | ✅ **六处观测全部逐位一致**（金标准 `1975307218.862890`，10 位整数+6 位小数=**16 位有效数字**）：DB `total_amount` 差值 `0.000000` / DB `SUM(line_total_amount)` 差值 `0.000000` / API `totalAmount` 逐位同 / 导出 HTML `1975307218.86289`（去尾零符合设计）/ 导出 Excel `<v>1.97530721886289E9</v>`（OOXML 对 double 的科学计数法序列化，非精度问题）/ 前端列表页渲染 `1975307218.86289`（Playwright 1 passed）。逐行验算（抽 1/10/20 行）`line_total_amount - subtotal*annual_volume` 亦全为 `0.000000`。 | 🔑 **EDGE-13 需要第二组数据才能验到**：第一组总额 `1975307218.862890` 经 `stripTrailingZeros()` 后是 `197530721886289` = **正好 15 位**，`precision()>15` 为 false，走的是数值单元格 —— 光看第一组会**误以为兜底逻辑没生效**。另造一组（末行年用量改 `800000` 令总额末位非 0）得 `1975307342.319812` = **16 位**，Excel 单元格从 `t="n"` 变为 `t="s"` 指向 `sharedStrings[27]`，实证 B6 的 `stripTrailingZeros().precision()>15 → setCellValue(String)` 分支正确。**两组对照才是完整证据**。 | ⚠️ **首次前端验证 FAIL 是用例写错不是产品缺陷**：路由用了 `#/quotations`，实际是 `/quotations`（截图显示已登录但停在工作台）；改用点击侧边栏「报价中心 → 报价单管理」后 1 passed。再次印证 testcase.md 纪律「任何 FAIL 必须先区分产品坏了 vs 用例写错了」。 | **收尾**：40 行 line_item + submit 产生的 2 张 costing_order 已删，两张单恢复 `DRAFT`/`total_amount=0`，库中 `AC14-TEST-*`/`EDGE13-*` 残留 **0 行**；临时 Playwright 脚本与截图已删除。 | 遗留：[[BL-0078]]（E2E 夹具集体失效）与 [[BL-0092]]（删单不清占号）仍 TODO，与本次验证无关。

---

[2026-08-02] BL-0092 孤儿 pending 占号 —— 体检与兜底清理（含原判断更正） - **已交付合 master（fix `fe612af9` / merge `3fc3f204`）** | 新增 `quotation/service/PendingHygieneService.java` + `quotation/resource/PendingHygieneResource.java` + `PendingHygieneServiceTest`（4 用例全绿），改 `basicdata/v6/repository/MaterialMasterRepository.java`（加 `deleteOrphanPendingWithGuard()`） | ⚠️ **先更正 BL-0092 的原始判断（登记时错了）**：原条目断言「删除路径无任何清理，只有 `QuoteBackfillService:145` 在审批转正时清占号」——**不成立**。`QuotationService.delete()` 早已调 `cleanupPendingV6Data()`（task-0721 B8），覆盖 `B8_PENDING_TABLES` 8 张表 + `material_master` 的 `deletePendingWithGuard`，**9 张表全覆盖**；实测造 pending 行→调 DELETE 端点→残留 **0 条**，删单回收路径完整有效。登记时只看转正路径就下结论、未穷举调用点，属判断失误。 | 🔑 **241 条孤儿的真实来源靠时间线定位**：task-0721 B8 修复合并于 **07-21**，而孤儿数据全部产生于 **07-27~28** —— 正是 `cpq_db_0724` 迁库重建期，即**绕过应用层直接重建 `quotation` 表**的产物（与 [[BL-0078]] 同一批操作）。**判断"代码缺陷 vs 历史脏数据"必须比对修复合并时间与数据产生时间，不能只看现象。** | **真实缺口是两个防御空白**（本次修的对象）：①9 张表的 `pending_quotation_id` **零外键约束**，任何绕过应用层的操作都会留永久僵尸且无机制发现；②`material_master` 引用守卫**有意**留悬空（只打 WARN 说"需人工核查"），从无兜底去核查。孤儿危害 = `is_current=false` 且归属单不存在，业务查询永不命中却占着 uq 位置（后续导入撞键），并让以 `pending_quotation_id IS NULL` 为条件的查询返空（`material_customer_map` 曾因此对所有客户返空）。 | **设计要点**：清理语义与删单回收**完全一致**（8 张表直删、`material_master` 走同款守卫），区别仅在筛选条件从「属于某张单」换「归属单已不存在」；守卫"悬空版"三处引用检查逐条**对称改写**（原判据「引用方不属于本单」→ 新判据「引用方是正式行或归属单仍存在」，引用方自己是孤儿则不算有效引用）；`inspect()` 用 `information_schema` 反查所有带该列的表做**交叉校验**，漏加新表会在 `unmanagedTables` 暴露不会静默；端点 `dryRun` **默认 true**，真删须显式传 false（DELETE 横跨 9 张基础资料表，误触代价高）；**新建独立 Resource 而非改 `QuotationAdminResource`**——后者正被并发会话修改，避开合并冲突。 | ✅ **清理结果（经端点执行，已备份 `bl0092_orphan_backup_20260802` 241 行可回滚）**：8 张表 239 条全清零；`material_master` 2 条被守卫拦下（`W-1001` 被 BOM 子件引用 19 次+客户映射 1 次、`S-80011` 被子件 18+母件 13+映射 1）——**守卫拦得对**，这两个料号正在被有效数据使用，合理处置是转正（`pending_quotation_id` 置 NULL）而非删除，属业务判断故未擅自执行。**活数据完好性逐表核验**：`unit_price 94-12=82`/`material_bom_item 170-85=85`/`element_bom_item 93-43=50`/`capacity 64-32=32` 全部精确匹配，挂活单的 pending 行一条未动。 | 🐛 **踩坑：worktree 跑测试撞 `FlywayValidateException`** —— `Detected applied migration not resolved locally: 368/369/370`，根因是并发会话的 V368~V370 已应用到共享 test 库但文件在主工作区仍 untracked，新建 worktree 里没有。按既有做法临时 copy 进 worktree 跑完再删（**注意别提交**）。这是 [[BL-0095]] 的同型问题，只要"共享测试库 + 未提交迁移"这个组合存在就会反复出现。 | **有意未做**：不给 `pending_quotation_id` 加外键 —— `ON DELETE CASCADE` 会绕过 `material_master` 引用守卫，`ON DELETE SET NULL` 会把 pending 行变成"无主僵尸"仍占 uq 位，两种语义都不对；现方案 = 应用层删单即时回收 + 端点兜底体检清理。

---

[2026-08-02] task-0728 组件 SQL 视图可视化配置（取数配置器）—— 七轮需求澄清完成后**用户主动暂停**，登记续接点 | 产物：`dev-docs/task-260728-组件SQL视图的用户便捷化配置功能/`（`需求说明.md` v7 773 行 + `原型-取数配置器.html` 交互原型 + `原型-页签字段匹配关系.html` 审核矩阵）；`BACKLOG.md` 新增 BL-0088/0089/0090 | **无代码改动**（未建 worktree、未拆任务文档、未写实现）。 | **方案内核**：把 `dev-docs/rule-260724-组件模板配置/` 的配方从「Agent 读的文档」变成「代码里的配方注册表 + 编译器」——用户**只选页签类型**一项，主数据源/判别式/取数锚点/附属 Sheet/客户收窄方式全部推导；`component_sql_view` 加 `builder_config` JSONB（**NULL = 存量手写模式**，20+ 存量视图原样共存不迁移）；SQL 是只读编译产物 ⇒ **报价渲染链路零改动**（`SqlViewExecutor`/`ComponentDriverService`/快照组装只读 `sql_template`，不知道 builder 存在）。一体化：一列 = 视图 SELECT 列 + 组件字段 + `default_source` 绑定，三者同一次保存原子产出，杜绝「逐字对齐」这个当前最高频静默故障源。 | 🔑 **七轮里被实测数据推翻的四个自己的方案（这是本次澄清的主要价值）**：①「取数锚点二选一」→ **锚点与页签类型是两条正交轴**（`unit_price(INCOMING_MATERIAL_PROCESS)` 的 `code` 20/20 是材质料号、`finished_material_no` 20/20 有值 ⇒ 锚点=成品、页签=材质元素，原模型配不出）；②「客户过滤无条件固定生成」→ `material_master` **无 `customer_no`/`is_current`/`system_type` 任何一列**，必须 `JOIN material_customer_map ... AND mcm.customer_no=:customerCode` 收窄，`capacity`/`plating_scheme` 无客户列只能加版本过滤；③「全部列 = 表的业务列」→ `unit_price` 被 **8 张 sheet 共用**，按表给会混进别的 sheet 的列（对本 sheet 永远为空），正解是**该 Sheet 真实导入列**（3~9 个，中文名用 **Excel 原列名**如「值」「比例」「计价单位」），并据此新建 `QuoteSheetRegistry` 声明式登记 + 单测双向断言（同 `PricingSheetRegistry` 镜像纪律，见 [[v6-handler-registry-mirror-trap]]）；④「附属源一律 `LIMIT 1` 标量子查询」→ **6 个附属源里 3 个对页签行身份不唯一**（成品其他费用 4 行/电镀费用 2 行/组装加工费 2 行），`LIMIT 1` 是**静默取错值**，正解是让页签变多行。 | **最终关联规则（三层，§4.3.3.3.4）**：每 Sheet 登记「附加维度」→ **页签行粒度 = 所选字段各源附加维度的并集**（由用户搭配字段动态决定，主件页签因此可能 1 行/2 行(工序)/4 行(要素)）→ 关联条件与 SQL 形态自动推导：**提供粒度 → `JOIN` 且 `is_current`/判别式放顶层 WHERE**（§3.6 铁律：版本化表进带 `is_current` 的 JOIN ON 会让 pending 改写产出畸形 SQL）、**粒度更粗 → 相关标量子查询**、同表同行 → 直接取列。两条 err 级阻断：**粒度冲突**（两个互不包含的维度同选 ⇒ 笛卡尔积）、**粗粒度列勾小计**（每行重复 ⇒ 重复计算）。 | ⚠️ **改名影响面（§4.3.3.6，实测）**：全链路**没有一处用稳定 ID**——`snapshot_rows.driverRow` = `{"_销售料号":...}`（视图别名做 key）、`basicDataValues` = `{"{$cp_view._销售料号}":...}`（**完整路径字符串**做 key）、公式 token `{"type":"field","value":"单价"}`、`row_key_fields`=`["料号","材质","元素"]`、`part_no_field`/`sort_field`、`cross_tab_ref.match`=`[{"a":"料件","b":"料件"}]` ⇒ 改一个列名须**同事务同步 6 处**，且**冻结单（SUBMITTED/APPROVED/PUBLISHED）引用时必须阻断改名**（其快照 key 是旧名且不可重刷，强改会让旧单该列渲染成空）。根治方案（渲染层改用稳定 `fieldKey`）代价极大、触及 AP-44 协议面与全量历史数据迁移，登记 **BL-0090** 独立立项。 | **其它实测校正**：`FINISHED_MATERIAL_OTHER` 的值在 `cost_ratio`（4/4，比例制）、`INCOMING_MATERIAL_PROCESS` 的值在 `base_value`（2/2），**`pricing_price` 均全空** —— 按直觉绑 `pricing_price` 会配出整列 NULL（这条自证了「真实数据预览」的必要性，dry-run 查不出）；`plating_scheme.hf_part_no` 与 `unit_price(PLATING).plating_scheme_no` **双向全空**，电镀方案在现网是孤立数据无关联路径，本期移出；`material_bom_item` 三态 `RECIPE 78/OUTSOURCED 34/ASSEMBLY 33/NULL 1`，四种用法（外购件主源/材质元素附属/零件附属/BOM树主源全部边）由页签类型推导，用户不接触 `characteristic`。 | 🚧 **暂停时卡在 3 项待用户拍板**（见 `需求说明.md` §0.1）：①来料回收折扣归哪页签（建议费用类）；②BOM 树是否本期（+30% 工作量：双分支 `UNION ALL` 编译 + 写 `costing_bom_tree_config` + 预览走 `BomTreeRenderService`）；③血统匹配是否本期（BL-0088，**实测规模 S~M 远小于初估**：核心 `buildMatch()` 12 行纯函数 + 6 调用点全在 `formulaSerialize.ts`、`TabFieldMatrix.tsx` 1 行、Drawer ~80 行 UI，**后端 0 行**因 `RowKeyCompare`/`TokenMappabilityValidator` 均为零生产调用的休眠类；**token 格式不变**故存量零影响、求值链路零改动；现网 43 个 ACTIVE 组件仅 2 个用跨页签且都是手写视图无血统 ⇒ 零行为变化）。 | ⚠️ **复现前提**：全部实测取自 `10.177.152.12:5432/cpq_db_0724`（07-27/28）；`CLAUDE.md` profile 表此后有更新，**续接前先确认 8081 实际连的哪个库**，若已切库则带具体行数的结论须重验。

---

[2026-08-02] 「该料号卡片数据待重算」根因排查（QT-20260802-0049） - **诊断结论 + 立项 BL-0097/0098/0099，无代码改动** | 用户报障 → 技术总监逐层定位，产出 3 条 BACKLOG 条目 | 🔑 **根因是「局部 catch 兜不住事务毒化」**：组件 `$view`（尤其 `tab_type='BOM'` 树页签）查询在 PG 层报错 → `componentDriverService.expand` 抛的异常**被 Java 侧 catch 住看似已处理** → **但 PG 已把当前事务置 aborted，此后同事务每条 SQL 都失败** → `buildCardValues` 抛异常返 null → 落 `__cardValueFailed` 哨兵 → 前端显示「待重算」。`CardSnapshotService:2179-2183` 注释（2026-07-22 事故）已记载该陷阱，当时处置是"树组件跳过平铺展开"属点状规避，**通用问题未根治** → 立 [[BL-0097]] P0。 | ⚠️ **报价侧哨兵不带 `__errorMsg`**（只有核价树整单渲染失败才走 `failedSentinelWithError`，BL-0030 的能力报价侧缺失），故用户与排查者都看不到是哪条 SQL 出错，dev 环境日志又不落文件 —— **排查成本极高，这是本次耗时最多的一环**。 | 🔑 **第二发现：FORMULA 字段的 positional fallback 致配置静默漂移** —— `resolveFormula` 4 级回退的第 3 级按「字段在 FORMULA 字段中的相对位置」取 `formulas[同位置]`。实证：`COMP-0157「物料」`的 `材料成本` 字段 `formula_name` 为空、无同名公式，按位置匹配到 `formulas[2]`=「银点材料成本公式」。**用户本人认为"这条公式没被任何字段绑定"，实际它正在决定「材料成本」列的算法** —— 用户改动该公式后故障消失，正是此机制的实证。危害：公式列表增删/改序会让未显式绑定的字段**静默换算法**，不报错不提示 → 立 [[BL-0098]] P1。 | **第三发现**：`formulaSerialize.ts:642-647` 构造跨组件引用 token 时 `tab_name: col` 填的是**列名**而非页签名，后端第 2 级回退会拼出 `"税率#税率"` 恒不命中；当前靠第 1 级 `component_code#col` 兜住未出事，但 alias 变化即静默跌到"取整个组件小计合计"（如「产品」= 管理费+税率之和）→ 立 [[BL-0099]] P2。 | ✅ **排除了自己的嫌疑（重要）**：本次故障发生在 task-0801 精度改造合并之后，故先做 A/B —— ①同期建的 `QT-20260802-0048`/`0050` 卡片值**正常**（0050 与 0049 同模板同料号）；②`evaluateExpression` 的 `try/catch` 在 B2 改造后**完好**，`cross_tab_ref` 的两处 `throw`（B2 把原 `"(0/0)"`+NaN 降级改成显式 throw）均被它捕获返 ZERO，**不会**传播到 `buildCardValues`。故精度改造在该路径无回归。 | 🐛 **诊断方法论沉淀**：①「同模板同料号一成一败」是时序问题而非配置问题的强信号，应立刻查配置变更时间线（本次 `COMP-0157` 与模板在 02:04:52 被改，正卡在 0049 算值 02:03:14 与 0050 算值 02:05:28 之间）；②`quotation_view_structure` 会被后续 refresh 覆盖，**故障后再去查冻结结构可能已看不到故障时的配置**（本次即如此，只能靠用户口述原始 token）；③组件级改动**无变更日志表**（只有 `basic_data_change_log`/`global_variable_change_log`），配置类故障事后无法追溯改了什么 —— 这本身是个可登记的可观测性缺口。

---

[2026-08-02] 变更日志「只读空壳」查证 —— 立项 BL-0100 - **诊断 + 立项，无代码改动** | 排查 [[BL-0097]]「卡片数据待重算」时被"组件改动无从追溯"直接卡住，顺藤查证变更日志基建的真实状态 | 🔴 **查证结论：整套变更审计能力只有读没有写** —— 表 `basic_data_change_log`（24 列，`field_changes` jsonb / `old_value` / `new_value` / `change_source` / `affects_calculation` 设计完善）✅、查询 API `ChangeLogService.search()` ✅、导出 `export()` ✅、REST 端点 `ChangeLogResource` ✅、每月定时清理 `ScheduledTaskService.cleanupChangeLog()`（CL-RETENTION-07，删 5 年前数据）✅，**唯独写入方 ❌**：全工程零 `INSERT`、无实体类、`ChangeLogService` 只有 `search`/`export` 两个方法，表 **0 行**。即**系统有一个每月定时任务，专门清理一张从未被写入过任何数据的表**。`global_variable_change_log` 有一处 INSERT（`GlobalVariableService:664`）但表同样 0 行（路径未被触发过）。 | ⚠️ **比没有更危险**：前端若有「变更日志」入口，查出来恒空 → 给人"已有审计"的错觉。 | **实证代价**：本次排查中 `COMP-0157「物料」`与模板在 `02:04:52` 被改过（正卡在故障单算值 02:03:14 与正常单算值 02:05:28 之间，是判断因果的关键证据），但**改了哪个字段、从什么改成什么，库里查不到任何记录**，只能靠用户口述；组件/模板配置直接决定报价金额算法，改动无留痕 = 算错钱无法追责。 | **实现注意（已写进条目）**：①组件保存是整份 jsonb 覆盖，必须做**字段级 diff**，只记"整份旧值→新值"等于没记（一次改动几十 KB）；②写日志不得阻断主流程；③⚠️ 注意 [[BL-0097]] 教训——日志写入若与主保存同事务且 SQL 出错会**毒化整个事务**，需评估独立事务/Savepoint。

---

[2026-08-02] 基础数据导入(repair-0802) 电镀费用/电镀成本增加「投入料号」，`unit_price` 两列语义归队 - **已交付合 master**（分支 `feat/repair-0802-plating-input-part-no`，7 提交） | 改 `basicdata/v6/quote/Q17PlatingCostHandler.java` + `QuoteImportValidator.java` + `basicdata/v6/pricing/P22PlatingCostHandler.java`，新增 `V372__repair0802_plating_input_part_no.sql`，测试 `Q17PlatingCostHandlerTest`(6) / `QuoteImportValidatorPlatingTest`(5) / `P22PlatingCostHandlerTest`(4) | **问题**：电镀两个 sheet 把销售料号写进 `unit_price.code` 且 `finished_material_no` 恒空，与该表其余 14 个 handler 口径相反（`code`=零件料号、`finished_material_no`=成品料号，见 `dev-docs/rule-260724-组件模板配置/4-页签属性与树.md` §零件）。实证：现役 18 行 `QUOTE/PLATING` 的 `code` 全是 `S-80011`，而该料号在 `material_master` 里 `material_name='投入零件1'`、`material_type='零件'` —— **业务本就按零件维度记电镀费用，只是 sheet 没有对应的列**。后果：费用无法上卷成品、BOM 闭包类视图取不到数。 | **改动**：两个 sheet 增加**非必填**的【投入料号】【投入料号名称】列。报价侧 Q17 照 Q06/Q07 三分支解析（有码沿用 / 仅名称反查铸号 + `material_master` upsert / 皆空回退销售料号），groupKey 加 `finished_material_no`；核价侧 P22 照 P15 范式把分组锚点从 `code` 切到 `finished_material_no`、`code` 进 content（核价侧不做名称反查，与 P15/P16/P17「品名」列一致）；`QuoteImportValidator.validatePlatingCost` 把名称反查失败提前到 Phase 1 零写库阶段拦截；V372 清理旧语义存量 + `dj_view` 改绑成品轴并暴露 `input_part_no`/`input_part_name` + 电镀成本组件加两个字段。 | **关键决策**：①**「非必填」不等于「可跳过」** —— `code` 是 NOT NULL 且在 `uq_unit_price` 13 维唯一键内，两列皆空必须回退为销售料号（语义=电镀针对成品自身），与 Q15 组装加工费年降的退化范式一致，且使存量语义零漂移。②**零 DDL** —— `finished_material_no` 早在 V276 就进了唯一键。③**存量必须清理而非迁移**：旧行 groupKey 缺一维，重导时不会被下线，两组 `is_current=true` 并存会让视图出重复行（需求说明 §6 已授权清空重导）。④**两侧范式故意不同**：报价侧 gk 含 `code`（对齐 Q06/Q07），核价侧 gk 锚 `finished_material_no`（对齐 P15），各自与本侧邻居一致优先于跨侧统一。 | 🐛 **踩坑一（协议级）**：读料号列必须用 `row.exact("投入料号")` —— `getStr` 是 contains 匹配，会命中「投入料号名称」列并**静默取错值**。 | 🚨 **踩坑二（迁移必看，本次由 dry-run 抓出，原计划写错）**：**迁移里禁止用 `component.code` 定位组件**。`component.code` 是各库独立自增的配置数据，**同一个 code 在不同库指向完全不同的组件** —— 实测 dev 库 `cpq_db_0724` 的电镀成本是 `COMP-0063`，而 test 库 `cpq_db` 的 `COMP-0063` 是「材质/元素/材料成本」组件（`COMP-0057` 才是电镀成本）。按 code 硬编码会在别的库里改错组件、污染无关配置。正解是**语义锚点** `WHERE trim(data_driver_path)='$dj_view'`。同理**字段插入位置也不能硬编码下标**（两库该组件字段序不同：test 库销售料号在第 1 位、dev 库在第 2 位），要按「销售料号」的 `WITH ORDINALITY` 下标 +0.1/+0.2 动态算。验证已确认：test 库 `COMP-0057` 正确加了两列且位置正确，无关的 `COMP-0063` 分毫未动。 | 🐛 **踩坑三（操作纪律）**：做 A/B 归因时用 `git checkout master -- <file>` 还原对照组，会**静默丢弃该文件未提交的实现改动**。正确顺序是先提交（或 stash）再 A/B。 | ⚠️ **遗留（非本次引入，已 A/B 证伪）**：`PricingVersioningImportE2ETest` 3 个用例全部失败，报 `Cannot find zip signature within the first 4096 bytes`。根因是仓库里两个 Excel 夹具（`docs/table/核价测试数据/核价系统功能基础数据功能结构所需字段（6.0版） .xlsx`、`docs/table/报价测试数据/报价系统功能基础数据功能结构所需字段V3.xlsx`）**本身不是有效 zip**（首字节 `87 7d 1c`，正常 xlsx 应为 `PK`），且 git HEAD 里存的就是这样，工作区与主仓 md5 完全一致。A/B 实证：把 P22 还原成 master 版跑同一测试，**同样 3 失败**，与本次改动无关。需要单独立项修复夹具。

---

[2026-08-03] repair-0803 页签依赖建图改「列粒度」—— 消除 `component_subtotal` 引用输入列造成的**假环** - **已交付**（分支 `fix/repair-0803-false-tab-cycle`，commit `1052bd9c`） | 改 `quotation/service/CrossTabComponentOrder.java`（新增 `TabDep` + `buildComponentDeps`）、`quotation/service/CardSnapshotService.java`、`configure/service/ConfigureSnapshotService.java`；测试 `CrossTabComponentOrderTest`（+6 用例）、新增 `CrossTabDepsRealSnapshotReplayTest`（4 用例）+ replay 资源 `src/test/resources/repair-0803/qt-20260803-0052-quote-card.json`；`BACKLOG.md` 新增 [[BL-0101]] | **故障**：QT-20260803-0052（模板 施耐德BUG2 v1.3）报「卡片渲染失败: 页签公式存在循环引用: [56c8a517(COMP-0160 产品), 74c0cede(COMP-0157 物料)]」，整卡不可用。 | 🔑 **根因＝粒度不匹配，不是配置错**：`component_subtotal` 依赖此前按**页签**粒度建边——只要 A 的公式引用了 B 的任意一列小计，就判「B 必须先算」。但被引用列若是 `INPUT_NUMBER` 这类零依赖列，其值在 PASS1 之前就已由 baseRows/editRows/driver 定死，**与页签算序无关**。本例 `产品.税率` 正是 `INPUT_NUMBER`（手工输入），`产品.管理费` 才是 FORMULA。列级依赖其实是一条直线：`产品.税率(输入) → 物料.原材料成本 → 物料.材料成本/回收成本 → 产品.管理费`；折成页签粒度后成了 `产品⇄物料` 闭环 → `topoOrder` 误判 → 整卡渲染中止。**配置本身合法且业务合理**（原材料成本除以产品税率）。 | **判据来源**：`FormulaCalculator.collectFormulaFields:1393` 只对 `field_type=="FORMULA"` 的字段求公式，且 `formula_assignments`（结构是 `{字段完整下标: 公式名}`）只在 FORMULA 列内部生效 ⇒ **只有引用公式列或整页签合计时，计算次序才有意义**。 | **修法**：`buildComponentDeps` 统一建图 —— `cross_tab_ref` 全量建边；`component_subtotal` 仅在「被引用列是公式列（`FORMULA` / `*_FORMULA`）」「整页签合计（`is_tab_total` / `__amount_total__`）」「列名缺失」「目标 fields 不可读」「查无此列」时建边。**保守优先**：只有能确证非公式列才省略边，守住 QT-1743（管理费=0）的修复意图。`extractSubtotalRefs` 保留并委托新实现，行为逐位不变（零破坏）。 | ⚠️ **前后端口径不一致（本次发现，未修，登记 [[BL-0101]]）**：前端 `crossTabOrder.ts` 只有 `extractSourceRefs`（仅收 `cross_tab_ref`），**全工程没有 `extractSubtotalRefs`**；`CrossTabComponentOrder.java:66` 原注释「与前端 `extractSubtotalRefs` 对齐」是不实描述（本次已订正）。正因前端不收这类边，此单**编辑页看着正常、后端渲染即崩**。修前端时不可照搬页签粒度，否则前端也会开始报环。 | ✅ **验证（三层）**：①单测 `CrossTabComponentOrderTest` 13/13；②**真实快照 replay** —— 把出错那份 `quotation_view_structure.QUOTE_CARD`（9 页签）逐字导出做成测试资源，旧页签粒度建图**复现**假环、新列粒度建图 8 页签全排序且物料先于产品；③**端到端**：worktree 起临时 8099 连同一 dev 库，正常单 QT-20260802-0050 `refreshed=1` 零错误；故障单临时置 DRAFT 重算后 `refreshed=1`、错误文案归零、无 NaN/Infinity，验后状态已还原 CANCELLED。 | ⚠️ **重要：修复后该单多数值仍为 0，属数据问题不是修复问题**（勿误判为回归）——`其他费用` 页签实际类别是「外购件管理费/材料管理费」而公式 predicate 写的是 `类别='管理费'`，恒不匹配 ⇒ 管理费=0；物料 driverRow 的 `_产出类型/_损耗率/_材料净重/_组成数量` 全为 null ⇒ 银点/非银点分支不触发 ⇒ 材料成本=0。修复的效果是**从「整卡崩溃什么都看不到」变成「正常渲染出结构与已有值」**（加工费 14 / 组装加工费 12.08 / 税率 1.13 / 报价合计 14）。 | **影响面**：全库扫描 ACTIVE 组件同型双向引用共 2 对——`COMP-0157⇄COMP-0160`（已爆，施耐德BUG2 v1.0~v1.3 四个版本都含）、`COMP-0032⇄COMP-0135`（完全同构，当前无模板引用，属潜伏；修复后即使建边也不再成环）。 | 🐛 **踩坑一（javadoc）**：注释里写 `INPUT_*/BASIC_DATA` 会因 `*/` **提前闭合注释块**，报一堆「illegal character: '（'」全角括号错——中文注释里列举字段类型时别用 `*/` 相邻写法。 | 🐛 **踩坑二（worktree 起后端）**：新建 worktree 起 Quarkus 必撞 `FlywayValidateException: Detected applied migration not resolved locally: 368/369`（并发会话的 V368~V371 已应用到共享库但文件仍 untracked）。**不要跑 `flyway repair`**（会破坏共享历史），验证用途加 `-Dquarkus.flyway.validate-on-migrate=false -Dquarkus.flyway.migrate-at-start=false` 即可。同 [[BL-0095]]。 | ✅ **A/B 归因（全量测试 1893 项：1 Failure + 5 Errors）**：切到干净基线 `26310aa1` 跑同样三个测试类，失败**逐条一致**（`QuotePendingScopeOpenWhitelistTest` 白名单恒红、`SessionLifecycleTest` Quarkus 起不来、`DataLoaderTest` ×4 `sqlViewExecutor` 未注入）⇒ 全为既有基线问题，非本次引入，正是 [[BL-0094]]/[[BL-0095]] 描述的「后端回归网基本失效」。**判定回归必须自己在基线跑 A/B，不能凭直觉认领或撇清。**

[2026-08-03] 报价渲染/物化(repair-0803-snapshot) 行数据即快照 —— 默认值只在「键不存在」时烘一次，用户清空可持久化 - **已实现待验收**（分支 `fix/repair-0803-snapshot-authority`，7 提交） | 改 `quotation/service/FormulaCalculator.java`(resolveRowByFieldName INPUT 分支) + `configure/service/ConfigureSnapshotService.java`(新增 static `overlayExistingInputKeys` + `loadRowDataByComp` + 接线 `computeRowDataFromSnap`)；前端新增 `pages/quotation/keyPresenceAuthority.ts`(唯一判据模块) + 改 `QuotationStep2.tsx`(bake effect) / `QuotationWizard.tsx`(§1.5/§1.6 回填 + `hasUserInput`) / `components/ComponentCell.tsx`(只读分支)；测试 `InputKeyPresenceAuthorityTest`(10) + `OverlayExistingInputKeysTest`(6) + `keyPresenceAuthority.test.ts`(12)；spec `docs/superpowers/specs/2026-08-03-row-data-snapshot-authority-design.md`，计划 `docs/superpowers/plans/2026-08-03-row-data-snapshot-authority.md` | **故障**：用户清空报价单卡片里的数字输入格（如「元素单价」「损耗率」），保存草稿后重新打开，**数字又回来了**。 | 🔑 **根因＝`''`(用户清空) 与「键不存在」(从未填过) 在 6 处判空口径里被折叠成同一个「空」**：①前端 bake effect(`QuotationStep2:1870`) 判 `cur===''` 即空 → 重新烘 `default_source`；②保存回填 §1.6(`QuotationWizard:1083`) 值为空即填回 `content`（**在点保存那一刻**就发作，比重开更早）；③§1.5 `FIXED_VALUE` 同型；④后端 `resolveRowByFieldName:1281` `if(nonEmpty(v)) out.put(...)` **空值不写键** → 库里「用户清空」与「从来没这一列」物理同形（**数据层病根**）；⑤`hasUserInput`(`QuotationWizard:519`) 要求「有非空值」→ 整行只剩空值时退回 enriched 默认行；⑥`ComponentCell:596` 只读态值空回退显示 default_source → 编辑页空白而详情页有数字（AP-50 双端不一致）。**关键放大器**：bake 守卫 `bakedRef` 是 `useRef`，**只活在当前挂载的卡片实例里**，刷新即清零 —— 所以同一会话内清空不回弹（守卫已置位），**一旦重开必被烘回**。 | **不变式（本次确立）**：`row_data[row][fieldName]` **键存在 = 已定值，任何路径不得写入；键不存在 = 从未定值，仅此时允许烘一次默认值**。清空 = 写空串 `""`，**不是删键**（`null` 按键缺失处理，不引入第三种空语义 —— 后端 `mergeRowDataInputsIntoEdits` 的 `!v.isNull()` 会跳过 null，落 null 等于键丢了）。 | ✅ **先例佐证，非新规则**：该口径在 `FormulaCalculator.fillInputDefaultSourceByFieldName:1866` 注释原文「键存在(driver/手填/显式清空'')均不补…仅"键缺失"才补 default_source」已正确实现，前端 `currentRowForEval` 同款。本次是把其余 6 处**对齐到既有先例**。更妙的是 `resolveRowByFieldName` 被删掉的旧注释写着「显式编辑(含清空'')优先且独占」——**原作者意图本就正确，只是最后一行 `if (nonEmpty(v))` 把自己的意图否定了**。 | **实测爆炸半径**：ACTIVE 组件共 **91 个 `INPUT_NUMBER`，其中 81 个配了 `default_source`、11 个配了静态 `content`** —— 几乎所有数字输入列都在这条路径上（组成数量/材料毛重/损耗率/加工费/费用/汇率/元素单价）。全库 `row_data` 仅 1 条记录含空串（`QT-20260731-0037` COMP-0090 第 0 行 `"组成含量(%)": ""`），它之所以能留住，正是因为该行 `{$mc_view._组成含量}` 是 null（源解析不出→烘不了）；同行 `{$mc_view.元素单价}=216770` 有值，故那一格清空必被烘回。**判据：清空能留住 ⟺ 该行 default_source 解析不出值。** | **决策**：①判据用「键存在」而非「行级 `__baked` 标记」（后者判空口径仍模糊且新元数据键要跟着行走遍所有物化/复制/快照路径）；②显式刷新「一律不动 INPUT 列」，只重取 driver/BASIC_DATA/DATA_SOURCE + 重算 FORMULA；③**不做数据迁移**，存量键缺失的格子下次打开自然烘一次并固化（代价：历史上被丢键的「已清空」会被填上一次，再清一次即真正留住）；④架构选「前端 bake 保留只改判据」而非「烘焙下沉后端」——后者语义更纯但手动新增行无后端物化链路需另补一套，风险不成比例。 | ⚠️ **Task 6 是本次最高风险处**：`lineNeedsExpand` 为真的行（某 driver 组件 snapshot_rows 缺失/空）会走 `computeRowDataFromSnap` **用空 editRows 重物化整行所有组件的 row_data**，把刚保存的手工输入连同「键存在」这一事实一起冲掉。修法＝落库前 `overlayExistingInputKeys` 按 **`row_index` 对齐**（不是数组下标 —— 行增删时下标会错位，row_index 匹配不上就退化为用新值，安全；AP-54 同族纪律）盖回 INPUT_* 键，BASIC_DATA/DATA_SOURCE/FORMULA 仍用新算值。 | 🚨 **验证证据的诚实边界（务必看）**：①**E2E 无绿灯** —— `quotation-flow.spec.ts` / `composite-product-flow.spec.ts` 夹具已随迁库失效（[[BL-0078]]），本次不跑、也不拿单测冒充；②**原定的值中性锚点全哑** —— `GoldenCardValuesEquivTest` 2 例、`RowDataWholeBatchEquivTest` 2 例、`PersistWholeBatchEquivTest` 2 例**全部 SKIPPED**（`Assumptions.assumeTrue` 守卫的硬编码夹具单在测试库无数据，同 [[BL-0021]]/[[BL-0078]] 夹具腐烂族），故改以自包含矩阵单测 `InputKeyPresenceAuthorityTest` 承担该职责；③**后端全量回归网当前基本不可用** —— 实测 1936 项 **158 Failures + 386 Errors**，几乎全是 `current transaction is aborted` 级联，唯一非级联真错误是 `ScheduledTasksTest.setupSeed` 的 `ARJUNA016051: thread is already associated with a transaction`。**注意这与本 RECORD 上一条记载的「同基线 1893 项 1F+5E」相差悬殊，说明共享测试库 `cpq_db` 在 2026-08-03 上午被搞坏了**（疑与多会话并发跑全量 + 该事务泄漏有关），非本次引入。 | ✅ **A/B 归因（按 RECORD 上一条立的规矩自己跑，不凭直觉撇清）**：切到基线 `af377b0b` 跑同一套 —— **Failures=158 / Errors=386 与含改动版逐一相等**，失败清单不含任何本次触碰的类，新增两个测试类 16/16 全过。⚠️ 但**测试总数不可比**（`mvn test` 不 clean，基线那次 `target/surefire-reports/` 里残留了我两个新类的旧报告，聚合口径被污染）——判定只看 Failures/Errors。 | 🐛 **踩坑一（worktree 起后端/跑测试）**：Flyway 必因「库有 V368~V371、本地无」validate 失败（并发会话 task-0729 的迁移已应用到共享库但文件仍 untracked）。本次做法是把那 4 个文件**复制进 worktree（保持 untracked、绝不 git add、收尾删除）**；RECORD 上一条给的另一解是加 `-Dquarkus.flyway.validate-on-migrate=false`。**都不要跑 `flyway repair`**（会破坏共享历史）。 | 🐛 **踩坑二（计划编写）**：计划里给子代理的提交步骤若写 `git commit -- <新文件路径>` 会报 `pathspec did not match any file(s) known to git` —— 新文件**必须先 `git add`**。写计划时统一成「先 add 再 commit -- 同路径」。 | 🐛 **踩坑三（BL 号撞车）**：并发会话同日已占用 [[BL-0101]]（前后端 `extractSubtotalRefs` 口径不对齐），本次改用 [[BL-0102]]。**多会话并发时登记 BACKLOG 前必须先 grep 现有最大号**。 | **遗留**：[[BL-0102]] —— `PriceReconciler`（task-0729，未提交但已在共享 8081 热加载生效）对「元素 ∈ 调价清单」的行每次 saveDraft **无条件覆盖 `元素单价`**（解不出价直接 remove 该键），而 UI 上该列仍是可编辑输入框 → 与本不变式直接冲突，需产品裁定置灰或允许覆盖。实测 `CUST-0001 罗克韦尔` 已有 enabled 策略（08-03 07:23 建，scope=ALL，元素 Ag/Cu），8 个组件配了 `element_price_field`。
...
[2026-08-03] 公式引擎 / task-0803 - Task 9-A：前后端 BOM 父子取值求值引擎逐位比对（共享夹具，零分歧） | 新增 `dev-docs/task-260803-BOM页签增加父子取值公式/fixtures/tree-formula-parity-cases.json`（16 条用例，单一物理文件） + `cpq-backend/src/test/java/com/cpq/quotation/service/TreeFormulaParityFixtureTest.java` + `cpq-frontend/src/pages/quotation/treeFormulaParityFixture.test.ts` | **背景**：Task 7 工程师此前只做了「旁证」——读后端实现逐行镜像 + 前后端各自独立设计出同一棵测试树/同一组断言值，从未真正拿同一份输入喂两套独立实现对比。既有的 `TreeFormulaEvalTest`（后端）/ `treeFormula.test.ts`（前端）也只单测各自的低层 `evaluateExpression` 手搭 `TreeEvalContext`，绕开了 `calculate()`/`computeTabFormulasTree()` 的整页签单元格拓扑排序 + 环检测 + effKey 产出流程。本任务补的是这道「真正对比」的缺口。 | **落点决策**：没有沿用 `cross-tab-cases.json` 的「两份手动同步拷贝」模式（那种模式已在自己的 javadoc 里承认"必须手动同步，divergence 需暴露不能隐藏"，即隐含会漂移的风险）；改为把夹具放在仓库根下 `dev-docs/task-0803.../fixtures/`（不属于任何一侧 src 树，避免暗示归属），两端测试各自向上遍历目录找到同时含 `cpq-backend`/`cpq-frontend` 的仓库根后再定位同一个物理文件读取（后端 `Files.readString`，前端 `fs.readFileSync` + `import.meta.url`），真正做到「改一处、两端同步生效」。 | **覆盖**：AC-1~AC-11、AC-14、AC-15、判据5/判据6、G5（悬空父引用）、G6（单节点树）；AC-12（墓碑删除行）以「行从数组中省略」建模而非真实走 tombstone 参数——因为两端引擎在各自真实调用契约里都是「墓碑行在到达这层之前已被上游过滤/标记死亡效果等价于行不存在」，`TreeRelations.of` 对 `parentId` 指向不在 `byNodeId` 里的节点与「该父节点已墓碑」处理路径完全相同（均落到 `parent[i]=-1`），故此建模对求值层是充分且诚实的。 | **结果**：两端各 16/16 全绿，用例数逐位相同，**未发现任何数值分歧**——BigDecimal(后端)/JS number(前端) 的已知精度差异在本次全整数/一个精确二分之一小数（13/2=6.5）的取值范围内未被触发。 | **自检**：`./mvnw -q -o test -Dtest=TreeFormulaParityFixtureTest` 16 passed；`npx vitest run src/pages/quotation/treeFormulaParityFixture.test.ts` 16 passed；`npx tsc --noEmit` 0 错误；旧测试 `TreeFormulaEvalTest`(11)/`treeFormula.test.ts`(18)/`FormulaCalculatorCrossTabFixtureTest`(46) 均未受影响。

---

[2026-08-03] 测试 / task-0803 - Task 9-B：端到端验证（五道闸真实 HTTP + 落库 token + 性能实测） | 新增 `cpq-backend/src/test/java/com/cpq/perf/BomTreeFormulaPerfTest.java`（纯 JUnit，无 CDI，直调 `FormulaCalculator.calculate`）；无其余代码改动 | **环境**：worktree 临时后端 8099（连共享 `cpq_db_0724`），cookie 会话鉴权（`admin`/`Admin@2026`），测试组件全部 `T0803TEST_` 前缀，收尾已清理（`component`/`component_directory` 表 `T0803TEST%` 残留计数均为 **0**，`COMP-0157`/`COMP-0032`/`COMP-0048` 三个共享组件 `updated_at` 核对未被本次触碰）。 | **①报价侧配置期验证**：`POST /api/cpq/components` 建 BOM 测试组件（`子件合计=CSUM([单价])` + `父单价=PGET([单价])`）→ **200**，返回体与库内 `formulas` 逐字段核对均为 `{"type":"tree_ref","dir":...,"agg":...,"targetExpr":[...]}` 结构（非文本，证明五道闸吃的是 token JSON 而非字符串）；`PUT` 原样回灌（round-trip）后 `GET` 比对 `id`/`formula_id`/`expression` **逐位不变**（仅 `updatedAt` 变化）——即"保存→重新 GET→往返无损"成立；**未做**真实单据渲染取值端到端（AC-27/28 完整链路）——现有测试组件无 `data_driver_path`/无真实 driver 行，要接出真实树形行集须另建 SQL 视图/模板/报价单，超出本次 curl 验证范畴，详见下方"未验用例"。 | **②五道闸真实 HTTP 结果**（均与 `ComponentServiceTreeTokenGateTest` 单测断言逐句吻合，证明单测覆盖与真实接口行为一致）：非 BOM(`零件`)+`CSUM` → 400「…该功能仅支持 tabType="BOM"…」；非 BOM+`tree_attr`([层级]) → 400 同一口径（验证 F-2 评审订正落地：树属性与函数一视同仁）；BOM+`previous_row_subtotal` → 400「…BOM 树页签禁止使用该 token…」；`CSUM` 内层嵌 `cross_tab_ref` → 400「…tree_ref.targetExpr 内出现不允许的 token 类型「cross_tab_ref」…」（闸①，与 tabType 无关，BOM 组件同样命中）；BOM+合法父子公式 → 200。闸③（反向）：已配父子公式的 BOM 组件试图改 `tabType='零件'` → 400「请先删除公式中的父子取值引用」且**事务回滚**（GET 复核 `tabType` 仍为 `BOM`）；反例 TC-GATE-06——无父子公式的 BOM 组件同样改法 → **200**（证明闸③精确命中"已配置"这个条件，非无差别拦截）。闸④（导入）：`POST .../component-directories/{id}/import/commit` 送一个 2 组件 bundle（1 合法 BOM + 1 非法非 BOM，均含 `tree_ref`）→ **400**，DB 核对两个 code 均 **0 行**（G4"整包回滚"裁决——合法的那个也未落库，非"部分成功"）。 | **③性能实测（AC-29，供 G7 定阈参考）**：现网实查 `cpq_db_0724` 全部 BOM 组件 `snapshot_rows` 最大仅 **7 行**（`COMP-0026`），`COMP-0157` 实际使用中只 6 行——需求点名的"最大 BOM 树"现网规模远小于测试分档。分档矩阵（root+branches+leaves 三层合成树，CSUM/PGET 交替列，7 次采样取 min/median/max，warmup 3 次）：10 行×3 列 0ms，50×3 0ms，200×3 0~3ms，10×8 0ms，50×8 0~1ms，200×8(1600 cell) 0~6ms；延展探测 1000×8(8000 cell) 7~24ms，5000×8(40000 cell) 23~79ms。**趋势近线性**（cell 数越大越稳定，小规模下 JIT/GC 抖动占比更明显）。**建议阈值**：现网规模（≤200 行×8 列）下页签级求值几乎不可测量（<5ms），**建议 P1 阈值定在 500ms/5000 cell**（约等于本次 5000 行×8 列延展档的 15~20 倍安全余量），业务侧若认可可直接采纳，需求 §11.5 G7 仍待正式拍板。 | **踩坑（继承自 BL-0095 已知模式，非新问题）**：worktree 起 8099 必撞 `FlywayValidateException`（并发主工作区未提交的 V368~V371/V373~V374 已应用到共享库但本 worktree 无文件），沿用既有做法临时 copy 6 个文件进 `db/migration/` 跑完即删（未 `git add`，收尾 `git status` 确认已还原干净）；副作用是这批文件被复制期间顺带让 `@QuarkusTest`（`RefreshQuoteCardValuesTreeResilienceTest` 等 194 个 tree 相关用例）在共享 **test** 库 `cpq_db` 上把 Flyway 推进到 V376（此前卡在 372，`@QuarkusTest` 全起不来，同 BL-0095/BL-0098 记录）——**这是被动的正向副作用**（test 库 schema 向前追平），非本次刻意所为，且是 DDL-only、非破坏性迁移，194 个 tree 测试全绿（0 failure/error）验证了这一批 migration 应用后无回归。 | **未验用例（如实说明，非凑数）**：TC-E2E-01/02/03（真实报价单/核价单渲染 + `row_data` 落库 + 三视图一致）、TC-REL-03/04（`cross_tab_ref` 共存 + Excel 列模型降级）、闸⑤（`__nodeId` 缺失兜底，属纯计算层行为非 REST 层，Task 9-A 共享夹具 + 既有 `RefreshQuoteCardValuesTreeResilienceTest` 已间接覆盖树组件求值异常不 500 的路径，但未专项构造"缺 `__nodeId` 行"用例）——原因：这些均需要真实 `data_driver_path`/模板/报价单全链路搭建，超出"直接建测试组件 + curl 打接口"的验证边界，需求文档 §5 也明确"无试算预览端点"，本次按任务书授权的"退而验证保存→GET 往返无损"降级处理，已在此记录降级理由供后续如需补测参考。

---

[2026-08-03] 公式引擎 / 组件管理 - **task-0803 BOM 页签父子取值公式（交付）** | `formula/TreeRelations.java`(新) / `formula/CellGraph.java`(新) / `FormulaCalculator.java` / `ComponentService.java` / `ComponentImportService.java` / `TokenMappabilityValidator.java` / `TabJoinPlanEvaluator.java` / `FormulaCalculationService.java` / 前端 `formulaSerialize.ts` / `TabJoinFormulaDrawer.tsx` / `QuotationStep2.tsx` / `ReadonlyProductCard.tsx` / `QuotationWizard.tsx` / `bomTreeRelations.ts`(新) / `cellGraph.ts`(新) |

**交付**：`tabType='BOM'` 树页签新增父子取值原语 —— `PGET`（子取父）、`CSUM/CAVG/CMAX/CMIN/CCOUNT`（父取**直接**子）、树属性 `[层级]/[是否叶子]/[是否根]`。让成本 rollup、用量累乘这类 BOM 算法由用户用公式配出来，兑现 2026-06-05「后台只给原语、算法归业务公式」的既定方针。**无 DB 迁移**（token 存 JSONB）。

**核心架构决定**：求值单元从「行」下沉到 **「单元格 (nodeId × 列名)」** —— `PGET` 要父先算（自上而下）、`C*` 要子先算（自下而上），方向相反，原「逐行 × 行内列拓扑」单遍不可能同时满足。建图后 Kahn 拓扑求值，环上及其下游置 0、环外照常。**非 BOM 页签走原路径，`computeAllFormulas` 函数体 306 行逐字未动**（零回归门禁，已逐字 diff 自证）。

**🚨 三条写代码前必看的坑**：
① **行内依赖边必须用精确列依赖，不能「按列拓扑序串成链」**。链边会在双向混用时造**假环**：反例 `R.y=CSUM(C.x)` + `C.w=PGET(R.z)`，若列序上 y 早于 z、w 早于 x，链边把它们连成环而精确图不会。后端用 `buildFormulaDeps`、前端用 `getFormulaDeps`+`condTreeColumns`。
② **父子边按 `__nodeId`/`__parentId` 认，禁止按料号**。BOM 是 DAG，同料号挂多父（现网 `3110520789` 同挂两个父件），按料号认边必然串号把两棵子树混成一坨。
③ **校验闸的 token 扫描必须递归**。首版平铺遍历顶层 token，导致 `tree_ref` 嵌进 `cross_tab_ref.targetExpr` 可绕过闸②（评审退回修复）。递归 walker 的下钻判据是「每个元素都带 `type` 键的 `List<Map>`」——`match`/`sources` 天然不被误钻。

**「有值」判据（§4.3.4，最容易做错）**：判据 4「**数值 0 是有值**」在前端是经典 JS falsy 陷阱（`if (!v)` 会把 0 判成空 → `CMIN` 算错），实现用布尔存在标志 + `=== true` 规避；判据 6「**公式列恒有值**」是用户明示接受的取舍 —— 「空值不参与聚合」只对取数/输入列生效，聚合公式列时 spine 补位空行仍参与。

**⚠️ 需求文档曾把配置 UI 指错入口（技术总监自纠）**：`FormulaBuilder.tsx`/`CrossTabRefDrawer.tsx` 是**孤儿组件**（生产零引用），真实入口是 `TabJoinFormulaDrawer.tsx` —— **文本表达式编辑器**（用户编辑字符串 → `formulaSerialize.ts` 解析成 token）。父子取值最终作为该文本语言里的函数接入，照 KSUM 先例。**教训：不要靠文件名推断入口，先 grep 谁引用它。** 详见 [[BL-0106]]。

**验证**：后端 **343 个纯 JUnit 零失败**；前端 **1004 测试通过**（A/B 确认 63 个文件收集失败在 master 上一模一样，既有非本次引入），tsc 0 错误。**前后端共享夹具 16 条逐位比对零分歧**（`dev-docs/task-0803-.../fixtures/tree-formula-parity-cases.json`，单一物理文件两端同读；经**变异测试**证明有效——注入 JS falsy 陷阱后 AC-6 立刻炸、其余 15 条不动）。五道闸真实 HTTP 全部 400/200 符合预期，含闸③反例证明精确命中。性能：200行×8列(1600 cell) ≤1ms，5000行×8列(40000 cell) 23~79ms；现网最大 BOM 树实查仅 7 行。
...
[2026-08-03] task-0729 repair-0803 续集（BL-0109）—— 预算/dryRun 路径同根因 ContextNotActiveException，两层嵌套 REQUIRES_NEW 各自补 @ActivateRequestContext | `cpq-backend/src/main/java/com/cpq/priceadjust/service/PriceAdjustBudgetService.java`（`processMaterial`/`runDryRunSnapshot` 各补一个 `@ActivateRequestContext`）、`dev-docs/task-0729-.../repair-260803-报价单删除阻塞外键/{需求文档.md,test-report.md}`（续集章节追加）、`BACKLOG.md`（新登记 BL-0109） | 同分支 `fix/repair-0803-quotation-delete-fk-block` 延续（未新开），BL-0108 ①②合入 master 后测试立即撞到。
...
[2026-08-04] 公式引擎 / task-0803 - **修复：C* 族聚合取数列(BASIC_DATA)静默返 0**（核价侧端到端验收时发现，已合 master merge fix/repair-0803-tree-agg-basicdata） | `FormulaCalculator.java`（新增 `buildTreeAggPresenceView`）/ `TreeFormulaParityFixtureTest.java` / 前端 `QuotationStep2.tsx`（`resolveRowForTree` 的 partNo 守卫收窄）/ `treeFormulaParityFixture.test.ts` / 共享夹具补第 18 条 | **发现方式**：验收 AC-28（核价侧与报价侧公式能力一致）时，在真实核价单（COMP-0048「物料BOM」，17 行 4 层 spine 树、根为补位空节点）上配 10 个父子公式实测。`PGET`/`层级`/`是否叶子`/`是否根` 全对，但 `CSUM/CAVG/CMAX/CMIN/CCOUNT` 作用在 `[组成用量]`（BASIC_DATA）上**全部返 0**。 | 🔑 **定位靠的是一列对照**：同一行、同一棵树、同一个 COUNT —— `CCOUNT([父用量])`（聚合 **FORMULA** 列）= 8 正确，`CCOUNT([组成用量])`（聚合 **BASIC_DATA** 列）= 0 错误；而 `PGET([组成用量])` = 0.36 正确，证明该列本身可解析、问题出在聚合的「有值」过滤上。**这列对照是我为验判据 6 顺手加的，若只配常规几列会误判成「核价侧整体不支持父子公式」**。 | **根因**：`hasValueForAgg` 读 `ctx.currentRowRaw`，其键是 **driver 视图列名**（`composition_qty`）；`buildRowEvalCtx` 之后调的 `fillInputDefaultSourceByFieldName:2352` 只按字段名回填 `INPUT_NUMBER/INPUT_TEXT/INPUT` **三种类型**，`BASIC_DATA`/`DATA_SOURCE` 字段按字段名（`组成用量`）永远取不到 → `isBlank` 恒真 → `evalTreeRef` 的 CHILD 分支把子行全过滤 → 聚合返 0。BOM 页签的列绝大多数是取数列，等于 C* 族在其主用途（成本 rollup）上不可用。 | **修法**：新增 `buildTreeAggPresenceView` —— 以 `currentRowRaw` 为底（保留 driver 列名直读 + 显式清空 `""` 语义），叠加 `resolveRowByFieldName` 的按字段名解析（覆盖全字段类型，仅 `nonEmpty` 时落键 = 键缺失即无值，与 `isBlank` 同义）。`TreeEvalContext.resolvedRaw` 目前**仅** `hasValueForAgg` 一个消费点，改动不外溢。FORMULA 列走 `formulaColumns` 短路不受影响。 | **前端同族隐患一并修**：`resolveRowForTree` 的 BASIC_DATA 分支整体挡在 `&& partNo` 之后，但 `basicDataValues` 是行级已解析值、取它不需要 partNo，只有 pathCache 兜底才需要；**后端无 partNo 概念、直接读 basicDataValues**，故旧写法是一处两端可分歧点。已把 partNo 要求收窄到 pathCache 兜底那一步。 | 🚨 **夹具盲区（教训）**：原 16 条用例的 `rows.values` 直接落进 `driverRow` 且键就是字段名，两端都能平凡解析，**从未覆盖「driver 列名 ≠ 字段名」这一真实形态** —— 所以"零分歧"是在一个过窄的输入空间里得出的。两端 harness 已加 `basicDataValues` 支持，补第 18 条回归用例。**按 TDD 先跑红**：后端红、前端在无 partNo 下同红，证明用例有鉴别力，再修复 → 两端 18/18。 | **另一处教训**：早前报价侧端到端"全部通过"没有错，但覆盖有洞 —— 测试组件 COMP-0089 的 `组成数量` 恰好是 `INPUT_NUMBER`，正落在 `fillInputDefaultSourceByFieldName` 覆盖的三种类型里。**选 e2e 载体时必须让被聚合列覆盖 BASIC_DATA，否则这类缺陷照样漏**。 | **验证**：真实核价单 17 行 × 10 列全部命中"修复前就写死"的预期（含补位空节点作父 → `PGET`=0、`CCOUNT(公式列)`=8 判据 6、非整数父值 0.36）；后端公式相关 334 测试 0 失败（1 个 `@QuarkusTest` 环境性错误 = BL-0095 worktree 缺 V377，非本次引入）；前端 tsc 0 错误 + quotation 目录 43 文件 378 测试全绿。共享组件 COMP-0048/COMP-0089 已还原，全库 `tree_ref/tree_attr` 公式 0、模板快照残留 0。

---

[2026-08-04] repair-0803 公式 `SUM` 内引用宿主页签字段取值修复 + 环链路可视化 - **已交付合 master（merge `b80b85b2`）** | 改 `quotation/service/FormulaCalculator.java`（取值链 + 依赖递归 + 结构化环）、`CrossTabComponentOrder.java`（环路径提取 + 文案去 UUID）、`CardSnapshotService.java`、`ConfigureSnapshotService.java`、`component/service/ComponentService.java`、`template/service/TemplateService.java`，新建 `common/exception/FormulaCycleException.java`；前端 `utils/formulaEngine.ts`、`pages/quotation/QuotationStep2.tsx`、新建 `pages/component/FormulaCycleDrawer.tsx`、`ComponentManagement.tsx`、`TemplateConfiguration.tsx`、`CrossTabRefDrawer.tsx`；文档 `dev-docs/repair-260803-公式SUM内引用宿主页签字段/`（需求文档/api/backtask/fronttask/test/原型 六件套） | 🔑 **根因（两层，前后端同构）**：①`b_field` token 读 `ctx.currentRowRaw`，而 FORMULA 字段结果**只回填 `fieldValues` 从不写回 `currentRowRaw`**（`computeRows` 尾部），`fillInputDefaultSourceByFieldName` 又只补 `INPUT_*` 类型 ⇒ `SUM(...)` 的 `targetExpr` 内引用本页签**公式列**恒取 0（静默少算，不报错不提示）；②`addExprFieldDeps` / 前端 `getFormulaDeps` **只扫顶层、只认 `type=="field"`**，不递归 `targetExpr`、不认 `b_field` ⇒ 即使值取得到，字段级拓扑序也不知道要先算被引用字段。**两层必须一起修**，只修①会得到"时对时错"的结果，比恒 0 更难查。 | 🔑 **语义裁决（D-1）**：定为**逐行广播**（`SUM(aᵢ+D)=Σaᵢ+N·D`）。这**不是新语义**——实证 `b_field` 指向 `INPUT_NUMBER` 时本就逐行广播、`targetExpr` 内嵌套 `cross_tab_ref` 也是（N=2 实测 20.0=Σaᵢ+2×5），FORMULA 取 0 才是异类，修复=让它归队。 | ⚠️ **两个实现期订正（原方案是错的）**：①**不能回落 `fieldValues`** —— 在 `targetRowValue` 子上下文里 `fieldValues` 装的是**被聚合源页签当前行的列**（从 `arow` 灌入），宿主已算字段根本不在里面，直接回落会让 `b_field` 串到源页签同名列上。故新增 `RowContext.hostFieldValues` 专用通道（顶层与 `fieldValues` 同引用故能取到刚算出的值，`targetRowValue` 原样透传），前端加等价的第 15 参。②**`targetExpr` 内的 `field` 一律不收依赖** —— 它指源页签列，与宿主同名纯属巧合，收了会凭空建边甚至误报环；规则实为「任意层级 `b_field` 收 + 仅顶层 `field` 收」，遇 `projectToHostKey`（KSUM 子 token）停止下探。 | ✅ **一处改动三处生效**：`topoOrder` / `cyclicFormulaNodes` / `describeFormulaCycles` **共用 `buildFormulaDeps`**，故只改 `addExprFieldDeps` 一处，求值排序与环检测自动同口径，不存在"排序认这条边、检测不认"的分裂。实现时**不得**为二者各写一份收集逻辑。 | **环链路可视化**：`FormulaCycleException` 照 `RowKeyConflictException` 既有形态（`GlobalExceptionMapper` 加一个 `instanceof` 分支走 `ApiResponse.error(code,msg,data)`），**`BusinessException` 基类不动、响应体格式零改动**；`message` **保持原多行定位文案逐字不变**（被 `ComponentServiceConditionalValidationTest.cycleMessage_containsLocation` 锁死，也是"不消费 data 的路径仍可只读 message"的落地），`data.{errorType,cycles}` 纯新增；前端只认 `errorType` 判定、**禁止 message 文本匹配**。删除 `ComponentService.dfsCycleDetect`（零定位信息，能力被完全覆盖，留着会让同一个环报出两种文案）。**页签级环的发布期校验改用 `buildComponentDeps` 与渲染期同口径** —— 旧实现只收 `cross_tab_ref` 边，这正是「模板发布通过、一渲染就报环」的成因。 | ✅ **验收证据**：全量 A/B —— 纯 master 基线 2167 项 `159F+393E`，本分支 2199 项 **`159F+393E` 逐条一致**，多出的 32 项全绿（零回归）；`AC-2 前后端对拍`补进既有共享夹具 `cross-tab-cases.json`（前后端各一份必须字节一致），新增 3 例锁死取值链三分支（回落 20 / 不回落 16 / 置空 10），后端 49/49 + 前端 109/109 同结果；`AC-6` 连真库扫 87 组件断言受影响集合恰好 4 条公式并逐条核对引用字段名；4 张受影响 DRAFT 单真实重算后无 `__cardValueFailed`、无 `NaN`、卡片正常。 | ⚠️ **AC-8 金额未变化的正解**（勿误判为"修复没生效"）：那 4 张单的 `来料加工费`/`来料损耗率` **字段自身就算出 0**（driverRow 的 `_产出类型`/`_损耗率` 全为 null，其取值公式的 predicate 不匹配），取值链已正确回落，只是回落到的值本身是 0。修复生效的证据在单测/对拍/replay，不在这批数据。 | 🐛 **踩坑**：①javadoc 里写 `INPUT_*/BASIC_DATA` 会因 `*/` **提前闭合注释块**，报一串"illegal character 全角括号"；②`cyclePathIn` 返回的是**显式闭合**形态 `[A,B,A]`（`dfsBackTo` 末尾 `add(target)`），直接拿来做结构化 `nodes` 会首尾重复且多绕一条自环边（测试工程师 AC-10 抓出）；③worktree 里 `@QuarkusTest` 撞 `FlywayValidateException`（缺主仓未提交的 V368+ 迁移），单跑加 `-Dquarkus.flyway.validate-on-migrate=false -Dquarkus.flyway.migrate-at-start=false` 可绕过，但**跑全量时加这两个参数反而会让几百个测试失败**，合并到主仓后自愈。 | **范围裁决（D-4，用户 2026-08-04）**：**引擎只按公式忠实计算，不改任何配置** —— 公式写在 SUM 内就逐行计入、写在外就整单计入一次，选哪种是配置者的业务决定。原方案把「存量 8 条公式逐条确认意图并改写」纳入交付属越界，已收回；`§5.4` 清单改为**告知性材料**（含"损耗率在乘法位置两种写法等价、加工费在加法位置不等价"的参考说明）。 | **遗留**：E2E `quotation-flow` 3 failed 属既有基线（夹具单缺产品分类致 Step1「下一步」禁用，见记忆 `task0712-update071501-category-axis`），失败点在 Step1 **未触及本次改动**；AC-9/AC-12/AC-15 前端 UI 无自动化，需人工验；新登记 BL-0110/BL-0111。

---

[2026-08-04] 前端构建 - **修复：`npm run build` 因测试文件缺 node 类型报 7 个 TS 错误** | 涉及文件：`cpq-frontend/tsconfig.json`（加 test 项目引用）/ `cpq-frontend/tsconfig.app.json`（exclude 测试文件）/ `cpq-frontend/tsconfig.test.json`（新建） | **根因**：`tsconfig.app.json` 的 `include: ["src"]` 把 src 下 76 个测试文件一并纳入 `tsc -b`，而它是浏览器端项目、`types` 只有 `vite/client`；task-0729 的 `formulaGolden.test.ts` 与 task-0803 的 `treeFormulaParityFixture.test.ts` 需要 `node:fs/path/url` 读仓库根 `formula-golden/*.json` 黄金用例 → 6 个 TS2591 +1 个连带 `f: any`（`readdirSync` 返 any）。**`npm run test` 一直是绿的**，因为 vitest 只转译不做类型检查，故障只在 build 暴露。 | **修法取舍**：没有给 `tsconfig.app.json` 的 `types` 直接加 `"node"`（一行即可修好），因为那会让应用源码误用 `process`/`Buffer`/`fs` 时 TS 不再报错、故障推迟到浏览器运行时；也没有简单 exclude 测试文件了事（等于 76 个测试文件永久失去类型检查，vitest 也不补这一层）。改为新增 `tsconfig.test.json` 项目引用：测试文件从 app 项目移出、由 test 项目（`types: ["node","vite/client"]`）检查，二者都在 `tsc -b` 覆盖内。 | **验证（双向探针，非仅看构建变绿）**：①临时探针 `src/__typecheck_probe__.test.ts` 写入 `const bad: number = "字符串"` → 报 TS2322，证明测试文件**没有**被静默跳过检查，同文件的 `node:fs` 导入无报错证明 node 类型确已生效；②临时探针 `src/__app_probe__.ts`（非测试）导入 `node:fs` → 仍报 TS2591，证明应用侧的浏览器语义保护面完整。两探针均已删除，`tsc -b` 干净退出。 | **值中性**：`vite build` 产物 hash 修复前后一致（`index-D4I4d2yg.js`）。 | ⚠️ **顺带发现（既有，非本次引入，未修）**：`npm run test` 有 2 条真实失败 —— `formulaGolden.test.ts` 的 `amt-002`/`amt-003`。**已做背靠背 A/B**（stash 掉本次 tsconfig 改动后跑，同样 `2 failed | 1055 passed`、同样这两条），与本次无关。根因同一个：`04-amount-total.json` 的 `amt-002` 用 `tab_name: "投料"` 作 `component_subtotal` token 的键（`expectedSource=manual-computed` 期望 12.34），前端引擎只认 `component_code`（`amt-001` 同数据走 `component_code` 通过），取值返 0；`amt-003` 是它的连带（88.88+12.34 期望 101.22，实得 88.88）。**注意不是** `formulaGolden.test.ts` 文件头注释里记的那批已知分歧（那批是 `10-decimal-precision.json` 的 `dec-001/002/004`）。按黄金用例机制的纪律「不要修复到消失，除非拿到明确裁定」，未擅动——待裁定的是：`component_subtotal` token 是否应支持 `tab_name` 取键（引擎补能力），还是该用例的假设本身不成立（改用例）。

---

[2026-08-04] 部署/建库脚本 - **内网库结构同步：建库脚本补全至 V378 + 产出等价增量脚本** | 涉及文件：`deploy/cpq-init-empty-navicat.sql`(+783/-28) / `deploy/0804-dbupdate.sql`(新建, 528 行) | **背景**：内网无 Flyway，靠 `cpq-init-empty-navicat.sql` 建库。该脚本上次同步停在 **V365**（2026-07-31），而开发库 `cpq_db_0724` 已跑到 **V378**，缺口 13 个迁移（V366~V378）。 | **范围裁决（用户 2026-08-04）**：`0804-dbupdate.sql` **只要表结构 DDL**，不含业务配置 DML（V366/V369/V370/V372~V376 对 `component.fields` / `component_sql_view.sql_template` / 公式稳定 id 的 UPDATE，以及 V372/V377 的 `DELETE FROM unit_price`）。内网库形态 = 用 init 建的、**已录入业务数据**。⚠️ 由此带来的已知后果已写进脚本头：结构就位但调价相关的组件/视图配置仍是旧的，内网若要启用 task-0729 调价功能需另行同步配置。 | 🔑 **唯一保留的 DML 例外**：V377 的 `UPDATE annual_discount SET discount_type='FINISHED' WHERE discount_type='INCOMING'`。它**不是**业务配置同步，而是新 CHECK 约束能建立的前提 —— 旧值 `'INCOMING'` 不在新枚举 `(INCOMING_MATERIAL/ASSEMBLY_PROCESS/FINISHED)` 内，不改则 `ADD CONSTRAINT` 必然失败。属等价改名不丢数据，已在脚本内就地注明理由。 | 🔑 **方法论：不手抄迁移，改用「真库 dump + 建库重放 + 多维 diff」**。13 个迁移叠加效果靠手推极易漏（且会漏掉迁移之外的漂移），故改为：① `pg_dump` 开发库拿权威 DDL；② 建临时库重放脚本；③ 与开发库做 **表/列定义/列相对顺序/索引/约束/函数/视图** 七维比对。两个交付物都用这套闭环验证，最终**全维零差异**。 | ⚠️ **踩坑 1（FK 拓扑序）**：新表间有 13 条外键。幂等形态把约束内联进 `CREATE TABLE IF NOT EXISTS`（为避开 `ADD CONSTRAINT` 不支持 IF NOT EXISTS），但内联 FK 要求**被引用表先建** —— 按字母序建表会让 `customer_price_adjust_element` 先于 `customer_price_adjust_strategy`，实测报 `relation does not exist`。已改为拓扑排序建表。init 侧不受影响（pg_dump 风格里 FK 是独立的 ALTER 区）。 | ⚠️ **踩坑 2（`ADD COLUMN` 落位）**：`quotation_line_item.row_version` 我按语义插在 `line_total_amount` 后，但 V368 走 `ALTER TABLE ADD COLUMN` **实际落在表末尾**（`deleted_tree_nodes` 之后）。加了 `ordinal_position` 维度才暴露。本库存在靠位置对齐的批量 upsert SQL（见反模式「位置性 NULL 占位」），列序必须逐位一致，已修正。**教训：比对列时不带 `ordinal_position` 会漏掉列顺序错位**。 | ✅ **无害差异 2 类（已证伪不是 bug，勿再"修")**：① `annual_discount` 的 `ordinal_position` 在开发库跳号（2,3,6,7…）—— V377 的两个 `DROP COLUMN` 留下 attnum 空洞而 PG 不重排，init 全新建表则连续；**列相对顺序完全一致**，`pg_dump` 输出逐字相同。② 12 处 CHECK 的两种打印形式（`(ARRAY[...])::text[]` vs `ARRAY[x::text,...]`）—— 源于 `CHECK (x IN (...))` 原生写法与 pg_dump 文本重建后 PG 规范化的差异；已用**行为对照实验**证伪（两库同插合法值均接受、同插非法值均被同名约束拒绝），且 `pg_dump` 归一化后该差异消失。 | 🚨 **baseline 必须 362 -> 378（本次唯一的"非加法"改动）**：V363~V365 那批当初可以"基线仍停 362"，因为它们幂等可重放；但 **V368 含 `CREATE TABLE`(无 IF NOT EXISTS)、V377 含 `RENAME COLUMN`，非幂等** —— 基线不上调则新库连 Quarkus 时会重放 V366~V378 并因「表已存在 / 列 biz_type 不存在」启动失败。已在文件两处（头部 + baseline 段）写明此处与前几次处理方式不同，防止后人照抄。 | **验证证据**：init 单跑建库 → 143 表 / 3 视图 / 5 函数 / baseline 378 / 参数种子 1 行，与开发库七维零差异，`pg_dump` 归一化后 38 行差异**全部**是函数体中文注释(14)+CHECK 打印形式(24)；`0804-dbupdate.sql` 在 git 原始 V365 版 init 造的基线库上跑通，产物与开发库**七维零差异含列绝对位置**（走 ALTER 路径故连 attnum 空洞都一致）。两个临时校验库 `cpq_tmp_initcheck_0804` / `cpq_tmp_upd_0804` 已删除。 | **幂等性**：增量脚本第 1~4、6 节可重复执行；第 5 节含 `RENAME COLUMN` **只能跑一次**（已在节首标注，重复执行报 `column "biz_type" does not exist` 属预期）。全文不用 `DO $$` 匿名块，幂等靠 `IF NOT EXISTS` / `DROP IF EXISTS` 实现，函数体是全文唯一美元引号且置于末尾 —— 与建库脚本同一套规避 Navicat 解析中断的思路。脚本头附「执行前自检」，其中**第 2 条是硬前置**：`annual_discount` 存量若在新 7 维唯一键下撞键，第 5 节建唯一索引会失败，需先人工清理。

---

[2026-08-05] 报价渲染 - **repair-0805：绑了 `formula_id` 的公式列整列空白（[[BL-0112]] P0，用户报障 QT-20260804-0068）** | `cpq-frontend/src/pages/quotation/{enrichComponentData,QuotationStep2,useCardSnapshots,ReadonlyProductCard,QuotationWizard}.ts(x)` + 4 个测试文件 + 真实单据夹具 | 合并 master（分支 `fix/repair-0805-formula-id-lost-in-enrich`，5 个提交）。文档 `dev-docs/repair-260803-BL0098-公式绑定改绑ID/repair-260805-渲染侧丢公式id致公式列全空/` | 🔑 **根因 = BL-0098「公式绑定改绑稳定 ID」只搬了钥匙没搬锁**：`resolveFormula` 改按 `field.formula_id` 查（且刻意不回落，设计正确），但渲染侧 `enrichComponentData` 两条组装路径的白名单 `.map()` **没搬公式自己的 `id`**（`ComponentFormula` 接口压根没声明 `id`，三处查找全写 `(x as any).id` 把类型缺失绕过去了）→ 查找恒不命中 → 字段整个不进计算列表 → `formulaCache[列]` 为 `undefined` → 单元格 `—`。后端完全正常（`formulaResults` 值全对），故障纯在前端。 | ⚠️ **触发条件不是「保存组件」**：`V375` 迁移一次性把 component/template 的字段翻译成 `formula_id`，但**刻意未迁 `quotation_view_structure`**（V375 注释 D3）→ 实测只有 V375 之后新建的单据冻结结构带 `formulaId`（12 张单里仅 1 张），存量单据走 `formula_name` 回退不受影响。**影响面 = 今后新建的每一张单**。 | 🔑 **第二根因（放大器）**：`useCardSnapshots.computeRowKey` 读驼峰 `f.defaultSource`，而渲染模型是蛇形 `f.default_source`（`enrichComponentData:286` 写的），调用点又 `as any` 抹掉类型差 → rowKey 退化成行号、与后端内容键永久对不上（`deletedRows.ts:56` 2026-07-14 已记载此分歧但当时绕开了）。它**单独无害**（查不到快照就走本地兜底），但**拆掉了根因 A 的安全网** —— 两个凑一起才炸。已按后端 `resolveRowByFieldName` 逐条镜像重写，含 `BASIC_DATA`/`basic_data_path` 通路（用户裁决扩围：只改大小写仅覆盖 179/221 处绑定，42 处/17 个核价组件修不到）。全库对拍 **497/497**（修复前 snake 形状 **1/497**）。 | 🚨 **交付验收抓到自引回归（最重要的一条）**：`QuotationWizard.snapshotRows`（保存 payload 构建器）调 `computeAllFormulas` 时 `crossTabRows` 传 `undefined`（纯 PASS1 口径，`QuotationStep2.tsx:1801` 早有警告「保存/折扣/导出一律用完整口径」）。修复前那些列**连 key 都没有** → `if (formulaCache[k] != null)` 为假 → 不写 → `row_data` 保住真值；修复后有 key 了 → `cross_tab_ref` 无 store 全算 0 → **把 0 写进 row_data**。即「**原来的 bug 一直在替另一个 bug 兜底**」。受控 A/B（同单/同操作/同后端）：master 保住 `608.7758112094`、分支清零、退掉阶段二仍清零 → 归因阶段一。已修（抽 `buildLineItemEvalContext` 在 lineItem 层算一次共享，不在组件层重复算）。 | ⚠️ **48 条单测全绿却没拦住这个回归** —— 用例全在纯函数层，**没有一条覆盖「保存 payload 构建」链路**，是靠人工受控 A/B 逮到的（[[BL-0119]] 已登记补测试债）。 | 📌 **自检口径修正**：`npx tsc --noEmit -p tsconfig.json` 是**空跑**（solution 文件 `files:[]`，恒返 0，不构成证据），前端 tsc 自检**一律用 `tsconfig.app.json`**。 | 📌 涟漪（已量化）：修复让 6 个组件的 10 个「名字绑定」cross_tab 列开始往 `row_data` 落真值（master 一直在写 0）→ 按列折扣会真正生效（[[BL-0059]] 的症状面），但实测全库 `discount_source` 全为 NULL、**当前受影响单据 0 张**，属前瞻性行为变更。 | 遗留登记 [[BL-0113]]~[[BL-0119]]
...
[2026-08-05] 价格调整策略（task-0729 验收期修复） - bug #47：元素∈清单但解不出价 → 值被删、锁还留着 = 死格 | 涉及文件：`cpq-backend/src/main/java/com/cpq/priceadjust/service/PriceReconciler.java`（`reconcileRows` 的 row_data 分支 + 类内 javadoc） |

**现象**：元素在调价清单里、但该元素在当期版本明细中从无价格（`current_price` 全程 NULL）时，Step2「元素单价」列渲染成 `— 🔒V26xxxx` —— 只读文本 + 锁徽标、无输入框、无值，既填不进去也拿不到系统价，且手动行外无绕开手段。违反验收 #47 / §11.3.2.1「从无历史价 → 视同不在清单、解锁可编辑」。

**根因**：`snapshot_rows`（driverRow）分支有 `if (ep == null || ep.price == null) continue;` guard，`row_data` 分支没有同款 guard —— 解不出价时走 else 把价格/货币键 `remove` 掉了，但紧随其后的 `dataRow.put("__priceLocked", true)` / `put("__priceVersion", ...)` 是**无条件执行**的。前端 `!!(driverRow?.__priceLocked ?? rawRow?.__priceLocked)`：driverRow 因 guard 生效没有标记 → `undefined ?? true` → `true` → `ComponentCell` 走只读分支。

**修法**：把两行 `put` 收进 `if (ep != null && ep.price != null)` 分支；else 分支删值的同时**一并删掉** `__priceLocked` / `__priceVersion`。**规范：清值与撤锁必须同进同出** —— 只删值不删锁 = 只读的空格 = 死格，正是 §11.4.1 改判前那条已被作废的口径。

**验证（真实数据，合成客户 `ZZ47-C` + 报价单 `ZZ47-Q1`，绑真实价格承载组件 COMP-0102，验完已全量清理零残留）**：一次归位覆盖 5 类行 —— `ZZ47A`(∈清单本期有价) → 1234.56/CNY/`__priceLocked=true`/`V26ZZ4701` ✅；`ZZ47B`(∈清单本期无价但 `inherited_from_previous=true`) → 800.00/CNY/锁保留 ✅（PASS 场景①不回归）；`ZZ47C`(∈清单从无历史价) → 价格键/货币键/`__priceLocked`/`__priceVersion` **四键全清** ✅（修复目标）；`ZZ47X`(∉清单) → `row_data` 逐字节未动 ✅；`snapshot_rows` 侧 C/X driverRow 逐字节未动、guard 未受影响 ✅。连跑 3 次 `row_data`+`snapshot_rows` md5 不变（归位幂等 #45 未破）。

**同族评估结论（未改，供决策）**：交付报告 §2.5 把「元素∉清单」推迟的理由是"元素被停用与用户改行在后端完全一样、要区分必须改数据协议"——**该前提与实现不符**：`customer_price_adjust_element` 无 enabled 列，§11.2.1 明令**元素停用保留在清单里**（验收 #51 实测计数仍为 1），停用根本不走「∉清单」分支。剩下两种成因（财务手动取消勾选 / 用户改行元素）期望行为**一致都是解锁**（验收 #58 逐字："确认后该元素出清单、对应单价列恢复可编辑"），故**无需区分、无需改数据协议**。
...
**遗留**（`dev-docs/repair-260803-BL0098-公式绑定改绑ID/repair-260805-行内公式列不随编辑重算/需求文档.md §5`）：S2 可见性保险（快照值与本地值不一致时 ⚠）、AC-3 自动化回归（改 INPUT → saveDraft → 重开）、AC-5/AC-6（比对视图 / 价格升版两条链路实测）、AC-7 三视图人工验收。另发现一个理论洞（**全库 0 条、未实际发生**）：`editCardValue` 在 `quote_card_values` 恰为 NULL 的窗口会写出「非空但空壳」的快照，`ensureCardValues` 的 `IS NULL` 判据此后再也选不中它。
...
**合并**：`24b68b20`（父任务 `task-0729`，目录 `dev-docs/task-260729-客户价格调整策略和价格版本/repair-260807-更正任务价格丢失与版本错乱/`）
...
## [2026-08-09] 模板管理 / 报价渲染 —— task-0806 模板发布全量冻结（P0，`BL-0133`）

**合并**：`8d04336a`（27 commits，37 文件 / +5058 行）｜**文档**：`dev-docs/task-260806-模板发布全量冻结/`｜**反模式**：`AP-63`

### 解决了什么

模板 `PUBLISHED` 后「内容层」仍是活的：在组件管理保存一次组件，所有引用它的已发布模板快照被**自动、静默、无提示**改写，连带改变在途报价单。

**实测爆炸半径比直觉大得多**：全库**零个组件跨 `template_series_id` 共享**，所以「一个组件被 6 个模板引用」必然是**同一系列的 6 个历史版本**。改一次组件 → v1.0~v1.5 的快照同时被改写。**版本号是假的** —— 挂在 v1.0 的报价单以为冻在 8-05 的配置，实则显示今天的。

**这是代码漂移，不是既定设计**：`docs/三大核心模块基线.md:178/:182` + §10.2#4 白纸黑字写的就是「不可变」「只在用户明确触发时刷新」「不直接动 PUBLISHED」，代码没做到。

### 怎么修的

权威源改为关系表 `template_component_snapshot`（18 个渲染配置字段 + tc 三列）。`publish()` 是**唯一写入点**；`components_snapshot` jsonb 改为**由该表派生**（保前端契约）；渲染期收口到 `PublishedTemplateReader`（**只给批量方法**，不给单条查询——给了就是给 N+1 留后门）；快照读不到**显式报错，绝不回落活表**。

`refreshSnapshotsByComponent`（H1）整体退役，连同两个端点删除。三个 admin 后门保留但加「预览 + `operation_log` 审计 + WARN」。

**存量不迁移**（D16 推翻原 D3）：改由人工「首次冻结」。三态语义 —— 零快照行 = 未冻结（409 `TEMPLATE_NOT_FROZEN`，正常过渡态）／行数与 jsonb 长度不符 = **损坏（500）**。两者必须分开判，混了会让「快照被后门删了」被误判成「还没发布」。

### 🔑 最值得记的教训：「写了但不生效」在一个任务里重复了 6 次

立项估「10 处读取点收口」，实际做到 B23。**多出来的工作量几乎全部来自同一个形态** —— 代码写对了，但输入到不了 / 产物没人读 / 旧数据架空它 / 文案指向不存在的操作：

1. 「10 处活表读」清单实际 **18 处** —— 只 grep 了已知的 3 个文件，**确认偏误**
2. `lookupForResolver` 三层 fallback 齐全，**前两层从未执行过**（上游恒传 `templateId=null`/`status=null`）
3. `findTab` 实现了损坏检测，**零生产调用方**
4. V382 清了快照表却留着旧 jsonb → 渲染产出「六个空白页签」，**不报错也不提示**
5. 409 文案写「请重新发布」，但 `publish()` **只收 DRAFT** —— 该操作不存在（催生 D20「首次冻结」）
6. `componentsSnapshot == null` 被当作「这是 DRAFT」的代理信号 —— 改造后未冻结的 PUBLISHED 也是 NULL → **误判回落活表**。同一文件的兄弟方法先修了一处，**没传播到隔壁**

> **验证「逻辑对」远不等于验证「它真的会跑」。** 每加一道守卫 / 每冻一个字段 / 每写一句用户提示，都要**端到端证明它被真实触达**。尤其警惕：① 依赖上下文注入的守卫（先查上游传没传）② 新旧两份数据并存（旧的会不会架空新的）③ 用空值推断状态（改造后这个推断还成立吗）。
> **元教训：同一个 bug 修一处必须全仓扫同类** —— 上面 ①⑥ 都是「修一处漏一处」。

### 其他值得记的

- **`| tail` 会吞掉退出码**：我因此三次拿到假的「exit 0」（baseline 测试、端口冲突、全量测试套件）。**判构建/测试结果不要看被管道包过的退出码**
- **探服务器前先验明身份**：一次 `mvnw quarkus:dev -Dport=8099` 因端口被并发会话占用而静默失败，随后几发 curl **全打在别人的实例上**，三个新端点齐刷刷 404，差点得出「端点没生效」的错误结论。戳破它的是「探那个本该被删的端点，结果它还在」→ 查 `/proc/<pid>/cwd` 指向别的 worktree
- **两个 Quarkus 实例会互相破坏 Flyway 历史**：主仓无 V382 → 8081 重启记 `type=DELETE` 标记 → worktree 有 V382 的 8095 认为「未应用」→ 重跑撞「表已存在」。解法是**主仓也放一份副本**（合并前删）。我一度误判这是「重复应用、合并时会炸」并删过记录，实为 Flyway 对「历史有、盘上无」的正常记账（375/376 早就这样）
- **AC 判据可能自相矛盾**：AC-5 原写「三文件内活表读 grep 零命中」，与 §5.1.2「DRAFT 走活表」直接打架。判据要按路径分，不能按文件数
...
**涉及文件**：`dev-docs/rule-260724-组件模板配置/` 7 份（`README` / `1-总则与工作流` / `2-组件与字段` / `4-页签属性与树` / `5-公式与Excel列` / `AGENT-配置入口` / `附录-速查`）· `deploy/0809-dbupdate.sql`（顺带纠错）
...
**涉及文件**：`客户组件模板/报价通用组件/`（`材料成本.sql` / `材料成本.json` / `README.md`）· `dev-docs/rule-260724-组件模板配置/`（`AGENT-配置入口.md` / `报价侧.md` / `核价侧.md`）
...
## [2026-08-09] 文档 - BACKLOG / dev-docs INDEX 状态回写：3 条状态失真 + 14 条已完成条目归档

**涉及文件**：`BACKLOG.md` · `dev-docs/INDEX.md`

**触发**：用户指出「部分已经交付的事项仍显示进行中」。先确认工作区干净且已推送 —— **是内容陈旧，不是漏提交**。

**逐条 git 核实后改的 3 处状态失真**（不凭文字判断，全部落 commit hash）：
1. **BL-0133**（模板发布全量冻结）标 `[~] 进行中（待出实现计划）`，实际 2026-08-07 已合 master `8d04336a`。按 BACKLOG 自身规则移入「已完成」区，并把两处**交付期被推翻的立项决策**写进条目：D3「存量一次性对齐」→ D16 改「不迁移存量」；「重新发布即可恢复」→ B22-c 证伪（`publish()` 只收 DRAFT，补冻走 `POST /templates/{id}/freeze`）。
2. **BL-0137 阶段③a** 标「开发中（分支 `feat/task-0806-lazy-rowdata`）」，实际该分支已合并并删除（merge `e40ab0fb` / 实现 `a582935d` / 收尾 `b186ec21`）。条目本身因阶段②④⑤ 未开工，**保持 TODO 不结案**。同步改 INDEX §3 对应行。
3. **BL-0097**（`$view` 报错毒化事务）标 `TODO（未排期）`，而 INDEX §0.0 标「状态存疑」。核实结论：**部分交付** —— 错误可见性已修（`2d12bde2`），但**事务毒化根因未解且 Savepoint 方案已证否**（同提交含 `SavepointIsolationFeasibilityTest`）。两边都改成这个口径，存疑解除。

**另挖出的批量问题：14 条状态已是 `[x]`/`✅`/`[DONE` 的条目一直留在未完成区**（BL-0112/0120/0124/0098/0101/0017×2/0031/0045/0096/0092/0072/0030/0040），使「还剩多少没做」严重虚高、扫读时误导。按 BACKLOG 开头自己写的规则（「完成后把条目移到文末『已完成』并标 `[DONE 日期]`，不要删除」）统一移入，加归档小标题标明来历。

**搬移做了无损校验**（不是肉眼看）：脚本比对搬移前后所有 `###` 块的**集合**，结果「丢失块：无」、新增块只有归档小标题一个、`BL-\d{4}` 编号集合前后完全相等。

**修正后的真实计数**（INDEX §0.0 原写「登记 140 条 / 未完成约 103 条 / P0×2」全部过时）：未完成区 **135** 条（TODO 101 · 待开发 13 · BLOCKED 2 · `[-]` 2 · 其他 3），优先级 **P0×4 / P1×39 / P2×89**；已完成区 **42** 条。

**INDEX §0.0 态势快照**同步刷新：2026-08-06 / `717ca4f0` → 2026-08-09 / `b3cf4872`（落后 111 个提交），补录 `repair-0808` 交付与 2026-08-09 的文档/部署基建同步。
...
**涉及文件**：后端精度基础设施、公式/JEXL/报价/核价/价格调整/导出链路、V385；前端 Decimal.js 精度边界、报价三视图、快照无损解析、HTML 打印；`dev-docs/task-260801-公式计算精度优化/task-260810-公式计算12位显示9位/`；`dev-docs/main-api.md`；`docs/PRD-v3.md`。
...
**涉及文件**：`DecimalRequestValidator.java`、`PrecisionPolicy.java`、`QuotationResource.java`、`FormulaCalculator.java`、`CardSnapshotService.java`、`QuotationService.java`、`precision.ts`、`losslessJson.ts`、`draftPrecision.ts`、报价 Wizard/Step2/Step3 及对应测试；任务目录 `repair-260811-输入值原样与结果精度分层/`。
...
**涉及文件**：`cpq-frontend/src/pages/config/MaterialRecipeEditDrawer.tsx`（唯一代码改动）；文档 `dev-docs/task-260812-材质元素改下拉选择/`（需求/前端/后端/接口/原型/用例/报告/主线亲验记录）。合并 `92d09aa4`。
...
**涉及文件**：`PricingHandlerCatalog.java`（`all()` 摘除 4 项，20→20 保序）、`PricingImportService.java`（`orderedHandlers()` 摘除 4 项，20→16 保序）、`PricingTemplateServiceTest.java`（24→20 收敛，含 `DISABLED_HANDLERS` 常量避免「未登记进 catalog」守卫误判）；任务目录 `dev-docs/task-260812-核价导入停用四个Sheet/`。
...
**详见**：`dev-docs/task-260812-核价导入停用四个Sheet/test.md`（48 条用例）+ `test-report.md`（AC-1~12 逐条对照表 + 证据原文）。
...
## [2026-08-13] 报价对账 - repair-0812 对账阈值与结果尺度不对称致误报（已合 master）

**问题**：报价单编辑页改「材料净重」失焦后，公式列弹 ⚠「前后端算值不一致」（前端 `63.211125158028` / 后端 `63.211125158`），且提交被 `409 RECONCILE_PENDING` 阻断、**怎么改都消解不掉**。

**根因（三层）**：① 前端对账取的 `formulaCache` 是 12 位工作值（`toCalculationString`），后端快照 `formulaResults` 是 9 位结果值（`PrecisionPolicy.roundFormulaResult`），两侧尺度天然差 3 位、差值上界 `5e-10`；② `isWithinTolerance` 默认容差被 task-0810（`fd83cac1`，2026-08-11）从 `0.01` 改成写死 `1e-12`，**比结构性差距小 500 倍** ⇒ 凡工作值第 10~12 位非零的公式列 100% 误报；③ 病根是 **task-0810 FR-12 规格自相矛盾** —— 要求「按 12 位工作值判定」，而后端从不产出 12 位对比基准，判定条件恒不成立。回归自 2026-08-11，此前阈值是 `10^-DISPLAY_SCALE`(1e-6) 不误报。

**修法（用户裁决方案 A）**：`QuotationStep2.tsx#valuesReconcile` 比较前把两侧按 `formatFormulaResult` 归一到 `FORMULA_RESULT_SCALE`(9)，**容差参数不动**；`valuesReconcile` 从组件内闭包提到模块作用域并导出供单测直接校验生产逻辑。**判定用归一值、展示与上报仍用原始 12 位**（真出分歧时靠第 10~12 位排查）。后端零改动、契约零变更、不改数。

**涉及文件**：`cpq-frontend/src/pages/quotation/QuotationStep2.tsx`（`valuesReconcile` + import）｜`cpq-frontend/src/utils/formulaEngine.ts`（`isWithinTolerance` 注释，默认值未动）｜新增 `reconcileScale.repair0812.test.ts`（15 条）｜`e2e/precision-flow.spec.ts` + `e2e/fixtures/precision.ts`（TC-076 重写 + TC-076b 新增）｜`task-0810/需求文档.md`（FR-12 作废改写 + §10 演进史）+ `fronttask.md` + `task-0806/需求文档.md` D4｜`dev-docs/task-260801-公式计算精度优化/repair-260812-对账阈值与结果尺度不对称致误报/`

**关键决策与注意事项**：
1. **明确排除的错误做法**：不要让前端 `computeAllFormulas` 直接产出 9 位——后端链式公式以 12 位 `working` 向下游传播，前端跟着改成 9 位传播会累积出**真实**偏差。正确模型 = **传播 12 位，只在「落快照 / 显示 / 比较」三个边界收 9 位**。
2. **已知限制 L-1~L-4**：归一后 `1e-12` 容差在本调用点已成死代码（判定等价于 9 位精确相等）；若两侧 12 位工作值有 1 ulp 差异且恰落在 9 位舍入中点附近仍会误报，**前端单侧堵不上**（手上没有后端 12 位工作值），只有方案 C 能闭合；且该风险**当前不可观测**（后端 `working` 从不落库/进 DTO/写日志）。
3. **交付口径（禁止放大）**：只能声明「结构性误报的判定口径已修正，经真实故障值单测 A/B 证明有效」；**不得**声明「前后端已一致」（AC-6 普查裁决不做）、「⚠ 已在真机消失」（AC-1 未验证）、「提交闸门已恢复放行」（AC-2 未验证）。
4. **AP-65 教训（本次最大收获）**：作废需求条款时只改了 3 处**文档**，漏了它的**可执行形态** —— `e2e/precision-flow.spec.ts` TC-076 把「第 10~12 位差异必须阻断」写死成 P0 断言，修复后该用例数学上必然失败。**搜索要按语义不按编号**（`grep FR-12` 搜不到任何东西）。
...
**涉及文件**：`component.row_key_fields`（数据，3 行 UPDATE）｜`客户组件模板/核价组件/核价组件模板-简易.json`（按当前库整体重导，3→5 组件 + `⚠️行键口径` 警告段）｜`核价组件模板-简易-组件.sql`（注释）｜`dev-docs/task-260801-页签连表公式配置优化/repair-260814-行键中英口径混存致连表可比判定失真/`（问题说明 + test-report）｜`BACKLOG.md`（BL-0169/BL-0170）
...
**归属**：`dev-docs/task-260806-模板发布全量冻结/repair-260814-发布冻结后tabType护栏误拦/`｜`BL-0168`｜合 master `c2f485d5`（当日立项当日交付，⏳ 闸门 B 待用户真机）
...
**涉及文件**：`FieldConfigTable.tsx` ｜ `ComponentManagement.tsx` ｜ `ComponentService.java` ｜ 新增 `cpq-frontend/e2e/rowkey-fieldname-contract.spec.ts`（S1 阴性 / S2 勾选态 / S3 写入与取消 / S4 存量清除）｜ `dev-docs/task-260801-页签连表公式配置优化/repair-260814-行键复选框写列名致口径污染/`
...
**涉及文件**：`ReadonlyProductCard.tsx` ｜ `QuotationStep2.tsx` ｜ `e2e/quotation-bom-tree.spec.ts` ｜ `dev-docs/task-260721-报价侧树状结构与页签类型属性/repair-260814-详情页报价树多出版本列/`
