...
| 17 | 电镀费用（加工费） | `unit_price` | `PLATING 电镀` | `电镀加工费` |
| 17 | 电镀费用（材料费） | `unit_price` | `PLATING 电镀` | `电镀材料费` |
...
| **price_type 与 cost_type 区别** | `price_type` 标识价格来源分类（ELEMENT/MATERIAL/COMPONENT/PLATING），`cost_type` 标识费用用途分类，两字段独立写入 `unit_price`。其中「电镀费用」Sheet 落库的 `price_type=PLATING`（与代码 `Q17PlatingCostHandler` 一致）。 |
