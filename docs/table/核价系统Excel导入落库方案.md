...
| Excel 列名 | 目标表字段 | 是否导入 | 备注说明 |
|-----------|-----------|:-------:|---------|
| 销售料号（历史模板兼容读「宏丰料号」） | `finished_material_no` | ✅ | 成品料号；**升版轴锚点**（该销售料号下全部投入料号 × 加工费/材料费共享同一版本组） |
| 投入料号 | `code` | ✅ | **非必填**；零件料号（组内明细维度）。空则回退销售料号 |
| 投入料号名称 | — | ❌ | **非必填**；不落库、不参与解析（与 §15/§16/§17「品名」一致，渲染取名由视图 JOIN `material_master` 负责） |
| 生产料号 | `production_no` | ✅ | 描述列 |
| 电镀方案编号 | — | ❌ | **不落库**，仅判断用：不为空时整行跳过不导入 |
| 版本编号 | — | ❌ | **不落库**：`version_no` 系统自增生成，忽略 Excel 该列 |
...
| Excel 列名 | 目标表字段 | 是否导入 | 备注说明 |
|-----------|-----------|:-------:|---------|
| 销售料号（历史模板兼容读「宏丰料号」） | `finished_material_no` | ✅ | 成品料号；**升版轴锚点** |
| 投入料号 | `code` | ✅ | **非必填**；零件料号（组内明细维度）。空则回退销售料号 |
| 投入料号名称 | — | ❌ | **非必填**；不落库、不参与解析（同上） |
| 生产料号 | `production_no` | ✅ | 描述列 |
| 电镀方案编号 | — | ❌ | 同上，不为空则整行跳过 |
| 版本编号 | — | ❌ | 不导入 |
| 电镀加工费 | — | ❌ | 不导入（归第 1 条记录） |
...
> 📌 **repair-0802**：`groupKey = (system_type, price_type=PLATING, finished_material_no)` —— 分组锚点从 `code` 改为销售料号，与 §15 来料加工费同构：同一销售料号下的全部投入料号 × 加工费/材料费共享一个版本组，任一变化整组升版。`code`（投入料号）进 content，组内去重键 = (code, cost_type)。核价侧**不做名称反查/铸号**（核价侧物料均已有正式码）。配套 `dj_view` 的 `material_no`/`hf_part_no` 与 `:versionFilter` 料号键列一并改绑 `finished_material_no`，并新增 `input_part_no` / `input_part_name` 两列（名称由 JOIN `material_master` / `material_recipe` 取得）。
