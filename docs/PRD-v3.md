...
- **添加产品**:点击"添加产品"按钮 → `BulkImportPartsDrawer` 抽屉
  ...
  - 也支持手填 HF 料号 + 数量
  - 批量加入 lineItems
- **选配添加**（2026-09-03 task-260902 落地，三层模型）：产品库里没有的产品，由销售在报价单内**现场配出来**，提交时自动铸销售料号
  - **入口**：第二步「添加产品」旁的「选配添加」→ `ConfigureProductDrawer` 四步抽屉
  - **三层模型**：产品（客户产品编号）→ 配件 1..N（零件 / 外购件）→ 零件挂材质 1..N（各带占比，Σ=100%）→ 每材质选一条含量配置
    - **零件**：新建（填品名/规格/尺寸/单重 + 加材质）或从产品库选已有零件
    - **外购件**：只能**从现有料号库选**（`material_master.material_type='外购件'`），不建新主数据
    - **占比**：定点小数 `numeric(24,12)`，合计判等必须用 `BigDecimal.compareTo` / 前端定点整数，🚫 浮点会错误拒绝 `0.000000000001 + 99.999999999998 + 0.000000000001`
    - **自定义含量**：材质 `allow_custom_content=true` 时可现场改配比，**只用于本次选配，不回流材质库**
  - **销售指纹去重**（`sel_part_signature`，`structure_version='v2'`）：提交前按配置算指纹比对
    - 命中 → **复用已有销售料号**并带出 `reusedProductInfo`（品名/规格/材质/占比）供确认
    - 未命中 → 铸新销售料号（`QuoteMaterialNoAllocator`，格式 `<客户码>-<年月><流水>`）
    - 指纹维度：客户 + 品名/规格/尺寸（长度前缀编码防拼接歧义）+ 单重 + 材质码:占比(元素:含量) + 工序集
    - **全类 token 顺序无关**（均 `sorted()`）—— 「焊接→铆接」与「铆接→焊接」是同一个产品；但**重复次数是维度**（`Z100→Z101→Z100` ≠ `Z100→Z101`），🚫 `sorted()` 旁不可加 `distinct()`
    - 🚫 **配方编号不进指纹**：含量逐字相同即视为同一种材料（裁决 D-5）
  - **客户产品编号**：第一步录入，落 **`sel_product_no`**（`customer_no` + `customer_product_no` + `quote_part_no`）
    - 编号已被占用 → 挡住并指路「从产品库添加」（占用口径 = `sel_product_no` ∪ `material_customer_map`）
    - **一个销售料号可对多个客户产品编号**（不同客户编号配出相同配置时复用同一料号）；「从产品库添加」列表按销售料号去重成一行，`customerProductNos` 字段带出该料号名下全部编号
    - 🚫 **不写 `material_customer_map` 的 `customer_product_no`** —— 该表的 `uq_mcm_quote_no` 同时是导入 `upsertQuote` 的 ON CONFLICT target 与**跨客户串号防线**，且 4 个组件视图的 JOIN 不含编号维度，占用它会产生重复行
    - ⚠️ 铸号时 `QuoteMaterialNoAllocator` 会在 mcm 写一行**占号行**（`customer_product_no IS NULL`），那是防重复分配的固有机制，与上一条不冲突
  - **选配模板**（`sel_template`）：**2026-09-03 起入口下线**。实测三个参数开关与值域限定全部空转，唯一生效的是「没配模板就不让选配」这道门禁 —— 弊大于利。路由、页面、表与数据均保留，将来按新模型重新引入时管**步骤可见性 / 值域 / 必填性**，🚫 不管自定义含量（那归材质管理）
...
- repair-0811 分层补充：INPUT_TEXT、INPUT_NUMBER 与基础数据带入值保持用户/单元格当前原文，不按公式显示精度回写；结构整数（如 `项次`）可使用安全 JSON number。公式依赖链工作值保留 12 位，单元格公式最终结果最多 9 位。产品卡片最终小计与报价单总金额分别使用独立的结果精度变量，当前默认均为 9，未来由系统参数分别覆盖。
...
#### 5.5.0 发布后不可变 + 版本演进（2026-08-07 task-0806 确立）

**产品语义（这是本节最重要的一条，改变了配置的生效方式）**：

> **模板一经发布（PUBLISHED），其渲染所依赖的全部配置即冻结，不可再更改。**
> 组件管理里修改任何配置（字段、公式、驱动路径、行键、排序列、元素价格列……），**对已发布模板和已生成报价单一律无影响**。
> 想让新配置生效，**只能派生新草稿 → 修改 → 发布为新版本**。

| 场景 | 行为 |
|---|---|
| 改组件配置 | 只影响**草稿模板**和**将来新发布的版本**；所有已发布版本纹丝不动 |
| 想让某已发布模板用上新配置 | 「创建为新草稿」→ 改 → 发布 → 得到新版本号 |
| 已有报价单 | 继续绑在它原来的模板版本上，渲染结果不变。要用新版本须显式换模板（⚠️ 换模板会清空已填数据，见 `BL-0129`） |
| 老版本是否自动归档 | **否**。同系列多个 PUBLISHED 版本可共存，模板选择列表按发布时间倒序，最新版在最前 |
| 组件被停用 | 只表示「新模板不能再选它」；**已发布模板与已生成报价单不受影响，照常渲染** |
| 想知道某已发布版本落后多少 | 「与当前组件配置的差异」视图（`frozen-drift`）。**差异随时间增长是正常的**，它是「要不要发新版」的决策依据，不是告警 |

**为什么要这样**：此前实现存在缺陷 —— 保存一次组件即静默改写所有引用它的已发布模板快照，导致**整条版本历史被改写**（实测：一个系列的 v1.0~v1.5 六个版本连同其下 18 张报价单同时被改），版本号无法对应确定的渲染结果，历史报价不可追溯。详见 `dev-docs/task-260806-模板发布全量冻结/需求文档.md` 与 `docs/反模式.md` AP-63。

**运维后门**：`SYSTEM_ADMIN` 保留少量可强制改写已发布快照的端点（用于平台级紧急修复），但一律需二次确认、留审计、打告警 —— 属于明确「破坏版本不可变性」的例外操作，不进日常流程。详见 `docs/三大核心模块基线.md` §3.5。

##### 5.5.0.1 过渡期：「尚未冻结」是一个正常状态（2026-08-07 决策调整）

本机制**不对存量模板做数据迁移**。上线后，此前已发布的模板都处于「**已发布但尚未冻结**」状态，需要人工执行一次冻结才进入新语义。这是预期的过渡态，不是故障。

| 状态 | 表现 | 怎么处理 |
|---|---|---|
| **尚未冻结** | 打开报价单时提示「该模板尚未冻结，请联系管理员冻结后再试」 | 管理员执行**「首次冻结」**（可单个、可一次性全量） |
| **已冻结** | 正常渲染 | —— |
| **快照损坏** | 明确报错并给出模板/组件/页签定位 | 属异常，需排查（正常操作产生不了这个状态） |

**「首次冻结」与「重新发布」不是一回事**：

- **首次冻结** —— 把该模板**当前**的配置就地冻成快照。**只对从未冻结过的模板可用**，不改版本号、不改状态。它不是在改写什么，是给从来没冻过的东西补上快照，因此不破坏不可变性
- **重新发布** —— 想让**新的组件配置**在业务上生效时走的正常路径：派生新草稿 → 修改 → 发布出新版本号

**归档特例**：归档是终态、无法再发布，因此模板归档时若尚未冻结，系统会**自动补冻**一份，确保其历史报价单将来仍打得开。
...
### 9.25 v4.7(2026-09-03)— 选配从两层模型升到三层，客户产品编号改由独立映射表承载

**决策点**：选配此前的「配件」在数据上**直接等于一个材质料号**（`PartRequest.recipeCode` 单值、落库只写 1 行自指物料行），装不下真实产品结构 —— 一个零件由多种材质按占比构成，是触点行业的常态。本次升为三层：**产品（客户产品编号）→ 配件 1..N（零件 / 外购件）→ 零件挂材质 1..N（各带占比）→ 每材质选一条含量配置**。

**五项裁决**：D-1 三层且零件含多材质；D-2 材质填**占比**、零件填**总重**（不逐个填克重）；D-3 客户产品编号已存在则**挡住**、改走产品库入口；D-4 外购件**只从现有料号库选**、不建新主数据；D-5 指纹新增「材质占比 / 零件总重 / 品名规格尺寸」三维度，而**配方编号不进指纹** —— 含量逐字相同即同一种材料。

**一处架构否决**：AC-2「编号唯一」与 AC-7「指纹复用」在 `uq_mcm_quote_no` 下正面冲突（该索引把销售料号与客户产品编号锁成 1:1）。**放宽索引的方案被影响面调查否决** —— 它同时是导入 `upsertQuote` 的 ON CONFLICT target 与**跨客户串号检测**的载体（森萨塔串号事故的防线），且 4 个组件视图的 JOIN 不含编号维度，放宽会让渲染出现重复行。改为**新建 `sel_product_no` 映射表**（V401），让一个销售料号可对多个客户产品编号，`material_customer_map` 一个字节不动。

**顺带收敛了一个一列两义**：`material_master.material_type` 被导入侧当「料号类型」、被选配侧写入「材质名」，两种语义共存至今没爆，**只因 custom 路径从未跑通**（实测 `material_recipe_id` / `config_fingerprint` 全表 NULL）。而 `task-260901` 打开 `allow_custom_content` 正要给它通电。已裁决归位为料号类型（配套 V403 视图补 `material_part_no` 维度、V404 外购件不进材质页签）。

**选配模板入口下线**：核查发现三个参数开关与值域限定**全部空转**（`projectEnabledParams` 恒加槽位不读 `enabledTypes`、`allowed.isEmpty()` 全放行且现网 0 行配置），唯一真正生效的是「没配模板就不让选配」这道门禁 —— 弊远大于利。路由 / 页面 / 表 / 数据一律保留，将来按新模型重新引入时管**步骤可见性、值域、必填性**，不管自定义含量。

**已知未闭合**：AC-10（同材质下含量逐字相同的两条配置 → 仍复用）在当前 schema 下**造不出前置** —— `uq_recipe_element = UNIQUE (recipe_id, element_code)` 禁止同材质出现同名元素，而「逐字相同」必然同元素码。解除条件为 `task-260901` 的 `DROP CONSTRAINT` 迁移落地；D-5 规则本身未失去验证（AC-22③ 走同一条判同路径且可构造）。

**关联**：`dev-docs/task-260902-选配流程重构/`、V401~V404、merge `29f9d5e6`。
...
**关联**：`dev-docs/task-260801-公式计算精度优化/task-260810-公式计算12位显示9位/`、V385、`BL-0159`、`BL-0160`、`BL-0161`。

### 9.23 v4.5(2026-08-07)— 模板发布后全量冻结，配置变更改为严格版本化生效

**决策点**：把「模板发布 = 快照冻结」从**名义**变成**事实**。

此前实现有三条彼此独立的活穿透路径：① 保存组件即自动改写所有已发布模板快照；② 6 个渲染配置字段（`rowKeyFields` / `sortField` / `element*Field` / `columnCount`）从未进过快照；③ 渲染期 10 处直接读活表、压根不读快照。三者叠加的后果是**整条版本历史被改写** —— 实测一个模板系列的 v1.0~v1.5 六个已发布版本连同其下 18 张报价单同时受影响，**版本号无法对应确定的渲染结果**。

**这不是架构取舍，是实现漂移**：`docs/三大核心模块基线.md` 原本就写明「组件管理改字段后，老模板的历史报价单不能反向受影响」，代码没做到。

**本次确立的产品语义**（详见 §5.5.0）：发布即冻结；配置变更只能通过**派生新草稿 → 发布新版本**生效；已有报价单继续绑原版本；组件停用不影响已发布模板。同时新增「与当前组件配置的差异」视图，让「这个历史版本到底冻的是什么」第一次变得可查。

**刻意未改**：老版本发布后**不自动归档**（同系列多 PUBLISHED 共存，维持既有行为，仅修正基线文档中「自动 ARCHIVED」的错误表述）。

**关联**：`BL-0133`（本任务）／`BL-0129` 因换模板成为高频路径而提级 P1／`BL-0138`（批量发版工具）／`BL-0139`（差异视图 UI）／`docs/反模式.md` AP-63。
