...
**为什么要这样**：此前实现存在缺陷 —— 保存一次组件即静默改写所有引用它的已发布模板快照，导致**整条版本历史被改写**（实测：一个系列的 v1.0~v1.5 六个版本连同其下 18 张报价单同时被改），版本号无法对应确定的渲染结果，历史报价不可追溯。详见 `dev-docs/task-0806-模板发布全量冻结/需求文档.md` 与 `docs/反模式.md` AP-63。
