# CPQ 开发记录 — 历史归档 (2026-05-20 及更早)

> 本文件是 RECORD.md 的历史归档,收录 **2026-05-20 及更早** 的开发条目。
> 当前活跃记录(2026-05-21 基线锁定起)见 [RECORD.md](./RECORD.md)。
> 归档日期: 2026-06-03。倒序排列,与主文件一致。
...
[2026-06-01] 报价单整份快照 Phase4 Task6 + Phase4 收口 - 公式引擎防漂移红线(前后端共享样本逐分对账) | cpq-frontend formula-reconcile-cases.json + formulaReconcile.test.ts + cpq-backend FormulaCalculatorTest#reconcileFixture (commit bd8ecad)
- 用户决议(S1 审计后): 保守收口。row_data/snapshot_rows 是 ExcelViewService(报价/核价 Excel 视图)/QuotationExportService(导出)/SnapshotCollectorService(提交快照)/QuotationService(copy)/CardSnapshotService.buildCardValues(快照 baseRows 种子来源本身)/ComponentDriverService 的承重消费方 → **无法退役**。card 渲染计算/driver 读已由 Task3/4/5 脱钩到快照, INPUT 读 row_data = 与 Excel/导出/提交同源(single-source 合理保留)。Task3 deferred(INPUT 本地态/editRows 持久 rebuild/停写 row_data)因 row_data 必须保留而变为不必要, 关闭。
- 防漂移红线: 唯一权威样本 formula-reconcile-cases.json(11 例: 四则/括号/×÷归一/material_cost/跨tab component_subtotal/工序累加 previous_row_subtotal/除零/缺值/4位HALF_UP/path BASIC_DATA/gvar)。前端 vitest formulaReconcile.test.ts 跑 formulaEngine.evaluateExpression(round4 后比对, 12/12) + 后端 FormulaCalculatorTest#reconcileFixture 跑 FormulaCalculator.evaluateExpression(setScale4, 16/16) 同读一文件 → 任一引擎漂移一侧变红。刻意微偏离(除零前端 Infinity vs 后端 0)在前端 round4 归 0 编码契约。
...
- **未完(拆为独立后续任务, 决议 C)**: Task 9(读这 5 表的 PG 视图 + 配置 SQL 模板加 is_current) + Task 10(E2E)。详见 docs/table/报价系统版本号-视图is_current过滤-实现计划.md + worklist。

[2026-06-02] 基础数据导入/版本化 - Task 9/10 视图 is_current 过滤(写入路径后续) | V280/V281 迁移 + worklist | 计划 docs/table/报价系统版本号-视图is_current过滤-实现计划.md
- Task 9a 范围清点(活库 pg_views/component_sql_view/template_sql_view 实测): 引用串命中 46 个, 真正 FROM/JOIN 这 5 表仅 26 个(20 个是列名误报如 standard_labor_rate AS unit_price)。IN-scope 18: 4 PG 视图(v_composite_child_*) + 13 component_sql_view + summary_material(部分)。OUT: v12_* 核价 PRICING / 列名误报 v_c_*。capacity 无任何视图消费(走 driver)。产出 docs/table/视图is_current-worklist.md。commit 1fdce40。
- Task 9b(V280): 4 个 v_composite_child_* 用 CREATE OR REPLACE(不 DROP CASCADE, 仅加 WHERE) 加 is_current。element_bom_item/unit_price 必需, material_bom_item 防御。flyway success, 4 视图可查。commit ae46fc9。
- Task 9c-1(V281): 7 个配置 SQL 模板必需过滤(gx_view/zcj_view 的 unit_price, v12_plating_scheme 的 plating_scheme, composite_child_elements_mirror/v12_raw_element_bom/ys_view 的 element_bom_item, summary_material 的 element_bom_item)。机制 UPDATE regexp_replace(\s+ 弹性匹配 CRLF, 原始文本含 ^M+换行, replace 单空格失败), 每锚点 dry-run count=1。flyway success, BnfTableMetaSyncer 重新同步 OK。commit f6e0950。
- 关键决策: material_bom_item 防御性过滤有意省略(Q03/Q12 deleteNonCurrent 保证只剩当前版本); 实证全库 material_bom_item/element_bom_item/unit_price-QUOTE 的 is_current=false 行=0, no-op 判断确认正确。capacity 无视图消费, 9c-2 无需改。
...
[2026-06-03] CardEffectiveRows共享工具 - 新增纯函数工具类:卡片值快照JSON→每页签有效合并行+小计;报价/核价共用(中性命名);合并口径=driverRow∪basicDataValues∪formulaResults∪editRows(后者优先);tabKey=componentId:sortOrder(sortOrder从componentsSnapshot取);rowKey优先按rowKeyFields拼,否则按位置idx | cpq-backend/src/main/java/com/cpq/quotation/service/card/CardEffectiveRows.java + CardEffectiveRowsTest.java | TDD:先红后绿;2 tests passed;仅新增2文件不改任何现有文件;供后续CARD_FORMULA Excel列求值使用

[2026-06-03] 文档 - 物料BOM⇄组成件BOM 同料号去重合并规则(仅落文档, 代码走architect评审) | docs/table/报价系统Excel导入落库方案.md (V3.0→V3.1) | 仅改MD未动代码; 架构反转待评审
- **需求**: 用户把本应放组成件BOM(§12 Q12, bom_type=ASSEMBLY)的组成件料号重复填进物料BOM(§3 Q03, bom_type=MATERIAL), 导致同一料号在 material_bom 出现两条记录(characteristic=NULL + characteristic='ASSEMBLY')。要求去重: 同一料号(system_type+customer_no+material_no)只保留一条当前行。
- **规则(已确认, 已写入文档)**: ①类型组成件优先(本次出现在组成件BOM→ASSEMBLY/characteristic='ASSEMBLY', 否则MATERIAL/characteristic=NULL); ②子行合并键=(system_type,customer_no,material_no,component_no) 不含characteristic; ③字段冲突取并集、冲突处组成件赢, 独有字段(物料:base_qty/scrap_rate/defect_rate/component_usage_type; 组成件:operation_no/item_seq)全保留; ④版本血缘按料号(不含characteristic)升版翻转is_current; ⑤文件即权威(只填一边重导→另一边明细不保留, 降为旧版本)。
- **关键架构事实(查证 V219建表 + V277加is_current)**: 仅 material_bom 主表有 bom_version; material_bom_item 子表**无版本列**, 靠 is_current 翻转 + VersionedV6Writer.deleteNonCurrent 物理删非当前子行 = 子表只留当前版本明细, 无子行级历史。故"升版"救不了被删的另一来源子行 → 这就是为何"文件即权威"是唯一自洽选项。
- **架构反转(本次未做, 待architect)**: 推翻 V6 基线"characteristic(NULL vs 'ASSEMBLY')作 Q03/Q12 物理隔离键、允许同料号两行并存"的设计, characteristic 降级为"当前版本类型属性"。落地点: ①VersionedV6Writer 版本/翻转 groupKey 去掉 characteristic 改料号级; ②QuoteImportService/Q03MaterialBomHandler/Q12AssemblyBomHandler 改为按料号跨两sheet汇总后单次 writeMasterDetail(否则同文件后写者flip+删先写者子行); ③material_bom 加 partial unique index (system_type,customer_no,material_no) WHERE is_current=true; ④复核所有 FROM material_bom* 下游视图是否依赖 characteristic='ASSEMBLY'/bom_type 过滤(AP-53类断链风险)。
- **现状代码取证**: Q03MaterialBomHandler:86 childGk.put("characteristic", null); Q12AssemblyBomHandler:68 masterGk.put("characteristic","ASSEMBLY")(注释原话"uq 隔离 Q03(NULL)/Q12(ASSEMBLY)") — 即本规则要反转的隔离设计来源。

[2026-06-03] 物料BOM去重合并 - architect评审结论(更正上条"架构反转"方向) | docs/table/报价系统Excel导入落库方案.md (评审后再修) | 上条留痕的反转点①③有害,已废弃
- **评审最大发现**: material_bom 有第二写入方——选配运行时 ConfigureProductService.writeCombomaterialBomV6(行547-590) 对同一COMBO料号**故意写两条主行** (bom_type=MATERIAL/characteristic=NULL) + (ASSEMBLY/characteristic='ASSEMBLY'),分别喂 composite_child_materials_mirror(characteristic IS NULL)/zcj_bom(='ASSEMBLY')/composite_child_elements_mirror/composite_child_processes_mirror 四个mirror视图。characteristic"同料号双行并存"是选配契约,非导入偶然。
- **更正**: 上条留痕的反转点① 改VersionedV6Writer groupKey 去characteristic、③ 加 (system_type,customer_no,material_no) WHERE is_current=true 唯一约束/去characteristic索引 —— **均有害,会致选配保存409 + 4个Tab全空(AP-53类断链),已废弃**。
- **最终方案(零DDL)**: 6条规则全部收敛在导入器层。新增 MaterialBomMergeHandler: 解析物料BOM+组成件BOM两sheet后按material_no单一事务内汇总子行(合并键去characteristic)→算characteristic/bom_type(组成件优先)→每料号单次 writeVersionedMasterDetail; Q03/Q12退化为纯parser(material_master upsert副作用保留)。VersionedV6Writer不改;不加任何DB约束/索引;"唯一当前行"靠导入器单次写入保证。下游mirror视图零改动但E2E双spec必须回归。
- **PM待决(实现前置,未决不开工)**: 导入数据 vs 选配运行时数据是否共享同一(customer_no,material_no)命名空间? 共享则导入会覆盖选配写的行,需确认是否按来源隔离。
- **现状**: 文档已按评审结论二次修正(改方向①③为"不做"+补选配契约说明+PM待决块)。代码未动。

[2026-06-03] 物料BOM去重合并 - PM澄清命名空间(关闭architect R5前置项,放行实现) | docs/table/报价系统Excel导入落库方案.md(PM待决块改✅) | 两料号集合天然不相交,可安全实现
- **R5结论(PM)**: 导入侧 material_no=Excel人工填ERP物料料号(如9996/3120012574); 选配侧 material_no(parentHfPartNo)=AutoAllocatePartNoProvider生成,固定 CFG- 前缀(CFG-COMBO-000001)。两集合靠CFG-前缀天然不相交(共享customer_no但料号不撞), 导入去重与选配双行不会互相覆盖。两链路均写 system_type=QUOTE,故system_type不能用于隔离。
- **决策**: 无需加 data_source 来源列 / 无需system_type隔离; MaterialBomMergeHandler 导入器层合并可安全实现。
- **加固(实现时落地)**: 导入校验层加前置——material_no 以 CFG- 开头的行拒绝导入并报错, 把隔离从操作规范升级为系统强制。
- **状态**: 6条规则✅ + 零DDL实现方案(architect)✅ + 命名空间(PM)✅ 全部澄清, 实现前置项清零。代码尚未动, 待用户决定是否开工(建议走 cpq-deliver: TDD MaterialBomMergeHandler + E2E双spec回归选配4Tab)。
