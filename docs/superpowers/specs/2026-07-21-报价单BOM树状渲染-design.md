...
| 2 | 料号出现在「零件」类型页签 | 零件 |
| 3 | 料号未出现在「零件」页签，但其**下级挂有材质** | 零件（结构推导） |
| 4 | 料号出现在「外购件」类型页签 | 外购件 |
| 5 | 以上皆不满足（零命中） | **错误**——该料号不是有效的报价产品，拒绝新增 |
...
树上**非叶子节点行整行只读**，只有叶子行的数据可编辑。非叶子节点代表 BOM 结构，其数据由结构决定。

只读判定为**动态**——按该节点当前是否拥有子节点计算，而非物化时固化。

> **待确认 C**：一个**已填数据的叶子**被加上子叶子后即变为非叶子，按上述规则会转为只读，其已填数据的归属需要明确。
>
> 按 BOM 语义，叶子是最底层物料（承载数据）、中间节点是组成件（不承载数据），因此「在有数据的叶子下加子」本身即是业务矛盾——它说明该节点原本就不该被当作叶子。可选处理：(a) 禁止在已填数据的叶子下加子，加行入口置灰并给出原因；(b) 允许但提示数据将转为只读保留；(c) 允许并清空其数据。倾向 (a)，但需业务确认。
...
- **待确认 A**（§4.2）：6 类页签类型值域与 `characteristic` 三态的映射；「零件」与「组成件」的语义区别
- **待确认 B**（§6.2）：手工叶子的业务列数据来源（当前设计为留空手填）
- **待确认 C**（§6.1）：已填数据的叶子被加上子叶子后，其已填数据的归属（倾向禁止此操作）
- **待核实**：报价单编辑者的实际角色，用于确认相关端点鉴权（现有 lookup 端点为 `SALES_MANAGER` / `PRICING_MANAGER` / `SYSTEM_ADMIN`）
