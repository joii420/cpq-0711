...
### 2.2 金额 (is_amount)
- **依赖**：必须先勾小计才能勾；**取消小计时自动联动取消金额**；金额勾选框在未勾小计时置灰（disabled）。
- **效果**：金额列的小计单元格前加 **¥**，数字**跟随通用数值精度**（最多 4 位小数、去末尾 0，与其它小计列同款格式，仅多 ¥ 前缀）；并计入本页签总计。
- **本页签总计** = 仅所有**金额列**的小计之和，格式 **¥ + 2 位小数**（财务惯例）；**无任何金额列时，整行隐藏本页签总计**。
...
### C3 — 本页签总计：仅金额列之和 + 无金额列隐藏
- `tabTotalLines.ts#sumTabColumns`：过滤条件 `f.is_subtotal` → **`f.is_amount`**；更新注释与 `CompLike.fields` 类型补 `is_amount`。
- 本页签总计行门槛：`QuotationStep2.tsx:2579`、`ReadonlyProductCard.tsx:681` 的 `some(f => f.is_subtotal)` → **`some(f => f.is_amount)`**。
- 本页签总计数值格式：保持 `formatCurrency`（¥ + 2 位）。
...
- 行键相关全部逻辑。
- Excel 视图（单独评估）。
- 存量数据迁移（只改代码；存量未勾金额页签的本页签总计将隐藏，已确认接受）。

## 4. 边界场景

| 场景 | 期望 |
|---|---|
| 页签有 is_subtotal 列、无 is_amount 列 | 小计行显示（纯数字、无 ¥）；本页签总计行隐藏 |
| 页签无 is_subtotal 列 | 小计行 + 本页签总计行均隐藏 |
| 文本列勾 is_subtotal | 小计行该列显示 0 |
| 取消某列 is_subtotal（原已勾 is_amount） | is_amount 同步取消；该列退出小计行与本页签总计 |
...
- 人工验收：报价单 / 核价单 / 详情三视图，对照「未勾小计列留空」「金额列 ¥+精度」「本页签总计仅金额列、无金额列隐藏」「组件管理金额置灰+联动」。
