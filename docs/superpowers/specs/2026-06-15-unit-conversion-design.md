...
| 吨 / T | KG | 1000（×1000） |
| 片 / PCS | PCS | 1 |
| KPCS / 千片 | PCS | **0.001（÷1000）** ⬅ 2026-07-28 修订（原 ×1000） |
| g/PCS（克/片，斜杠两侧大小写不限） | kg/PCS | 0.001（÷1000） |
| **kg/KPCS（千克/千片，斜杠两侧大小写不限）** | kg/PCS | **0.001（÷1000）** ⬅ 2026-07-28 新增 |
...
- **`kg/KPCS` 与 `g/PCS` 数学等价**：1 kg/千片 = 1000 g / 1000 片 = 1 g/片，故两者系数必须恒等（有专项断言守护）。
- **中英别名必须同系数**：`KPCS`/`千片`、`KG`/`千克`、`G`/`克`、`T`/`吨`、`PCS`/`片` 各为同一单位的两种写法，任何一侧改动都要同步另一侧，否则同一行换个写法结果就变（KPCS 档差 100 万倍）。前后端各有 `aliasPairs` 断言守护。

> **2026-07-28 修订说明**：`KPCS / 千片` 由 ×1000 改为 ÷1000（语义从「千片这个数量单位换算成片」改为「每千片的量归一到每片」），并新增 `kg/KPCS`。改动前实测存量数据：裸 `KPCS` 与 `千片` **各 0 处**使用，含 KPCS 的 28 处全部是 `g/KPCS`（系数不变），故对既有报价数据零影响。
