...
| 同组内年降顺序重复 | **组内归并成一行，逐字段末值非空胜**（详见下方 §9.1） |
| 同 groupKey 并发写 | `pg_advisory_xact_lock` 串行化，与其它 6 张表一致 |

### 9.1 同 `(groupKey, discount_order)` 多行 → 逐字段末值非空胜（2026-08-03 修正）

> ⚠️ **本条推翻了本文档初稿的判断。** 初稿写的是「重复即撞唯一键报错（fail loud，不静默覆盖）」，实现后被全量回归推翻。

**业务事实**：同一 `(销售料号, 年降顺序)` 在一个 Sheet 内**允许出现多行** —— 典型场景是一行填「年降系数（%）」、另一行填「单次固定年降值」，二者同属一条年降规则。落库时必须归并成一行：新行该字段非 null 就覆盖，为 null 则保留旧值。

**两条证据**：

1. 被删除的 `AnnualDiscountRepository.accDiscount` 的 javadoc 明确记载该语义（「同冲突键的批内重复必须由调用方按**末值非空胜（逐字段）**归并后再传入」）。初稿把它读成了批量 INSERT 的机械约束，**误判为实现细节而非业务契约**。
2. `MaterialMasterBatchImportIntegrationTest:186-187` 有一行**故意造的重复夹具**，注释写着「dup：AD1/order=1 再来一行只带 fixed（逐字段末值非空胜：ratio 保留、fixed 补上）」—— 这是被测试显式钉死的契约。

**先例**：`docs/table/报价系统Excel导入落库方案.md` 顶部 V3.6（2026-07-30，task-0730）记载，「来料回收折扣」§9 遇到同型问题时的结论同样是「同一 `(成品料号, 投入料号, COALESCE(项次,0))` 的重复行走**组内 upsert 末值胜**，不再撞唯一键报错」。本次与该先例保持一致。

**实现位置**：`AnnualDiscountWriter.accumulate` —— 组内先按 `discount_order`（`Objects.equals`，null 安全）查找已有行，命中则逐字段合并、**不新增行**，未命中才 append。三个 Sheet 统一适用。

**教训**：读到"为了满足 X 技术约束，调用方必须先做 Y"这类注释时，要追问 Y 本身是不是业务语义 —— 删掉承担 Y 的代码时，Y 会一起消失。
