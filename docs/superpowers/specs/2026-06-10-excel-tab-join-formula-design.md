...
2. 逐 `group`：
   ...
   4. **聚合**：把宽表行集喂 `FormulaEngine` 算 `aggExpression`（`SUM([投料.金额])` → 对行集合聚合）→ 标量。除数取不到/为 0 → 按 `1`。裸明细字段无聚合 → 取过滤后第一行（缺则 0）。
   5. 把标量绑到变量 `组N`。
3. `FormulaEngine` 算 `finalExpression`（`组N` 当变量 + 四则 + 常数）→ 最终标量。
4. 写入 Excel 单元格。

注意：小计/总计是页签级单值，在 JOIN 宽表里广播到每行；对其再 `SUM` 会被行数放大（AP-22 教训，UI 给提示，但语义按用户自负）。
