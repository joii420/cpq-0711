...
### 4.6.1 antd 会给两字中文按钮自动插空格（**精确匹配永不命中**）

```
DOM 实际文本： "保 存"  "查 询"  "删 除"       ← 中间是空格
你写的选择器： /^保存$/                        ← 永远不命中
```

**这是 antd 对两个汉字的按钮文本自动加间距的行为**，对**所有中文界面**成立，不限于本项目某个页面。
...
**同族第三坑**：`.ant-pagination` 渲染在 `.ant-table` **之外**（两者同属 `.ant-table-wrapper`）。容器锚点若取 `.ant-table`，**翻页器定位恒空** —— 要同时拿到表格和翻页器，锚点必须取 `.ant-table-wrapper`。
...
**纪律**：断言前先把状态归零，或改成**基于当前实际状态的相对操作**（"只点当前未勾选的行"），不要盲点下标。

> 与 §4.7「量具可信度」同族：**前者是尺子不准，这里是起点不对**。两者产生的假差异都长得和真 bug 一模一样。

---

### 4.6.3 「错误路径上会弹二次确认」时，确认框的出现与否本身要进断言

**场景**：某缺陷会导致提交的 payload 少一个字段，而后端把「字段缺失」判为「用户取消勾选」→ 弹二次确认 → 用户点确认后数据落库正确。

**只查库会得到最坏的结论**：库里对了 → 判 PASS → 但用户每次保存都要面对一个**不该出现**的确认框，**点确认才碰巧对，点取消反而错**。

> **「碰巧对」和「对」在验收里必须分开。**

**通则**：二次确认的有效性建立在「**用户能判断该不该确认**」这个前提上。一旦弹窗内容看起来完全正常（用户没有任何线索知道自己不该点），这个前提就不成立 —— **确认框此时不是保护，而是把用户变成损害链条的一环**，且它往往是该缺陷**唯一可见的症状**。

**断言写法**：`弹窗不出现` **且** `库里数据正确`，两侧都要。

> 实例：task-0729 验收「元素已勾选并落库 → 被停用 → 以 `includeDisabled=false` 打开矩阵 → 保存」。若 `preserveSelectedRowKeys` 失效，payload 缺该元素 → 后端 `doPutElements` 判为取消勾选 → 弹「N 张存量单该元素单价列将解锁」。财务看到这个多半会顺手点确认（**看起来像例行操作**）→ 元素被静默踢出调价清单。
