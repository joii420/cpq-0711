...
/**
 * TAX_INCLUDED(price, customer_id) — 返回含税价（price × (1 + tax_rate)）。
 *
 * <p>v5.1 §3.2 货币函数。查询 customer_tax 表获取税率。
 *
 * <p>V50 migration 已移除 customer_tax.tax_type 字段，schema 与公式契约对齐。
 * 查询 is_current=true 行，按 effective_date DESC 取最新生效税率。
 */
@ApplicationScoped
public class TaxIncludedFunction implements FormulaFunction {

    private static final Logger LOG = Logger.getLogger(TaxIncludedFunction.class);
    ...
    public Object invoke(List<Object> args, EvaluationContext ctx) {
        if (args == null || args.size() < 2) {
            return FormulaError.invalidArgs("TAX_INCLUDED", "需要 2 个参数：TAX_INCLUDED(price, customer_id)");
        }
        ...
        if (customerIdStr == null || customerIdStr.isBlank()) {
            return FormulaError.invalidArgs("TAX_INCLUDED", "customer_id 不能为空");
        }
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    return new FormulaError("TAX_INCLUDED 未找到客户税率：customerId=" + customerIdStr, "NOT_FOUND");
                }
                BigDecimal taxRate = rs.getBigDecimal(1);
                return price.multiply(BigDecimal.ONE.add(taxRate));
            }
        } catch (Exception e) {
            LOG.warnf("TAX_INCLUDED query failed: %s", e.getMessage());
            return new FormulaError("TAX_INCLUDED 查询失败：" + e.getMessage(), "QUERY_ERROR");
        }
    }
