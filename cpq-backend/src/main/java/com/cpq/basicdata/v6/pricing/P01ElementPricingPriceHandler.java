...
/**
 * P01 元素核价价格表 → unit_price (PRICING/ELEMENT/元素核价价格) 整组版本化。
 * <p>版本按 元素代码 分组，忽略 Excel「元素价格版本」列，由 {@link VersionedV6Writer} 系统自增
...
public class P01ElementPricingPriceHandler implements SheetHandler {
    ...
    public SheetImportResult handle(List<SheetRow> rows, ImportContext ctx) {
        ...
        for (SheetRow row : rows) {
            ...
            c.put("pricing_price", DecimalScale.at(row.getDecimal("核价单价"), 12));
            c.put("market_ref_price", DecimalScale.at(row.getDecimal("市场参考价"), 12));
            ...
            c.put("recovery_discount", DecimalScale.at(row.getDecimal("回收折扣"), 12));
        ...
        for (Map.Entry<String, List<Map<String, Object>>> e : byCode.entrySet()) {
            ...
            gk.put("price_type", "ELEMENT");   // PricingPriceType 类不含 ELEMENT（与 P13 CONSUMABLE 同为保留值,不参与拆分,见类注释）
            gk.put("cost_type", "元素核价价格");
