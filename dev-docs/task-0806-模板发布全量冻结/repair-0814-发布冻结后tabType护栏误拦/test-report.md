...
| **AC-4** | 核价渲染逐位不变 ★一票否决 | 🟡 **结构性 + 单测级 + 指纹级达成；"重渲后仍逐位相同"待闸门 B 真机** | 见 §7；合并前后 `costing_card_values` 全量指纹**逐字节一致**（`587f0a61…`），证明无自发漂移；重渲等价性靠 13 项核价树回归 + 18/18 数据事实支撑 |
...
| **AC-12** | 强制自检 | ✅ | 见 §6（端点 401 ✅ / N+1 逐循环声明 ✅ / E2E 已跑且失败全部归因清楚 见 §7.1） |
...
| 后端端点存活（`/api/cpq/components` 期望 401） | ✅ **401**（合并后在主工作区 dev server 实测；首次请求 000 是合并触发 Quarkus 热重载重编译，等待后转 401） |
| E2E `quotation-flow.spec.ts` | ✅ **已跑，4 failed 全部为夹具/环境缺陷，与本次改动无关** —— 见 §7.1 |

---

## 7.1 E2E 执行结果（合并后，2026-08-14）

```
npx playwright test --config=e2e/playwright.config.ts e2e/quotation-flow.spec.ts --reporter=list
→ 4 failed
  1) :144  LEGACY SIMPLE smoke · 苏州西门子 + 报价模板0608 v1.10 + 10110002
  2) :463  TC-F1 打开 DRAFT 报价单不自动发 refresh-card-snapshot
  3) :522  TC-F2 显式刷新才触发 refresh-card-snapshot
  4) :624  TC-075 SIMPLE · Stage H 确定性精度单
```

**结论：4 个失败全部是夹具/环境缺陷，与本次改动无关。** 逐条给证据，不用"应该是既有的"糊过去：

| # | 失败点 | 报错原文 | 归因（**有实证**） |
|---|---|---|---|
| 1 | LEGACY smoke | `locator.click: Timeout` — 等 `.ant-select-item-option` 含「西门子」 | **DB 实查：`SELECT … FROM customer WHERE name LIKE '%西门子%'` → 0 行**。夹具客户在库里根本不存在（全库仅 6 个客户）→ `BL-0078`「E2E 夹具随 `cpq_db_0724` 迁库全部失效」 |
| 2 | TC-F1 | `编辑态 Step1 下一步应可点` → 按钮 `disabled`，`title="请先填写产品分类和报价模板"` | 夹具单缺产品分类 → 前端 Step1 守卫禁用「下一步」。与记忆 `task0712-update071501-category-axis` 记载的「干净 master 恒失败」原因**逐字一致**（全库 6 个客户里仅 3 个有产品分类） |
| 3 | TC-F2 | 同上，**同一条断言、同一个 title** | 同 #2 |
| 4 | TC-075 | `必须显式设置 PW_PRECISION_SEED_QUOTATION_NO…`（抛在 `fixtures/precision.ts:83`，**用例还没开始跑**） | `BL-0167`（2026-08-13 登记：该环境变量全仓零文档、账面结论无法复现） |

**为什么可以判定与本次改动无关（不是猜，是因果不可能）**：

- 本次**零前端改动**（`git show --stat` 可验），而 #1/#2/#3 全部挂在**前端**：一个是下拉选项不存在，两个是前端 Step1 守卫把按钮置灰；
- #2/#3 **停在 Step1**，根本没走到 Step2 渲染，更没触到我改的三条后端路径（组件保存校验 / publish / 核价树 `parent_no` 判定）；
- #4 抛在 fixture 装配阶段，连页面都没打开；
- 我改的三处，没有任何一处能产生「客户下拉少一项」或「产品分类为空」这种症状。

**关于「恒 3 失败」与本次 4 失败的差异**：记忆 `task0712-update071501-category-axis` 记的是 3 失败，但那条记忆写于 `BL-0167`（2026-08-13 登记）之前，`TC-075` 是后加入的用例。**3 → 4 的增量恰好等于 `TC-075`**，与本次改动无关。

> ⚠️ **诚实声明**：本次**未**做 `git checkout` 式的背靠背 A/B 对照跑。原因两条：
> ① 上述四条失败在**因果上不可能**由本次改动引起（全部发生在我改的代码路径之前，且本次零前端改动），每条都有独立实证（DB 查询 / 报错 title / 抛出位置）；
> ② 主工作区当前有**并发会话的未提交改动**，用 checkout 制造对照会破坏其工作区。
> 若后续有人怀疑此结论，正确做法是先补齐夹具（`BL-0078` / `BL-0167`）让 E2E 回到可用状态，再谈 A/B —— 拿一套四条全挂的 spec 做对照，本身也得不出有效结论。
...
- **合并后自检已补齐**：后端端点 401 ✅；`costing_card_values` 全量指纹合并前后**逐字节一致** ✅；
  E2E 已跑，4 failed **全部为夹具/环境缺陷且逐条有实证**（客户「西门子」DB 实查 0 行 / 产品分类缺失致 Step1 禁用 / `PW_PRECISION_SEED_QUOTATION_NO` 未设），
  与本次改动**因果不可能相关**（本次零前端改动，而三条挂在前端、一条挂在 fixture 装配）—— 详见 §7.1
