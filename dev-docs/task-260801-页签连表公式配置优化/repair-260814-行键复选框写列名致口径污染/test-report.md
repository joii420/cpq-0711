...
| 任务 | 职责 | 状态 |
|---|---|---|
| `repair-260814-行键中英口径混存致连表可比判定失真`（`BL-0169`） | **数据清理** —— 洗干净已被污染的 7 个组件（`COMP-0232~0234` / `COMP-0241~0244`） | ✅ 已交付 |
| `repair-260814-行键复选框写列名致口径污染`（`BL-0170`，本任务） | **根因修复** —— 让污染不再产生，并让存量残留在下次保存时自愈 | ✅ 已交付 |
