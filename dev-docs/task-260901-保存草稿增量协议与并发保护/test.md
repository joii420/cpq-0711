...
| B-1c 有条件置 NULL | 改回无条件置 NULL（逃生阀 `CPQ_SAVEDRAFT_CONDITIONAL_INVALIDATE=false`） | **T-9 变红**（不是 T-7）<br>📌 **2026-09-01 更正**：原写「T-7 变红（空值行数变 1845）」是**全量协议时代**的预期 —— 那时 payload 装整单，无条件置 NULL 会波及 1845 行。**增量协议下 payload 只含变化行，无条件置 NULL 也只波及那一行**，T-7 的「恰好 1 行」照样成立、根本测不出来。真正守住 B-1c 的是 **T-9**：它发的是「语义未变」的行，本该保住卡片值，破坏后就会被清空。<br>⚠️ **教训**：证伪清单是在闸门 A（协议尚未实现）时写的，协议落地后**某些期望值必须按实际协议重新校准** —— 否则「破坏了却没变红」会被误判成用例无分辨力，而实际是期望值指错了对象。 |
