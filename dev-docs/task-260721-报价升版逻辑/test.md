...
| **DAG / 主子同步 fixture** | 复用姊妹任务（`task-260721-报价侧树状结构与页签类型属性`）附录 A 的黄金 fixture：根料号 `3120018220` → 子 `2120011658`/`2120011659` → 两者均挂子 `3110520789`（DAG 重复子件，2 occurrence）→ 其子 `2101110225`。用于 UT-B5-DAG / UT-B5-MC 主子同步与去重验证 |
...
- backtask B4/B5 明确"读取树任务定义的系统列（`__v6_id` 注入点复用树任务的物化管线 / `deleted_tree_nodes` / 手工叶子标记）"，本任务与「task-260721-报价侧树状结构与页签类型属性」是两个并行开发的姊妹任务（各自独立 worktree/分支）。
