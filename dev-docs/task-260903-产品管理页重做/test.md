...
> 🚩🚩 **2026-09-03 方案变更（用户裁决）—— 本节的「走导入器」口径作废，改为直接 INSERT。**
>
> **变更原因**：测试代理干跑 Phase-1 校验，**33 条失败 ⇒ 整份拒收**（19×主数据不存在 · 11×必填项为空 · 3×客户编号未建档）。这是**旧表现网数据的真实状态**，不是样例缺陷。
>
> **判据**：本任务 17 条 AC **无一条验证导入链路**（那是 `task-260902` 的 AC-11/AC-36）。导入只是取数手段。
>
> **主线已实证可行**（事务内试插后 ROLLBACK，共享库零残留）：`material_name` 可空 · `ds_quote_customer_part` 仅 2 列 NOT NULL · **`ds_quote_*` 零外键约束** · LEFT JOIN 未建档客户实测**行保留名为 NULL**。
>
> ### 变更后的执行顺序（**顺序不可调**）
>
> | 步 | 动作 | 为什么是这个顺序 |
> |---|---|---|
> | **①** | **先跑 `E2E-13`（AC-13 空态）** | `ds_quote_material` 当前实测 **0 行**，是**真实空表**状态。灌了数据就再也回不去（清表是 §3.2 红线）。**错过这一步 AC-13 就永久失去可验窗口** |
> | ② | 直接 `INSERT` 样例数据进 `ds_quote_*` | 不走 `POST /dataset/quote/import` |
> | ③ | 造 `S-3120014539` 物料BOM 的**第二个版本**（AC-10 修订） | v2 为当前版、v1 整组移入 `ds_quote_material_bom_history`，与 `VersionedGroupWriter` 升版语义一致 |
> | ④ | **替换**（不是新增）某一行品名为 ≥60 字符合成值（AC-15 修订） | 新增会让总数变 43 与 AC-4 的 42 冲突 |
> | ⑤ | 跑其余 E2E | |
>
> ### 清理纪律（🚨 §3.2 红线相关）
>
> ✅ 清理条件必须落在**我方料号集合**上：`WHERE material_no IN (...)`
> 🚫 **严禁反向条件**（`WHERE material_no NOT LIKE 'TEST-DS-%'`）—— 会带走 `task-260902` 及他人的数据
> 🚫 仍然严禁 `TRUNCATE` / 无 `WHERE` 的 `DELETE`
>
> ### 下面 §1.1~§1.6 的「行数与合成值规则」**继续有效**，只有「用导入器写入」这一点作废。
