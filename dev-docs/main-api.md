...
| GET | `/{reviewId}` | 审核详情 | 响应 `ReviewDetailDTO`：`reviewId,customerNo,materialNo,materialName,currentVersionNo,targetVersionNo,budgetStatus,reviewStatus,elementChanges(ElementChange[]: elementCode,elementName,matchedRule,previousPrice,currentPrice,changeRate,**usageQty**,**unitPriceImpact**,noPrice,inheritedFromPrevious),**elementImpactTotal**,templateSeriesId,templateSeriesName,comparisonColumns(ColumnResult[]: columnId,label,threshold,quoteCurrent,...),quotations(QuotationRef[]: quotationId,quotationNo,createdAt,status,isBasis,quoteSubtotalCurrent,**quoteSubtotalAdjusted**,**adjustedComputed**,comparisonViewUrl)`<br>🔧 repair-0807：① `quoteSubtotalAdjusted` 此前**后端零赋值点**恒 `null`，现对 `isBasis=true` 的那一行跑隔离子事务 dryRun 取真值，其余行恒 `null` 且 `adjustedComputed=false`（前端据此渲染「未试算」而非「—」）；② `usageQty`/`unitPriceImpact` 此前恒 `null`，现按「单元素取整体 Δ、多元素逐个跑只改该元素的 dryRun」算出，`usageQty = unitPriceImpact ÷ (currentPrice − previousPrice)`，分母为 0 或任一侧 null 时返 `null`（**不得返 0 冒充无用量**）；③ `elementImpactTotal` 口径由「`col-default` 的 `diffAdjusted`（= 报价·调整后 − 核价·调整后）」**更正为**「判断依据单 dryRun 后 subtotal − 现 subtotal」。<br>⏱️ 含 dryRun：单元素版本 ≤3s，每多一个变价元素约 +1.6s。<br>🔒 判断依据单缺失/已删/dryRun 失败 → **降级 200 + 全部留白**（`adjustedComputed=false`、`elementImpactTotal=0`），**不得 500**（`basis_quotation_id` 无 FK 约束，报价单删除后会留悬空指针，现网已有真实样本） |
...
| 方法 | 路径 | 说明 | 请求/响应要点 |
|------|------|------|----------------|
| GET | `/jobs` | 任务列表 | 查询参数：`page,size,status,customerNo`（🔒 两个过滤条件都可选，Panache `Parameters` 不接受未出现在查询串里的 key，故按"是否有任何条件"分叉查询，不用 `1=1`+占位 key）；响应 `PageResult<JobDTO>`：`jobId,customerNo,versionNo,triggeredBy,triggeredAt,status,total,success,failed,conflict,stale,**skipped**,finishedAt,notified`<br>🔧 repair-0807 新增 `skipped`（wire 侧**无 `Count` 后缀**，与既有 `total/success/...` 同风格；实体侧是 `skippedCount`，映射 `dto.skipped = j.skippedCount`）。**批次状态口径同步变更**：只要存在 `SKIPPED` 项，批次即为 `PARTIAL`，**不再报 `SUCCESS`** —— 本次缺陷的传播路径正是"32/32 全成功"骗过财务，而其中至少 1 张单一个字节都没改 |
| GET | `/jobs/{jobId}` | 任务详情 | 不存在 404；响应 `JobDTO`（同上，含 `skipped`） |
| GET | `/jobs/{jobId}/items` | 任务明细行 | 查询参数：`page,size(默认 50),status`；响应 `PageResult<JobItemDTO>`：`itemId,quotationId,quotationNo,materialNo,lineItemId,status,errorCode,errorMessage,diffValue,retryCount,updatedAt`<br>🔧 repair-0807：`status` 枚举**新增 `SKIPPED`**（该单未被更新且重试无意义）。`SKIPPED` 的 `errorCode` **保持 `null`**（本模块语义是「`errorCode` 非空 = 非成功态需人工处理」，占用它会把屏 7 的可重试判定带偏），原因写在 `errorMessage`，且区分「冻结结构已补建但仍无价格承载组件」与「冻结结构缺失且补建失败」两态 |
| POST | `/jobs/{jobId}/retry` | 批量重试该批次全部 `FAILED`+`CONFLICT`（不含 `STALE`、**不含 `SKIPPED`**） | 异步 `retryJob`；202 空体。🔒 `loadWaitingItems` 只捞 `WAITING`/`CONFLICT`，`SKIPPED` 项**不会被重跑**（期望行为，repair-0807 AC-18） |
