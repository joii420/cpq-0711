...
| AC-27 | B-9·UPDATE 归零 | B-9 | T-21 |
| AC-28 | B-9·真变化仍写 | B-9 | T-21 |
| AC-29 | B-9·null 语义 | B-9 | T-21 |
| AC-30 | B-10·并发不再 409 | B-10 | T-22 |
| AC-31 | B-10·约束仍生效 | B-10 | T-22 |
| AC-32 | B-10·无冲突时等价 | B-10 | T-23 |
| AC-33 | B-10·物化侧不破坏 | B-10 | T-23 |
...
T-01→AC-1,2 ｜ T-02→AC-3,4 ｜ T-03→AC-5 ｜ T-04→AC-6 ｜ T-05→AC-7 ｜ T-06→AC-8 ｜ T-07→AC-9 ｜ T-08→AC-10 ｜ T-09→AC-11 ｜ T-09b→AC-11b ｜ T-12→AC-15 ｜ T-13→AC-16 ｜ T-14→AC-17 ｜ T-15→AC-18,19 ｜ T-16→AC-20 ｜ T-17→AC-21 ｜ T-18→AC-22,23 ｜ T-19→AC-24,25 ｜ T-20→AC-26 ｜ T-21→AC-27,28,29 ｜ T-22→AC-30,31 ｜ T-23→AC-32,33。**无孤儿用例。**
...
| **T-21** | 手工+SQL日志 | 开 `org.hibernate.SQL=DEBUG`，用真实 payload 保存；再改一行 subtotal 数值保存；再把某行 subtotal 置 null 保存 | ① `update quotation_line_item` 条数 = 0、端到端 < 30s；② 改值那行确实被更新；③ null 时行为不变 |

| **T-22** | 集成测试 | 模拟并发：saveDraft 事务内，用独立事务插入同 `(line_item_id, component_id)` 行后再 flush；另用裸 SQL 试插重复行 | ① 保存返回 200 且最终值 = payload 的值；② 裸 SQL 插重复仍被约束拒绝 |
| **T-23** | 手工+SQL | ① 无并发正常保存，与 B-10 前逐位对比；② 建单后立即打开保存，等物化也完成 | ① 五列逐位等价、耗时不劣化超 20%；② 两者都成功，最终行数 = 行数 × 页签数，无重复无缺失 |
