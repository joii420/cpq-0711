...
| 6 | **导入校验** | **严格校验 + 跳过 + 报告**：合规行落库，不合规行跳过并在返回报告里逐条列明原因。**任一行不合规不影响整单**。⚠️**2026-07-09 修订(推翻原"纯数字跳过")**：经实测，304/316/301/430/191/206/223/258/721 等数字是**合法组成项(不锈钢牌号等)**，是 `Cu/304`、`301/Cu/301` 等 65 个复合材质的主含量层——**不得当脏数据跳过**。真正的脏数据闸门只保留**材质级 Σ含量≈1** 校验。基线：**253 落库 + 1 跳(WZHF26-25，Σ=1.44 真错)**。 |
...
覆盖 Excel 37 种「元素」中可命名的约 28 种。数字牌号（304/316/301/430/191/206/223/258/721）**是合法组成项**，导入时按符号 upsert 进 `element`、`element_name` 回退=符号；**可选**在 seed 补中文名（304→"304不锈钢"、316→"316不锈钢"、301→"301不锈钢"、430→"430不锈钢"；191/206/223/258/721 含义不明暂留符号）：
...
- `元素名称` 非空（**注意：不再校验"非纯数字"——数字牌号是合法组成项，2026-07-09 修订**。删除原 `PURE_NUMBER ^\d+$` 跳过逻辑）
- `含量` 可解析为数值、且 `0 < 含量 ≤ 1`（否则 reason=`含量非法`）
- **去重**：同材质编号内同元素名称的重复行去重（末值胜，保插入序）——文件含 17 个材质有重复元素行（部分是两套配比版本，如 00016/00018），去重后各自 Σ=1；若不去重 material_recipe_element 会灌成 Σ=200。（现有实现已具备，保留）
...
**验收**：见 api.md 示例；用真实 `材质库.xlsx` 跑，`materialsUpserted=253`、`skipped` 恰含 1 条（`WZHF26-25` Σ=1.44 真错，**不含**任何数字牌号）、`durationMs<3000`。
...
1. **校验单测**：构造含①含量>1②某材质Σ≠1③材质无编号 的小 workbook（POI 内存生成），断言各自被跳过且 reason 正确、合规材质正常入库。**并加正向断言**：含数字牌号(如 `Cu/304`=Cu+304)的材质**必须正常入库**（304 作合法元素、Σ=1），不得被跳。
...
- [ ] 真实 `材质库.xlsx` 导入：报告 `materialsUpserted=253`、`durationMs<3000`、`skipped` 恰 1 条(WZHF26-25)、数字牌号材质(Cu/304 等)已入库
