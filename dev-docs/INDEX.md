...
| 　 └ `repair-0811-输入值原样与结果精度分层/` | 修正 task-0810 动态数值分类：INPUT_TEXT/INPUT_NUMBER 保留当前单元格原值；公式过程 12 位、结果 9 位；产品卡片小计与报价总额拆为独立精度变量，预留系统参数入口；闭合 `BL-0162` | 🚧 **闸门 A 已确认，开发中** | `DecimalRequestValidator.java` `PrecisionPolicy.java` `losslessJson.ts` `precision.ts` |
