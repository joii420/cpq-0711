...
| `task-0806-价格调整更新任务性能优化` | 📐 **刚立项**（需求文档 + 三件套齐） | 更新任务 58s(18项) → 目标 22~25s。`BL-0140`；**分组键必须含取价基准日**（实测反例：按 job 整批会写坏 5/18 张单的核价卡片）；**闸门 A 未过**，待用户确认后开 worktree |
...
| 文件 | 命中任务数 | 历史任务 |
|---|---|---|
| **`QuotationStep2.tsx`** | **24 / 38** 🔥 | 0708导入 · 0712核价展示 · 0712选配 · 0713版本选择 · 0715UI · 0717比对 · 0721树 · 0721升版 · 0722元素价格 · 0723清洗 · 0725空白 · 0728版式 · 0729客户价格 · 0801精度 · 0801连表 · 0803父子公式 · **0806编辑链路对账** · **0806模板冻结** · 删除行重构 · repair-0727回填 · repair-0729copy · repair-0803BL0098 · repair-0803SUM · rule-0724 |
| `ReadonlyProductCard.tsx` | 13 | 0712核价展示 · 0713版本选择 · 0721树 · 0722元素价格 · 0725空白 · 0728版式 · 0729客户价格 · 0801精度 · 0801连表 · 0803父子公式 · 删除行重构 · repair-0727回填 · repair-0803BL0098 |
| `CardSnapshotService.java` | 13 | 0712核价展示 · 0721树 · 0725空白 · 0728版式 · 0729客户价格 · 0801精度 · **0806模板冻结** · 0803父子公式 · 删除行重构 · repair-0729copy · repair-0803BL0098 · repair-0803SUM · repair-0803公式BUG |
| `QuotationWizard.tsx` | 11 | 0712核价展示 · 0712选配 · 0722元素价格 · 0723清洗 · 0725空白 · 0729客户价格 · 0729分类 · 0801精度 · 0801连表 · repair-0729copy · rule-0724 |
| `useDriverExpansions.ts` | 9 | 0713版本选择 · 0722元素价格 · 0725空白 · 0728版式 · 0801连表 · 删除行重构 · repair-0727回填 · repair-0803BL0098 · rule-0724 |
...
| `ComponentService.java` | 8 | 0721树 · 0723清洗 · 0729客户价格 · 0803父子公式 · **0806模板冻结** · repair-0803BL0098 · repair-0803SUM · repair-0803公式BUG |
| `ComponentCell.tsx` | 6 | 0729客户价格 · 0801精度 · 0803父子公式 · 删除行重构 · repair-0803BL0098 · rule-0724 |
| `formulaEngine.ts` | 6 | 0729客户价格 · 0801精度 · 0803父子公式 · **0806编辑链路对账** · repair-0803SUM · rule-0724 |
| `SqlViewExecutor.java` | 4 | 0722元素价格 · 0725空白 · 0729客户价格 · repair-0727回填 |

⚠️ **`QuotationStep2.tsx` 是全仓最危险的文件**：24 个任务在它身上叠了协议。它同时受 AP-31 / AP-37 / AP-38 / AP-44 / AP-50 / AP-51 / AP-54 约束，且属 `CLAUDE.md`「修改后强制自检」第 5 条的 **E2E 强制清单**。改它之前请连读：`task-0721树` + `task-删除行删错架构重构` + `repair-0727-报价单报价侧删除行BUG`（行身份四套口径）。
...
| 目录 | 一句话 | 状态 | 主战场文件 |
|---|---|---|---|
| `task-0806-报价编辑链路优化与前后端对账/` | 把后端从**显示权威降级为校验器**：DRAFT 行内走前端引擎（与列小计同源、零等待），后端异步照算做**对账**，不一致亮标记 + **禁提交**；其上叠异步/懒物化/缓存三级优化。实测一格编辑 900ms 分布：整行物化 `row_data` 356ms(45%) > 整卡重算 120ms > 小计+整单表头 78ms | 📐 **闸门 A 就绪**：需求/front/back/api 四件齐，D1~D11 已拍板，**待用户确认后开工**（`BL-0137` P0） | `QuotationStep2.tsx` `ReadonlyProductCard.tsx` `CardSnapshotService.java`（`editCardValue`/`materializeWholeLineRowData`） |
| `task-0806-价格调整更新任务性能优化/` | 价格调整通过后「更新任务」提速：18 项 job 实测 58s（3.22s/项）→ 目标 22~25s。热点 = 核价树 `render()` 43.5% + S0 口径守卫 14.3%。**实测否定两条路**：不是 N+1（一次 render 恰好 17 条 SQL、零重复）、不能上线程池（2026-06-22 同款设计已 revert）。真正浪费 = `refreshCostingCardValuesForLine` 逐项调 `render(List.of(li))` → 18 项发 306 条 SQL 而非 17 条 | 📐 **需求文档 + fronttask/backtask/api 三件套定稿，闸门 A 待确认**（`BL-0140` P1） | `BomTreeRenderService.java` `CardSnapshotService.java`（`refreshCostingCardValuesForLine`）`PriceAdjustJobExecutionService.java` `MaterialVersionUpgradeService.java` |
