...
| └ `task-0810-公式计算12位显示9位/` | 精度契约升级：后端 BigDecimal、前端 Decimal.js Decimal，精度链禁止 Double/JS number；公式节点/快照/持久化保留 12 位，最终显示最多 9 位；decimal string 无损传输；扩 21 个计算金额列；吸收 `BL-0159/0160` | ✅ **验收通过，待合并 master**（2026-08-11；exact-5 5/5；任务关键 37/37；TC-077 环境 BLOCKED） | `PrecisionPolicy.java` `FormulaCalculator.java` `precision.ts` `formulaEngine.ts` |
