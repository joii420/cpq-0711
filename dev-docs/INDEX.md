...
| └ `repair-260829-异步物化事务上下文缺失/` | 🔵 **立项**（2026-08-29）· ①~④ 已写完，**停在闸门 A0 等方案裁决** | **导入建单后卡片全空、金额全 0 —— 异步物化丢事务上下文，两层静默 catch 吞掉**。现象：1845 行导入建单后打开报价单，各页签全空、小计与总价 ¥0；`quotation_line_component_data` **0 行**。🚨 **没有任何「失败」外观**：HTTP 200、明细行 1845 齐全、`[create-quotation-timing]` 四步全绿（`①447 ②282 ③2767 ④3193 总计=6689ms`）、**零条 ERROR**。<br>🔑 **根因**：`ca243876`（D-5）把 `materialize()` 改 `managedExecutor.runAsync` fire-and-forget 后，HTTP 请求随即结束、CDI request context 销毁；① 步链路上 `ConfigureSnapshotService.loadComponentsSnapshot`（`:1068`）标 **`@Transactional(TxType.SUPPORTS)`**（不开事务、只借调用方的）→ 拿不到 `EntityManager` → 抛异常 → **两层静默 catch**（内部 catch 返 null + `snapshotQuotation:146` 顶层 catch 只 warn 不上抛）→ ① 步写 0 行 → ②③④ 在空数据上「成功」跑完。**鉴别点**：同序列 `loadCustomerId`/`loadDriverComponents`/`loadCustomerTemplateId` 全成功（都是 `REQUIRES_NEW`），**只有 `SUPPORTS` 那个失效**。<br>🔬 **还原实验六组**（临时探针，纯读，已删）：A/B/C/D（请求存活）全 `present(5 tabs)`；**E**（fire-and-forget 逐位复现）→ **`null`**；**F**（E + `QuarkusTransaction.run`）→ `present(5 tabs)`。E 触发的异常与生产 0204/0202 **逐字同源**。A/B/C/D 全绿本身说明：缺陷**只在原请求已结束时**触发 —— 这就是它穿过全部既有测试的原因（测试里请求总是活着）。<br>📊 生产日志 **3 条 timing = 3 条降级 WARN = 3 张空单，零反例**；本批 24 张单中 **19 张 comp_data=0**。<br>🚫 **三个假设已证伪，勿重走**：① 用户原判「`is_current` 被 FLIP 翻错」—— 44280 行新行 100% 带 `pending_quotation_id`，是 `task-0721 B2` pending 他单隔离**不变量**，**27 万+ 行一行都不许动**（刷 `is_current=true` 会撞 `uq_*` 并让 A 单数据串进 B 单）；② 「08-26 导入侧有改动」—— pending 行 **07-27** 就在产生，变的是量级与入口；③ 主线上一轮推测的「ThreadLocal 跨线程丢失」—— `snapshotQuotation`→`snapshotLines` 同线程直调，`QuotePendingScope.open()` 正常执行。<br>⚠️ 未坐实：08-26/08-27 的 14 张空单在 `ca243876` 之前，成因待核；5 张非空单的数据来源（`0180` 已知是事后 `ensure-card-values` 补的）待逐单核 |
| `task-260721-.../repair-260829-保存草稿树页签校验N+1` | 🔵 **开发中**（闸门 A0 两轮 + 闸门 A 已过 2026-08-29）· 分支 `fix/repair-260829-savedraft-tabmeta-n1` · 立项提交 `81ad23ee` | **1845 行单 `PUT /draft` 必然超时** —— 前端 30s `ERR_ABORTED`，后端 **60s Narayana reaper 强杀**，60 秒只跑完 **200/1845 行**，事务整体回滚 ⇒ 该单**永远存不进去**（现网 **22 张 ≥280 行的单**同此）。<br>🔑 **根因（四轮埋点实测）**：**①** 主循环内每行每页签调 `QuotationTreeService.assertCanAddRowsToRestrictedTab`，其 `loadSingleComponentTabMeta` **每次重查整单恒定的模板元数据** → **9,225 次调用 / 18,450 条跨网 SQL，应为 1 次**（占 **82.6%**，≈325 s）；**②** per-line `em.flush()` 1,845 次（≈57 s）。真正写库的部分只占 **0.15%**。<br>引入者 `8c084048`(2026-07-21 B8 真接线)，`task-260806` 冻结改造把单次成本从 1 条查询涨到「查询+findById+解析 10KB jsonb」但 **N+1 结构原样保留**。<br>📌 **本期范围**：B-1 元数据整单取一次 · B-2 flush 外提 · **B-6 `componentData` 改记录级 UPSERT**（`snapshot_rows`/`deleted_row_keys` 不碰，省「删了又写回」的 5.8 MB）· B-7 迁移清 6 行重复 + 加 `uq_qlcd_line_component` · F-1 前端 axios 30→60s。**21 条 AC · 19 用例 · 双向覆盖闭合**。<br>🚫 **B-3（建单物化静默失败）已撤出**——与并发会话 `task-260825/repair-260829-异步物化事务上下文缺失` 撞车，对方根因更准（CDI 传播竞态 vs 我方「无 JTA 事务」）且已在实现期。<br>🔬 **三条推算被自己的实验证伪，勿重走**：① `@DynamicUpdate` 阻断 UPDATE 合批是 flush 主因 → 假（D-1 批量化仅值 **47 ms**，拿掉后 flush 反升）；② 修好建单物化会让 saveDraft 变慢 → 假（续存 **21~35 s** 反而快于首存 **40.6 s**，因 S2 从 14.1 s 掉到 1.7 s）；③ 调 `statement-batch-size` 能解 → 假（batch=1 时 >55 s 跑不完 ⇒ 批处理**已生效**，瓶颈在 8.8 MB jsonb 的序列化与解析）。<br>遗留 [[BL-0196]]~[[BL-0199]] |
...
| `fix/repair-260829-async-tx-context` | `.claude/worktrees/repair-260829-async-tx-context` | **0 笔**（建于 `161b06db`） | 🔵 **开发中** —— `repair-260829-异步物化事务上下文缺失`，闸门 A 已放行（2026-08-29）。B-1 是闸门：先证伪方案丙的假设，证伪则回落甲 |
