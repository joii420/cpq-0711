...
| 组件管理改页签类型报 **「该组件已被 N 处核价(COSTING)模板引用，不能设为 BOM 树页签」** | `task-0806-模板发布全量冻结/repair-0814-发布冻结后tabType护栏误拦`（护栏判定**不过滤模板 status**；0806 冻结后前提已消失 ⇒ 现网 22 个组件 100% 误拦。**但 DRAFT 核价模板 + 重新发布两条残余真风险仍在，不能一删了之**） |
| 连表公式抽屉里**行键显示成「中文 + 英文列名」混在一起** / 某源页签的明细 chip **全被置灰**只能引「(总计)」 | `task-0801-页签连表公式配置优化/repair-0814-行键中英口径混存致连表可比判定失真`（`row_key_fields` 混了「字段名」与「driver 列名」两套命名空间 → 可比判定 `isSubset` 集合虚胖 → 本该可比的页签对被判不可比。**口径以 `rule-0724 §2.4 C3`「存字段名」为准**；UI 复选框只读写列名是持续污染源 → `BL-0170`） |
| **详情页/预览**某列比**编辑页**多一列或少一列（如报价侧 BOM 树多出「版本」列） | `task-0721-报价侧树状结构与页签类型属性/repair-0814-详情页报价树多出版本列`（AP-50：`QuotationStep2.tsx` 与 `ReadonlyProductCard.tsx` 各存一份渲染副本，历史裁决常只落地一半。**先比对两文件同名渲染点的判据是否一致**，再查数据） |
...
| └ `repair-0814-详情页报价树多出版本列/` | 「报价侧不出版本列」裁决（`7fadf5e8`）**只落地了编辑页**，只读页 `ReadonlyProductCard.tsx` 一行未动 → 详情页恒多一列（AP-50 同族）。修法＝5 处补 `isCosting` 闸门 + 顺带补 `QuotationStep2` tfoot 合计行占位（`7fadf5e8` 漏改的第 4 处）。**核价侧不变由等价性证明**（`X → {isCosting && X}`），不必跑数值 A/B | ✅ **已交付合 master `ada23fd0`**（`BL-0173`；AC-1~8 全达成；还原实验复现故障态；遗留 [[BL-0174]] 系统列口径抽共享） | `ReadonlyProductCard.tsx` `QuotationStep2.tsx` `quotation-bom-tree.spec.ts` |
