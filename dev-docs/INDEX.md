...
| └ `repair-260828-建单物化逐行DB往返/` | **建单物化仍需 106 秒 —— 逐行 DB 往返三根因**。现象：导入建单 1845 行，抽屉「正在计算卡片值…」进度条卡 100 秒+（无报错/不丢单/无失败批，纯慢）。🟠 **停在闸门 A0，等方案裁决**（①~④ 已写，⑤⑥ 待补）。 | 🟠 立项中 | ⚠️ **归属存疑（2026-08-28 主线核对）**：目录物理位置在本任务下，但文档 `问题说明.md` 的「返修对象」写的是本任务、引用的却是 **merge `76c4b0ab`** —— 那是 `task-260825-大单量导入建单性能` 的合并（本任务合并是 `bef24579`）。内容（建单物化 / `CreateQuotationMaterializer`）也属那条线。**疑为并发会话建错父目录**，未擅自移动（改名会打断既有引用），待其作者确认。 |
