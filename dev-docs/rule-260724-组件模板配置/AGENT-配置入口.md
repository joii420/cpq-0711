...
> 🚨 **无增量表陷阱（2026-08-12 task-0812 起，核价侧）**：**表里有数据 ≠ 这张表还在收新数据**。核价导入停用了 5 个 Sheet（24→20→19），以下几处从此只剩存量：
> `unit_price[price_type='ELEMENT'/'MATERIAL_PRICE']`（元素/材料核价价格表停用）· `material_version_mgmt` · `exchange_rate_v6`（P03 在途）· `material_customer_map`（核价侧不再写，仅报价侧 `Q02` 维护）· `material_master` 的**品名/规格/尺寸/旧料号**对「只作父件的销售料号」无来源。
> 绑上去**当场看着正常**（老料号有存量值），换个新料号就整列空且**不报错**。上面 §3.5「主件」行里的 `mcm.*` 各列即属此列——核价侧本就不用 `:customerCode`，若某客户只导核价文件，其 `mcm` 行根本不存在。
> **完整清单 + 三条硬规则 + 「取用的计算版本」等不入库列见 `核价侧.md §导入口径收敛与数据来源边界`。**
...
**为什么要显式配、不靠解析 SQL**：升版要精确改「元素单价」这一个键、并靠「元素列」的值定位到 Ag/Cu 那些行。原方案是运行期正则解析视图 SQL 反推，**2026-07-29 推翻** —— `COALESCE(cep.unit_price,0) AS 单价`、`ON ebi.component_no = cep.element_code`（左右写反）、多条件 JOIN、带引号列名都会让正则抓空或抓错，**且失败全是静默的**（价格纹丝不动 / 改到别的字段，都不报错）。详见 `dev-docs/task-260729-客户价格调整策略和价格版本/需求说明.md §11.15.3`。
