...
| AC-21 | 单点 | B-15 | TC-21 | 双组字段 + SQL 含 LEFT JOIN + 12 行预览 |
| AC-22 | **边界** | B-13, B-15 | TC-22 | 3 个存量主件组件改动前后行数/列值比对 |

**三类覆盖齐全**：单点 10 条（AC-1~9, 21）· 序列 2 条（AC-10, 11）· 边界 6 条（AC-12~16, 22）· 反向 5 条（AC-8, 17, 18, 19, 20）。
...
| 用例 | 取数 | 现网可用量 |
|---|---|---|
| TC-04 材质 | `material_recipe.code` 命中的投入料号 | **6** 个 |
| TC-05 零件/外购件 | `ds_quote_material` 且 `material_type` 非空 | **2** 个 |
| TC-12 空值默认零件 | `ds_quote_material` 且 `material_type IS NULL` | **29** 个 |
| TC-06 不存在 | 两表都不在的投入料号 | **1** 个 |

> ⚠️ TC-05 只有 2 个可用料号，**且两个都是同一种类型时该用例会空跑**。执行前先跑：
