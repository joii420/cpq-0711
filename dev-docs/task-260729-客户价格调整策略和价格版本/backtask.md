...
- ⚠️ **必须同步更新 `dev-docs/rule-260724-组件模板配置/3-SQL视图.md` 与 `报价侧.md` 的配方**（加 JOIN 键铁律 + 别名 `cep` + 三个角色字段必配）—— 那是配置 Agent 的唯一入口，不改配方 = 新配的模板必然走回旧路（风险表「迁移后的漏配」）。
...
| 7 | `dev-docs/rule-260724-组件模板配置/3-SQL视图.md` + `报价侧.md` 配方更新（JOIN 键铁律 + 别名 `cep` + 三角色字段） | B2 风险表 |
