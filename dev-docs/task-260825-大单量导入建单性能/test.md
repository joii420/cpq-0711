...
| **AC-4**（INPUT 键盖回不回退；非 INPUT 列确实刷新） | **T-2** + 亲验-3 | 单元 + **真机序列** | T-2：`OverlayExistingInputKeysTest` 6 例通过数；亲验-3：UI 前后截图 + 库内 `row_data` 前后对照。归档 `证据/` |
...
| 🔴 **AC-11**（③ 单事务 ≤20s / 单批耗时不随 N 增长 / 无 ARJUNA） | **T-8** + 亲验-7 | 集成 + **真机** | T-8：单批耗时与批数断言；亲验-7：分批埋点原始输出，**与改动前 `③=58679ms` 直接对照**。归档 `证据/` |
| 🔴 **AC-12**（部分失败可恢复 · 已提交批保留 · 重跑只补未完成行） | **T-9** | 集成 | 构造第 k 批失败后的库内状态 + 重跑后的补算行数 |

**反向覆盖（每个测试都指得回 AC）**：
T-1→AC-3 ｜ T-2→AC-4 ｜ T-3→AC-5 ｜ T-4→AC-6 ｜ T-5→AC-7 ｜ T-6→AC-8 ｜ T-7→AC-10 ｜
T-8→AC-11 ｜ T-9→AC-12 ｜
亲验-1→AC-1 ｜ 亲验-2→AC-2 ｜ 亲验-3→AC-4,AC-10 ｜ 亲验-4→AC-6 ｜ 亲验-5→AC-7 ｜ 亲验-6→AC-9 ｜ 亲验-7→AC-11。
**无孤儿测试，无未覆盖 AC。**

> 🔴 **2026-08-25 交付缺口（如实登记，闸门 B 必须显式汇报）**：
> 首轮 `cpq-tester` 交付了 **T-1（含还原实验，RED 已证）/ T-2 / T-6（部分）**，
> **T-3 / T-4 / T-7 未写、T-5 不完整** → **AC-5 / AC-6 / AC-10 目前零自动化覆盖**。
> 卡点是这三项需要内部新方法名，而测试方仅有 AC、被禁读实现且未向主线索取。
> **补齐方式**：主线在补派时**直接提供所需方法签名**（签名是契约不是实现细节，给签名不破坏「测试不从实现派生」）。
...
- `OverlayExistingInputKeysTest` 现有 **6 例**，**不得修改其断言**，必须继续全绿。
...
### T-8 · 事务分批（AC-11 · D-4 的核心护栏）

- 断言 ③ 的**单个事务**最长耗时 ≤ 20,000ms；批数 = ⌈N/chunk⌉；**单批耗时不随 N 增长**（两组 N 对照）。
- 🔒 **同时复核 `quotation_view_structure` 计数仍 ≤4**（不是 ≤4×批数）——
  防 B-12 的自伤：prefetch 若被拆进批事务，D-3 就被打回原形。**这条必须和 T-1 一起跑。**
- 🚨 **还原实验**：把分批改回单事务，T-8 必须变红。

### T-9 · 部分失败可恢复（AC-12）

- 构造第 k 批失败：断言已提交批的卡片值**保留**、未完成行仍 NULL、
  重跑时 `IS NULL` 谓词只补未完成行（打印补算行数证明「只补未完成」而非「全量重算」）、
  接口仍返回 `cardValuesReady=false` + `warnings`。
...
| 测试 | 守什么 | 备注 |
|---|---|---|
| `OverlayExistingInputKeysTest`（**6 例**） | E-4 INPUT 键盖回语义 | 断言不许改 |
