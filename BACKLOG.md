...
### [BL-0162] task-0810 精度校验误伤 INPUT_NUMBER，保存草稿被阻断
- **优先级**：P0
- **来源**：2026-08-11 用户报保存草稿 `rowData[0].项次 must be a decimal string`
- **状态**：[~] 闸门 A 已确认，开发中
- **登记日期**：2026-08-11
- **背景**：task-0810 对动态 JSON number 做全局拒绝，没有依据组件字段 `field_type` 区分输入值与公式结果；`项次:1` 因物化字段名未在白名单中被误判。前端 `losslessJson.ts` 的 `项次/_项次` 白名单还存在乱码。
- **范围**：输入字段原值保存/重开/参与运算；公式过程 12 位、结果 9 位；产品卡片小计和报价总额使用两个独立精度变量并预留系统参数入口。
- **依赖**：无；无数据库结构变化。
- **验收要点**：真实草稿保存 `项次:1` 成功；`1.2300`逐字保持；公式 number 仍拒绝；小计/总额 scale 可独立测试。
