...
### [BL-0129] 换模板复制后总价停在兜底 0，直到一次卡片值失效才纠正
- **优先级**：P2
- **来源**：方向 3 验收 `D3-06`（2026-08-06）
- **状态**：待开发
- **说明**：`QuotationService:1546` 的 `sameTemplate` else 分支只写 `newLi.subtotal = ZERO`、未赋卡片值，**按读代码 `quote_card_values` 应为 NULL**；但实测复制出来的单**卡片值非空（8 个 tabs）** → `ensureCardValues` 的 `IS NULL` 谓词**不会选中它** → 收敛点不触发 → `li.subtotal` **停在兜底 `0`**，要等一次卡片值失效（saveDraft / 刷新基础数据）才被纠正
- **当前未暴露的原因**：验收时用的新模板真值恰好也是 `0.000000`，两者重合
- ⚠️ **换一个能算出非 0 的模板就会有一段窗口显示 0** —— 用户在这段窗口内看到的总价是错的
- **待查**：卡片值非空是从哪来的（复制路径的其他分支？触发器？）—— 与代码读到的不一致，需先查明再定修法
- **预估规模**：S（查明后）
