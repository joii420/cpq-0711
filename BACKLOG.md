...
- **登记日期**：2026-08-05
- **任务目录**：`dev-docs/repair-0803-BL0098-公式绑定改绑ID/repair-0805-行内公式列不随编辑重算/`（`需求文档.md` = 唯一权威口径；`问题分析报告.md` = 证据链）
- **根因（三段）**：
...
- **状态**：**阶段⓪① 已交付合 master**（merge `2942a4d8` / 实现 `286def1c`，2026-08-07）；阶段②③④⑤ 未开工
- **登记日期**：2026-08-06
...
- **预估规模**：M
- **验收要点**：与 [[BL-0140]] 同一套门禁 —— 四项落库产物逐字节全等、**必测 `c2915208`**（跨 6 建单日的已知反例）、连跑两次一致、单组件失败只落该 item。基线 MD5 直接复用 `dev-docs/task-0806-价格调整更新任务性能优化/baseline.md`（75 个 line item，由未改动代码算出）
