...
- **状态**：**阶段⓪① 已交付合 master**（merge `2942a4d8` / 实现 `286def1c`，2026-08-07）；**阶段③a 开发中**（分支 `feat/task-0806-lazy-rowdata`）；**阶段③b 已裁定不做** → [[BL-0154]]（D17）；阶段②④⑤ 未开工
- **登记日期**：2026-08-06
...
- **四阶段（顺序不可换，① 是后三步的安全网）**：
  ...
  3. ~~**W3 懒物化 `row_data`**（抄 `lazy-excel` 现成模式）→ **~356ms/次**~~ → **2026-08-07 D17 重新裁定**：真实可省 **294ms**（非 356ms），其中 **234ms 由 ③a 批量写拿到**（已交付），剩余 60ms 的懒物化转 [[BL-0154]] 不做
...
### [BL-0155] 🚨 19 个等价性护栏测试因基准单据消失而**全员失效**（静默跳过 / 计入 403 Errors）
...
### [BL-0154] `row_data` 懒物化（task-0806 阶段③b）—— **已实测裁定暂不做，长期观察**
- **优先级**：P2
