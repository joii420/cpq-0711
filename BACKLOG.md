...
- **范围**：
  1. **固化存量**：保存组件时，对无 `formula_name` 的 FORMULA 字段，把当前按位置/同名匹配到的公式名**写死**进
     `formula_name`（一次性迁移 + 保存时补齐），此后不再依赖位置；
  2. **UI 显式化**：组件管理的字段配置里，未显式绑定的 FORMULA 字段应展示「当前按位置匹配到：XXX」的提示，
     并引导用户确认；
  3. positional fallback 保留但降级为**仅兼容存量**（新建/编辑必须显式绑定）。
- **依赖**：无。改动触及 `FormulaCalculator` 与组件管理前端，**不触发 AP-44**（无 field_type 变动）
- **预估规模**：M
- **验收要点**：①存量组件全部补齐 `formula_name`；②调整公式顺序后，各字段算法不变；③新建 FORMULA 字段未选公式时有明确提示。
