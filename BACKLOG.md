...
- **状态**：✅ **已完成**（2026-08-14 合 master `33deb4a7`；文档 `dev-docs/task-0801-页签连表公式配置优化/repair-0814-行键复选框写列名致口径污染/`，AC-1~AC-11 全绿含还原实验）
...
- **状态**：✅ **已交付合 master `c2f485d5`**（2026-08-14；闸门 A 当日通过，裁决 = 方案 D 修订版 + 原 `BL-0169`（现 [[BL-0172]]）并入本期）。
  **⏳ 闸门 B 待用户真机验收**（TC-24 `COMP-0233` 改 BOM / TC-25 重新发布核价模板-简易）。
  文档：`dev-docs/task-0806-模板发布全量冻结/repair-0814-发布冻结后tabType护栏误拦/`（问题说明 + backtask + api + fronttask + test + test-report）
...
- **状态**：✅ **已交付合 master `b64742fc`**（2026-08-13；`dev-docs/task-0813-基础资料数值列扩至12位小数/`）
...
- **状态**：待开发
- **登记日期**：2026-08-07
- **背景**：`dev-docs/task-0806-报价编辑链路优化与前后端对账/api.md` 声称 lineItem 不存在返 `404`，实测统一走 `400`（既有行为，该文件自述"后端零改动"，纯粹是文档写超前）。
...
- **📐 方案已就绪**：`dev-docs/task-0801-公式计算精度优化/repair-0809-4位截断残留两处/`（需求/back/front/api/test 五件齐，`aece64cc`）。
...
- **方案目录**：`dev-docs/task-0801-公式计算精度优化/task-0810-公式计算12位显示9位/`
...
- **状态**：✅ **已交付合 master `ada23fd0`**（2026-08-14；AC-1~8 全达成，见 `dev-docs/task-0721-报价侧树状结构与页签类型属性/repair-0814-详情页报价树多出版本列/test-report.md`）
- **登记日期**：2026-08-14
...
- **交付**：`dev-docs/task-0801-公式计算精度优化/repair-0811-输入值原样与结果精度分层/`；输入字段原值、公式 12/9 位分层、产品小计/报价总额独立精度入口全部落地，无 DDL/Flyway。
