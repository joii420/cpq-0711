...
- 🚫 **目录深度硬上限 = 2 层（主任务 / 子任务），严禁三层嵌套**（2026-08-13 立规）
  - **规则**：`dev-docs/<主任务>/<子任务>/<文档>.md`。**主任务下的所有子任务（含 `task-` 子任务、`repair-` 返修、二次返修）一律平铺在主任务目录下**，不得再往子任务里套子任务。
  - **正例**：`dev-docs/task-0801-公式计算精度优化/repair-0812-对账阈值与结果尺度不对称致误报/问题说明.md`
  - **反例**：`dev-docs/task-0801-公式计算精度优化/task-0810-公式计算12位显示9位/repair-0812-.../问题说明.md`（三层，禁止）
  - **"由子任务 B 引入的 BUG"怎么放**：仍平铺到**主任务**目录下建 `repair-MMDD-*`，在文档头部用 `**引入来源**：<子任务目录名>（提交 hash）` 一行写清血缘 —— **血缘靠文档字段表达，不靠目录层级表达**。
  - 理由：三层以后路径过长、`INDEX.md` 树形标记失真、跨文档引用极易写错；两层足以表达「主任务 → 子任务」，更深的关系用文档内的「引入来源 / 关联」字段描述。
- 🐞 **BUG 立项标准动作（发现问题 → 建 repair 目录 → 写 `问题说明.md`）**（2026-08-13 立规）
  1. **先找归属**：过 `dev-docs/INDEX.md` §0「按症状反查」+ §0.5「按代码文件反查」定位这个 BUG 属于哪个**主任务**（判不准就问用户，不要猜）。
  2. **在该主任务目录下**建 `repair-MMDD-<中文问题名>/`（遵守上一条两层上限）；确实不属于任何既有任务时才建 `dev-docs/repair-MMDD-*/`。
  3. 目录内写 **`问题说明.md`**（BUG 类任务的**必写首文档**，取代 task 类的 `需求文档.md` 作为立项文档），**六段齐全，缺一不可**：
     | 段 | 内容要求 |
     |---|---|
     | **① 问题描述** | 现象原文 / 报错文案；**用户若提供了截图，必须把图片文件拷进本 repair 目录并用 `![说明](./<图名>.png)` 内嵌进 md**，不能只写"见聊天记录" |
     | **② 复现 / 发现方式** | 谁怎么发现的（用户报 / 监控 / 测试）＋ 可执行的复现步骤（单号、页面、页签、操作、环境/库）＋ 触发条件的一般化描述 |
     | **③ 需求结果** | 期望行为逐条列（E-1、E-2…），**必须同时写清"该报的仍要报"这类反向期望**，防止修成"把告警关了" |
     | **④ 问题原因 / 报告** | 根因（分层写）＋ 证据链（SQL/代码行/实测数据，可复跑）＋ 引入时间线（哪个提交引入、之前为什么不犯）＋ 影响面 ＋ 已知限定 ＋ 为什么测试没拦住 |
     | **⑤ 修复方案** | 🚫 **只写用户裁决后的最终方案**（选定方案 + 为什么选它 + 改动面 + 已知风险 + 被否方案一句话带过）。**候选方案对比不进本文档** —— 见下方第 4 步：写本段之前必须先在**对话里**输出方案对比报告并与用户讨论定案。**未裁决时本段写「⏳ 待裁决（对比报告已于对话输出，待用户拍板）」**，不得自行填入倾向性方案 |
     | **⑥ 验收标准** | 逐条 AC + 判定方式，**必须含**：阴性（不再误报/不再复现）、阳性（真问题仍能被抓住）、无副作用（值/显示/导出不变）、以及 `CLAUDE.md`「修改后强制自检」要求的那几项证据 |
  4. **方案先讨论、后落文档**（2026-08-13 立规，顺序不可颠倒）：
     - 先写 `问题说明.md` 的 **①②③④⑥**（描述 / 复现 / 期望 / 根因 / 验收），**⑤ 暂留「⏳ 待裁决」**；
     - 再在**对话里输出「方案对比报告」**：候选方案 ≥2 个，每个写清 **改动面 / 优点 / 代价 / 风险 / 是否推翻既有需求条款（点名文档与条目）**，并给出主线推荐与推荐理由；
     - **等用户裁决** → 把**裁决结果**（含选定理由与被否方案的一句话说明）回填进 ⑤，此时 `问题说明.md` 才算完成。
     - 理由：文档是**决策台账**不是**思考草稿**。把 A/B/C 三选一长期留在文档里，后来人无法分辨「哪个是真做了的」，且容易把主线的倾向误读成已定结论。
  5. **闸门 A 不变**：`问题说明.md` 六段齐全（⑤ 已是裁决结果）→ 才建 worktree 写代码。其余开发文档（`fronttask.md` / `backtask.md` / `api.md` / `test.md` / `test-report.md`）按任务平台规则 §3 在闸门 A 通过后补齐。
