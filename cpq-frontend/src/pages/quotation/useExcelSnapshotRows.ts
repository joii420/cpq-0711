...
export function parseExcelSnapshotRows(params: UseExcelSnapshotRowsParams): LinkedExcelRow[] {
  ...
  return (lineItems || []).flatMap((li, i) => {
    ...
    let arr: any[] = [];
    const parsed = tryParseSnapshotJsonLossless<{ rows?: unknown[] }>(json);
    if (parsed) arr = Array.isArray(parsed.rows) ? parsed.rows : [];
...
export function useExcelSnapshotRows(params: UseExcelSnapshotRowsParams): { rows: LinkedExcelRow[] } {
  ...
  const rows = useMemo<LinkedExcelRow[]>(
    () => parseExcelSnapshotRows({ lineItems, side, parsedColumns }),
