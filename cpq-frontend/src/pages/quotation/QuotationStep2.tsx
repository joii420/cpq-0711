 ...
 *
 * ⚠️ 注意：归一之后两侧都是精确的 9 位小数，最小非零差是 1e-9，恒 > 默认容差 1e-12 ——
 * 也就是说 `isWithinTolerance` 的容差在**本调用点**永远不会生效，判定实际等价于
 * 「9 位上的精确相等」。保留它是为了与其它调用点保持同一入口，不要误以为它还在吸收噪声。
 * 已知限制（repair-0812 §5.1 L-2）：若前后端各自的 12 位工作值有 1 ulp 级差异、且恰好
 * 落在 9 位舍入的中点附近，会一个进位一个不进位 → 归一后差 1e-9 → 仍误报且不可消解。
 * 前端单侧堵不上（我们手上没有后端的 12 位工作值），只有方案 C 能闭合。
 *
