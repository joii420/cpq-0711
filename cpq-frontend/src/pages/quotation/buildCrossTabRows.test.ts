...
// ─────────────────────────────────────────────────────────────────────────────
// E3 护栏 — 材料成本逐行 = 各列同行之和 (field token 引用同组件 FIXED_VALUE 列)
//
// 修复后新语义：同组件列引用(无总计) → field token(同行值)，而非 component_subtotal(整列总计)。
// 本用例固化该行为：材料成本 = 来料材料费 + 外购件材料费，
//   行1: 5 + 2 = 7
//   行2: 0 + 3 = 3
//   列小计(is_subtotal): 10
// 若回退到旧 component_subtotal 语义，allComponentSubtotals['ll_test'] 会被当作输入，
// 初始为 0 → 每行材料成本=0, 列小计=0, 与期望不符 —— 自动揭露回退。
// ─────────────────────────────────────────────────────────────────────────────

// 来料材料费和外购件材料费：FIXED_VALUE（值来自 row）
const e3Fields = [
  { name: '来料材料费', field_type: 'FIXED_VALUE', content: '' },
  { name: '外购件材料费', field_type: 'FIXED_VALUE', content: '' },
  { name: '材料成本', field_type: 'FORMULA', is_subtotal: true },
] as any;

// 材料成本 = field(来料材料费) + field(外购件材料费)
// field token 引用同组件 FIXED_VALUE 字段（由 formulaSerialize.ts E2 修复后生成）
const e3MaterialCostExpr = [
  { type: 'field', value: '来料材料费' },
  { type: 'operator', value: '+' },
  { type: 'field', value: '外购件材料费' },
] as any;
...
const e3ComponentData = [
  {
    componentId: 'll_test',
    componentCode: 'll_test',
    tabName: '来料',
    componentType: 'NORMAL',
    fields: e3Fields,
    formulas: [
      { name: '材料成本', expression: e3MaterialCostExpr },
    ],
    rows: e3Rows,
    componentData: [],
    snapshotRows: 2,
...
const e3LookupExpansion = (comp: any) => {
  if (comp.componentId === 'll_test') return e3Expansion;
  return undefined;
};
