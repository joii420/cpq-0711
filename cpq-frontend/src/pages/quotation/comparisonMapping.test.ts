...
describe('columnDecimals', () => {
  it('PRODUCT_TOTAL 列最多显示 9 位', () => {
    expect(columnDecimals(makeDefaultColumn())).toBe(9);
  });
  it('TAB_PAIR 列最多显示 9 位', () => {
    expect(columnDecimals(tabPairCol())).toBe(9);
  });
});
...
describe('formatComparisonNumber / formatDiffNumber', () => {
  ...
  it('最多保留配置小数位并去尾零', () => {
    expect(formatComparisonNumber('15500', 2)).toBe('15500');
    expect(formatComparisonNumber('8000.1234567895', 9)).toBe('8000.12345679');
  });
  it('差异值正数带 + 号', () => {
    expect(formatDiffNumber('1000', 2)).toBe('+1000');
  });
  it('差异值负数保留 - 号，不重复加号', () => {
    expect(formatDiffNumber('-500', 2)).toBe('-500');
  });
  it('差异值 0 不带符号', () => {
    expect(formatDiffNumber('0', 2)).toBe('0');
  });
});
