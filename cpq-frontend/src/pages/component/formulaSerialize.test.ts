...
describe('expressionToTokens — basic', () => {
  ...
  it('whole-tab total [COMP_RL(总计)] → component_subtotal 指向金额哨兵键 + is_tab_total', () => {
    // BL-0017: value/tab_name 指向哨兵 __amount_total__（命中各装配点 code#__amount_total__ = Σ金额列），
    // 不再是首个小计列；is_tab_total 仍供序列化忠实回显 [COMP_RL(总计)]；label 为纯组件名。
...
describe('component_subtotal — subtotal column references', () => {
  ...
  it('[alias(总计)] bare-total maps to component_subtotal (BL-0017: 指向金额哨兵键)', () => {
    // BL-0017: 整页签总计走哨兵 __amount_total__（= Σ金额列），非首个小计列。
    const tokens = expressionToTokens('[COMP_RL(总计)]', allTabs, selfRKF);
    expect(tokens).toHaveLength(1);
    expect(tokens[0]).toMatchObject({
      type: 'component_subtotal',
...
describe('expressionToTokens / 回显 — 页签名称(componentName)作公式标识', () => {
  ...
  it('用名称解析整页签总计 [回料(总计)] → component_subtotal(BL-0017 金额哨兵键)', () => {
    ...
    expect(t[0]).toMatchObject({ type: 'component_subtotal', component_code: 'COMP_RL', value: '__amount_total__', is_tab_total: true });
  });
...
describe('WYSIWYG: 整页签总计 [页签(总计)] 与小计列 [页签.列] 可区分且各自忠实往返', () => {
  ...
  it('BL-0017 口径切换：[COMP_RL(总计)] 命中金额哨兵键，非首列、非裸键', () => {
    const tok = expressionToTokens('[COMP_RL(总计)]', allTabs, selfRKF);
