 ...
 *   5. CrossTabRefDrawer (标题「插入跨页签引用」) 打开 → 源页签下拉被真实兄弟组件填充
...
test('cross_tab_ref 配置路径: 抽屉填充兄弟组件 + 确定后公式区出现跨页签 chip', async ({ page }) => {
  ...
  const drawer = page.locator('.ant-drawer').filter({ hasText: '插入跨页签引用' }).first();
  await expect(drawer, '应弹出「插入跨页签引用」抽屉').toBeVisible({ timeout: 8000 });
