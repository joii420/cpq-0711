...
test('报价单流程: 苏州西门子 + 报价模板0608 v1.10 + 10110002(渲染层无回归)', async ({ page }) => {
  ...
  // 顺带捕获自动保存命中的报价单 id —— 末尾"懒算护栏"用例需要 reopen 这张
  // 已建好(含产品 + 卡片值快照)的报价单来验证打开阶段无 batch-expand 风暴。
  let draftPutCount = 0;
  let savedQuotationId: string | null = null;
  page.on('request', (req) => {
    if (req.method() === 'PUT') {
      const m = req.url().match(/\/quotations\/([^/]+)\/draft/);
      if (m) { draftPutCount += 1; savedQuotationId = m[1]; }
    }
  });
  ...
  await page.keyboard.press('Escape');  // 关闭下拉, 确保受控 onChange 提交(否则 step1Valid 可能不翻 true)
  await page.waitForTimeout(300);
  ...
  await page.waitForTimeout(1200);  // 等模板加载
  {
    ...
    await opt.click();
    await page.waitForTimeout(400);
    await page.keyboard.press('Escape');  // 关闭下拉, 提交受控值(搜索态下 antd 偶尔不 commit → step1Valid 卡 false)
    await page.waitForTimeout(300);
  }
  await shot(page, 'template-selected');

  // ── 7) 下一步 → Step2 (等校验通过, 按钮 enabled 再点) ──
  const nextBtn = page.getByRole('button', { name: /下一步/ }).first();
  await expect(nextBtn, 'Step1 校验应通过(产品分类+报价模板已选), 下一步按钮应可点').toBeEnabled({ timeout: 15000 });
  await nextBtn.click();
  await page.waitForLoadState('networkidle');
  ...
  // ── 9) 选配添加抽屉（task-0712 F5 明细表重构，D11）：
  //     单屏明细表(左) + 3D 预览常驻(右)，无 P0~P5 逐步向导。
  //     若客户/行业无有效选配模板，抽屉显示「缺少选配模板」空态——用等待兜底两种态其一。
  const emptyTplHint = page.locator('.ant-drawer').getByText('缺少选配模板', { exact: false });
  const addRowBtn = page.locator('.ant-drawer button:has-text("新增材质料号")');
  await Promise.race([
    emptyTplHint.waitFor({ state: 'visible', timeout: 8000 }).catch(() => {}),
    addRowBtn.waitFor({ state: 'visible', timeout: 8000 }).catch(() => {}),
  ]);
  await shot(page, 'configure-drawer-open');

  if (await emptyTplHint.count() > 0) {
    console.log('[选配添加] ⚠️ 该客户/行业无有效选配模板(D6 兜底链未命中)，跳过选配子流程，改走"确认加入"前置校验(0 行应不可点)');
    const confirmDisabled = page.locator('.ant-drawer button:has-text("确认加入")');
    console.log(`[选配添加] 确认加入按钮存在(禁用态)=${await confirmDisabled.count() > 0}`);
  } else {
    // ── 10) 明细表「+ 新增材质料号」→ 内层子框三步：① 材质 → ② 元素含量 → ③ 工序 ──
    await addRowBtn.first().click();
    await page.waitForTimeout(500);
    await shot(page, 'sub-step1-material');

    // Step① 材质：网格卡片，选候选 00001(Ag，单元素、默认配比=100%，免微调、必过校验)。
    const materialCard = page.locator('.ant-drawer').getByText('00001', { exact: true }).first();
    await materialCard.click();
    await page.waitForTimeout(300);
    await shot(page, 'sub-step1-material-selected');
    await page.locator('.ant-drawer button:has-text("下一步")').last().click();
    await page.waitForTimeout(700);
    await shot(page, 'sub-step2-elements');

    // Step② 元素含量：00001 单元素默认配比恰=100%（不微调也能过 confirmAdd 校验），直接下一步。
    await page.locator('.ant-drawer button:has-text("下一步")').last().click();
    await page.waitForTimeout(500);
    await shot(page, 'sub-step3-process');

    // Step③ 工序：勾选第一个候选（非必选，选一个覆盖更真实的落库路径）。
    ...
    await shot(page, 'sub-step3-process-checked');
    await page.locator('.ant-drawer button:has-text("确认添加")').last().click();
    await page.waitForTimeout(600);
    await shot(page, 'sub-confirmed-back-to-table');

    // ── 11) 明细表：数量默认 1（Σqty==1 → SIMPLE，D11/D12 判定同口径），直接确认加入 ──
    const rowCount = await page.locator('.ant-drawer .ant-table-tbody tr.ant-table-row').count();
    console.log(`[选配添加] 明细表行数 = ${rowCount} (期望 1)`);
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 懒算护栏(lazy-card-values 修复): 打开已保存报价单读卡片值快照, 不再风暴
  // ───────────────────────────────────────────────────────────────────────
  // 背景: 修复前, 打开一张已保存的报价单会对每个 Tab 现场 batch-expand +
  //       batch-evaluate 重算一遍("打开即风暴", 拖慢首屏 / autosave 风暴)。
  //       修复后, 打开走 loadQuotation → ensureCardValues 暖快照/兜底, 直接读
  //       已存的卡片值快照渲染 → 打开阶段对这两个端点的请求数应为 0。
  //       (ensureCardValues 的暖/兜底逻辑负责把快照填好, 使该读快照门闸生效。)
  //
  // 作用域: 监听器在 reopen 的 page.goto 之前注册, batchCalls 从空开始, 只统计
  //         "打开已保存报价单"这一段 —— 不含上面"新建 + 添加产品"阶段(那个阶段
  //         调 batch-expand / batch-evaluate 是合理的, 故必须新起一个干净监听器
  //         而非复用全程计数)。复用上面已建好(含产品 + 快照)的报价单 reopen,
  //         不另造流程。
  // ═══════════════════════════════════════════════════════════════════════
  if (savedQuotationId) {
    const batchCalls: string[] = [];
    const onBatchReq = (r: import('@playwright/test').Request) => {
      const u = r.url();
      if (u.includes('/components/batch-expand') || u.includes('/formulas/batch-evaluate')) {
        batchCalls.push(`${r.method()} ${u}`);
        console.log(`[lazy-card] ⚠️ 打开阶段检测到风暴请求: ${r.method()} ${u}`);
      }
    };
    page.on('request', onBatchReq);

    // 重新打开已保存的报价单(走 loadQuotation → ensureCardValues 读快照路径)
    // /edit 路由落地 Step1(currentStep 恒 useState(0) 起步)，点「下一步」进 Step2 才能看到产品卡片。
    await page.goto(`/quotations/${savedQuotationId}/edit`);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(3000);  // 给"打开后若仍有现场重算"充足触发时间, 让风暴请求先飞出来
    const reopenNextBtn = page.getByRole('button', { name: /下一步/ }).first();
    if (await reopenNextBtn.isEnabled().catch(() => false)) {
      await reopenNextBtn.click();
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(500);
    }
    await shot(page, 'reopen-saved-quotation');

    // 等渲染落定: 产品卡片 Tab 出现 + 无"加载中"占位(镜像上面 8-Tab 落定判定)
    await page.locator('button.qt-tab-btn').first()
    ...
    await expect
      .poll(() => batchCalls.length, {
        timeout: 8000,
        message:
          `打开已保存报价单不应触发 batch-expand / batch-evaluate 风暴(懒算修复应读卡片值快照), ` +
          `但检测到 ${batchCalls.length} 次: ${batchCalls.join(', ')}`,
      })
      .toBe(0);

    page.off('request', onBatchReq);
    console.log('[lazy-card] ✅ 打开已保存报价单 batch-expand/batch-evaluate = 0 (懒算护栏通过)');
  } else {
    console.log('[lazy-card] ⚠️ 未捕获到已保存报价单 id(全程无 PUT /draft), 跳过 reopen 风暴断言');
  }
...
test('TC-F1: 打开 DRAFT 报价单不自动发 refresh-card-snapshot', async ({ page }) => {
  ...
  // ── 打开报价单（落地 Step1，向导不会因带 id 的 /edit 路由自动跳转 Step2——
  //     currentStep 恒以 useState(0) 起步，仅用户点「下一步」才前进），点「下一步」进 Step2 ──
  await page.goto(`/quotations/${quotationId}/edit`);
  await page.waitForLoadState('networkidle');
  // 额外等待 3s，确保任何"打开后自动刷新"逻辑（若还存在）有充足时间触发
  await page.waitForTimeout(3000);
  const step1NextBtn = page.getByRole('button', { name: /下一步/ }).first();
  await expect(step1NextBtn, '编辑态 Step1(客户/模板已锁定预填)下一步应可点').toBeEnabled({ timeout: 15000 });
...
test('TC-F2: 显式刷新才触发 refresh-card-snapshot', async ({ page }) => {
  ...
  // ── 打开报价单，点「下一步」进 Step2（/edit 路由落地 Step1，不自动跳转，见 TC-F1 同注）──
  await page.goto(`/quotations/${quotationId}/edit`);
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(1500);
  const tcf2NextBtn = page.getByRole('button', { name: /下一步/ }).first();
  await expect(tcf2NextBtn, '编辑态 Step1(客户/模板已锁定预填)下一步应可点').toBeEnabled({ timeout: 15000 });
  await tcf2NextBtn.click();
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(1000);
