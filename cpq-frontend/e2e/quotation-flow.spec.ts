...
test('报价单流程: 苏州西门子 + 报价模板0608 v1.10 + 10110002(渲染层无回归)', async ({ page }) => {
  ...
  // 本次 fix 验证: 捕获 PUT /draft 响应体 —— 新行落快照后, 后端应返回携带 quoteCardValues 的
  // 新鲜 DTO(前端 syncLineItemsFromResponse 据此翻入快照模式, 否则 driver 行删除"点了无反应")。
  const draftRespHasQuoteSnap: boolean[] = [];
  page.on('response', async (resp) => {
    const req = resp.request();
    if (req.method() === 'PUT' && /\/quotations\/[^/]+\/draft/.test(resp.url())) {
      try {
        const body = await resp.json();
        const lis = body?.data?.lineItems ?? [];
        draftRespHasQuoteSnap.push(lis.some((li: any) => !!li.quoteCardValues));
      } catch { /* 非 JSON / 解析失败忽略 */ }
    }
  });
  ...
  // ── 14) 本次 fix 核心验证: 导入/新增流程下 driver 行删除即时生效 ──
  // 14a. saveDraft 响应应至少有一次携带 quoteCardValues(后端 fix: 新行落快照后返回新鲜 DTO),
  //      否则前端停在实时展开模式(useSnapQuote=false), driver 行删除墓碑不被过滤 → "点了无反应"。
  const snapRespCount = draftRespHasQuoteSnap.filter(Boolean).length;
  console.log(`\n=== PUT /draft 响应含 quoteCardValues 的次数: ${snapRespCount}/${draftRespHasQuoteSnap.length} (期望 ≥1) ===`);
  expect(snapRespCount, 'saveDraft 响应应至少一次携带 quoteCardValues(→前端翻快照模式)').toBeGreaterThanOrEqual(1);

  // 14b. 找一个含 driver 删除 ✕ 的 Tab, 删一行, 断言该 Tab 行数减少(删除即时生效)。
  ...
  for (const tabName of tabNames) {
    ...
    await tab.click();
    await page.waitForTimeout(1500);
    const delBtns = page.locator('button[title*="删除行（永久"]');
    const delCount = await delBtns.count();
    if (delCount === 0) continue;
    const rowsBefore = await page.locator('.ant-table-row').count();
    console.log(`  [删除验证 Tab '${tabName}'] driver删除按钮=${delCount}, 删前行数=${rowsBefore}`);
    await delBtns.first().click();
    await page.waitForTimeout(2000);
    const rowsAfter = await page.locator('.ant-table-row').count();
    console.log(`  [删除验证 Tab '${tabName}'] 删后行数=${rowsAfter} (期望 < ${rowsBefore})`);
    await shot(page, `delete-driver-row-${tabName}`);
    expect(rowsAfter, `删除 driver 行后该 Tab 行数应减少(删前 ${rowsBefore})`).toBeLessThan(rowsBefore);
    deleteVerified = true;
    break;
  }
  console.log(`\n=== driver 行删除验证: ${deleteVerified ? '✅ 已完成' : '❌ 未找到可删行'} ===`);
  expect(deleteVerified, '应至少找到一个含 driver 行的 Tab 完成删除验证').toBe(true);
