...
test('builder-UI: 标题含「跨页签公式构建器」+ 简单/高级 Segmented + 简单模式操作仅含取一个值/求和', async ({ page }) => {
  ...
  // 2) 简单模式：找「要算什么」section，打开其操作 Select，断言选项
  //    定位策略：找 hasText='要算什么' 的 section div，取其后的第一个 .ant-select
  const operationSection = drawer.locator('div').filter({ hasText: /^要算什么/ }).first();
  const operationSelect = operationSection.locator('~ div .ant-select').first()
    .or(drawer.locator('.ant-select').nth(3));
  // 更稳健：直接在 drawer 内找「要算什么」label 后方的 ant-select，
  // 但 DOM 是兄弟关系，用 nextSibling 不可靠；改为：drawer 内先找含「要算什么」的 div，
  // 再在该 div 的父级里取 .ant-select（就是紧跟的操作 select）。
  // 由于 drawer 内 .ant-select 顺序是: [0]=源页签 [1]=A.列 [2]=本.列 [3]=操作 [4]=目标列
  // 简单模式无公式子 select，操作 select = index 3（已由原 spec 确认 selCount≥5，[3]=操作）
...
test('builder-UI: 高级模式选好源+匹配+求和+目标后 rawText 自动以「求和 | 源:」开头', async ({ page }) => {
  ...
  await expect(rawTextArea, 'rawText 应以「求和 | 源:」开头').toHaveValue(/^求和 \| 源:/, { timeout: 2000 });
